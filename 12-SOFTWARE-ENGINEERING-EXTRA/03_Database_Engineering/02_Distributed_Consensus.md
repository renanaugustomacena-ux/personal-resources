...
modulo: "02"
titolo: "Distributed Consensus & Replication"
versione: "etcd 3.5 / CockroachDB 24.x / ZooKeeper 3.9"
livello: "Advanced"
prerequisiti:
  - "Networking fundamentals: TCP, latency, packet loss, network partitions"
  - "Concurrency concepts: threads, locks, happens-before, linearizability intuition"
  - "Basic distributed-systems vocabulary: replicas, quorum, leader/follower, CAP theorem"
  - "Familiarity with log-structured commit protocols (WAL concepts from Module 3.1)"
obiettivi:
  - "Trace a Raft leader election step-by-step, including split-vote and partition scenarios, and explain why the safety invariants hold"
  - "Compare Paxos, Multi-Paxos, Raft, ZAB, and Viewstamped Replication on message complexity, leader requirements, and production adoption"
  - "Design a replication topology (sync, async, semi-sync, leaderless) for a given latency/durability requirement and justify the trade-offs using CAP/PACELC"
  - "Implement or trace CRDT merge operations (G-Counter, PN-Counter, OR-Set) and explain when CRDTs are preferable to consensus-based replication"
  - "Evaluate a distributed system's consistency guarantees (linearizable, sequential, causal, eventual) and select the appropriate model for a given use case"
tag: [consensus, raft, paxos, replication, crdt, distributed-systems, zab, gossip, causal-consistency, two-phase-commit]
---
...
### Exercise 1: Trace a Raft Election (Paper Walk-Through)

Using the Raft paper (Ongaro & Ousterhout, 2014), trace through the following
scenario on paper or whiteboard:

1. A 5-node cluster (N1--N5) is in steady state with N1 as leader in term 4.
2. N1 crashes. N3's election timer fires first.
3. Draw the `RequestVote` messages, the vote responses, and the resulting leader.
4. Now add a twist: N2's timer fires at the same time as N3's (split vote).
   Show what happens, including the randomized timeout resolution.
5. After the new leader is elected, show how a log entry that was replicated to
   N1 and N2 (but not yet committed) is handled by the new leader.

Deliverable: A sequence diagram (hand-drawn or Mermaid) covering all messages
and state transitions for both the clean and split-vote paths.

### Exercise 2: Compare Replication Topologies

Given these three application profiles, design the replication topology and justify
each choice with CAP/PACELC reasoning:

| Application | Latency SLA | Durability requirement | Read pattern |
|---|---|---|---|
| Payment ledger | p99 < 50ms | Zero data loss | Read-after-write |
| Social media feed | p99 < 200ms | Tolerate 1s data loss | Eventually consistent |
| Global config store | p99 < 10ms | Zero data loss | Infrequent writes, many reads |

For each, specify: sync vs. async replication, quorum sizes (R, W, N), whether
to use leader-based or leaderless, and the consistency model.

### Exercise 3: Implement G-Counter and PN-Counter CRDTs

In your language of choice, implement:

1. A **G-Counter** with `increment(node_id)`, `value()`, and `merge(other)` methods.
2. A **PN-Counter** built on two G-Counters.
3. Simulate 3 nodes each performing 100 random increments/decrements.
4. Merge all three states pairwise and verify convergence (all nodes agree).
5. Measure the metadata overhead: how large is the serialized state for N=3 vs. N=100 nodes?

### Exercise 4: Observe Gossip Convergence

Using a local Docker Compose setup with 5 Consul agents (or Serf nodes):

1. Start all 5 nodes and observe the initial gossip protocol via logs.
2. Inject a key-value write on node 1.
3. Query all 5 nodes at 100ms intervals and record when each node has the value.
4. Plot convergence time vs. cluster size (repeat with 3, 5, 10, 20 nodes).
5. Partition node 5 (iptables or Docker network disconnect), wait 30 seconds,
   reconnect, and measure how long anti-entropy takes to sync the missed updates.

### Exercise 5: Two-Phase Commit Failure Modes

Simulate 2PC with a coordinator and 3 participants (can use simple TCP sockets or
message queues):

1. Happy path: all participants vote YES, coordinator sends COMMIT.
2. One participant votes NO: coordinator sends ABORT.
3. Coordinator crashes after collecting YES votes but before sending COMMIT.
   Observe participant behavior (stuck holding locks).
4. Implement a timeout-based recovery: participants abort after T seconds of
   coordinator silence. Document when this violates atomicity.
5. Compare with the Saga pattern: implement the same 3-step transaction as
   forward actions + compensating actions and test a mid-sequence failure.
...
- etcd/raft library (Go): https://github.com/etcd-io/raft
- etcd/raft Go package docs: https://pkg.go.dev/go.etcd.io/raft/v3
...
- Raft Consensus Algorithm (visualization + resources): https://raft.github.io/
- CRDT.tech (papers and implementations): https://crdt.tech/papers.html

**Books:**

- Kleppmann, Martin. *Designing Data-Intensive Applications*. O'Reilly, 2017.
  Chapters 5 (Replication), 8 (The Trouble with Distributed Systems),
  9 (Consistency and Consensus).
- Cachin, Christian; Guerraoui, Rachid; Rodrigues, Luis. *Introduction to
  Reliable and Secure Distributed Programming*. 2nd ed., Springer, 2011.

**Papers:**

- Lamport, Leslie. "Paxos Made Simple." *ACM SIGACT News* 32(4), December 2001.
  https://lamport.azurewebsites.net/pubs/paxos-simple.pdf
- Ongaro, Diego and Ousterhout, John. "In Search of an Understandable Consensus
  Algorithm (Extended Version)." *USENIX ATC*, 2014. Best Paper Award.
  https://raft.github.io/raft.pdf
- Fischer, Michael J.; Lynch, Nancy A.; Paterson, Michael S. "Impossibility of
...
- Shapiro, Marc et al. "A Comprehensive Study of Convergent and Commutative
  Replicated Data Types." INRIA Research Report 7506, January 2011.
- Das, Abhinandan; Gupta, Indranil; Motivala, Ashish. "SWIM: Scalable
  Weakly-consistent Infection-style Process Group Membership Protocol."
  *DSN*, 2002.
- Liskov, Barbara and Cowling, James. "Viewstamped Replication Revisited."
  MIT Technical Report MIT-CSAIL-TR-2012-021, 2012.
- Kulkarni, Sandeep et al. "Logical Physical Clocks and Consistent Snapshots
  in Globally Distributed Databases." *OPODIS*, 2014.
- Moraru, Iulian; Andersen, David; Kaminsky, Michael. "There Is More Consensus
  in Egalitarian Parliaments." *SOSP*, 2013. (EPaxos)
- Corbett, James C. et al. "Spanner: Google's Globally-Distributed Database."
  *OSDI*, 2012. (TrueTime and external consistency)

**Online Resources (retrieved: 2026-05-29):**

- The Morning Paper — Paxos Made Simple walkthrough: https://blog.acolyer.org/2015/03/04/paxos-made-simple/
- Hashicorp Serf / Memberlist (SWIM + Lifeguard): https://github.com/hashicorp/memberlist
- Jepsen — Distributed systems correctness testing: https://jepsen.io/
...
|---|---|---|
| 3.1 — Relational Database Internals | `03_Database_Engineering/01_Relational_Internals.md` | WAL mechanics and streaming replication are the transport layer for consensus-driven replicas |
| 3.3 — Storage Engines & NoSQL | `03_Database_Engineering/03_Storage_Engines_NoSQL.md` | NoSQL systems (Cassandra, DynamoDB) use gossip and leaderless replication covered here |
| 3.4 — Data Engineering OLAP/OLTP | `03_Database_Engineering/04_Data_Engineering_OLAP_OLTP.md` | OLAP pipelines depend on replication lag characteristics and consistency models |
| 2.2 — Distributed System Patterns | `02_Architecture_Design/02_Distributed_System_Patterns.md` | Saga, 2PC, and event sourcing patterns build on the transaction and consensus primitives here |
| 2.4 — System Design (CAP/PACELC) | `02_Architecture_Design/04_System_Design_CAP_PACELC.md` | CAP theorem and PACELC framework provide the theoretical backdrop for replication trade-offs |
| 5.2 — Kubernetes Architecture | `05_DevOps_Cloud_Native/02_Kubernetes_Architecture.md` | etcd (Raft-based) is the control-plane store for Kubernetes; consensus health directly affects cluster availability |
...
| Term | Definition |
|---|---|
| **Consensus** | Agreement among distributed nodes on a single value (or sequence of values) despite crashes, message loss, or partitions. |
| **Term (Raft)** | Monotonically increasing logical clock in Raft; each term has at most one leader and is used to detect stale messages. |
| **Quorum** | The minimum number of nodes (typically a majority, f+1 out of 2f+1) that must agree for a decision to be considered committed. |
| **Leader Lease** | A time-bounded guarantee that a node is the current leader; enables local reads without an extra round of consensus. |
| **Split Vote** | A Raft election failure where two candidates divide the votes and neither achieves a majority; resolved by randomized election timeouts. |
| **Log Compaction** | Replacing a prefix of the replicated log with a snapshot of the state machine to bound log growth; implemented as InstallSnapshot in Raft. |
| **CRDT (Conflict-Free Replicated Data Type)** | A data structure whose replicas can be updated independently and merged deterministically, guaranteeing eventual convergence without coordination. |
| **Vector Clock** | A logical clock consisting of one counter per node, enabling detection of causal ordering and concurrency between events. |
| **HLC (Hybrid Logical Clock)** | A clock combining physical time with a logical counter, providing causal ordering with bounded drift from wall-clock time; used by CockroachDB. |
| **Gossip Protocol** | An epidemic-style protocol where each node periodically exchanges state with random peers, achieving O(log N) convergence for information dissemination. |
| **Linearizability** | The strongest single-object consistency model: every operation appears to take effect atomically at some point between its invocation and response, respecting real-time order. |
| **2PC (Two-Phase Commit)** | A distributed transaction protocol with PREPARE and COMMIT phases; guarantees atomicity but is blocking if the coordinator fails after PREPARE. |
| **Saga** | A sequence of local transactions with compensating actions for rollback; used for long-running distributed transactions where 2PC lock-holding is impractical. |
| **FLP Impossibility** | The 1985 proof (Fischer, Lynch, Paterson) that no deterministic consensus algorithm can guarantee termination in a purely asynchronous system with even one crash failure. |
| **Sloppy Quorum** | A variant of quorum replication (used by DynamoDB) where writes during a partition may go to non-home nodes via hinted handoff, trading consistency for availability. |
