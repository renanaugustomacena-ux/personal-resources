...
modulo: "2.1"
titolo: "Code-Level Architecture & Principles"
versione: "2026-05 (post-DDD Renaissance)"
livello: "Advanced"
prerequisiti:
  - "Solid understanding of at least one OOP and one FP language"
  - "Experience building multi-module applications (web, CLI, or services)"
  - "Familiarity with basic design patterns (GoF: Strategy, Observer, Factory)"
  - "Understanding of relational databases and basic SQL"
  - "Version control with Git (branching, merging)"
obiettivi:
  - "Apply Clean / Hexagonal / Onion architecture to enforce the Dependency Rule in a greenfield or brownfield codebase"
  - "Implement CQRS with separate read and write models and measure the resulting performance delta"
  - "Design an event-sourced aggregate with snapshots, upcasters, and temporal query capability"
  - "Model a bounded context using DDD tactical patterns (Entities, Value Objects, Aggregates, Domain Events, Specifications)"
  - "Write architectural fitness functions that enforce structural invariants in CI"
tag: [clean-architecture, hexagonal, ports-and-adapters, onion, cqrs, event-sourcing, ddd, dependency-injection, architectural-fitness, module-structure]
---
...
> - Distinguish Clean, Hexagonal, and Onion architectures and select the right one for a given project's complexity profile
> - Enforce the Dependency Rule through package structure, DI wiring, and automated architecture tests
> - Implement a full CQRS pipeline: command handlers, event handlers, read-model projections, and eventual-consistency monitoring
> - Build an event-sourced aggregate lifecycle: event replay, snapshotting, schema evolution via upcasters, and temporal queries
> - Apply DDD tactical patterns (Aggregates, Value Objects, Domain Services, Specifications) to protect domain invariants at the code level
...
## Exercises — Cross-Cutting Synthesis

> These exercises complement the hands-on exercises in §12 by requiring integration across multiple architectural concepts from this module and related modules.

### Synthesis Exercise 1: Full-Stack Bounded Context (Intermediate)

Design and implement a complete "Reservations" bounded context that combines:
- Hexagonal Architecture for structure (ports/adapters)
- CQRS for separate booking commands vs. availability queries
- A domain model with at least two aggregates sharing a domain event

**Deliverable:** A running service with architecture tests (ArchUnit or import-linter) enforcing the dependency rule, plus a written ADR justifying your architecture choice using the Decision Matrix from §18.

### Synthesis Exercise 2: Event Sourcing + Read Model Rebuild Pipeline (Advanced)

Starting from the event-sourced shopping cart in Exercise 3 (§12), build a production-grade rebuild pipeline:
1. Introduce a **projection bug** (e.g., incorrect total calculation).
2. Fix the projector logic.
3. Build a replay tool that rebuilds the read model from the event store.
4. Add an **idempotency guard** so replayed events do not duplicate side effects (e.g., notification emails).
5. Measure rebuild throughput and set an SLO (e.g., 50,000 events/second on a single thread).

**Deliverable:** A benchmark report comparing snapshot-accelerated replay vs. full replay, plus the idempotency mechanism documented inline.

### Synthesis Exercise 3: Migration Dry Run — Layered to Hexagonal (Intermediate)

Take an open-source layered MVC project (Spring PetClinic, Django Polls, or Rails Blog) and apply the Migration Playbook from §19:
1. Complete Phases 1-3 on a branch.
2. Measure before/after: afferent coupling (Ca), efferent coupling (Ce), instability (I = Ce / (Ca + Ce)) per package.
3. Write architecture fitness functions that would have caught the original layering violations.

**Deliverable:** A diff showing the refactored structure, metrics comparison table, and fitness function test suite.

### Synthesis Exercise 4: Cross-Module Integration — Architecture Meets Distributed Systems (Advanced)

Design a multi-service system for an e-commerce platform:
1. Use Module 2.2 (Distributed System Patterns) for service communication (Saga, Circuit Breaker).
2. Use this module's CQRS + Event Sourcing for the Order service internally.
3. Use Module 2.4 (CAP/PACELC) to justify your database choices per service.
4. Define an Anti-Corruption Layer between the Order and Inventory bounded contexts.

**Deliverable:** An architecture diagram, PACELC classification per data store, and a Saga definition for the order fulfillment workflow.

### Synthesis Exercise 5: Architectural Fitness Dashboard (Advanced)

Build an automated dashboard that continuously monitors architectural health:
1. Dependency rule violations (ArchUnit / import-linter).
2. Cyclomatic complexity per module (threshold: 10).
3. Coupling metrics (Ca, Ce, Instability, Abstractness) with a target zone on the Main Sequence.
4. Event schema compatibility checks (backward + forward).
5. CQRS projection lag (time between event publication and read model update).

**Deliverable:** A CI pipeline that publishes metrics to a dashboard and fails the build on regression.

---

## Readings and References — Extended

> Supplements the reference list in §16 with additional resources and verified URLs.

### Books

1. Martin, R.C. *Clean Architecture: A Craftsman's Guide to Software Structure and Design*. Prentice Hall, 2017. ISBN 978-0134494166.
2. Evans, E. *Domain-Driven Design: Tackling Complexity in the Heart of Software*. Addison-Wesley, 2003. ISBN 978-0321125217.
3. Vernon, V. *Implementing Domain-Driven Design*. Addison-Wesley, 2013. ISBN 978-0321834577.
4. Vernon, V. *Domain-Driven Design Distilled*. Addison-Wesley, 2016. ISBN 978-0134434421.
5. Seemann, M. *Dependency Injection: Principles, Practices, and Patterns*. 2nd ed., Manning, 2019. ISBN 978-1617294730.
6. Ford, N., Parsons, R., Kua, P. *Building Evolutionary Architectures*. 2nd ed., O'Reilly, 2023. ISBN 978-1492097549.
7. Kleppmann, M. *Designing Data-Intensive Applications*. O'Reilly, 2017. ISBN 978-1449373320.
8. Gamma, E., Helm, R., Johnson, R., Vlissides, J. *Design Patterns: Elements of Reusable Object-Oriented Software*. Addison-Wesley, 1994. ISBN 978-0201633610.

### Articles and Online Resources

9. Martin, R.C. "The Clean Architecture" (2012). Retrieved: 2026-05-29. https://blog.cleancoder.com/uncle-bob/2012/08/13/the-clean-architecture.html
10. Cockburn, A. "Hexagonal Architecture" (2005). Retrieved: 2026-05-29. https://alistair.cockburn.us/hexagonal-architecture/
11. Fowler, M. "CQRS" — bliki. Retrieved: 2026-05-29. https://martinfowler.com/bliki/CQRS.html
12. Fowler, M. "Event Sourcing" (2005). Retrieved: 2026-05-29. https://martinfowler.com/eaaDev/EventSourcing.html
13. Fowler, M. *Catalog of Patterns of Enterprise Application Architecture*. Retrieved: 2026-05-29. https://martinfowler.com/eaaCatalog/
14. Young, G. "CQRS Documents" (2010). Retrieved: 2026-05-29. https://cqrs.files.wordpress.com/2010/11/cqrs_documents.pdf
15. Parnas, D.L. "On the Criteria To Be Used in Decomposing Systems into Modules." *Communications of the ACM*, 15(12), 1972, pp. 1053-1058.
16. Graça, H. "DDD, Hexagonal, Onion, Clean, CQRS — How I Put It All Together" (2017). Retrieved: 2026-05-29. https://herbertograca.com/2017/11/16/explicit-architecture-01-ddd-hexagonal-onion-clean-cqrs-how-i-put-it-all-together/
17. Microsoft. "Event Sourcing Pattern — Azure Architecture Center." Retrieved: 2026-05-29. https://learn.microsoft.com/en-us/azure/architecture/patterns/event-sourcing

### Papers

18. Martin, R.C. "Design Principles and Design Patterns." *objectmentor.com*, 2000.
...
|---|---|
| [02_Distributed_System_Patterns.md](./02_Distributed_System_Patterns.md) | Saga, Circuit Breaker, and service mesh patterns that connect hexagonal services in a distributed topology |
| [02_b_High_Performance_System_Design.md](./02_b_High_Performance_System_Design.md) | Performance optimization techniques (connection pooling, async I/O, backpressure) applied within ports and adapters |
| [02_c_API_Design_Evolution.md](./02_c_API_Design_Evolution.md) | API versioning and contract design that define the external surface of adapter-in layers |
| [03_Design_Principles_SOLID_etc.md](./03_Design_Principles_SOLID_etc.md) | SOLID principles (especially DIP and ISP) that underpin the Dependency Rule and port design |
| [04_System_Design_CAP_PACELC.md](./04_System_Design_CAP_PACELC.md) | CAP/PACELC trade-offs that drive database selection for event stores and read-model projections |
| [../03_Database_Engineering/](../03_Database_Engineering/) | Schema design, indexing, and transaction patterns used by persistence adapters and event stores |

---

## Glossary — Supplementary Terms

> Supplements the glossary in §17 with additional terms referenced in this module.
...
| **Afferent Coupling (Ca)** | The number of external modules that depend on a given module. High Ca indicates the module is a core dependency. |
| **Clean Architecture** | Robert C. Martin's synthesis of Hexagonal, Onion, and BCE architectures into a concentric-ring model governed by the Dependency Rule. |
| **Efferent Coupling (Ce)** | The number of external modules a given module depends on. High Ce indicates the module is volatile and change-prone. |
| **Event Store** | A persistence mechanism optimized for append-only storage and sequential retrieval of domain events. |
| **Instability (I)** | A metric computed as Ce / (Ca + Ce). I = 0 is maximally stable; I = 1 is maximally unstable. |
| **Main Sequence** | The ideal zone on the Abstractness vs. Instability graph where A + I ≈ 1. Modules far from this line are in the Zone of Pain or Zone of Uselessness. |
| **Onion Architecture** | Jeffrey Palermo's architecture style (2008) that layers domain, domain services, and application services concentrically, with infrastructure at the outermost ring. |
| **Primary Port (Driving Port)** | An interface through which external actors drive the application (e.g., HTTP controller calls a use case). |
| **Projection** | A read-model representation derived from domain events. Projections are disposable and rebuildable from the event stream. |
| **Saga** | A coordination pattern for managing distributed transactions across services using a sequence of local transactions and compensating actions. |
| **Secondary Port (Driven Port)** | An interface through which the application reaches out to external systems (e.g., database, message broker, third-party API). |
| **Sidecar** | A co-located process that extends or enhances a service without modifying its code (e.g., Envoy proxy, logging agent). |
| **Temporal Query** | A query that reconstructs system state at a specific point in time by replaying events up to that timestamp. |
| **Idempotency** | A property of an operation where applying it multiple times produces the same result as applying it once. Critical for event replay and at-least-once delivery guarantees. |
