...
fase: "6 — Backend & Frontend"
modulo: "06.3"
titolo: "Mobile & Cross-Platform Engineering"
versione: "Flutter 3.38, React Native 0.79 (New Architecture), KMP / Kotlin 2.2, Swift 6, Compose Multiplatform 1.8"
livello: "Advanced"
prerequisiti:
  - "Proficiency in at least one of Dart, Kotlin, Swift, or TypeScript"
  - "Mobile app lifecycle basics: foreground, background, terminated states"
  - "REST/GraphQL API consumption and JSON serialisation"
  - "Understanding of platform sandboxing, permissions, and secure storage"
obiettivi:
  - "Architect a cross-platform application selecting the optimal framework (Flutter, React Native, KMP) based on team skills, performance requirements, and platform-API surface"
  - "Implement offline-first data synchronisation with conflict resolution using SQLite, optimistic writes, and background sync schedulers"
  - "Profile rendering performance on both iOS and Android, achieving consistent 60 FPS scrolling in lists of 10,000+ items"
  - "Configure a mobile CI/CD pipeline (GitHub Actions or Fastlane) covering lint, unit tests, build, screenshot tests, and automated store deployment"
  - "Harden a mobile application with certificate pinning, secure token storage (Keychain/Keystore), obfuscation, and root/jailbreak detection"
tag: [mobile, flutter, react-native, kotlin-multiplatform, swift, compose-multiplatform, offline-first, push-notifications, deep-linking, mobile-security]
---
...
>
> 1. Compare native (Swift/Kotlin), Flutter (Dart/Impeller), React Native (JSI/Fabric), and KMP (shared Kotlin + native UI) on startup time, render pipeline, and native-API access — and defend a framework choice for a given product brief.
> 2. Design and implement an offline-first architecture with local-first writes, background queue sync, last-writer-wins or CRDT-based conflict resolution, and a user-visible sync-status indicator.
> 3. Integrate push notifications (APNs + FCM) end-to-end: permission request, token registration, server dispatch, foreground display, and deep-link navigation on tap.
> 4. Detect and fix common mobile performance issues — jank, excessive widget rebuilds, oversized images, main-thread blocking — using platform profilers (Xcode Instruments, Android Studio Profiler, Flutter DevTools).
> 5. Build a reproducible mobile CI/CD pipeline that gates merges on lint + unit-test + build success, runs screenshot regression on every PR, and deploys to TestFlight / Play Internal Track on tag push.
...
### Exercise A: Cross-Platform Performance Showdown

1. Build the **same** application — a scrollable product catalog with images,
   search filtering, and pull-to-refresh — in three frameworks: Flutter,
   React Native (New Architecture), and KMP with Compose Multiplatform.
2. Populate each with 10,000 items (use a local JSON fixture, not a network
   call, to isolate rendering performance).
3. On a physical mid-range Android device (e.g., Pixel 6a) and an iPhone 13
   mini, measure for each framework:
   - Cold-start time (process launch → first frame rendered)
   - Scroll FPS during a fast fling through the list (use `adb shell dumpsys gfxinfo` on Android, Instruments on iOS)
   - Peak memory consumption during scroll
   - APK/IPA size (release build)
4. Record results in a comparison table. Identify which framework wins on
   each metric and which metric has the largest variance.
5. For the worst-performing framework on scroll FPS, apply framework-specific
   optimisations (e.g., `const` widgets in Flutter, `React.memo` +
   `getItemLayout` in React Native, `LazyColumn` keys in Compose). Re-measure.
6. **Deliverable:** the comparison table, the optimisation diff, and a
   one-paragraph analysis of when each framework is the right choice based on
   these numbers.

### Exercise B: Offline-First Sync with Conflict Resolution

1. Build a shared shopping list app (Flutter or React Native) backed by
   SQLite locally and a REST API remotely.
2. Implement **optimistic local writes**: when the user adds or edits an
   item, persist to SQLite immediately and enqueue a sync operation.
3. Implement a **background sync worker** (WorkManager on Android,
   BGTaskScheduler on iOS) that retries queued operations with exponential
   backoff.
4. Simulate a **conflict**: edit the same item on two devices while offline,
   then bring both online. Implement a last-writer-wins strategy with a
   vector clock timestamp, and show a conflict-resolution UI when timestamps
   are within 1 second of each other.
5. Add a **sync-status indicator** (synced / pending / error) next to each
   list item.
6. **Deliverable:** a screen recording showing the offline-edit → sync →
   conflict-resolution flow, plus the SQLite schema and sync-queue data model.

### Exercise C: Native Bridge Performance Boundary

1. In a React Native project (New Architecture enabled), create a
   **TurboModule** that performs a CPU-intensive operation (e.g., SHA-256
   hash of a 1 MB buffer) in native code (Kotlin/Swift).
2. Call the TurboModule from JavaScript 1,000 times in a loop. Measure:
   - Total wall time
   - Per-call JSI overhead (compare with a pure-JS SHA-256 implementation)
3. Now create the equivalent in Flutter using a **platform channel**
   (MethodChannel) and measure the same metrics.
4. Finally, implement a Flutter **FFI** binding to a C SHA-256 library.
   Measure again — FFI should eliminate the platform-channel serialisation
   overhead.
5. Tabulate: JS-only vs. TurboModule vs. MethodChannel vs. FFI. Identify
   the crossover point (payload size) where going native becomes worth the
   bridge cost.
6. **Deliverable:** benchmark results table and a decision flowchart for
   "when to use a native bridge."

### Exercise D: Mobile CI/CD Pipeline with Screenshot Regression

1. Set up a GitHub Actions workflow for a Flutter or React Native project
   with the following stages: lint → unit tests → build (Android + iOS) →
   screenshot tests → deploy.
2. For screenshot regression testing, use Maestro (cross-platform) or
   Paparazzi (Android-only). Capture screenshots of 5 key screens.
3. Store reference screenshots in the repository. On each PR, the workflow
   compares current screenshots against references and fails if pixel diff
   exceeds a threshold (e.g., 0.1%).
4. Automate version bumping: on merge to `main`, bump the version code
   using a conventional-commits-based script.
5. On tag push (`v*`), deploy the Android build to Play Store Internal Track
   (via Fastlane or Gradle Play Publisher) and the iOS build to TestFlight
   (via Fastlane).
6. **Deliverable:** the complete GitHub Actions workflow YAML, Fastlane
   configuration, and screenshots of a passing and a failing screenshot-diff
   check.

### Exercise E: Mobile Security Hardening Audit

1. Take an existing mobile application (your own project or a sample app)
   and perform a security audit against the OWASP Mobile Top 10 (2024).
2. Implement **certificate pinning** using OkHttp CertificatePinner
   (Android) or TrustKit (iOS). Verify with mitmproxy: the app should refuse
   to connect through the proxy.
3. Migrate token storage from SharedPreferences/UserDefaults to
   Android Keystore / iOS Keychain. Verify tokens are not visible in a
   device backup.
4. Enable code obfuscation: R8 with ProGuard rules on Android,
   `--obfuscate` flag on Flutter, Hermes bytecode on React Native.
   Decompile the release build and verify class/function names are mangled.
5. Add root/jailbreak detection (e.g., `rootbeer` on Android,
   `IOSSecuritySuite` on iOS). On detection, show a warning but allow
   continued use (graceful degradation, not hard block).
6. Configure Android Network Security Config to block all cleartext traffic.
   Verify with `adb shell run-as <package> curl http://...` — the request
   should fail.
7. **Deliverable:** a checklist mapping each OWASP Mobile Top 10 item to the
   mitigation applied (or "N/A" with justification), plus the mitmproxy and
...
- Windmill, Eric. *Flutter in Action*. Manning, 2020.
- Rappl, Florian. *React Native in Action*. Manning, 2019.
- Moskala, Marcin. *Kotlin Coroutines: Deep Dive*. Kt. Academy, 2022.
- Neuburg, Matt. *iOS Programming Fundamentals with Swift*. O'Reilly, 2023.
- Phillips, Bill; Stewart, Chris; Marsicano, Kristin. *Android Programming: The Big Nerd Ranch Guide*. 5th ed., Big Nerd Ranch, 2022.
...
- Fastlane — mobile CI/CD automation. https://fastlane.tools/
- Maestro — mobile UI testing. https://maestro.mobile.dev/
- KMP production case studies — Android Developers. https://developer.android.com/kotlin/multiplatform
...
| Topic | Module | Link |
|---|---|---|
| Backend concurrency (Kotlin coroutines context) | 06 — Backend & Frontend | `01_Backend_Concurrency.md` |
| Frontend engineering (React, state management) | 06 — Backend & Frontend | `02_Frontend_Engineering.md` |
| OS internals: processes, memory, sandboxing | 01 — Foundations | `../01_Foundations/01_OS_Internals_Processes_Memory.md` |
| Networking: TCP/IP, HTTP, TLS | 01 — Foundations | `../01_Foundations/02_Networking_TCP_IP_Deep_Dive.md` |
| Site Reliability Engineering (SRE, incident response) | 05 — DevOps & Cloud Native | `../05_DevOps_Cloud_Native/03_Site_Reliability_Engineering.md` |
| Distributed system patterns (offline sync, consistency) | 02 — Architecture & Design | `../02_Architecture_Design/02_Distributed_System_Patterns.md` |
...
| Term | Definition |
|---|---|
| **Bridgeless architecture** | React Native's New Architecture design that replaces the asynchronous JSON bridge with JSI, enabling synchronous, direct communication between JavaScript and native code. |
| **Certificate pinning** | A mobile security technique that restricts which TLS certificates an app trusts, preventing man-in-the-middle attacks even when a rogue CA certificate is installed on the device. |
| **Compose Multiplatform** | JetBrains' extension of Jetpack Compose to iOS, desktop, and web, allowing shared declarative UI code from a single Kotlin codebase. |
| **Dart isolate** | A Dart concurrency primitive — an independent execution context with its own memory heap, communicating via message passing; used in Flutter for CPU-intensive background work. |
| **Deep linking** | The ability to navigate a user directly to a specific screen within a mobile app from an external URL, push notification, or another app. |
| **Fabric** | React Native's new rendering system that replaces the legacy UI Manager, using a shared C++ core for cross-platform rendering with concurrent and synchronous layout capabilities. |
| **Impeller** | Flutter's rendering engine (replacing Skia for mobile), which pre-compiles all shaders at build time, eliminating first-run shader-compilation jank and delivering consistent frame rates. |
| **JSI (JavaScript Interface)** | The C++ layer in React Native's New Architecture that replaces the asynchronous bridge, enabling direct, synchronous function calls between JavaScript and native code. |
| **KMP (Kotlin Multiplatform)** | JetBrains' technology for sharing Kotlin code (business logic, networking, data) across Android, iOS, desktop, web, and server targets from a single codebase. |
| **Offline-first** | An architecture pattern where the application reads from and writes to a local database (SQLite, Realm) as the primary store, syncing with a remote server asynchronously in the background. |
| **Platform channel** | Flutter's mechanism for invoking native platform code (Kotlin/Swift) from Dart via serialised method calls; higher overhead than FFI but simpler to use for standard platform APIs. |
| **TurboModule** | A React Native native module backed by JSI that loads lazily (only when first accessed) and communicates synchronously with JavaScript, replacing the legacy NativeModules system. |
| **Widget rebuild** | In Flutter, the process of reconstructing the widget tree when state changes; excessive rebuilds (rebuilding widgets that did not change) are the primary cause of UI jank in Flutter apps. |
| **WorkManager** | Android's recommended API for deferrable, guaranteed background work (e.g., sync, upload) that survives process death and respects battery/network constraints. |
