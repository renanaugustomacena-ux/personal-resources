...
modulo: "06.1"
titolo: "Backend Concurrency Models"
versione: "Go 1.23, Python 3.14 (free-threading), Java 24 (JEP 491), Node.js 24 LTS, Erlang/OTP 27"
livello: "Advanced"
prerequisiti:
  - "Operating-system fundamentals: processes, threads, virtual memory (Module 01.1)"
  - "Networking basics: TCP, HTTP, connection lifecycle (Module 01.2)"
  - "Proficiency in at least one of Go, Java, Python, or JavaScript"
  - "Understanding of Big-O complexity and basic data structures"
obiettivi:
  - "Compare thread-per-request, event-loop, goroutine, virtual-thread, and actor concurrency models on throughput, latency, and resource cost"
  - "Implement structured concurrency scopes that guarantee no orphan tasks and correct cancellation propagation"
  - "Design backpressure-aware data pipelines that bound memory under sustained producer-consumer rate mismatch"
  - "Detect and resolve concurrency bugs — data races, deadlocks, live-locks — using runtime tooling (Go race detector, Java Flight Recorder, ThreadSanitizer)"
  - "Benchmark concurrent workloads end-to-end and interpret scheduling traces to identify bottlenecks"
tag: [concurrency, parallelism, goroutines, virtual-threads, async-await, actor-model, CSP, backpressure, structured-concurrency, event-loop]
---
...
>
> 1. Explain the trade-offs between OS threads, green threads, coroutines, virtual threads, and actors — and select the right model for a given workload profile.
> 2. Apply structured concurrency (errgroup, TaskGroup, StructuredTaskScope) to eliminate goroutine/thread leaks in production code.
> 3. Build bounded producer-consumer pipelines with explicit backpressure, measuring producer stall time, consumer utilisation, and tail latency.
> 4. Use lock-free primitives (CAS, atomic operations) and understand their memory-ordering guarantees across x86 and ARM.
> 5. Profile and trace concurrent programs with `runtime/trace` (Go), JFR/async-profiler (Java), py-spy (Python), and Clinic.js (Node.js), turning flame graphs into actionable optimisations.
...
### Exercise A: Goroutine Leak Detection

1. Write a Go HTTP server that spawns a goroutine per request to call a slow
   upstream service (simulate with `time.Sleep(30s)`). Do **not** wire a
   context-cancellation path.
2. Hit the server with 500 requests, then immediately shut down the client
   connections (TCP RST). Observe goroutine count via `runtime.NumGoroutine()`
   and the `/debug/pprof/goroutine` endpoint.
3. Identify the leak: goroutines blocked on the upstream call with no
   cancellation signal.
4. Fix: propagate `http.Request.Context()` into the upstream call and use
   `context.WithTimeout`. Re-run and confirm goroutine count returns to
   baseline within seconds.
5. Add a Prometheus gauge (`go_goroutines`) and a Grafana alert rule that
   fires when the goroutine count exceeds 2x the steady-state baseline for
   more than 60 seconds.
6. **Deliverable:** a before/after pprof goroutine dump and a one-paragraph
   write-up explaining the root cause and the fix.

### Exercise B: Async Pipeline with Backpressure (Python 3.14 free-threaded)

1. Build a three-stage pipeline: **Producer** (reads lines from a 1 GB log
   file) → **Transformer** (parses JSON, extracts fields) → **Writer**
   (batch-inserts into SQLite).
2. Implement first with `asyncio.Queue(maxsize=1000)` between stages.
   Measure: producer stall time, transformer throughput, writer batch
   latency, peak RSS.
3. Now build the same pipeline using Python 3.14 free-threaded mode
   (`python3.14t`) with `threading.Thread` and `queue.Queue(maxsize=1000)`.
   Measure the same metrics.
4. Compare: which mode wins on throughput? Which wins on peak memory? Why?
5. Introduce a poisoned JSON line mid-file. Verify the pipeline does **not**
   silently drop it — the error must surface with line number and stage name.
6. **Deliverable:** a comparison table (async vs free-threaded) with
   throughput (lines/s), p99 latency (ms), and peak RSS (MB).

### Exercise C: Virtual Thread Pinning Hunt (Java 24)

1. Write a Java 24 HTTP server using `Executors.newVirtualThreadPerTaskExecutor()`.
   Each request acquires a `ReentrantLock`, calls `Thread.sleep(100)` inside
   the critical section, and returns.
2. Run with `-Djdk.tracePinnedThreads=full`. Observe: **no** pinning
   (JEP 491 fixed `synchronized` pinning, but `ReentrantLock` never pinned).
3. Replace `ReentrantLock` with a `synchronized` block and repeat on JDK 21
   vs JDK 24. On JDK 21 you should see pinning warnings; on JDK 24 you
   should not.
4. Introduce a native JNI call inside the `synchronized` block. Observe
   that JNI still causes pinning even on JDK 24.
5. Measure throughput (requests/s) under 10,000 concurrent virtual threads
   for each variant. Record the carrier-thread utilisation via JFR.
6. **Deliverable:** JFR recording with annotated screenshots of the
   carrier-thread timeline, plus a markdown summary of when pinning still
   occurs in JDK 24.

### Exercise D: Event-Loop Starvation (Node.js)

1. Create an Express server with two endpoints: `/fast` (returns "ok") and
   `/slow` (computes a 10-million-iteration Fibonacci synchronously).
2. Measure `/fast` p99 latency while `/slow` is being hit concurrently.
   Observe event-loop starvation.
3. Fix option 1: move Fibonacci to a `worker_threads` Worker. Measure again.
4. Fix option 2: break Fibonacci into chunks yielding via `setImmediate()`
   after every 100,000 iterations. Measure again.
5. Use `clinic doctor` and `clinic flame` to visualise the starvation
   before and after each fix.
6. **Deliverable:** Clinic.js HTML reports (before, worker fix, chunked fix)
   with a one-paragraph analysis of each.

### Exercise E: Actor Supervision Tree (Erlang/Elixir)

1. Build an Elixir `DynamicSupervisor` managing a pool of `WorkerGenServer`
   processes. Each worker accepts a `{:process, payload}` message and may
   crash on malformed payloads.
2. Configure `:one_for_one` restart strategy with `max_restarts: 5` in 10
   seconds.
3. Send 100 messages, 10% deliberately malformed. Verify: crashed workers
   restart, non-crashed workers are unaffected, no message is lost (use a
   dead-letter queue for failed messages).
4. Switch to `:rest_for_all` strategy. Observe the difference in restart
   behaviour.
5. Add telemetry: track restart count per worker and total message throughput
   via `:telemetry` and LiveDashboard.
6. **Deliverable:** a supervision tree diagram (ASCII or Mermaid) and a log
   showing crash → restart → successful redelivery.
...
- Butcher, Paul. *Seven Concurrency Models in Seven Weeks*. Pragmatic Bookshelf, 2014.
- Goetz, Brian et al. *Java Concurrency in Practice*. Addison-Wesley, 2006.
- Armstrong, Joe. *Programming Erlang: Software for a Concurrent World*. 2nd ed., Pragmatic Bookshelf, 2013.
- Kleppmann, Martin. *Designing Data-Intensive Applications*. O'Reilly, 2017. (Chapters 8-9 on distributed concurrency.)
...
- Sutter, Herb. "The Free Lunch Is Over: A Fundamental Turn Toward Concurrency in Software." *Dr. Dobb's Journal* 30.3 (2005).
...
- Sourcegraph `conc` — structured concurrency for Go. https://github.com/sourcegraph/conc
- Nystrom, Bob. "What Color is Your Function?" (2015). https://journal.stuffwithstuff.com/2015/02/01/what-color-is-your-function/
- Elizarov, Roman. "Structured Concurrency." Kotlin design document. https://kotlinlang.org/docs/coroutines-basics.html
- Erlang/OTP documentation — Supervisor Behaviour. https://www.erlang.org/doc/design_principles/sup_princ
...
| Topic | Module | Link |
|---|---|---|
| OS processes, threads, context switching | 01 — Foundations | `../01_Foundations/01_OS_Internals_Processes_Memory.md` |
| TCP, HTTP, connection lifecycle | 01 — Foundations | `../01_Foundations/02_Networking_TCP_IP_Deep_Dive.md` |
| Distributed system patterns (consensus, replication) | 02 — Architecture & Design | `../02_Architecture_Design/02_Distributed_System_Patterns.md` |
| API design and evolution | 02 — Architecture & Design | `../02_Architecture_Design/02_c_API_Design_Evolution.md` |
| Frontend engineering (Web Workers, service workers) | 06 — Backend & Frontend | `02_Frontend_Engineering.md` |
| Mobile concurrency (Kotlin coroutines, Dart isolates) | 06 — Backend & Frontend | `03_Mobile_CrossPlatform.md` |
...
| Term | Definition |
|---|---|
| **Backpressure** | Flow-control mechanism where a consumer signals a producer to slow down when the consumer cannot keep up with the ingestion rate. |
| **Carrier thread** | An OS-level platform thread that a Java virtual thread is mounted on during execution; virtual threads unmount from carriers during blocking I/O. |
| **CAS (Compare-And-Swap)** | An atomic CPU instruction that updates a memory location only if it still holds an expected value; the foundation of lock-free algorithms. |
| **Coroutine** | A generalised subroutine that can suspend execution at defined yield points and resume later, enabling cooperative concurrency without OS-thread overhead. |
| **CSP (Communicating Sequential Processes)** | A concurrency model (Hoare, 1978) where independent processes synchronise by sending messages over typed channels rather than sharing memory. |
| **Event loop** | A single-threaded execution model (Node.js, browser JS) that dispatches I/O callbacks from a queue, achieving high concurrency for I/O-bound work without parallelism. |
| **GIL (Global Interpreter Lock)** | A mutex in CPython that allows only one thread to execute Python bytecode at a time; Python 3.13+ offers an experimental free-threaded build without the GIL. |
| **Goroutine** | A lightweight, user-space thread managed by the Go runtime's M:N scheduler; creation cost is ~2 KB of stack (growable), orders of magnitude cheaper than an OS thread. |
| **Pinning** | The condition where a Java virtual thread cannot unmount from its carrier thread — typically caused by native JNI code — blocking the carrier for the duration. |
| **Structured concurrency** | A programming discipline that scopes concurrent tasks within a lexical block, guaranteeing that all child tasks complete (or are cancelled) before the scope exits. |
| **Virtual thread** | A JVM-managed lightweight thread (Project Loom, JEP 444) that maps many-to-few onto platform threads, enabling thread-per-request at scale without thread-pool tuning. |
| **Work stealing** | A scheduling strategy where an idle processor dequeues tasks from another processor's run queue, balancing load dynamically across cores. |
| **Deadlock** | A state where two or more tasks each hold a resource the other needs, and none can proceed; prevented by consistent lock ordering or lock-free designs. |
| **Memory ordering** | The guarantees a CPU/compiler provides about the visibility order of memory writes across threads; critical for correct lock-free code (e.g., acquire/release semantics). |
| **Actor model** | A concurrency model (Hewitt, 1973) where isolated actors communicate exclusively via asynchronous messages; each actor processes one message at a time, eliminating shared-state races. |
