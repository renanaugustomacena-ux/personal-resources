...
modulo: "06.2"
titolo: "Frontend Engineering"
versione: "React 19, Next.js 15, Vite 6 / Rolldown, TypeScript 5.7, Astro 5"
livello: "Advanced"
prerequisiti:
  - "Solid JavaScript/TypeScript (ES2024+, strict mode, generics)"
  - "HTML semantics and CSS layout (flexbox, grid, custom properties)"
  - "HTTP request lifecycle and browser networking (Module 01.2)"
  - "Basic React or equivalent component-model experience"
obiettivi:
  - "Select and justify a rendering strategy (CSR, SSR, SSG, ISR, streaming SSR) given SEO, interactivity, and infrastructure constraints"
  - "Diagnose and fix hydration mismatches, measuring TTI improvement with React DevTools and Performance API"
  - "Reduce JavaScript bundle size below 150 KB gzipped for landing pages using code splitting, tree shaking, and dynamic imports"
  - "Achieve passing Core Web Vitals (LCP < 2.5 s, INP < 200 ms, CLS < 0.1) on a production page and verify with Lighthouse CI"
  - "Architect a micro-frontend system with Module Federation 2.0, defining shared dependency contracts and independent deployment pipelines"
tag: [frontend, react, server-components, hydration, bundle-optimization, web-vitals, signals, micro-frontends, accessibility, design-systems]
---
...
>
> 1. Map each rendering strategy (CSR, SSR, SSG, ISR, streaming SSR, React Server Components) to its impact on FCP, LCP, TTI, and SEO — and choose accordingly.
> 2. Implement React Server Components with streaming Suspense boundaries, reducing client JS bundle by 40%+ compared to a full-CSR baseline.
> 3. Perform a bundle audit with `source-map-explorer` or `vite-bundle-visualizer`, identify the top three oversized dependencies, and replace or code-split them.
> 4. Build an accessible, theme-aware design system grounded in CSS custom-property tokens, passing WCAG 2.2 AA automated and manual checks.
> 5. Instrument a production page with CrUX, Lighthouse CI, and web-vitals library, setting performance budgets that block deployment on regression.
...
### Exercise A: Bundle Forensics and Diet

1. Clone a medium-complexity React application (e.g., a dashboard with charts,
   date pickers, rich-text editor).
2. Run `npx source-map-explorer build/static/js/*.js` (or
   `npx vite-bundle-visualizer` for Vite projects). Screenshot the treemap.
3. Identify the top 3 dependencies by size. For each one, determine:
   - Is this dependency actually used? (search for imports)
   - Is there a lighter alternative? (e.g., `date-fns` instead of `moment`,
     `lightweight-charts` instead of `chart.js`)
   - Can it be code-split behind `React.lazy()` + `<Suspense>`?
4. Apply at least two of the above strategies. Re-run the bundle analysis.
5. Record before/after: total JS size (gzipped), largest chunk size, number
   of chunks.
6. **Deliverable:** two treemap screenshots (before/after) and a markdown
   table showing size reductions per dependency.

### Exercise B: Hydration Debugging and Streaming SSR

1. Create a Next.js 15 App Router page with three data-fetching components:
   `<UserProfile>` (fast, 50 ms), `<ActivityFeed>` (medium, 500 ms),
   `<Recommendations>` (slow, 2 s).
2. Render all three without Suspense. Measure LCP with Lighthouse — the page
   is blocked by the slowest component.
3. Wrap `<ActivityFeed>` and `<Recommendations>` in `<Suspense>` boundaries
   with skeleton fallbacks. Enable streaming SSR.
4. Re-measure LCP. It should now reflect `<UserProfile>` (the first
   meaningful paint), not `<Recommendations>`.
5. Open Chrome DevTools → Performance → record a page load. Identify the
   hydration phase. Note which components hydrate eagerly vs. after streaming
   completes.
6. **Deliverable:** Lighthouse reports (before/after) and a timeline
   screenshot annotating FCP, LCP, and hydration boundaries.

### Exercise C: Core Web Vitals Performance Budget CI Gate

1. Set up Lighthouse CI in a GitHub Actions workflow for a production or
   staging URL.
2. Define performance budgets: LCP < 2.5 s, INP < 200 ms, CLS < 0.1,
   total JS < 150 KB gzipped.
3. Configure the workflow to **fail the PR check** if any budget is
   exceeded.
4. Introduce a deliberate regression (add a 500 KB uncompressed library to
   the critical path). Verify the CI check fails with a clear message.
5. Fix the regression (lazy-load the library). Verify the CI check passes.
6. **Deliverable:** the GitHub Actions workflow YAML and screenshots of a
   failing and passing check.

### Exercise D: Micro-Frontend with Module Federation 2.0

1. Create two independent Vite/Rspack applications: a **Shell** (navigation,
   authentication) and a **Remote** (product catalog page).
2. Configure Module Federation 2.0: the Shell consumes the Remote's
   `ProductList` component at runtime.
3. Ensure shared dependencies (`react`, `react-dom`) are singletons with
   version pinning to prevent duplicate React instances.
4. Deploy both applications to different origins. Verify: the Shell loads the
   Remote's component cross-origin, navigation between shell and remote
   routes is seamless, and a version mismatch in `react` triggers a clear
   error (not a silent hydration failure).
5. Add independent deployment: update the Remote's `ProductList` without
   redeploying the Shell. Verify the change is picked up on next navigation.
6. **Deliverable:** a Mermaid architecture diagram showing the federation
   graph, plus a screen recording of the cross-origin loading.

### Exercise E: Accessibility Audit and Remediation

1. Select a web application page with at least a form, a modal, and a data
   table.
2. Run axe-core (`npx @axe-core/cli <url>`). Record all Critical and
   Serious findings.
3. Test keyboard navigation: Tab through all interactive elements, confirm
   focus is visible, confirm Escape closes modals, confirm Enter submits
   forms.
4. Test with a screen reader (NVDA on Windows, VoiceOver on macOS). Record
   any announcements that are missing, misleading, or duplicated.
5. Fix all Critical and Serious axe findings. Add `aria-labelledby`,
   `role`, focus trapping, and `prefers-reduced-motion` media queries as
   needed.
6. Re-run axe and re-test with keyboard and screen reader. Verify zero
   Critical/Serious findings remain.
7. **Deliverable:** axe reports (before/after), a list of manual keyboard/SR
   findings, and the code diff of fixes applied.
...
- Archibald, Jake. *JavaScript: The Definitive Guide*. 7th ed., O'Reilly, 2020. (Chapters on async, Workers, and streams.)
- Rappl, Florian. *The Art of Micro Frontends*. Packt, 2021.
- Dodson, Rob & Surma. *Web Performance in Action*. Manning, 2024.
- Faulkner, Steve et al. *Using ARIA*. W3C Working Group Note (ongoing).
...
- web.dev — Core Web Vitals. https://web.dev/vitals/
- Patterns.dev — rendering and performance patterns. https://www.patterns.dev/
- Astro documentation — Islands Architecture. https://docs.astro.build/en/concepts/islands/
- SolidJS — fine-grained reactivity. https://www.solidjs.com/tutorial/introduction_signals
...
| Topic | Module | Link |
|---|---|---|
| HTTP, QUIC, and modern protocols | 01 — Foundations | `../01_Foundations/02_a_Modern_Protocols_HTTP_QUIC.md` |
| Backend concurrency (event loop, async I/O) | 06 — Backend & Frontend | `01_Backend_Concurrency.md` |
| Mobile and cross-platform (React Native, Flutter web) | 06 — Backend & Frontend | `03_Mobile_CrossPlatform.md` |
| API design and evolution (REST, GraphQL, gRPC) | 02 — Architecture & Design | `../02_Architecture_Design/02_c_API_Design_Evolution.md` |
| Observability, SLIs, SLOs, SLAs | 05 — DevOps & Cloud Native | `../05_DevOps_Cloud_Native/05_Observability_SLI_SLO_SLA.md` |
| Container internals (Docker, deployment targets) | 05 — DevOps & Cloud Native | `../05_DevOps_Cloud_Native/01_Container_Internals.md` |
...
| Term | Definition |
|---|---|
| **Bundle splitting** | The technique of dividing application JavaScript into multiple chunks so the browser only downloads code required for the current route or interaction. |
| **CLS (Cumulative Layout Shift)** | A Core Web Vital measuring unexpected visual movement of page elements during load; a good score is below 0.1. |
| **CSR (Client-Side Rendering)** | A rendering strategy where the server sends a minimal HTML shell and JavaScript constructs the entire UI on the client. |
| **Hydration** | The process by which client-side JavaScript attaches event listeners and state to server-rendered HTML, making it interactive. |
| **INP (Interaction to Next Paint)** | A Core Web Vital measuring the latency between a user interaction (click, tap, key press) and the next visual update; a good score is below 200 ms. |
| **Island architecture** | A rendering pattern (Astro, Fresh) where most of the page is static HTML and only isolated interactive "islands" ship JavaScript. |
| **LCP (Largest Contentful Paint)** | A Core Web Vital measuring how quickly the largest visible element (image, heading, video) renders; a good score is below 2.5 seconds. |
| **Module Federation** | A Webpack/Rspack/Vite feature that allows independently deployed applications to share modules at runtime, enabling micro-frontend architectures. |
| **RSC (React Server Components)** | React components that execute on the server and stream a serialised component tree (React Flight protocol) to the client, shipping zero client JS for server-only components. |
| **Signals** | A fine-grained reactivity primitive (SolidJS, Angular Signals, Preact Signals) that tracks dependencies at the individual-value level, avoiding virtual-DOM diffing overhead. |
| **SSG (Static Site Generation)** | A rendering strategy where HTML pages are generated at build time and served as static files, providing the fastest possible TTFB. |
| **SSR (Server-Side Rendering)** | A rendering strategy where the server generates full HTML for each request; the browser displays content immediately and JavaScript hydrates afterward. |
| **Streaming SSR** | An SSR variant where the server sends HTML in chunks as data becomes available, using HTTP chunked transfer encoding; React integrates this via `<Suspense>` boundaries. |
| **Tree shaking** | A dead-code elimination technique used by bundlers (Rollup, esbuild, Rolldown) to remove unused exports from the final bundle, relying on ES module static analysis. |
| **TTI (Time to Interactive)** | The point at which the page is fully rendered and the main thread is idle enough to respond to user input within 50 ms. |
