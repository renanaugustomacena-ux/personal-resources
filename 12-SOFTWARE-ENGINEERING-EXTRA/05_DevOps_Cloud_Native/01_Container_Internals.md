...
modulo: "5.1"
titolo: "Container Internals (Linux Plumbing)"
versione: "Docker 27.x, runc 1.2.x, cgroups v2, OCI Runtime Spec 1.2"
livello: "Advanced"
prerequisiti:
  - "Solid Linux administration (shell, process model, filesystems)"
  - "Understanding of syscalls and kernel–userspace boundary"
  - "Familiarity with networking fundamentals (IP, TCP, DNS)"
  - "Basic experience running containers with Docker or Podman"
obiettivi:
  - "Explain how namespaces, cgroups, and capabilities compose a container"
  - "Build and optimise multi-stage container images below 50 MB"
  - "Debug namespace isolation failures using nsenter and /proc inspection"
  - "Configure seccomp profiles and capability dropping for production hardening"
  - "Compare OCI-compliant runtimes (runc, crun, gVisor, Kata) and choose per workload"
tag: [containers, linux, namespaces, cgroups, oci, docker, podman, seccomp, rootless, security]
---
...
> - Trace the kernel primitives (namespaces, cgroups v2, capabilities) that constitute a Linux container and explain how each restricts visibility, resource usage, or syscall access.
> - Build minimal, multi-stage OCI images using Docker or Buildah and analyse layer composition with `dive`.
> - Debug container isolation problems by inspecting `/proc/<pid>/ns/`, using `nsenter`, and reading cgroup controller files.
> - Write and apply custom seccomp profiles and drop unnecessary capabilities to enforce least-privilege at the syscall level.
> - Evaluate OCI runtime alternatives (runc, crun, gVisor, Kata Containers) and justify a selection based on security, performance, and compliance requirements.
...
### Exercise 1: Build a Minimal Multi-Stage Image

Write a multi-stage Dockerfile for a Go HTTP server (or any compiled language). The final image must:
- Use `scratch` or `distroless` as the final base.
- Be under 15 MB compressed.
- Run as a non-root user (numeric UID).
- Pass `docker scout cves` or `trivy image` with zero critical/high CVEs.

Analyse the layer structure with `dive` and document each layer's purpose.

### Exercise 2: Debug Namespace Isolation

1. Launch two containers on the same host: one with `--pid=host` and one with default PID namespace.
2. From inside each container, list processes (`ps aux`) and explain the difference.
3. Use `nsenter -t <PID> -n` from the host to enter a container's network namespace and run `ss -tlnp`. Verify the container's listening ports are visible.
4. Demonstrate that a container in the default PID namespace cannot send signals to host processes or processes in sibling containers.

### Exercise 3: cgroups v2 Resource Enforcement

1. Run a container with `--memory=128m --cpus=0.5`.
2. Inside the container, run a memory stress tool (`stress-ng --vm 1 --vm-bytes 200M`) and observe the OOM kill.
3. Locate the cgroup directory under `/sys/fs/cgroup/` on the host and read `memory.max`, `cpu.max`, and `memory.events`.
4. Modify `memory.max` via the cgroup filesystem directly and verify the container picks up the new limit.

### Exercise 4: Custom Seccomp Profile

1. Generate a default seccomp profile using `docker run --security-opt seccomp=unconfined` and record syscalls with `strace`.
2. Create a allowlist-only seccomp profile JSON that permits only the syscalls your application actually uses.
3. Run the container with `--security-opt seccomp=custom-profile.json` and verify it works.
4. Remove one required syscall from the profile and document the resulting failure mode.

### Exercise 5: Runtime Comparison

Deploy the same OCI image on three different runtimes:
1. **runc** (default) — measure startup time and memory overhead.
2. **gVisor (runsc)** — repeat measurements, attempt a `mount` syscall from inside and document the interception.
3. **Kata Containers** — repeat measurements, verify that `/proc/cpuinfo` shows a virtual CPU.

Produce a comparison table covering: startup latency, memory overhead, syscall compatibility, and isolation boundary (shared kernel vs. microVM).
...
|:------|:--------|:----------|:-----|:------|
| *Container Security: Fundamental Technology Concepts that Protect Containerized Applications*, 2nd ed. | Liz Rice | O'Reilly Media | 2025 | Definitive treatment of namespaces, cgroups, capabilities, and seccomp from a security perspective |
| *Docker Deep Dive* | Nigel Poulton | Independently published | 2024 | Practical deep dive into Docker engine internals and image management |
| *Linux Containers and Virtualisation*, 2nd ed. | Shashank Mohan Jain | Apress | 2023 | Kernel-level coverage of namespaces, cgroups, and overlay filesystems |

### Online Resources

| Resource | URL | Retrieved |
|:---------|:----|:----------|
| OCI Runtime Specification (Linux config) | https://github.com/opencontainers/runtime-spec/blob/main/config-linux.md | 2026-05-29 |
| OCI Image Specification | https://github.com/opencontainers/image-spec | 2026-05-29 |
| iximiuz Labs — OCI Containers Explained | https://labs.iximiuz.com/tutorials/oci-containers-explained | 2026-05-29 |
| Docker Documentation — Storage Drivers | https://docs.docker.com/storage/storagedriver/ | 2026-05-29 |
| man7.org — namespaces(7) | https://man7.org/linux/man-pages/man7/namespaces.7.html | 2026-05-29 |
| man7.org — cgroups(7) | https://man7.org/linux/man-pages/man7/cgroups.7.html | 2026-05-29 |
| Podman Quadlet Documentation | https://docs.podman.io/en/latest/markdown/podman-systemd.unit.5.html | 2026-05-29 |
| gVisor Architecture Guide | https://gvisor.dev/docs/architecture_guide/ | 2026-05-29 |
| Kata Containers Architecture | https://katacontainers.io/docs/ | 2026-05-29 |

### Papers and Specifications

- Biederman, E. W. & Networker, L. (2006). *Multiple Instances of the Global Linux Namespaces*. Proceedings of the Linux Symposium.
- Heo, T. (2015). *Control Group v2*. Linux kernel documentation (`Documentation/admin-guide/cgroup-v2.rst`).
- Open Container Initiative (2024). *OCI Runtime Specification v1.2.0*. https://github.com/opencontainers/runtime-spec/releases
...
|:-------|:-----|:-------------|
| 5.2 — Kubernetes Architecture | [02_Kubernetes_Architecture.md](./02_Kubernetes_Architecture.md) | Kubernetes CRI pipeline invokes OCI runtimes (runc/crun) covered here |
| 5.3 — Site Reliability Engineering | [03_Site_Reliability_Engineering.md](./03_Site_Reliability_Engineering.md) | Container resource limits (cgroups) directly affect SLI measurement and capacity planning |
| 5.4 — Infrastructure as Code | [04_Infrastructure_as_Code.md](./04_Infrastructure_as_Code.md) | IaC tools provision the hosts and registries on which containers run |
| 5.5 — Observability, SLI/SLO/SLA | [05_Observability_SLI_SLO_SLA.md](./05_Observability_SLI_SLO_SLA.md) | Container metrics (cgroup stats, OOM events) feed observability pipelines |
| 5.6 — Cloud Architecture | [06_Cloud_Architecture_AWS_Azure_GCP.md](./06_Cloud_Architecture_AWS_Azure_GCP.md) | Managed container services (ECS, Cloud Run, ACI) abstract the primitives discussed here |
| Foundations | [01_Foundations/](../01_Foundations/) | OS process model, filesystem hierarchy, and syscall interface underpin all container primitives |
...
| Term | Definition |
|:-----|:-----------|
| **cgroup** | Control group — kernel mechanism that limits, accounts for, and isolates resource usage (CPU, memory, I/O) of process groups |
| **Capability** | A distinct unit of privilege that can be independently enabled or disabled for a process, replacing monolithic root privilege (e.g., `CAP_NET_BIND_SERVICE`) |
| **Container Runtime** | Software that creates and runs containers using OS-level primitives; low-level (runc, crun) or high-level (containerd, CRI-O) |
| **Distroless** | Container base image containing only the application and its runtime dependencies, with no package manager, shell, or OS utilities |
| **Idmapped Mount** | A mount where user/group IDs are remapped, enabling rootless containers to access host-owned files without privilege escalation |
| **Namespace** | Kernel abstraction that partitions a global system resource (PID tree, network stack, mount table, etc.) so processes see an isolated instance |
| **OCI** | Open Container Initiative — governance body maintaining the runtime-spec and image-spec standards for containers |
| **OverlayFS** | Union filesystem used by most container runtimes to stack image layers into a single merged view with a writable upper layer |
| **Rootless Container** | A container whose entire runtime stack (runtime, image pull, networking) operates without any root privilege, relying on user namespaces |
| **runc** | Reference implementation of the OCI runtime specification; the default low-level runtime for Docker and containerd |
| **Seccomp** | Secure Computing Mode — kernel facility that restricts the set of syscalls a process may invoke, configured via BPF filter programs |
| **User Namespace** | Namespace type that maps container UIDs/GIDs to different host UIDs/GIDs, enabling unprivileged users to appear as root inside the container |
| **gVisor** | Application kernel written in Go that intercepts syscalls in userspace, providing stronger isolation than shared-kernel containers |
| **Kata Containers** | Lightweight VM-based container runtime that runs each container (or pod) inside a dedicated micro-VM with its own kernel |
| **BuildKit** | Next-generation container image build engine with parallel stage execution, cache mounts, and secret mounts; default builder in Docker 23+ |
