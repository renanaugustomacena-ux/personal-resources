...
fase: "1 — Foundations"
modulo: "01.1.a"
titolo: "The CPU & The Kernel Boundary Deep Dive"
versione: "Linux 6.x"
livello: "Advanced"
prerequisiti:
  - "x86_64 assembly basics (registers, MOV, CALL/RET, INT)"
  - "Module 01.1 — OS Internals: Process & Memory Management"
  - "Familiarity with C and system-level programming on Linux"
obiettivi:
  - "Explain hardware privilege rings (Ring 0-3, VMX root) and how CPL/DPL/RPL enforce access control"
  - "Trace the full syscall path from userspace SYSCALL instruction through kernel entry, dispatch, and SYSRET"
  - "Analyse VDSO/vsyscall acceleration and identify which calls bypass the kernel boundary entirely"
  - "Evaluate the performance cost of Spectre/Meltdown mitigations (KPTI, retpoline) on syscall-heavy workloads"
  - "Use strace, perf, and VDSO inspection tools to measure and debug user-kernel transitions"
tag: [cpu-architecture, kernel-boundary, syscalls, privilege-rings, vdso, spectre, meltdown, x86_64, kpti]
---
...
### Exercise 1 — Observing the Syscall Boundary with strace

Trace system calls of a simple program and measure transition overhead.

1. Write a minimal C program that calls `getpid()` in a loop 1 000 000 times and prints the elapsed time (use `clock_gettime(CLOCK_MONOTONIC)`).
2. Run it normally and record the wall time.
3. Run under `strace -c ./program` — examine the syscall count and cumulative time for `getpid`.
4. Now replace `getpid()` with `clock_gettime(CLOCK_REALTIME)` (a VDSO-accelerated call) and re-run both measurements.
5. Compare the per-call cost: genuine syscall (`getpid`) vs VDSO-resolved call (`clock_gettime`).

**Expected output:** `getpid` shows microsecond-scale overhead per call in strace; `clock_gettime` via VDSO is 10-50x faster since it never enters the kernel.

### Exercise 2 — Inspecting the VDSO

Examine the VDSO shared object mapped into every process.

1. Run: `cat /proc/self/maps | grep vdso` — note the virtual address range.
2. Extract the VDSO ELF: `dd if=/proc/self/mem bs=1 skip=$((0x<start_addr>)) count=$((0x<end_addr> - 0x<start_addr>)) of=/tmp/vdso.so 2>/dev/null` (substitute actual hex addresses).
3. Alternatively, use: `LD_SHOW_AUXV=1 /bin/true | grep AT_SYSINFO_EHDR` to find the VDSO base address.
4. Disassemble it: `objdump -d /tmp/vdso.so` — identify `__vdso_clock_gettime`, `__vdso_gettimeofday`, `__vdso_getcpu`.
5. Run: `readelf -Ws /tmp/vdso.so` — list all exported symbols.
6. Verify that the `[vvar]` mapping (visible in `/proc/self/maps`) is read-only data shared from the kernel to support these VDSO functions.

**Expected output:** The VDSO is a tiny ELF (~4 KB) exporting time/CPU functions that read kernel-maintained data in `[vvar]` without a ring transition.

### Exercise 3 — Privilege Ring Verification via /proc and CPL

Confirm the current privilege level from userspace and observe ring transitions.

1. Write a C program that attempts a privileged instruction directly (e.g., `asm volatile("cli");` to clear the interrupt flag).
2. Compile and run — observe the `SIGSEGV` or `SIGILL` (General Protection Fault from Ring 3 attempting a Ring 0 instruction).
3. Run: `dmesg | tail` — look for the `traps:` log line showing the faulting instruction and CPL.
4. Now write a kernel module (or use an existing one) that executes `cli` successfully from Ring 0 — confirm no fault.
5. Use `rdmsr` (via `msr-tools` package) to read `IA32_LSTAR` (`0xC0000082`): `sudo rdmsr 0xC0000082` — this is the kernel's SYSCALL entry point address.
6. Cross-check with `/proc/kallsyms`: `sudo grep entry_SYSCALL_64 /proc/kallsyms` — the address should match.

**Expected output:** Privileged instructions fault in Ring 3; `IA32_LSTAR` points to `entry_SYSCALL_64` confirming the SYSCALL entry vector.

### Exercise 4 — Measuring KPTI Overhead

Quantify the performance impact of Kernel Page Table Isolation (Meltdown mitigation).

1. Check current KPTI status: `cat /sys/devices/system/cpu/vulnerabilities/meltdown`
2. Run a syscall-intensive benchmark with KPTI enabled:
   ```bash
   perf stat -e context-switches,cpu-cycles,instructions -- \
     stress-ng --syscall 1 --timeout 10
   ```
3. Record cycles-per-syscall.
4. Reboot with KPTI disabled (if on a test system): add `nopti` to kernel command line.
5. Re-run the same benchmark and compare cycles-per-syscall.
6. Also check other mitigations: `cat /sys/devices/system/cpu/vulnerabilities/spectre_v2` — note whether retpoline or IBRS is active.
7. Examine all mitigations at once: `grep . /sys/devices/system/cpu/vulnerabilities/*`

**Expected output:** KPTI adds measurable overhead (typically 5-15%) on syscall-heavy workloads due to page-table switching on every ring transition.

### Exercise 5 — Full Syscall Path Tracing with ftrace

Trace the kernel-side execution path of a single syscall using ftrace.

1. Enable the function_graph tracer:
   ```bash
   echo function_graph | sudo tee /sys/kernel/debug/tracing/current_tracer
   echo __x64_sys_getpid | sudo tee /sys/kernel/debug/tracing/set_graph_function
   echo 1 | sudo tee /sys/kernel/debug/tracing/tracing_on
   ```
2. Trigger the syscall: `getconf CLK_TCK` (or any command that calls `getpid`).
3. Read the trace:
   ```bash
   sudo cat /sys/kernel/debug/tracing/trace | head -50
   ```
4. Observe the call graph: `entry_SYSCALL_64` → `do_syscall_64` → `__x64_sys_getpid` → `task_tgid_vnr` → return.
5. Disable tracing:
   ```bash
   echo 0 | sudo tee /sys/kernel/debug/tracing/tracing_on
   echo nop | sudo tee /sys/kernel/debug/tracing/current_tracer
   ```
6. Repeat with a more complex syscall like `openat` and compare the depth of the call graph.

**Expected output:** The function_graph tracer shows the complete kernel call tree for a syscall, with entry/exit timestamps at each function level.
...
- **vdso(7) — Linux manual page** — Authoritative reference for the virtual dynamic shared object, exported symbols, and architecture-specific behaviour. <https://man7.org/linux/man-pages/man7/vdso.7.html> (retrieved: 2026-05-29)
- **Linux Kernel — EEVDF Scheduler** — Kernel docs on the scheduler that dispatches tasks after syscall returns. <https://docs.kernel.org/scheduler/sched-eevdf.html> (retrieved: 2026-05-29)
- **Linux Kernel — Memory Management** — Page table and TLB documentation relevant to KPTI. <https://docs.kernel.org/mm/index.html> (retrieved: 2026-05-29)
- **CPU vulnerability sysfs interface** — Runtime mitigation status exposed at `/sys/devices/system/cpu/vulnerabilities/`. <https://docs.kernel.org/admin-guide/hw-vuln/index.html> (retrieved: 2026-05-29)
- **Implementing virtual system calls** — LWN article on the vDSO implementation internals. <https://lwn.net/Articles/615809/> (retrieved: 2026-05-29)
- **ftrace documentation** — Kernel source `Documentation/trace/ftrace.rst`; online at <https://docs.kernel.org/trace/ftrace.html> (retrieved: 2026-05-29)

### Books
- Hennessy, J. & Patterson, D., *Computer Architecture: A Quantitative Approach*, 6th ed., Morgan Kaufmann, 2017. (privilege modes, pipeline hazards, speculative execution)
- Perla, E. & Oldani, M., *A Guide to Kernel Exploitation: Attacking the Core*, Syngress, 2010. (kernel exploit techniques across ring boundaries)
- Kerrisk, M., *The Linux Programming Interface*, No Starch Press, 2010. (syscall conventions, signal delivery, process credentials)
- Gregg, B., *Systems Performance: Enterprise and the Cloud*, 2nd ed., Addison-Wesley, 2020. (perf, ftrace, syscall profiling methodology)
- Love, R., *Linux Kernel Development*, 3rd ed., Addison-Wesley, 2010. (kernel entry, interrupt handling, system call implementation)

### Papers and articles
- **Lipp, M. et al., "Meltdown: Reading Kernel Memory from User Space"**, 2018. USENIX Security. (the original Meltdown disclosure paper)
- **Kocher, P. et al., "Spectre Attacks: Exploiting Speculative Execution"**, 2019. IEEE S&P. (Spectre v1/v2 attack variants and retpoline mitigation)
- **"Meltdown & Spectre in 2024: Fixes, Impact, and What's Next"** — Status update on mitigation evolution and VMScape disclosure. <https://www.starwindsoftware.com/blog/meltdown-spectre-2024-status-update/> (retrieved: 2026-05-29)
- **Fog, A., "Software optimization resources"** — Instruction latency tables for Intel/AMD microarchitectures. <https://www.agner.org/optimize/> (retrieved: 2026-05-29)
...
| Module | Relationship |
|---|---|
| [01.1 — OS Internals: Process & Memory Management](01_OS_Internals_Processes_Memory.md) | Parent module; introduces processes, scheduling, and memory at a higher level before this deep dive |
| [01.1.b — Scheduler Data Structures](01_b_Scheduler_Data_Structures.md) | Scheduler dispatch happens immediately after syscall return; covers run-queue and EEVDF internals |
| [01.1.c — Memory Management Algorithms](01_c_Memory_Management_Algorithms.md) | KPTI page-table splitting and TLB flush costs connect directly to page table structures covered there |
| [01.1.d — File Systems & Storage](01_d_File_Systems_Storage.md) | File I/O syscalls (open, read, write) are the most common kernel boundary crossings in practice |
| [02 — Networking & TCP/IP Deep Dive](02_Networking_TCP_IP_Deep_Dive.md) | Socket syscalls (send, recv, epoll) are high-frequency ring transitions; io_uring bypasses the boundary |
| [04 — Compilers & Interpreters](04_Compilers_Interpreters.md) | Compiler-inserted retpoline sequences and speculative-execution barriers are mitigation primitives |
...
| Term | Definition |
|---|---|
| **Protection Ring** | Hardware privilege level enforced by the CPU; x86_64 uses Ring 0 (kernel) and Ring 3 (user), with Ring -1 for hypervisors (VMX root) |
| **CPL** | Current Privilege Level — the active ring stored in the low 2 bits of the CS register; determines what instructions the CPU will permit |
| **SYSCALL** | x86_64 instruction that performs a fast user-to-kernel transition by loading RIP from IA32_LSTAR and swapping to kernel GS base |
| **SYSRET** | x86_64 instruction returning from kernel to user mode after a SYSCALL; restores RIP from RCX and RFLAGS from R11 |
| **VDSO** | Virtual Dynamic Shared Object — a small kernel-provided ELF library mapped into every process, implementing certain syscalls entirely in userspace |
| **vvar** | Read-only memory region mapped by the kernel into userspace containing data (e.g., clock counters) that VDSO functions read without a ring transition |
| **KPTI** | Kernel Page Table Isolation — Meltdown mitigation that maintains separate page tables for user and kernel mode, flushing TLB on every transition |
| **Retpoline** | A compiler-generated code sequence (return trampoline) that replaces indirect branches to mitigate Spectre v2 speculative execution attacks |
| **GDT** | Global Descriptor Table — CPU table defining memory segments and their privilege levels; used during ring transitions |
| **IDT** | Interrupt Descriptor Table — CPU table mapping interrupt/exception vectors to handler addresses; gate DPL controls which rings can invoke each vector |
| **MSR** | Model-Specific Register — CPU configuration registers (read via RDMSR/WRMSR); IA32_LSTAR, IA32_STAR, IA32_FMASK control SYSCALL behaviour |
| **INT 0x80** | Legacy 32-bit Linux syscall entry mechanism using a software interrupt; replaced by the faster SYSCALL instruction on x86_64 |
| **entry_SYSCALL_64** | The kernel function (in `arch/x86/entry/entry_64.S`) that is the first code executed after a SYSCALL instruction transfers control to Ring 0 |
| **Speculative Execution** | CPU optimization that executes instructions ahead of branch resolution; Spectre/Meltdown exploit transient execution to leak data across privilege boundaries |
