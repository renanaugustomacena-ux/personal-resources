...
modulo: "25.1"
titolo: "Threat Intelligence and Adversary Tracking"
versione: "STIX 2.1 Errata 01 / TAXII 2.1 / MISP 2.4.x / ATT&CK v16 / OpenCTI v7"
livello: "Advanced"
prerequisiti:
  - "Network security fundamentals (protocols, packet analysis, DNS)"
  - "Malware analysis concepts (static/dynamic analysis, C2 protocols)"
  - "Familiarity with SIEM platforms and log-management workflows"
  - "Basic understanding of geopolitics and nation-state cyber operations"
  - "Python programming for security automation"
obiettivi:
  - "Apply the intelligence cycle, Diamond Model, MITRE ATT&CK, and Cyber Kill Chain to structure threat intelligence collection, analysis, and dissemination"
  - "Construct and exchange STIX 2.1 bundles (SDOs, SROs, patterns) via TAXII 2.1 and MISP for automated indicator sharing"
  - "Profile nation-state (APT1, APT28/29, Lazarus, Sandworm, Turla, APT41) and eCrime (Conti, LockBit, ALPHV, Clop) threat actors with ATT&CK mappings, YARA rules, and Sigma detections"
  - "Develop actor-specific detection packages combining Sigma rules, YARA rules, and hunt queries targeting each actor's dominant operational domain"
  - "Operationalize threat intelligence through confidence scoring, indicator lifecycle management, PIR prioritization, and TI-to-detection pipeline integration"
tag: [security, threat-intelligence, adversary-tracking, stix, taxii, misp, mitre-attack, apt, diamond-model, yara, sigma]
---
...
>
> 1. Apply the intelligence cycle, Diamond Model, MITRE ATT&CK, and Cyber Kill Chain to structure threat intelligence collection, analysis, and dissemination.
> 2. Construct and exchange STIX 2.1 bundles (SDOs, SROs, patterns) via TAXII 2.1 and MISP for automated indicator sharing.
> 3. Profile nation-state and eCrime threat actors with ATT&CK technique mappings, YARA detection rules, and Sigma behavioral signatures.
> 4. Develop actor-specific detection packages combining Sigma rules, YARA rules, and hunt queries targeting each actor's dominant operational domain.
> 5. Operationalize threat intelligence through confidence scoring, indicator lifecycle management, PIR prioritization, and TI-to-detection pipeline integration.
...
1. **STIX 2.1 bundle construction.** Using the Python `stix2` library, construct a complete STIX 2.1 bundle for a simulated phishing campaign: create a `threat-actor` SDO (with aliases, motivation, and sophistication), a `malware` SDO (with malware_types and kill_chain_phases), three `indicator` SDOs (one domain pattern, one file-hash pattern, one email-address pattern with MATCHES operator), and `relationship` SROs linking them (`uses`, `indicates`, `targets`). Serialize the bundle to JSON, validate it against the STIX 2.1 schema, and publish it to a local TAXII 2.1 server using `taxii2-client`.

2. **Diamond Model analysis of a real APT campaign.** Select a publicly-documented APT campaign (e.g., SolarWinds/SUNBURST, MOVEit/Clop, or Colonial Pipeline/DarkSide). For each intrusion event in the campaign, map the four Diamond Model vertices (Adversary, Capability, Infrastructure, Victim). Demonstrate at least three analytical pivots between vertices (e.g., infrastructure IP to co-hosted domains to additional victims). Document the activity threads and activity groups that emerge from the analysis.

3. **YARA and Sigma rule development from TI report.** Given a threat intelligence report describing a new malware family (provided as a PDF with IOCs, behavioral descriptions, and ATT&CK mappings), develop: (a) a YARA rule targeting the malware's unique string constants and code patterns, (b) two Sigma rules targeting the malware's process-creation and network-connection behaviors, and (c) a Snort/Suricata rule targeting the C2 protocol's network signature. Test all rules against provided benign and malicious samples to verify zero false positives and full detection coverage.

4. **ATT&CK Navigator coverage gap analysis.** Using the `attackcti` Python library, retrieve all techniques used by two threat actors in your organization's threat model (e.g., APT29 and Lazarus). Generate ATT&CK Navigator layer JSON files for both actors. Create a third layer representing your organization's current detection coverage (from your SIEM's deployed Sigma rules). Overlay the layers in Navigator to identify detection gaps — techniques used by the actors but not covered by your detections. Prioritize the top five gaps and write Sigma rules to close them.

5. **MISP event creation and feed synchronization.** On a local MISP instance, create an event for a simulated incident: add at least 10 attributes (ip-dst, domain, sha256, url, email-src) with appropriate categories, `to_ids` flags, and TLP tags. Tag the event with ATT&CK technique galaxies and a threat-actor galaxy. Configure a MISP-to-MISP synchronization feed with a partner instance. Verify that the event propagates correctly, respecting TLP:AMBER sharing restrictions. Export the event's IOCs in CSV format suitable for SIEM ingestion.

---

## Readings and References

- MITRE ATT&CK Enterprise Matrix (v16). <https://attack.mitre.org/matrices/enterprise/> (retrieved: 2026-05-29)
- OASIS STIX 2.1 Specification (Errata 01). <https://docs.oasis-open.org/cti/stix/v2.1/stix-v2.1.html> (retrieved: 2026-05-29)
- OASIS TAXII 2.1 Specification. <https://docs.oasis-open.org/cti/taxii/v2.1/taxii-v2.1.html> (retrieved: 2026-05-29)
- MISP Project — documentation and user guide. <https://www.misp-project.org/documentation/> (retrieved: 2026-05-29)
- Caltagirone, S., Pendergast, A., Betz, C. *The Diamond Model of Intrusion Analysis*. Center for Cyber Intelligence Analysis and Threat Research, 2013. <https://www.activeresponse.org/wp-content/uploads/2013/07/diamond.pdf> (retrieved: 2026-05-29)
- Mandiant. *APT1: Exposing One of China's Cyber Espionage Units*. 2013. <https://www.mandiant.com/resources/apt1-exposing-one-of-chinas-cyber-espionage-units> (retrieved: 2026-05-29)
- CISA — *Russian State-Sponsored and Criminal Cyber Threats to Critical Infrastructure*. <https://www.cisa.gov/news-events/cybersecurity-advisories> (retrieved: 2026-05-29)
- SigmaHQ — community Sigma rule repository. <https://github.com/SigmaHQ/sigma> (retrieved: 2026-05-29)
- Hutchins, E., Cloppert, M., Amin, R. *Intelligence-Driven Computer Network Defense Informed by Analysis of Adversary Campaigns and Intrusion Kill Chains*. Lockheed Martin, 2011.
...
|---|---|---|
| Domain 11 — Malware Analysis | Malware families (X-Agent, ShadowPad, Snake, Cobalt Strike) use injection, persistence, and C2 from Chapters 11A/11B | §2.1–§2.5, §3.1 |
| Domain 14 — Active Directory | APT operations target AD (DCSync, Golden Ticket, Kerberoasting); Conti playbooks describe AD attack chains | §2.2, §3.1 |
| Domain 16 — ICS Security | Sandworm's ICS campaigns (BlackEnergy, Industroyer, NotPetya) inform OT threat modeling | §2.2 |
| Domain 19 — Supply Chain | SolarWinds/APT29, 3CX/Lazarus, CCleaner/APT41 are supply-chain attacks tracked by TI | §2.1–§2.3 |
| Domain 24 — DFIR | TI provides IOCs, actor profiles, and campaign context for IR; YARA/Sigma rules are tactical TI output | §1.5, §4.1–§4.5 |
| Domain 25B — Operationalizing TI | Extends TI fundamentals into pipeline engineering, infrastructure tracking, attribution, and sharing frameworks | §4.1–§4.5, §5.1–§5.4 |

---

## Glossary

- **ATT&CK Navigator:** Web-based tool for creating heatmap visualizations of the MITRE ATT&CK matrix, used for coverage analysis and incident mapping.
- **Campaign (STIX):** A coordinated set of adversary activities sharing objectives, targets, or infrastructure, represented as a STIX Domain Object.
- **Cyber Kill Chain:** Lockheed Martin's seven-phase intrusion model (Reconnaissance through Actions on Objectives) used to structure defensive analysis.
- **Diamond Model:** Intrusion-analysis framework with four vertices (Adversary, Capability, Infrastructure, Victim) enabling analytical pivots between related events.
- **DGA (Domain Generation Algorithm):** Malware technique generating pseudo-random domain names for C2 resilience; tracked by predicting and pre-registering future domains.
- **Galaxy (MISP):** Knowledge base providing contextual enrichment (threat actor profiles, ATT&CK techniques, malware families) for tagging MISP events.
- **IOC (Indicator of Compromise):** Observable artifact (hash, IP, domain, URL, YARA rule) indicating potential malicious activity on a system or network.
- **ISAC (Information Sharing and Analysis Center):** Sector-specific organization facilitating threat intelligence sharing among member organizations.
- **PIR (Priority Intelligence Requirement):** Formally-defined intelligence question that guides TI collection and analysis priorities based on organizational risk.
- **RaaS (Ransomware-as-a-Service):** Criminal business model where ransomware developers provide malware and infrastructure to affiliates in exchange for revenue shares.
- **SDO (STIX Domain Object):** Core STIX 2.1 object types (indicator, malware, threat-actor, campaign, attack-pattern, etc.) representing intelligence entities.
- **SRO (STIX Relationship Object):** STIX 2.1 objects (`relationship`, `sighting`) that link SDOs together, expressing associations like "uses," "indicates," or "targets."
- **TLP (Traffic Light Protocol):** Information-sharing marking protocol (RED, AMBER+STRICT, AMBER, GREEN, CLEAR) controlling redistribution scope.
- **YARA rule:** Pattern-matching signature for malware detection based on byte sequences, strings, regular expressions, and file-structure conditions.
