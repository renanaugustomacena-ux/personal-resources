...
modulo: "9.2"
titolo: "Layer 2, Wireless, Telecom, and SDN Security"
versione: "WPA3 (802.11ax/be), 5G SA (3GPP Rel-18), Bluetooth 5.4, OpenFlow 1.5"
livello: "Advanced"
prerequisiti:
  - "Domain 9 Chapter 9A (TCP/IP, DNS, TLS fundamentals)"
  - "Networking fundamentals (OSI model, Ethernet, IP routing)"
  - "Basic Linux CLI and Wireshark familiarity"
obiettivi:
  - "Execute and detect ARP spoofing, MAC flooding, VLAN hopping, and STP manipulation attacks in a switched environment"
  - "Capture WPA2 four-way handshakes and PMKID hashes, and crack PSKs using GPU-accelerated offline attacks"
  - "Assess WPA3-SAE deployments against Dragonblood side-channel and downgrade vulnerabilities"
  - "Explain SS7/Diameter/5G-AKA signaling attack chains and apply firewall rules to mitigate location tracking and SMS interception"
  - "Exploit and defend SDN controller northbound APIs, flow table poisoning, and overlay encapsulation weaknesses"
tag: [security, layer2, wireless, wifi, bluetooth, telecom, ss7, 5g, sdn, openflow, vlan, arp-spoofing, wpa3, ble, imsi-catcher]
---
...
> 1. Perform Layer 2 reconnaissance and exploitation (ARP spoofing, MAC flooding, VLAN hopping, STP root-bridge hijacking) and deploy switch-level countermeasures (DAI, port security, BPDU Guard).
> 2. Conduct wireless penetration testing across WPA2-PSK (handshake/PMKID capture and cracking), WPA2-Enterprise (EAP credential interception), and WPA3-SAE (Dragonblood and downgrade assessment).
> 3. Exploit Bluetooth Classic (BIAS, KNOB, Bluebugging) and BLE (GATT enumeration, relay attacks, BLESA) vulnerabilities using appropriate hardware and software tooling.
> 4. Demonstrate SS7 MAP abuse (location tracking, SMS interception, call redirection), Diameter attacks, and IMSI catcher operation and detection techniques.
> 5. Assess SDN controller security (northbound API abuse, flow table poisoning, LLDP topology spoofing) and implement micro-segmentation using eBPF/Cilium and OVS flow rules.
...
> **Lab environment.** See `tutorials/tutorial_domain9_ch9B_l2_wireless_lab.md` for full setup instructions, topology diagrams, and step-by-step walkthroughs.

**Exercise 1 — ARP Spoofing and DAI Deployment (Layer 2 MitM)**

Set up a three-host GNS3/EVE-NG lab on a single VLAN. From the attacker host, use Bettercap (`arp.spoof on` with `set arp.spoof.fullduplex true`) to intercept traffic between a victim and the default gateway. Capture HTTP credentials in transit with `net.sniff on`. Verify the MitM by checking `arp -a` on the victim — the gateway MAC should match the attacker's. Then deploy Dynamic ARP Inspection (DAI) on the switch with DHCP snooping (`ip dhcp snooping`, `ip arp inspection vlan 10`) and confirm that the ARP poisoning now fails (the switch drops forged ARP replies and logs violations). Analyze the violation logs with `show ip arp inspection statistics`.

**Exercise 2 — WPA2 Handshake Capture, PMKID Extraction, and Offline Cracking**

Using a wireless adapter in monitor mode (e.g., Alfa AWUS036ACH), scan with `airodump-ng wlan0mon` to identify a WPA2-PSK target network. Capture the four-way handshake by deauthenticating a connected client (`aireplay-ng -0 5 -a <BSSID> -c <CLIENT_MAC> wlan0mon`) and observing "WPA handshake" in airodump-ng output. In parallel, attempt clientless PMKID extraction with `hcxdumptool -i wlan0mon -o pmkid.pcapng --enable_status=1`. Convert captures to hashcat format (`hcxpcapngtool -o hash.22000 pmkid.pcapng`) and crack with `hashcat -m 22000 hash.22000 rockyou.txt`. Compare the time-to-crack between a weak passphrase (8 characters, dictionary word) and a strong passphrase (16+ characters, mixed). Document which attack (handshake vs PMKID) is stealthier and why.

**Exercise 3 — WPA3-SAE Downgrade and Transition Mode Assessment**

Configure a lab AP with WPA3 Transition Mode (SAE + WPA2-PSK fallback). Use `airodump-ng --wpa-type 3 wlan0mon` to identify the AP's RSN capabilities (look for SAE and MFPC/MFPR flags). Create an evil twin with `hostapd-mana` advertising only WPA2-PSK for the same SSID. Confirm that a client with permissive supplicant settings downgrades to WPA2, losing SAE forward secrecy. Then reconfigure the AP in WPA3-only mode (`wpa_key_mgmt=SAE`, `ieee80211w=2` for MFPR). Verify that the evil twin downgrade no longer succeeds and that management-frame deauthentication is blocked by 802.11w PMF. Document the RSN IE differences between transition-mode and SAE-only beacons in Wireshark.

**Exercise 4 — Bluetooth GATT Enumeration and BLE Relay Attack**

Using a BLE-capable adapter and `gatttool`, enumerate a BLE peripheral's services and characteristics (`gatttool -b <ADDR> --primary`, `--characteristics`). Read publicly accessible handles and identify any handles exposing sensitive data without pairing. Then set up a BLE relay using GATTacker (two nRF52840 dongles or Ubertooth One): clone the peripheral's advertisement on one device, connect to the real peripheral from the other, and relay GATT operations. Verify that the target device (e.g., a smart lock simulator) grants access through the relay. Measure relay latency with Wireshark BLE captures and discuss why distance-bounding protocols are the primary countermeasure.

**Exercise 5 — SDN Flow Table Poisoning via LLDP Injection**

Deploy a Mininet topology with an ONOS or Ryu SDN controller. From an attacker host connected to switch S1, capture legitimate LLDP frames with `tcpdump -i eth0 ether proto 0x88cc`. Forge an LLDP frame with Scapy claiming a link exists between S1 and S3 (where no physical link exists), and send it (`sendp(lldp_forged, iface="eth0")`). Observe the controller's topology view — it should now show the phantom link. Verify that traffic destined for S3's subnet is misrouted through the attacker's host. Then enable LLDP HMAC authentication on the controller (`onos-cfg set org.onosproject.provider.lldp ...`) and confirm the forged LLDP is rejected. Document the topology graph before and after the attack.
...
- IEEE 802.1AE-2018 — MACsec: Media Access Control Security. (retrieved: 2026-05-29)
- IEEE 802.1X-2020 — Port-Based Network Access Control. (retrieved: 2026-05-29)
- IEEE 802.11-2020 (incorporating 802.11ax/Wi-Fi 6) and IEEE 802.11be (Wi-Fi 7 draft). (retrieved: 2026-05-29)
- 3GPP TS 33.501 — Security architecture and procedures for 5G System. (retrieved: 2026-05-29)
- RFC 7664 — Dragonfly Key Exchange (SAE). (retrieved: 2026-05-29)

**MITRE ATT&CK:**
- T1557 — Adversary-in-the-Middle: https://attack.mitre.org/techniques/T1557/ (retrieved: 2026-05-29)
- T1557.002 — ARP Cache Poisoning: https://attack.mitre.org/techniques/T1557/002/ (retrieved: 2026-05-29)
- T1595.002 — Active Scanning: Vulnerability Scanning: https://attack.mitre.org/techniques/T1595/002/ (retrieved: 2026-05-29)
- T1200 — Hardware Additions (rogue devices, IMSI catchers): https://attack.mitre.org/techniques/T1200/ (retrieved: 2026-05-29)

**CVEs Referenced:**
- CVE-2017-13082 — KRACK: Key Reinstallation Attack against WPA2 (CVSS 6.9, CWE-323). (retrieved: 2026-05-29)
- CVE-2019-9494 through CVE-2019-9497 — Dragonblood: WPA3-SAE side-channel and reflection attacks (CWE-208, CWE-924). (retrieved: 2026-05-29)
- CVE-2019-9506 — KNOB: Key Negotiation of Bluetooth (CVSS 8.1, CWE-327). (retrieved: 2026-05-29)
- CVE-2020-10135 — BIAS: Bluetooth Impersonation Attacks on Secure Simple Pairing (CVSS 5.4). (retrieved: 2026-05-29)
- CVE-2020-15802 — BLURtooth: Cross-Transport Key Derivation bypass (CWE-287). (retrieved: 2026-05-29)
- CVE-2020-9770 — BLESA: BLE Spoofing Attack on reconnection (iOS/Linux/Android). (retrieved: 2026-05-29)
- CVE-2017-0781 / CVE-2017-1000251 — BlueBorne: Bluetooth RCE without pairing (CVSS 8.8). (retrieved: 2026-05-29)
- CVE-2025-36911 — WhisperPair: Bluetooth forced pairing without user interaction. (retrieved: 2026-05-29)

**Tools and Frameworks:**
- Aircrack-ng suite: https://aircrack-ng.org/ (retrieved: 2026-05-29)
- hcxdumptool / hcxpcapngtool: https://github.com/ZerBea/hcxdumptool (retrieved: 2026-05-29)
- Bettercap: https://www.bettercap.org/ (retrieved: 2026-05-29)
- hostapd-mana: https://github.com/sensepost/hostapd-mana (retrieved: 2026-05-29)
- SigPloit (SS7/Diameter): https://github.com/SigPloiter/SigPloit (retrieved: 2026-05-29)
- Crocodile Hunter (IMSI catcher detection): https://github.com/EFForg/crocodilehunter (retrieved: 2026-05-29)
- Synacktiv — Wireless-(in)Fidelity: Pentesting Wi-Fi in 2025: https://www.synacktiv.com/en/publications/wireless-infidelity-pentesting-wi-fi-in-2025 (retrieved: 2026-05-29)
- Help Net Security — Wireless vulnerabilities doubling every few years (2026): https://www.helpnetsecurity.com/2026/03/12/report-wireless-security-vulnerabilities-2026/ (retrieved: 2026-05-29)
...
|-------|----------------|---------|-------------|
| ARP spoofing as MitM prerequisite for TCP/DNS/TLS attacks | Domain 9, Chapter 9A | §§1.4, 2.2, 4.1 | L2 MitM positioning enables TCP hijacking, DNS poisoning, and SSL stripping described in 9A |
| MACsec/802.1X as L2 analogue of TLS | Domain 9, Chapter 9A | §3 | Authentication and encryption at different OSI layers; MACsec protects the hop, TLS protects the session |
| Evil twin Wi-Fi as delivery mechanism for web attacks | Domain 8, Chapter 8A | §§1–3 | MitM via rogue AP enables JavaScript injection, session cookie theft, and phishing at the browser layer |
| eBPF/XDP networking and BPF verifier bypasses | Domain 5, Chapter 5B | §§4.3, 8 | Same BPF infrastructure; eBPF verifier CVEs can escalate from network data-plane to kernel code execution |
| SDN controller compromise chaining with container escape | Domain 6 | Container breakout | Containerized controllers (ODL in Docker) expose both SDN and container attack surfaces |
| Kernel modules for VLAN interfaces and netfilter conntrack | Domain 2, Chapters 2B–2C | §§3, 4 | 8021q module loading, seccomp policies, and netfilter conntrack exhaustion share kernel attack surface |
...
| Term | Definition |
|------|-----------|
| **CAM Table** | Content Addressable Memory table in a switch mapping MAC addresses to physical ports; overflow forces the switch into hub mode (broadcast) |
| **DAI** | Dynamic ARP Inspection — switch feature that validates ARP packets against the DHCP snooping binding table, dropping forged ARP replies |
| **DTP** | Dynamic Trunking Protocol — Cisco proprietary protocol for auto-negotiating trunk links; exploitation allows VLAN hopping |
| **BPDU** | Bridge Protocol Data Unit — STP control frame used to elect root bridges and compute spanning tree topology; forged BPDUs enable root-bridge hijacking |
| **PMKID** | Pairwise Master Key Identifier — value in the first EAPOL frame that enables clientless WPA2 offline cracking without capturing the full four-way handshake |
| **SAE** | Simultaneous Authentication of Equals — WPA3's Dragonfly-based key exchange providing forward secrecy and offline dictionary resistance |
| **PMF** | Protected Management Frames (802.11w) — encrypts and authenticates Wi-Fi management frames (deauth, disassoc) using IGTK, preventing deauthentication attacks |
| **GATT** | Generic Attribute Profile — BLE protocol defining services and characteristics exposed by peripherals; enumeration reveals device capabilities and potential data exposure |
| **CTKD** | Cross-Transport Key Derivation — Bluetooth mechanism allowing a device paired over one transport (BLE) to derive keys for another (BR/EDR); BLURtooth exploits weak CTKD |
| **SUCI** | Subscription Concealed Identifier — 5G mechanism that encrypts the SUPI (permanent subscriber ID) using the home network's public key, preventing passive IMSI catching |
| **SEPP** | Security Edge Protection Proxy — 5G network function securing the N32 inter-operator interface with mutual TLS and JOSE-based message protection |
| **VTEP** | VXLAN Tunnel Endpoint — the network node that encapsulates/decapsulates L2 frames into UDP-based VXLAN tunnels; spoofing a VTEP enables VNI injection attacks |
| **IMSI Catcher** | A device (e.g., Stingray) that impersonates a legitimate cell tower to capture subscriber identifiers, intercept communications, or track location |
| **MKA** | MACsec Key Agreement — 802.1X-2010 protocol that negotiates session keys for MACsec hop-by-hop encryption using EAP-TLS derived credentials |
| **Flow Table Poisoning** | Attack on SDN switches where an adversary injects or manipulates OpenFlow rules (via compromised controller, LLDP spoofing, or ARP proxy abuse) to redirect or drop traffic |
