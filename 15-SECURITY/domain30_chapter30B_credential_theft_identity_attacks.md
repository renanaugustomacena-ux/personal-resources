...
modulo: "30.2"
titolo: "Credential Theft Mechanics and Identity Attack Chains"
versione: "Mimikatz 2.2+, Impacket 0.12+, Rubeus 2.x, Certify/Certipy, Evilginx2 3.x"
livello: "Advanced"
prerequisiti:
  - "Windows authentication internals: NTLM, Kerberos, LSASS SSP architecture (Domain 14 Chapter 14B)"
  - "Active Directory: domain trusts, GPOs, ADCS, delegation (Domain 14 Chapter 14A)"
  - "C2 framework post-exploitation capabilities (Domain 30 Chapter 30A)"
  - "Cloud identity: Azure AD/Entra ID, SAML federation, OAuth 2.0, PRT"
  - "Sysmon and Windows Security event log analysis"
obiettivi:
  - "Parse LSASS memory structures to understand how Mimikatz extracts credentials from each SSP (MSV1_0, Wdigest, Kerberos, TsPkg) and how Credential Guard isolates them"
  - "Execute and detect the full spectrum of credential dumping techniques (comsvcs.dll, NanoDump, PPL bypass, DCSync, NTDS.DIT extraction) with specific event ID and Sigma rule mappings"
  - "Perform Kerberoasting, AS-REP Roasting, and targeted Kerberoasting attacks using Rubeus and Impacket, then build detection rules based on Event ID 4769/4768 patterns"
  - "Chain NTLM relay with authentication coercion (PetitPotam, PrinterBug, DFSCoerce) through ADCS ESC8 to achieve domain compromise, and implement hardening controls that break each link"
  - "Detect and respond to cloud credential theft (PRT extraction, OAuth consent grants, device code phishing) using Azure AD sign-in logs and Conditional Access policies"
tag: [security, credential-theft, mimikatz, kerberoasting, dcsync, ntlm-relay, golden-saml, prt, lsass, impacket, rubeus, detection-engineering]
---
...
> 1. Analyze LSASS memory architecture at the byte level, identifying encryption keys, SSP credential caches, and the structures Mimikatz parses for each credential type.
> 2. Execute credential dumping via at least five distinct methods (Mimikatz, comsvcs.dll, NanoDump, DCSync, NTDS.DIT extraction) in a lab environment and validate detection coverage for each.
> 3. Perform Kerberoasting and AS-REP Roasting attacks, crack extracted tickets, and build Sigma detection rules with tuned thresholds that distinguish attacks from legitimate Kerberos traffic.
> 4. Construct a full NTLM relay chain (coercion through ADCS to domain compromise) and implement the complete hardening stack (SMB signing, LDAP channel binding, EPA, Spooler disabling).
> 5. Prioritize credential rotation during incident response using the 10-step priority order (krbtgt through cloud tokens) and assess blast radius based on confirmed theft scope.
...
1. **LSASS credential dumping detection validation.** In a lab environment with Sysmon configured, execute credential dumping via five methods: (a) Mimikatz `sekurlsa::logonpasswords`, (b) `comsvcs.dll` MiniDump (both function name and ordinal `#24` variants), (c) NanoDump with `--fork`, (d) `procdump.exe -ma lsass.exe`, and (e) pypykatz against an offline dump. For each method, capture the Sysmon Event ID 10 (ProcessAccess) event and document: `SourceImage`, `TargetImage`, `GrantedAccess` value, and `CallTrace`. Verify that the Sigma rule from section 17.1.1 fires for each method and identify any method that evades it.

2. **Kerberoasting detection and response.** In a lab AD environment: (a) create three service accounts with SPNs and weak passwords, (b) execute Kerberoasting using `Rubeus.exe kerberoast /format:hashcat` and `impacket-GetUserSPNs`, (c) crack the extracted TGS hashes with hashcat (mode 13100 for RC4, mode 19700 for AES), (d) capture Event ID 4769 on the domain controller and validate the Sigma rule from section 17.1.3 fires correctly, (e) implement detection tuning: set the threshold to distinguish a targeted single-SPN request from bulk enumeration, and document the false positive rate from legitimate service ticket requests.

3. **DCSync detection and NTDS.DIT extraction forensics.** In a lab environment: (a) execute DCSync using both Mimikatz (`lsadump::dcsync /domain:lab.local /user:krbtgt`) and Impacket (`impacket-secretsdump -just-dc`), (b) extract NTDS.DIT via Volume Shadow Copy and parse offline with `impacket-secretsdump -ntds ntds.dit -system SYSTEM LOCAL`, (c) for each method, capture the detection events (Event ID 4662 for DCSync, Event ID 7045/4698 for VSS), (d) validate the Sigma rules from sections 17.1.2 and 17.1.6, and (e) perform the krbtgt password reset procedure (twice, 12 hours apart) and verify Golden Ticket invalidation.

4. **NTLM relay chain: PetitPotam through ADCS to domain compromise.** In a lab with ADCS configured with a vulnerable template (ESC8): (a) start `impacket-ntlmrelayx` targeting the ADCS web enrollment endpoint, (b) trigger PetitPotam coercion against a domain controller, (c) relay the DC's NTLM authentication to obtain a DC certificate, (d) authenticate with the certificate using `certipy auth` to obtain the DC machine account hash, (e) DCSync using the machine account hash. Document each step's artifacts and write detection rules for: NTLM authentication from a DC to non-DC targets, certificate enrollment for a machine account from a non-standard source, and the full chain correlation.

5. **Cloud credential theft detection.** In an Azure AD test tenant: (a) simulate a device code phishing attack by initiating a device code flow and completing authentication, (b) extract a PRT from a test device (non-TPM) using Mimikatz `sekurlsa::cloudap`, (c) create a malicious OAuth consent grant application requesting `Mail.Read` and `Files.ReadWrite.All`, (d) for each attack, write KQL queries against Azure AD Sign-in Logs and Audit Logs that detect the attack pattern, and (e) implement Conditional Access policies that block device code flow and restrict user consent to admin-approved applications.

---

## Readings and References

- MITRE ATT&CK. "OS Credential Dumping: LSASS Memory," T1003.001. https://attack.mitre.org/techniques/T1003/001/ (retrieved: 2026-05-29)
- MITRE ATT&CK. "Steal or Forge Kerberos Tickets: Kerberoasting," T1558.003. https://attack.mitre.org/techniques/T1558/003/ (retrieved: 2026-05-29)
- MITRE ATT&CK. "OS Credential Dumping: DCSync," T1003.006. https://attack.mitre.org/techniques/T1003/006/ (retrieved: 2026-05-29)
- GentilKiwi. "Mimikatz." https://github.com/gentilkiwi/mimikatz (retrieved: 2026-05-29)
- Fortra (SecureAuth). "Impacket." https://github.com/fortra/impacket (retrieved: 2026-05-29)
- GhostPack. "Rubeus — Kerberos Interaction and Abuse." https://github.com/GhostPack/Rubeus (retrieved: 2026-05-29)
- Helge Klein. "NanoDump — LSASS Credential Dumping." https://github.com/fortra/nanodump (retrieved: 2026-05-29)
- SpecterOps. "SharpDPAPI." https://github.com/GhostPack/SharpDPAPI (retrieved: 2026-05-29)
- Elastic Security. "Potential Credential Access via DCSync." https://www.elastic.co/guide/en/security/master/potential-credential-access-via-dcsync.html (retrieved: 2026-05-29)
- Detection.FYI. "Kerberoasting Activity — Initial Query (Sigma)." https://detection.fyi/sigmahq/sigma/windows/builtin/security/win_security_kerberoasting_activity/ (retrieved: 2026-05-29)
- CyberDefenders. "Credential Theft: Detection Strategies for SOC Analysts." https://cyberdefenders.org/blog/credential-theft-detection-hunting-for-soc-analysts/ (retrieved: 2026-05-29)
- PlainlySec. "Kerberoasting in 2026: From Foothold to Domain Admin in Four Hours." https://plainlysec.com/kerberoasting-in-2026-from-foothold-to-domain-admin-in-four-hours/ (retrieved: 2026-05-29)
...
|---|---|---|
| Domain 14 Chapter 14A | Active Directory attack paths | Kerberoasting, ADCS abuse (ESC1-ESC8), delegation attacks, and tiered administration model |
| Domain 14 Chapter 14B | Windows internals | Token architecture, LSASS SSP model, Credential Guard, process protection levels |
| Domain 29 Chapter 29A | Ransomware operations | Credential theft as the enabler of lateral movement and domain-wide ransomware deployment |
| Domain 29 Chapter 29B | Campaign dissections | Golden SAML in SolarWinds; credential replay in Change Healthcare |
| Domain 30 Chapter 30A | C2 framework internals | C2 platforms through which credential theft tools are executed in-memory |
| Domain 31 Chapter 31A | SIEM/SOAR detection engineering | Sigma rule deployment, correlation engine configuration, and alert triage for credential attack detections |
...
| Term | Definition |
|---|---|
| **LSASS (Local Security Authority Subsystem Service)** | Windows process (`lsass.exe`) that authenticates users, generates access tokens, and caches credential material in memory via Security Support Providers. |
| **SSP (Security Support Provider)** | DLL loaded by LSASS that handles a specific authentication protocol (MSV1_0 for NTLM, Kerberos, Wdigest, TsPkg, CredSSP) and maintains its own credential cache. |
| **Credential Guard** | Windows feature that isolates credential operations in a Hyper-V-protected enclave (`LsaIso.exe`), preventing LSASS memory dumping from extracting primary credential material. |
| **RunAsPPL** | Protected Process Light protection for LSASS that restricts which processes can open handles to it; bypassable via BYOVD attacks loading vulnerable signed drivers. |
| **Kerberoasting** | Attack where any authenticated domain user requests TGS tickets for service accounts and cracks them offline; RC4-encrypted tickets (etype 23) are crackable at ~700 GH/s on modern GPUs. |
| **AS-REP Roasting** | Attack targeting accounts with the `DONT_REQUIRE_PREAUTH` flag, allowing unauthenticated AS-REP requests whose encrypted portion can be cracked offline. |
| **DCSync** | Attack using the MS-DRSR replication protocol (`DrsGetNCChanges`) to request password data from a domain controller without direct access to the DC filesystem. |
| **Golden Ticket** | Forged Kerberos TGT created using the `krbtgt` account's NTLM hash, granting unrestricted domain access that persists until `krbtgt` is rotated twice. |
| **Silver Ticket** | Forged Kerberos service ticket created using a service account's NTLM hash, granting access to a specific service without domain controller interaction. |
| **PRT (Primary Refresh Token)** | Azure AD SSO token stored in LSASS (CloudAP SSP) on Azure AD joined devices; extractable on non-TPM systems and usable for cloud access without re-authentication. |
| **NTLM relay** | Attack exploiting NTLM's lack of channel binding to forward captured authentication to a different service, achieving access as the relayed user. |
| **PetitPotam** | Authentication coercion technique using the MS-EFSRPC protocol to force a target (typically a domain controller) to authenticate to an attacker-controlled host. |
| **DCC2 (Domain Cached Credentials v2)** | PBKDF2-based (10,240 iterations) cached domain credential format stored in the SECURITY hive for offline logon; significantly slower to crack than NTLM hashes. |
| **Evilginx2** | Real-time phishing proxy framework that intercepts the entire authentication flow (including MFA) and captures post-authentication session cookies; defeated only by FIDO2/WebAuthn. |
| **BYOVD (Bring Your Own Vulnerable Driver)** | Technique where attackers load a legitimately signed but vulnerable kernel driver to gain arbitrary kernel read/write, used to bypass LSASS PPL protection. |
