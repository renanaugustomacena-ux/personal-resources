...
modulo: "4.B"
titolo: "Hardware-Enforced Control Flow Integrity: Deep Internals, Bypass Techniques, and Detection Engineering"
versione: "Intel CET (Alder Lake+), ARM PAC (ARMv8.3+), ARM MTE (ARMv8.5+), ARM BTI (ARMv8.5+), Linux 6.6+ SHSTK, glibc 2.39+, Windows 11 HEST, LLVM/Clang 18+"
livello: "Advanced"
prerequisiti:
  - "Domain 4, Chapter 4A — Code reuse attack theory (ROP, JOP, COP, COOP, SROP, BROP)"
  - "Domain 1 — ELF PT_GNU_PROPERTY, PE load configuration directory"
  - "Domain 2 — Address space layout, page-table flags, sigreturn mechanics"
  - "Domain 3 — Memory corruption primitives (stack overflow, heap UAF, type confusion)"
  - "Domain 5 — Kernel exploitation context for supervisor-mode CFI"
obiettivi:
  - "Explain Intel CET shadow stack page-table encoding, token protocol, and WRSS semantics at the hardware level"
  - "Analyze ENDBR-gadget residual attack surface using automated harvesting tools against production binaries"
  - "Evaluate ARM PAC cryptographic foundations (QARMA cipher, key hierarchy, PAC bit allocation) and identify brute-force feasibility per configuration"
  - "Construct detection rules (Sigma, YARA, eBPF) for CET violations, CFG bitmap manipulation, and CFI bypass artifacts"
  - "Assess MTE tag-granule architecture, checking modes (sync/async/asymmetric), and quantify bypass probability for heap exploitation"
tag: [security, cet, pac, bti, mte, cfi, cfg, xfg, shadow-stack, endbr, bypass, detection, hardware-security, arm, intel]
---
...
>
> 1. Describe Intel CET shadow stack memory model, SSP management, and token-based stack switching at the microarchitectural level.
> 2. Harvest and classify ENDBR64 gadgets from production binaries to quantify residual IBT attack surface.
> 3. Analyze ARM PAC key hierarchy, QARMA cipher properties, and FEAT_FPAC behavior to assess PAC brute-force feasibility.
> 4. Implement detection engineering pipelines (Sigma, YARA, eBPF, ETW) for CET/CFG/CFI violation telemetry.
> 5. Evaluate MTE checking modes and calculate tag-collision probabilities for heap exploitation scenarios.
...
**Exercise 1 — ENDBR Gadget Harvesting and Attack Surface Quantification.**
Run the ENDBR gadget harvester (§2.6) against `/usr/lib/x86_64-linux-gnu/libc.so.6` and `/usr/lib/x86_64-linux-gnu/libcrypto.so`. Compare total ENDBR64 counts, classify gadgets by category (syscall, reg-control, stack-pivot, mem-write), and calculate the percentage of exploitable gadgets per library. Identify the three most dangerous gadgets from each library and explain why. Tools: Python 3, capstone, pyelftools. Reference: §2.6, `tutorials/tutorial_domain4_ch4B_cfi_bypass_lab.md`.

**Exercise 2 — CET Shadow Stack Signal Handler Desync Analysis.**
Compile the shadow stack desync demo (§2.7) with `gcc -fcf-protection=full` on a CET-capable system (or QEMU with CET emulation). Use GDB to trace SSP changes across `setjmp`/`longjmp` and signal delivery. Document: (a) how many stale tokens accumulate after N signal deliveries without `sigreturn`, (b) the SSP offset after each `longjmp`, (c) whether glibc's INCSSP compensation correctly aligns SSP. Tools: GDB with `info register ssp`, CET-enabled kernel. Reference: §2.2, §2.7.

**Exercise 3 — ARM PAC Brute-Force Feasibility Calculator.**
Write a Python script that, given a virtual address width (48 or 52 bits) and TBI configuration, calculates the PAC bit width, brute-force attempt count (average and worst-case), and expected wall-clock time at a given fork rate (e.g., 1000 forks/sec). Model both pre-FEAT_FPAC (crash oracle) and post-FEAT_FPAC (FPAC exception) scenarios. Compare 7-bit PAC (M1-class) vs. 16-bit PAC (server-class). Tools: Python 3. Reference: §3.3, §4.2, §4.3.

**Exercise 4 — XFG Type-Hash Collision Finder.**
Run the XFG collision finder (§7.6) against `ntdll.dll` and `kernel32.dll` from a Windows 11 installation. Report: (a) total unique XFG hashes, (b) number of collision sets (multiple functions sharing a hash), (c) the top-5 collision sets by function count with their prototypes. Assess whether any collision set contains a function useful for exploitation (e.g., `WinExec`, `CreateProcessW`). Tools: Python 3, pefile, Windows 11 system DLLs. Reference: §7.5, §7.6.

**Exercise 5 — MTE Tag Reuse Probability Experiment.**
On an MTE-capable device (Pixel 8+ or QEMU with MTE), compile and run the tag reuse demo (§6.9). Collect tag collision statistics over 10,000 malloc/free cycles. Compare observed collision rates against the theoretical 1/15 prediction. Test both Scudo (Android default) and a basic allocator without tag exclusion logic. Document the security implications of each result. Tools: aarch64-linux-gnu-gcc with `-march=armv8.5-a+memtag`, QEMU or physical MTE device. Reference: §6.6, §6.7, §6.9.

---

## Readings and References

- Intel Software Developer Manual, Vol. 3, Ch. 18 — Control-Flow Enforcement Technology. https://www.intel.com/content/www/us/en/developer/articles/technical/technical-look-control-flow-enforcement-technology.html (retrieved: 2026-05-29)
- Intel — Complex Shadow-Stack Updates (CET). https://www.intel.com/content/www/us/en/content-details/785687/complex-shadow-stack-updates-intel-control-flow-enforcement-technology.html (retrieved: 2026-05-29)
- Malvica, M. "Bypassing Intel CET with Counterfeit Objects." OffSec Blog, 2022. https://www.offsec.com/blog/bypassing-intel-cet-with-counterfeit-objects/ (retrieved: 2026-05-29)
...
- h3xduck — How to Enable Intel CET (2025). https://h3xduck.github.io/cfi/2025/06/26/enabling-intel-cet.html (retrieved: 2026-05-29)
- Ravichandran, J. et al. "PACMAN: Attacking ARM Pointer Authentication with Speculative Execution." ISCA 2022. https://dl.acm.org/doi/10.1145/3470496.3527429 (retrieved: 2026-05-29)
- ARM Architecture Reference Manual — Pointer Authentication (ARMv8.3-A). https://developer.arm.com/documentation/ddi0487/latest (retrieved: 2026-05-29)
- ARM Architecture Reference Manual — Memory Tagging Extension (ARMv8.5-A). https://developer.arm.com/documentation/ddi0487/latest (retrieved: 2026-05-29)
- MITRE ATT&CK T1068 — Exploitation for Privilege Escalation. https://attack.mitre.org/techniques/T1068/ (retrieved: 2026-05-29)
- CVE-2022-42856 — WebKit type confusion, PAC bypass on iOS. https://nvd.nist.gov/vuln/detail/CVE-2022-42856 (retrieved: 2026-05-29)
- CVE-2023-41993 — WebKit processing, PAC bypass chain on iOS 16. https://nvd.nist.gov/vuln/detail/CVE-2023-41993 (retrieved: 2026-05-29)
...
- IERAE CTF 2024 — Intel CET Bypass Challenge. https://github.com/exploits-club/bug.directory/blob/main/IERAE%20CTF%202024%20-%20Intel%20CET%20Bypass%20Challenge.md (retrieved: 2026-05-29)
...
|----------|---------|--------------|
| `domain4_code_reuse_attacks.md` | §2-§8 ROP/JOP/SROP/BROP theory | Prerequisite code reuse techniques that CET/PAC/CFI are designed to prevent |
| `domain5_chapter5A_kernel_exploitation.md` | §7.6 CONFIG_KCFI, §7.1 SMEP | Kernel-mode CFI deployment and supervisor shadow stack gaps |
| `domain6_mitigation_bypass.md` | §1-§3 ASLR/DEP bypass | CET/PAC operate after ASLR/DEP bypass; they prevent the subsequent control-flow hijack |
| `domain7_chapter7A_spectre_sidechannels.md` | §2 Spectre-v1, cache side-channels | PACMAN (§4.3) and speculative MTE bypass (§6.9) use Spectre-class cache oracles |
| `domain1_chapter1B_elf_advanced.md` | §5.3 PT_GNU_PROPERTY | CET IBT/SHSTK feature flags encoded in ELF note sections |
| `tutorials/tutorial_domain4_ch4B_cfi_bypass_lab.md` | Full lab | Hands-on CET bypass, ENDBR harvesting, and PAC oracle exercises |
...
| Term | Definition |
|------|------------|
| **CET (Control-flow Enforcement Technology)** | Intel hardware mechanism providing shadow stacks (backward-edge) and indirect branch tracking (forward-edge) CFI. |
| **Shadow stack** | A hardware-managed parallel stack storing return addresses, validated against the regular stack on every `ret`. |
| **ENDBR64** | x86_64 instruction marking a valid indirect branch target under CET IBT; opcode `F3 0F 1E FA`. |
| **SSP (Shadow Stack Pointer)** | Per-thread pointer to the current position on the shadow stack, stored in `IA32_PL3_SSP` MSR. |
| **WRSS** | CET instruction that writes a value to a shadow stack page; dangerous if available to attacker code. |
| **RSTORSSP / SAVEPREVSSP** | CET instructions for shadow stack switching via a token-based protocol during context switches and signal handling. |
| **PAC (Pointer Authentication Code)** | ARMv8.3 cryptographic MAC embedded in unused upper pointer bits, authenticating pointer integrity. |
| **QARMA** | Tweakable block cipher designed for low-latency PAC computation in ARM processors. |
| **FEAT_FPAC** | ARMv8.6 feature that generates an immediate fault on PAC mismatch instead of pointer corruption, closing brute-force oracles. |
| **BTI (Branch Target Identification)** | ARMv8.5 mechanism requiring indirect branches to land on `BTI` instructions; enforced per-page via the GP bit. |
| **MTE (Memory Tagging Extension)** | ARMv8.5 hardware memory safety mechanism associating 4-bit tags with 16-byte memory granules and pointers. |
| **CFG (Control Flow Guard)** | Microsoft forward-edge CFI using a process-wide bitmap of valid indirect call targets. |
| **XFG (eXtended Flow Guard)** | Microsoft enhancement to CFG adding per-call-site type-hash matching to constrain valid targets by function prototype. |
| **RAP (Reuse Attack Protector)** | grsecurity/PaX CFI implementation providing forward-edge (type-hash) and backward-edge (return-address encryption) protection. |
| **NOTRACK** | CET prefix (`3E`) that disables IBT checking for a specific indirect branch; dangerous if target is attacker-controlled. |
