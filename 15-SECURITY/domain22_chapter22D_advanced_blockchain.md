...
modulo: "22.4"
titolo: "Advanced Blockchain: Privacy, Layer 2, Bridges, and MEV"
versione: "Groth16 / PLONK / Halo 2 / BOLT 1-12"
livello: "Advanced"
prerequisiti:
  - "Domain 22 Chapter 22C — Bitcoin, Ethereum, and Smart Contract Security"
  - "Domain 13 Chapter 13A §2.2 — Elliptic-Curve Cryptography and Pairings"
  - "Familiarity with polynomial commitment schemes and finite-field arithmetic"
obiettivi:
  - "Compare Groth16, PLONK, and STARK proof systems on setup trust model, proof size, verification cost, and post-quantum resistance"
  - "Trace the Zcash Sapling spend circuit from note commitment through nullifier derivation to Groth16 proof generation"
  - "Analyze Monero ring-signature privacy — CLSAG construction, decoy selection weaknesses, and chain-reaction traceability attacks"
  - "Construct a Lightning Network HTLC payment path and evaluate channel jamming, replacement cycling, and wormhole attacks"
  - "Dissect cross-chain bridge exploits (Wormhole, Ronin, Nomad) and classify root causes across lock-and-mint, light-client, and optimistic architectures"
tag: [zk-proofs, groth16, plonk, stark, zcash, monero, lightning-network, bridges, mev, privacy-coins, layer-2]
---

# Domain 22, Chapter 22D — Advanced Blockchain: Privacy, Layer 2, Bridges, and MEV

> **Learning objectives.** After completing this chapter, the student will be able to: (1) explain the mathematical foundations of zk-SNARKs (R1CS, QAP, Groth16 pairing equations) and zk-STARKs (AIR, FRI folding protocol) at sufficient depth to evaluate their security properties; (2) analyze privacy-coin protocols — Zcash Sapling/Orchard note commitments, nullifiers, and viewing keys; Monero CLSAG ring signatures, stealth addresses, and Bulletproofs+ range proofs — and identify their traceability weaknesses; (3) construct Lightning Network commitment transactions with revocation mechanisms and evaluate the full attack taxonomy (channel jamming, replacement cycling, time-dilation, flood-and-loot); (4) classify cross-chain bridge architectures and reconstruct the root-cause exploit chains for Wormhole, Ronin, Poly Network, and Nomad; (5) analyze MEV extraction strategies, the MEV-Boost PBS architecture, and their implications for censorship resistance and DeFi fairness.
...
1. **ZK circuit construction and proving.** Using `circom` and `snarkjs`, build a Groth16 circuit that proves knowledge of two secret values whose hash (Poseidon) equals a public commitment, without revealing the secrets. Run the Powers of Tau phase-1 ceremony (simulated, 2 participants), then the circuit-specific phase-2 ceremony. Generate a proof and verify it on-chain using a Solidity verifier contract deployed on a local Hardhat/Foundry node. Measure: proof generation time, proof size, and on-chain verification gas cost.

2. **Lightning Network channel simulation.** Using `polar` (Lightning Network development tool), set up a 4-node topology (Alice-Bob-Carol-Dave). Open funded channels, route a multi-hop HTLC payment from Alice to Dave, then simulate a revoked-state broadcast: have Alice publish an old commitment transaction and verify that Bob's watchtower broadcasts the justice transaction. Measure the CSV delay and document the penalty mechanism. Then simulate a channel-jamming attack by sending 483 minimum-value HTLCs and measure the impact on routing capacity.

3. **Bridge exploit root-cause analysis.** For the Wormhole exploit ($326M, February 2022): read the vulnerable Solana program code (available in Wormhole's GitHub history pre-patch). Identify the missing `SignatureSet` account ownership check. Write a corrected version of the `verify_signatures` instruction that validates `sig_account.owner == secp256k1_program_id`. Explain why Solana's account model makes this class of vulnerability possible and how it differs from Ethereum's contract-call model.

4. **Monero ring-signature analysis.** Download the Monero blockchain (or use a public block explorer API) and analyze 1,000 recent transactions. For each ring, record the age distribution of decoy outputs. Fit a gamma distribution to the observed ages and compare against Monero's configured gamma parameters (shape=19.28, rate=1.61). Calculate the posterior probability that the youngest ring member is the real spend, given the fitted distribution. Discuss whether the current ring size of 16 provides adequate anonymity.

5. **MEV-Boost block analysis.** Using the Flashbots `mev-boost-relay` data API (https://boost-relay.flashbots.net), download 100 consecutive MEV-Boost blocks. For each block, identify: the builder, the proposer payment, the total gas fees, and the builder's retained MEV. Calculate the Herfindahl-Hirschman Index (HHI) of builder market share. Identify sandwich attacks in the blocks using the signature pattern (same-address front-run/back-run around a victim swap). Quantify total sandwich MEV extracted across the 100 blocks.

---

## Readings and References

- Groth, J., "On the Size of Pairing-Based Non-interactive Arguments," *EUROCRYPT 2016*. https://doi.org/10.1007/978-3-662-49896-5_11 (retrieved: 2026-05-29)
- Gabizon, A., Williamson, Z.J., and Ciobotaru, O., "PLONK: Permutations over Lagrange-bases for Oecumenical Noninteractive arguments of Knowledge," 2019. https://eprint.iacr.org/2019/953 (retrieved: 2026-05-29)
- Ben-Sasson, E. et al., "Scalable, transparent, and post-quantum secure computational integrity (STARKs)," 2018. https://eprint.iacr.org/2018/046 (retrieved: 2026-05-29)
- KuCoin Research, "Why Zero-Knowledge Proof Verification Logic Bugs Have Cost Crypto," 2025. https://www.kucoin.com/blog/why-zero-knowledge-proof-logic-bugs-cost-crypto (retrieved: 2026-05-29)
- IACR ePrint, "Zero-Knowledge Proof Vulnerability Analysis and Security Auditing," 2024/514. https://eprint.iacr.org/2024/514.pdf (retrieved: 2026-05-29)
- Zcash Protocol Specification, "Sapling and Orchard," Version 2024.5.1. https://zips.z.cash/protocol/protocol.pdf (retrieved: 2026-05-29)
- Noether, S., "Ring Confidential Transactions (RingCT)," *Ledger*, 2016. https://doi.org/10.5195/ledger.2016.34 (retrieved: 2026-05-29)
- Goodell, B. et al., "Compact Linkable Spontaneous Anonymous Group Signatures (CLSAG)," *ACISP 2021*. (retrieved: 2026-05-29)
- Riard, A., "Replacement Cycling Attacks on Lightning Network HTLCs," CVE-2023-40231, October 2023. https://github.com/ariard/bitcoin-development/blob/master/replacement-cycling.md (retrieved: 2026-05-29)
- Halborn, "Faulty Proof Validation in ZKPs: A Critical Blockchain Security Risk," 2024. https://www.halborn.com/blog/post/faulty-proof-validation-in-zkps-a-critical-blockchain-security-risk (retrieved: 2026-05-29)
...
|---|---|---|
| Elliptic-curve pairings and bilinear maps | Domain 13, Chapter 13A §2.2 | Groth16/PLONK verification uses BLS12-381 pairings |
| Bitcoin Script, SegWit, Taproot | Domain 22, Chapter 22C §1 | Lightning commitment transactions use CLTV/CSV timelocks |
| Smart-contract vulnerabilities and DeFi attacks | Domain 22, Chapter 22C §4-6 | Bridge exploits use reentrancy, access-control, and signature-verification flaws |
| Hash functions and post-quantum resistance | Domain 13, Chapter 13A §1 | STARK transparency relies on collision-resistant hashing; Groth16 is quantum-vulnerable |
| On-chain forensics and transaction-graph analysis | Domain 22, Chapter 22C §9 | Monero/Zcash traceability research extends UTXO-graph analysis |
| Payment channel networks and micropayments | Domain 22, Chapter 22B §6 | Lightning as an alternative to traditional card-network payment rails |

---

## Glossary

- **AIR (Algebraic Intermediate Representation):** The constraint system used by STARKs, expressing computation as polynomial identities over an execution trace.
- **Bulletproofs:** A zero-knowledge proof system based on inner-product arguments, used for range proofs in Monero and Mimblewimble.
- **CDA (Combined Dynamic Data Authentication):** Not to be confused with EMV CDA (Chapter 22A) — in this context, refers to combined techniques in ZK systems.
- **CLSAG (Compact Linkable Spontaneous Anonymous Group):** Monero's ring-signature scheme proving one of N ring members is the real signer without revealing which.
- **CRS (Common Reference String):** The public parameters generated during a trusted setup ceremony for SNARK proof systems.
- **FRI (Fast Reed-Solomon IOP of Proximity):** The core soundness mechanism in STARKs, verifying polynomial low-degree properties through iterative folding.
- **Groth16:** A zk-SNARK proof system producing 192-byte proofs with constant-time verification, requiring a circuit-specific trusted setup.
- **HTLC (Hash Time-Locked Contract):** A conditional payment in Lightning Network: pay if the preimage of a hash is revealed within a timelock, otherwise refund.
- **Key image:** A deterministic value derived from a Monero output's private key, published on-chain to prevent double-spending without revealing which output was spent.
- **MEV (Maximal Extractable Value):** Value extracted by block builders through transaction ordering — includes sandwich attacks, arbitrage, and liquidations.
- **Nullifier:** A unique value derived from a Zcash note's spending key, published on-chain to prevent double-spending without linking to the note commitment.
- **PBS (Proposer-Builder Separation):** Ethereum's architecture separating block construction (builders) from block proposal (validators) via MEV-Boost.
- **PLONK:** A universal, updatable zk-SNARK using permutation arguments and KZG polynomial commitments; any circuit can use the same SRS.
- **QAP (Quadratic Arithmetic Program):** The polynomial representation of an R1CS constraint system, enabling the Groth16 proof to verify constraint satisfaction via polynomial divisibility.
- **Ring signature:** A cryptographic signature proving that one member of a group signed, without identifying which member — used in Monero for transaction privacy.
