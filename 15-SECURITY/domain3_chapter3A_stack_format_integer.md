...
modulo: "3.1"
titolo: "Stack Corruption, Format Strings, and Integer Errors"
versione: "GCC 14 / Clang 18, glibc 2.39+, pwntools 4.x, GDB 15"
livello: "Advanced"
prerequisiti:
  - "Domain 1 (GOT/PLT for format-string GOT writes, ELF canary from AT_RANDOM)"
  - "Domain 2 (stack placement, ASLR, page permissions, sigreturn dispatch)"
  - "x86_64 calling convention (System V AMD64 ABI)"
  - "C memory model (stack vs heap, pointer types, array bounds)"
  - "GDB basics (breakpoints, examine memory, disassemble)"
obiettivi:
  - "Construct a complete x86_64 stack buffer overflow exploit including ROP chain, canary bypass, and ASLR defeat"
  - "Exploit format string vulnerabilities for both information disclosure (%p/%s leaks) and arbitrary write (%n GOT overwrite)"
  - "Identify and exploit integer overflow, truncation, and signedness errors that lead to heap or stack corruption"
  - "Verify stack protection mechanisms (canaries, FORTIFY_SOURCE, stack-clash-protection) and assess their bypass conditions"
  - "Apply SROP (sigreturn-oriented programming) concepts and demonstrate sigframe construction with pwntools"
tag: [stack-overflow, format-string, integer-overflow, rop, srop, canary, exploitation, binary-exploitation]
---
...
1. **Stack Buffer Overflow with ROP.** Compile a vulnerable program with a known buffer overflow, partial RELRO, and no PIE (`gcc -fno-stack-protector -no-pie -Wl,-z,relro -o vuln vuln.c`). Using pwntools and ROPgadget, construct a ROP chain that calls `system("/bin/sh")`. Then recompile with full protections (`-fstack-protector-all -pie -Wl,-z,relro,-z,now`) and document which mitigations block each step. Reference: `tutorials/tutorial_domain3_ch3A_stack_exploits_lab.md`.

2. **Format String Exploitation Lab.** Using the format-string-vulnerable program from §5, perform three tasks: (a) leak the stack canary using `%7$p` (adjust offset for your binary), (b) leak a libc return address and compute the libc base, (c) overwrite a GOT entry with `system()` using `%n`/`%hn` writes via `fmtstr_payload()` in pwntools. Document the entire chain from leak to shell.

3. **Integer Overflow to Heap Corruption.** Write a program with a signed integer overflow in a `malloc` size calculation (e.g., `malloc(user_count * sizeof(struct item))` where `user_count * sizeof(...)` wraps). Demonstrate the resulting too-small allocation and subsequent heap overflow. Then fix the code using `__builtin_mul_overflow()` and verify the fix.

4. **Canary Brute-Force in Fork Server.** Write a forking TCP server (fork-no-exec model) with a stack buffer overflow. Implement a canary brute-force attack that recovers the 8-byte canary one byte at a time by crashing children (256 attempts per byte, 2048 max total). Measure the time to recover the full canary. Then switch to `execve`-after-fork and verify the brute-force fails (new canary per exec).

5. **FORTIFY_SOURCE Evaluation.** Compile a program containing `strcpy`, `memcpy`, `sprintf`, and `gets` at `_FORTIFY_SOURCE=0`, `=1`, `=2`, and `=3`. For each level, disassemble the compiled output and identify which calls were replaced with `__*_chk` variants. Trigger a detectable overflow at each level and document the resulting behavior (abort, warning, silent corruption).

---

## Readings and References

- "Smashing the Stack for Fun and Profit" — Aleph One (Phrack 49): http://phrack.org/issues/49/14.html (retrieved: 2026-05-29)
- System V AMD64 ABI specification: https://gitlab.com/x86-psABIs/x86-64-ABI (retrieved: 2026-05-29)
- GCC Stack Protector documentation (`-fstack-protector`, `-fstack-clash-protection`): https://gcc.gnu.org/onlinedocs/gcc/Instrumentation-Options.html (retrieved: 2026-05-29)
- FORTIFY_SOURCE documentation: https://www.gnu.org/software/libc/manual/html_node/Source-Fortification.html (retrieved: 2026-05-29)
- CVE-2023-6246 — glibc `__vsyslog_internal` heap overflow (format-string adjacent): https://nvd.nist.gov/vuln/detail/CVE-2023-6246 (retrieved: 2026-05-29)
- MITRE ATT&CK T1203 — Exploitation for Client Execution: https://attack.mitre.org/techniques/T1203/ (retrieved: 2026-05-29)
- MITRE CWE-134 — Use of Externally-Controlled Format String: https://cwe.mitre.org/data/definitions/134.html (retrieved: 2026-05-29)
- MITRE CWE-190 — Integer Overflow or Wraparound: https://cwe.mitre.org/data/definitions/190.html (retrieved: 2026-05-29)
- MITRE CWE-121 — Stack-based Buffer Overflow: https://cwe.mitre.org/data/definitions/121.html (retrieved: 2026-05-29)
- pwntools documentation: https://docs.pwntools.com/en/stable/ (retrieved: 2026-05-29)
- ROPgadget tool: https://github.com/JonathanSalwan/ROPgadget (retrieved: 2026-05-29)
- HackTricks — ret2plt: https://book.hacktricks.wiki/en/binary-exploitation/common-binary-protections-and-bypasses/aslr/ret2plt.html (retrieved: 2026-05-29)
...
|---------|-------|-------------|
| `domain1_chapter1A_elf_foundations.md` | GOT/PLT, .got.plt entries, RELRO | Format string %n writes target GOT entries; full RELRO defeats GOT overwrites |
| `domain1_chapter1B_elf_advanced.md` | Stack canary from AT_RANDOM, TCB at fs:0x28 | Canary initialized from auxv; TLS corruption defeats canary protection |
| `domain2_chapter2A_process_memory.md` | Stack placement, RLIMIT_STACK, stack gap, ASLR | Stack clash targets guard pages; ASLR brute-force in fork-no-exec servers |
| `domain2_chapter2B_syscall_seccomp_ptrace.md` | Sigreturn dispatch, syscall mechanism | SROP exploits sigreturn to forge register state from crafted sigframes |
| `domain3_chapter3B_heap_uaf.md` | Heap exploitation, integer overflow root cause | Integer errors in size calculations lead to heap overflow; format writes target heap |
| `domain4_code_reuse_attacks.md` | ROP chains, stack pivoting, SROP, CFI | Stack overflow is the classic ROP entry point; canary brute-force enables ROP |

---

## Glossary

- **Stack Canary (Cookie):** Random value placed between local buffers and saved return address; verified before function return to detect overflow.
- **Format String Vulnerability:** Bug where user-controlled input is used as a `printf` format argument, enabling memory reads (%p/%s) and writes (%n).
- **ROP (Return-Oriented Programming):** Code reuse technique chaining existing code snippets ("gadgets") ending in RET to execute arbitrary computation.
- **SROP (Sigreturn-Oriented Programming):** Technique using a crafted signal frame and the `sigreturn` syscall to set all registers to attacker-controlled values.
- **Red Zone:** 128-byte region below RSP on x86_64 guaranteed not to be clobbered by signal handlers; usable by leaf functions without RSP adjustment.
- **FORTIFY_SOURCE:** Compiler hardening that replaces unsafe libc calls with bounds-checked `__*_chk` variants at compile time.
- **Integer Overflow:** Arithmetic result exceeding the type's representable range, wrapping around (unsigned) or invoking undefined behavior (signed).
- **Signedness Confusion:** Bug where a signed value is compared or converted to unsigned (or vice versa), causing unexpected behavior in size checks.
- **Stack Clash Protection:** Compiler mitigation (`-fstack-clash-protection`) that probes each page during large stack allocations to ensure guard pages are touched.
- **Frame Pointer Omission:** Optimization (`-fomit-frame-pointer`) that eliminates the RBP save/restore prologue, freeing RBP as a general register.
- **`__stack_chk_fail`:** libc function called when a stack canary mismatch is detected; terminates the process with a diagnostic message.
- **Positional Parameter:** Format string syntax `%N$` that directly accesses the Nth argument, enabling targeted reads and writes without sequential stack walking.
- **GOT Overwrite:** Exploitation technique replacing a GOT entry with an attacker-chosen address so the next PLT call redirects to malicious code.
- **VLA (Variable-Length Array):** C99 feature allocating arrays on the stack with runtime-determined size; potential source of stack clash if size is attacker-controlled.
