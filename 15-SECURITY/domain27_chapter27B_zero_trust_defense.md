...
modulo: "27.B"
titolo: "Zero Trust Architecture and Defensive System Design"
versione: "NIST SP 800-207 / NIST SP 800-207A / CISA ZTMM 2.0 / NIST CSF 2.0"
livello: "Advanced"
prerequisiti:
  - "Domain 27 Chapter 27A — threat modeling, Saltzer-Schroeder principles, secure boot, TPM 2.0, confidential computing"
  - "TLS 1.3 and TCP/IP fundamentals (Domain 9 Chapter 9A)"
  - "Cryptographic primitives and key management (Domain 13 Chapter 13A)"
  - "Active Directory, Kerberos, and identity attack paths (Domain 14 Chapter 14A)"
  - "Familiarity with cloud provider IAM (Domain 10 Chapter 10A)"
obiettivi:
  - "Design a Zero Trust architecture aligned to NIST SP 800-207 tenets, mapping Policy Engine, Policy Administrator, and Policy Enforcement Point roles to concrete infrastructure components"
  - "Implement phishing-resistant authentication using FIDO2/WebAuthn with passkeys and configure Entra ID Conditional Access policies enforcing Continuous Access Evaluation"
  - "Architect label-based microsegmentation policies using Illumio PCE or Kubernetes NetworkPolicy, progressing from discovery through monitoring to full enforcement"
  - "Deploy SPIFFE/SPIRE for workload identity in a Kubernetes cluster, configuring mTLS via Envoy SDS and establishing cross-domain federation"
  - "Evaluate API security architectures against the OWASP API Security Top 10 2023, implementing OAuth 2.0 PKCE + DPoP and GraphQL depth-limiting controls"
tag: [zero-trust, identity, fido2, microsegmentation, spiffe, spire, api-security, oauth, conditional-access, itdr, network-policy]
---

# Domain 27, Chapter 27B — Zero Trust Architecture and Defensive System Design

> **After completing this chapter, you will be able to:** (1) map the NIST SP 800-207 logical architecture (PE/PA/PEP) to real deployment models including device-agent/gateway, enclave-based, and resource-portal patterns; (2) configure and deploy FIDO2/WebAuthn registration and authentication ceremonies, including passkey flows with discoverable credentials; (3) design and enforce microsegmentation policies using label-based abstractions, validating through burn-in monitoring before enforcement; (4) implement SPIFFE/SPIRE workload identity with X.509-SVIDs and JWT-SVIDs, integrating with Envoy service mesh via SDS; (5) detect identity-layer attacks — token theft, MFA fatigue, AiTM session hijacking — using purpose-built Sigma rules and Continuous Access Evaluation.
...
1. **Zero Trust policy design.** For a mid-size enterprise (2,000 employees, hybrid cloud across AWS and Azure, 50 SaaS applications, 200 on-premises servers), design a Zero Trust policy framework aligned to CISA ZTMM 2.0. For each of the five pillars (Identity, Devices, Networks, Applications/Workloads, Data), assess the current state at the Traditional level and define the concrete controls, tooling, and milestones required to reach Advanced maturity within 18 months. Produce a roadmap with quarterly milestones, dependencies, and budget-class estimates.

2. **FIDO2/WebAuthn implementation lab.** Using a local web application (or the WebAuthn demo at webauthn.io), complete a full registration and authentication ceremony. Capture the `navigator.credentials.create()` and `navigator.credentials.get()` calls, inspect the attestation object and authenticator data, extract the AAGUID and credential public key, and verify the signature chain. Then configure a Conditional Access policy in Entra ID (or an equivalent IdP) that requires phishing-resistant MFA (Authentication Strength = FIDO2) for all cloud applications, deployed in report-only mode. Analyze the sign-in logs after 48 hours to identify users without registered FIDO2 credentials.

3. **Microsegmentation with Kubernetes NetworkPolicy.** In a Kubernetes cluster with Calico or Cilium as the CNI plugin, deploy a three-tier application (frontend, API, database) in a dedicated namespace. Implement: (a) a default-deny-all NetworkPolicy for both ingress and egress, (b) an allow rule permitting frontend pods to reach API pods on TCP 8080, (c) an allow rule permitting API pods to reach database pods on TCP 5432, (d) an egress rule permitting API pods to reach an external payment gateway CIDR on TCP 443 and DNS on UDP 53. Verify that direct frontend-to-database traffic is blocked. Generate a microsegmentation bypass Sigma rule that detects denied east-west flows exceeding 10 per source within 5 minutes.

4. **SPIFFE/SPIRE deployment and federation.** Deploy SPIRE Server and Agent in a Kubernetes cluster using the official Helm chart. Create registration entries for two workloads in different namespaces (payments and inventory). Verify that each workload receives an X.509-SVID with the correct SPIFFE ID in the SAN. Establish mTLS between the two workloads via Envoy sidecars configured with SPIRE's SDS API. Then deploy a second SPIRE Server in a separate trust domain and configure SPIFFE Federation between the two domains. Verify cross-domain authentication by presenting an SVID from trust domain A to a workload in trust domain B.

5. **OAuth 2.0 PKCE + DPoP security assessment.** Implement the full Authorization Code flow with PKCE from code-verifier generation through token exchange to resource access (using curl as the client). Then extend the flow with DPoP: generate an ephemeral ES256 key pair, construct DPoP proof JWTs for both the token endpoint and the resource server, and verify that the access token includes a `cnf` claim binding it to the DPoP key. Demonstrate that the access token cannot be replayed without the corresponding DPoP proof by attempting to use the token from a different client context. Document the attack scenario that DPoP prevents (token theft from proxy logs) with a concrete proof-of-concept.
...
- NIST SP 800-207A — Zero Trust Architecture Model for Access Control in Cloud-Native Applications: <https://csrc.nist.gov/pubs/sp/800/207/a/final> (retrieved: 2026-05-29)
- CISA Zero Trust Maturity Model v2.0: <https://www.cisa.gov/zero-trust-maturity-model> (retrieved: 2026-05-29)
- NIST Cybersecurity Framework (CSF) 2.0: <https://www.nist.gov/cyberframework> (retrieved: 2026-05-29)
- W3C Web Authentication (WebAuthn) Level 3: <https://www.w3.org/TR/webauthn-3/> (retrieved: 2026-05-29)
- FIDO Alliance — CTAP2 Specification: <https://fidoalliance.org/specs/fido-v2.2-ps-20250228/fido-client-to-authenticator-protocol-v2.2-ps-20250228.html> (retrieved: 2026-05-29)
- SPIFFE Specification: <https://spiffe.io/docs/latest/spiffe-about/overview/> (retrieved: 2026-05-29)
- SPIRE Documentation: <https://spiffe.io/docs/latest/spire-about/> (retrieved: 2026-05-29)
- OWASP API Security Top 10 (2023): <https://owasp.org/API-Security/> (retrieved: 2026-05-29)
- RFC 9449 — OAuth 2.0 Demonstrating Proof of Possession (DPoP): <https://datatracker.ietf.org/doc/rfc9449/> (retrieved: 2026-05-29)
- RFC 7636 — Proof Key for Code Exchange (PKCE): <https://datatracker.ietf.org/doc/rfc7636/> (retrieved: 2026-05-29)
- Illumio Core Documentation: <https://docs.illumio.com/> (retrieved: 2026-05-29)
- Google BeyondCorp Papers: <https://cloud.google.com/beyondcorp> (retrieved: 2026-05-29)
...
|---|---|---|---|
| §1 Zero Trust fundamentals | Domain 27A §2 | Saltzer-Schroeder design principles (complete mediation, least privilege) | Domain 27A §2 |
| §2 Identity-centric security | Domain 14 Ch.14A §2-5 | AD attack paths that Zero Trust identity controls mitigate | Domain 14A §2-5 |
| §3 Microsegmentation | Domain 9 Ch.9B §1 | Layer 2 / wireless network security extending segmentation | Domain 9B §1 |
| §4 SPIFFE/SPIRE | Domain 9 Ch.9A §3 | TLS 1.3 fundamentals underlying mTLS with SVIDs | Domain 9A §3 |
| §5 API security | Domain 8 Ch.8B §2-4 | Server-side API attacks and testing methodologies | Domain 8B §2-4 |
| §2.5 ITDR | Domain 30 Ch.30B §3 | AiTM phishing and credential theft techniques | Domain 30B §3 |
...
| Term | Definition |
|---|---|
| **ZTA** | Zero Trust Architecture — a security model assuming no implicit trust, verifying every access request regardless of network location |
| **PEP** | Policy Enforcement Point — the gate through which all access passes; intercepts requests and enforces PE decisions |
| **PE** | Policy Engine — the decision-maker evaluating access requests against dynamic policy, identity, device posture, and context |
| **PA** | Policy Administrator — establishes and tears down communication paths between subjects and resources per PE decisions |
| **FIDO2** | Fast Identity Online 2 — authentication standard combining WebAuthn (W3C) and CTAP2 (FIDO Alliance) for phishing-resistant passwordless authentication |
| **Passkey** | Discoverable FIDO2 credential stored on a platform authenticator, enabling passwordless login without the RP supplying credential IDs |
| **CAE** | Continuous Access Evaluation — protocol enabling resource providers to subscribe to critical identity events and revoke tokens in near-real-time |
| **ITDR** | Identity Threat Detection and Response — security discipline focused on detecting attacks targeting the identity layer |
| **SPIFFE** | Secure Production Identity Framework for Everyone — standard defining workload identity via URIs (`spiffe://trust-domain/path`) |
| **SVID** | SPIFFE Verifiable Identity Document — X.509 certificate or JWT binding a SPIFFE ID to a cryptographic key pair |
| **DPoP** | Demonstrating Proof of Possession — RFC 9449 mechanism binding OAuth tokens to the client's private key, converting bearer tokens to proof-of-possession tokens |
| **PKCE** | Proof Key for Code Exchange — RFC 7636 extension preventing authorization code interception in OAuth 2.0 flows |
| **IGA** | Identity Governance and Administration — lifecycle management of identities including provisioning, access reviews, and segregation of duties |
| **Microsegmentation** | Workload-level network segmentation enforcing per-workload communication policies, typically via host firewalls or hypervisor-level controls |
| **ZTMM** | Zero Trust Maturity Model — CISA framework assessing organizational Zero Trust adoption across five pillars and four maturity levels |
