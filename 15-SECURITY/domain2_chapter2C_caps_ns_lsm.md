...
fase: "Domain 2 — OS Internals and Process Security"
modulo: "2.3"
titolo: "Capabilities, Namespaces, Cgroups, LSMs, and Kernel Memory Security"
versione: "Linux 6.x, SELinux 3.x, AppArmor 4.x, Landlock ABI v4"
livello: "Advanced"
prerequisiti:
  - "Domain 2 Chapter 2A (process memory, task_struct, mm_struct, KPTI)"
  - "Domain 2 Chapter 2B (syscall dispatch, seccomp, ptrace)"
  - "Linux user/group permission model and setuid/setgid"
  - "Container fundamentals (Docker, namespaces concept)"
  - "Mandatory access control concepts (labels, policies, type enforcement)"
obiettivi:
  - "Enumerate Linux capabilities and trace the capability transformation rules across fork and execve"
  - "Configure and exploit user namespaces, evaluate their kernel attack surface, and apply sysctl restrictions"
  - "Write and deploy SELinux type-enforcement and AppArmor profile policies for container workloads"
  - "Implement Landlock unprivileged sandboxing for filesystem and network access control"
  - "Audit container isolation layers (capabilities + namespaces + cgroups + seccomp + LSM) and identify escape vectors"
tag: [capabilities, namespaces, selinux, apparmor, landlock, cgroups, container-security, lsm, kernel-hardening]
---
...
1. **Capability Transformation Lab.** Write a program that prints its capability sets (effective, permitted, inheritable, bounding, ambient) using `cap_get_proc()`. Compile it, set file capabilities with `setcap cap_net_bind_service=eip ./prog`, and run as a non-root user. Trace the capability transformation across `execve` by comparing the sets before and after. Then drop the capability from the bounding set with `prctl(PR_CAPBSET_DROP)` and verify it cannot be reacquired. Reference: `tutorials/tutorial_domain2_ch2C_caps_ns_lsm_lab.md`.

2. **User Namespace Attack Surface Audit.** Create a user namespace with `unshare -U -r` and from within, enumerate which kernel subsystems become accessible with namespace-scoped `CAP_SYS_ADMIN` (try `mount`, `nft`, `bpf prog load`). On a kernel with `kernel.unprivileged_userns_clone=0`, verify namespace creation fails. Document the sysctl controls (`unprivileged_userns_clone`, `unprivileged_bpf_disabled`, `user.max_user_namespaces`).

3. **SELinux Policy Development.** Write a minimal SELinux type-enforcement policy for a custom daemon: define a domain type, file types for its config and data, transition rules from `init_t`, and network permissions. Compile with `checkpolicy`, load with `semodule`, and verify enforcement with `sesearch` and `audit2why`. Intentionally trigger a denial and analyze the AVC log.

4. **Landlock Filesystem Sandbox.** Using the Landlock ABI, write a C program that restricts its own filesystem access to read-only on `/usr` and read-write on `/tmp`, blocking everything else. Verify that attempts to read `/etc/shadow` or write to `/home` fail with `EACCES`. Test stacking with an AppArmor profile that further restricts network access.

5. **Container Isolation Audit.** Using a Docker container with `--privileged` vs default capabilities, enumerate the effective capabilities with `capsh --print`, seccomp filter with `cat /proc/1/status | grep Seccomp`, and namespace isolation with `ls -la /proc/1/ns/`. For the privileged container, demonstrate cgroup `release_agent` escape (CVE-2022-0492 pattern) on a deliberately misconfigured host. Document each isolation layer's state.

---

## Readings and References

- Linux `capabilities(7)` man page: https://man7.org/linux/man-pages/man7/capabilities.7.html (retrieved: 2026-05-29)
- Linux `namespaces(7)` man page: https://man7.org/linux/man-pages/man7/namespaces.7.html (retrieved: 2026-05-29)
- SELinux Project documentation: https://selinuxproject.org/page/Main_Page (retrieved: 2026-05-29)
- AppArmor documentation: https://gitlab.com/apparmor/apparmor/-/wikis/home (retrieved: 2026-05-29)
- Landlock LSM documentation: https://docs.kernel.org/userspace-api/landlock.html (retrieved: 2026-05-29)
- CrackArmor — AppArmor privilege escalation vulnerabilities (2026): https://blog.qualys.com/vulnerabilities-threat-research/2026/03/12/crackarmor-critical-apparmor-flaws-enable-local-privilege-escalation-to-root (retrieved: 2026-05-29)
- CVE-2026-31431 — Container escape via kernel file-copy path: https://www.knightli.com/en/2026/05/01/copy-fail-cve-2026-31431-linux-kernel-container-escape/ (retrieved: 2026-05-29)
- CVE-2026-23405 — AppArmor policy namespace DoS: https://windowsnews.ai/article/cve-2026-23405-apparmor-policy-namespace-vulnerability-exposes-linux-kernel-to-denial-of-service-att.411358 (retrieved: 2026-05-29)
- runc container breakout vulnerabilities (2025): https://www.cncf.io/blog/2025/11/28/runc-container-breakout-vulnerabilities-a-technical-overview/ (retrieved: 2026-05-29)
- MITRE ATT&CK T1611 — Escape to Host: https://attack.mitre.org/techniques/T1611/ (retrieved: 2026-05-29)
- MITRE ATT&CK T1548.001 — Abuse Elevation Control Mechanism: Setuid and Setgid: https://attack.mitre.org/techniques/T1548/001/ (retrieved: 2026-05-29)
- CVE-2022-0492 — Cgroup release_agent container escape: https://nvd.nist.gov/vuln/detail/CVE-2022-0492 (retrieved: 2026-05-29)
- "Container Escape by Abusing Cgroups" — Aqua Security: https://www.aquasec.com/blog/new-linux-kernel-vulnerability-escaping-containers-by-abusing-cgroups/ (retrieved: 2026-05-29)
...
|---------|-------|-------------|
| `domain2_chapter2A_process_memory.md` | VMA permissions, mmap_min_addr, KPTI, /dev/mem | Capabilities gate mlock, pagemap; SELinux execmem constrains mprotect |
| `domain2_chapter2B_syscall_seccomp_ptrace.md` | Seccomp, ptrace, CAP_SYS_PTRACE, io_uring | Capabilities gate seccomp/ptrace; namespaces affect UID-based ptrace checks |
| `domain1_chapter1A_elf_foundations.md` | File capabilities in xattr, execve capability transformation | security.capability xattr consulted during ELF loading path |
| `domain5_chapter5A_kernel_exploitation.md` | Kernel heap exploitation, namespace-reachable attack surface | User namespace expands kernel attack surface (nf_tables, BPF, fsconfig) |
| `domain10_chapter10B_container_k8s_serverless.md` | Container runtime security, Kubernetes pod security | Full container isolation stack: caps + ns + cgroups + seccomp + LSM |
| `domain24_dfir.md` | Digital forensics, container forensics, evidence preservation | IMA integrity subsystem provides tamper evidence; LSM audit logs for IR |

---

## Glossary

- **Capability:** A discrete unit of privilege (e.g., `CAP_NET_BIND_SERVICE`) that can be independently granted, replacing the monolithic root model.
- **Namespace:** Kernel mechanism partitioning system resources (PID, mount, network, user, etc.) into isolated views per process group.
- **User Namespace:** Namespace type that remaps UID/GID ranges, granting namespace-scoped root to unprivileged users.
- **Cgroup (Control Group):** Kernel mechanism for limiting, accounting, and isolating resource usage (CPU, memory, I/O, devices) of process groups.
- **SELinux:** Mandatory access control LSM using type enforcement, MLS/MCS labels, and a compiled policy to restrict all subject-object interactions.
- **AppArmor:** Path-based mandatory access control LSM using per-program profiles to restrict file, network, and capability access.
- **Landlock:** Unprivileged, stackable LSM providing process self-sandboxing for filesystem and network access without root.
- **LSM (Linux Security Module):** Kernel framework providing hook points at security-relevant operations, enabling pluggable MAC implementations.
- **Bounding Set:** Hard ceiling on capabilities a process can ever acquire, even across execve of setuid binaries.
- **Ambient Capabilities:** Capabilities automatically inherited across execve for non-setuid, non-file-capability programs.
- **IMA (Integrity Measurement Architecture):** Kernel subsystem that measures (hashes) and optionally appraises files before execution.
- **KSM (Kernel Same-page Merging):** Memory deduplication mechanism that merges identical physical pages; exploitable as a side channel.
- **KASAN (Kernel Address Sanitizer):** Dynamic memory error detector for out-of-bounds and use-after-free bugs in kernel code.
- **KFENCE (Kernel Electric Fence):** Low-overhead kernel memory error detector using guard pages around sampled allocations.
- **release_agent:** Cgroup v1 mechanism that executes a program when a cgroup becomes empty; abused for container escapes when misconfigured.
