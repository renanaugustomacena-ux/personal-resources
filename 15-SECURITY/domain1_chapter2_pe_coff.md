...
modulo: "1.2"
titolo: "PE/COFF: The Windows Binary Format"
versione: "PE/COFF spec v11, Windows 11 24H2, MSVC 14.4x, pefile 2024.x"
livello: "Advanced"
prerequisiti:
  - "Domain 1 Chapter 1A (ELF structure for cross-format comparison)"
  - "Windows OS fundamentals (processes, DLLs, registry, services)"
  - "x86_64 and ARM64 instruction sets (basic)"
  - "Public key cryptography and X.509 certificate chains"
  - "Familiarity with Python scripting"
obiettivi:
  - "Map the complete PE32+ header chain from DOS stub through optional header, section table, and all 16 data directories"
  - "Trace the Windows import resolution mechanism (ILT, IAT, bound imports, delay-load) and compare with ELF GOT/PLT"
  - "Evaluate a PE binary's hardening posture across ASLR, DEP, CFG, XFG, CET, and Authenticode signing"
  - "Detect TLS callback anti-debug techniques and process hollowing artifacts using pefile and Sysinternals tools"
  - "Verify Authenticode signatures, extract PDB provenance data, and perform automated PE triage with Python"
tag: [pe-coff, windows, iat, cfg, xfg, authenticode, tls-callbacks, malware-analysis, reverse-engineering]
---
...
1. **PE Header Deep Inspection.** Using the `pefile` triage script from §19.1, analyze three Windows binaries: a system DLL (`kernel32.dll`), a signed third-party application, and a known-packed sample (e.g., UPX-packed). For each, extract: `DllCharacteristics` mitigation matrix, section entropy, TLS callback presence, and Authenticode status. Compare the results and identify which fields distinguish the packed sample. Reference: `tutorials/tutorial_domain1_ch2_pe_coff_lab.md`.

2. **IAT Hook Detection.** Write a Python script using `pefile` that compares the in-memory IAT of a running process (read via `ReadProcessMemory` or `ctypes` on Windows) against the freshly-resolved expected addresses (computed by loading each imported DLL and calling `GetProcAddress`). Test against a process with a known benign hook (e.g., an AV hook on `NtCreateFile`) and document the discrepancy.

3. **CFG Bitmap Analysis.** Using `dumpbin /loadconfig` on a CFG-enabled binary, extract the `GuardCFFunctionTable` entries. Cross-reference these RVAs against the binary's disassembly to verify they correspond to function entry points. Calculate the percentage of all functions that are CFG-valid indirect-call targets. Discuss the implications for ENDBR64-equivalent gadget availability.

4. **Authenticode Verification and Bypass.** Using `sigcheck64.exe -a -h` and PowerShell `Get-AuthenticodeSignature`, verify the signature of a Microsoft system binary. Then, using a hex editor, append 16 bytes of arbitrary data after the certificate table (the Authenticode exclusion region). Re-verify the signature and document whether modern Windows (post-MS13-098) rejects the modification.

5. **TLS Callback Anti-Debug Lab.** Write a minimal PE with a TLS callback that checks `IsDebuggerPresent()` and calls `ExitProcess(1)` if a debugger is attached. Compile with MSVC. Attach with WinDbg, set a breakpoint on `TlsCallback` (not `EntryPoint`), and step through the anti-debug check. Bypass it by patching the `BeingDebugged` flag in the PEB. Document the PEB structure offsets involved.

---

## Readings and References

- Microsoft PE Format Specification: https://learn.microsoft.com/en-us/windows/win32/debug/pe-format (retrieved: 2026-05-29)
- ECMA-335 — Common Language Infrastructure (CLI) Standard (.NET metadata): https://www.ecma-international.org/publications-and-standards/standards/ecma-335/ (retrieved: 2026-05-29)
- Microsoft Authenticode PE Image Signing Specification: https://learn.microsoft.com/en-us/windows-hardware/drivers/install/authenticode (retrieved: 2026-05-29)
- MITRE ATT&CK T1218 — System Binary Proxy Execution: https://attack.mitre.org/techniques/T1218/ (retrieved: 2026-05-29)
- MITRE ATT&CK T1055.012 — Process Hollowing: https://attack.mitre.org/techniques/T1055/012/ (retrieved: 2026-05-29)
- MITRE ATT&CK T1036 — Masquerading: https://attack.mitre.org/techniques/T1036/ (retrieved: 2026-05-29)
- pefile Python library documentation: https://github.com/erocarrera/pefile (retrieved: 2026-05-29)
- Sysinternals sigcheck documentation: https://learn.microsoft.com/en-us/sysinternals/downloads/sigcheck (retrieved: 2026-05-29)
- "Binary Memory Protection Measures on Windows OS" — NordVPN/Nord Security: https://nordvpn.com/blog/binary-memory-protection-measures-on-windows/ (retrieved: 2026-05-29)
- Windows Internals, 7th Edition — Russinovich, Solomon, Ionescu (Microsoft Press)
- Rich header analysis — "Leveraging the Rich Header for Malware Clustering": https://www.virusbulletin.com/virusbulletin/2020/01/vb2019-paper-rich-headers-leveraging-little-known-pe-artifact/ (retrieved: 2026-05-29)
...
|---------|-------|-------------|
| `domain1_chapter1A_elf_foundations.md` | ELF header, GOT/PLT, RELRO, dynamic linking | ELF counterpart — IAT ↔ GOT, delay-load ↔ lazy binding, .reloc ↔ .rela.dyn |
| `domain1_chapter1B_elf_advanced.md` | TLS, CET-IBT/SHSTK, .init_array constructors | TLS callbacks ↔ .init_array, CFG/XFG ↔ CET-IBT, /GS ↔ stack canary |
| `domain2_chapter2A_process_memory.md` | ASLR, virtual memory, page tables | ASLR with DYNAMIC_BASE + .reloc; ImageBase relocation vs PIE base selection |
| `domain3_chapter3A_stack_format_integer.md` | Stack canary (/GS), buffer overflow, format strings | __security_cookie initialization and bypass; Windows vs Linux canary comparison |
| `domain8_chapter8A_clientside_auth.md` | Browser security, web application attacks | CLR header and managed-code reflection; AMSI as managed-code inspection hook |
| `domain12_chapter12A_static_dynamic_analysis.md` | Static analysis, malware triage, binary diffing | PE triage with pefile/dumpbin; Authenticode and Rich header provenance analysis |

---

## Glossary

- **PE (Portable Executable):** The executable binary format used on Windows, derived from COFF, carrying DOS stub, COFF/optional headers, sections, and data directories.
- **RVA (Relative Virtual Address):** An offset from the image's `ImageBase`; the standard addressing mode for PE internal pointers.
- **IAT (Import Address Table):** Array of function pointers populated by the Windows loader with resolved addresses of imported functions; the PE equivalent of the ELF GOT.
- **ILT (Import Lookup Table):** The original, unmodified import reference table; preserved as a canonical record of imports after the IAT is overwritten by resolution.
- **CFG (Control Flow Guard):** Microsoft's coarse-grained forward-edge CFI: a process-wide bitmap of valid indirect-call targets, checked before every indirect call.
- **XFG (eXtended Flow Guard):** Refinement of CFG that adds per-call-site type-hash verification, restricting indirect calls to functions with matching prototypes.
- **Authenticode:** Microsoft's code-signing framework using PKCS#7 signatures embedded in or cataloged alongside PE binaries.
- **Rich Header:** Undocumented XOR-encoded metadata between the DOS stub and PE signature, recording compiler/linker `comp.id` values for build provenance.
- **TLS Callback:** Function pointer array in the TLS directory invoked before `EntryPoint` on process/thread attach; used for anti-debug and early code execution.
- **DllCharacteristics:** Bitfield in the PE optional header signaling participation in ASLR, DEP, CFG, AppContainer, and other mitigations.
- **Load Configuration Directory:** PE data directory containing security cookie address, SafeSEH tables, CFG/XFG function tables, CET compatibility flags, and other mitigation metadata.
- **Process Hollowing:** Injection technique that creates a suspended process, unmaps its image, writes a payload PE, and resumes execution at the payload's entry point (MITRE T1055.012).
- **Delay-Load Import:** PE mechanism for deferred DLL loading on first function call, implemented by user-mode `__delayLoadHelper2` rather than the OS loader.
- **AppContainer:** Windows sandboxing primitive providing a restricted execution environment with capability-based access control for UWP and select Win32 applications.
