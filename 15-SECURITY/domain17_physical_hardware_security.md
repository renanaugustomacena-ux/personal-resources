...
modulo: "17.1"
titolo: "Physical and Hardware Security"
versione: "ICD 705, NATO SDIP-27, OSDP v2 (SIA), Proxmark3 RRG/Iceman, Flipper Zero FW 1.x, IEEE 299-2006"
livello: "Advanced"
prerequisiti:
  - "Basic electronics (voltage, current, digital logic levels)"
  - "Linux CLI proficiency (USB device management, lsusb, udev)"
  - "Understanding of access control concepts (authentication, authorization)"
  - "Familiarity with RF fundamentals (frequency bands, modulation)"
  - "Knowledge of full-disk encryption concepts (BitLocker, LUKS)"
obiettivi:
  - "Demonstrate lock-bypass and pick techniques against pin-tumbler and disc-detainer mechanisms and assess high-security countermeasures"
  - "Clone RFID/NFC badges using Proxmark3 and Flipper Zero across 125 kHz and 13.56 MHz credential types and evaluate MIFARE Classic, iCLASS, and DESFire security"
  - "Intercept and replay Wiegand credentials using ESPKey and assess OSDP v2 as a mitigation"
  - "Execute USB HID injection attacks (Rubber Ducky, O.MG Cable) and deploy USBGuard countermeasures"
  - "Perform cold boot attacks for DRAM key extraction and evaluate hardware memory-encryption countermeasures (AMD SME/SEV, Intel TME)"
tag: [physical-security, lock-picking, rfid, nfc, proxmark3, flipper-zero, wiegand, osdp, usb-attacks, badusb, cold-boot, tempest, evil-maid, scif, social-engineering, supply-chain]
---

# Domain 17 — Physical and Hardware Security

> **Learning Objectives.** After completing this module the student will be able to: (1) assess physical security controls across CPTED zones, lock mechanisms (pin-tumbler, disc-detainer, high-security sidebar), and bypass techniques, and recommend appropriate countermeasures; (2) clone 125 kHz and 13.56 MHz access credentials using Proxmark3 and Flipper Zero, evaluate MIFARE Classic/iCLASS/DESFire cryptographic protections, and intercept Wiegand traffic with ESPKey; (3) execute USB HID injection attacks using Rubber Ducky and O.MG Cable payloads, deploy network implants (Packet Squirrel, LAN Turtle), and implement USBGuard policies for defense; (4) perform cold boot attacks for DRAM remanence key extraction and evil maid attacks against FDE, and evaluate TPM-measured boot, Intel Boot Guard, and hardware memory encryption as countermeasures; (5) assess TEMPEST/emanation risks using NATO SDIP-27 zone classifications and design Faraday cage shielding with proper power-line filtering and waveguide ventilation.
...
**Exercise 17.1-1 — RFID/NFC badge assessment with Proxmark3.**
Using a Proxmark3 (RDV4 or Easy) against a set of test cards (T5577 writable, MIFARE Classic 1K, DESFire EV2): (a) Read and identify a 125 kHz HID Prox card (`lf hid read`). Clone it to a T5577 blank (`lf hid clone`). Verify the clone reads identically. (b) Attack a MIFARE Classic 1K: run `hf mf autopwn` to recover all sector keys via the darkside/nested/hardnested attack chain. Dump all sectors (`hf mf dump`). Clone to a Magic Gen1a card (`hf mf cload`). Document which attack method was used (darkside vs. nested) and the number of authentication attempts required. (c) Attempt to read a DESFire EV2 card with default transport keys (`hf mfdes auth -n 0 -t aes -k 00000000000000000000000000000000`). Document whether the card uses default keys (configuration failure) or diversified keys (secure configuration). (d) Write a risk assessment comparing 125 kHz Prox, MIFARE Classic, iCLASS Standard, iCLASS SE, and DESFire EV3 for a corporate access control deployment. Deliverable: Proxmark3 session logs, cloned card verification, and the comparative risk assessment.

**Exercise 17.1-2 — Wiegand interception and OSDP v2 evaluation.**
In a lab access control setup (reader + controller + ESPKey or Flipper Zero GPIO): (a) Install the ESPKey inline between reader DATA0/DATA1 and controller. Present five different badges and capture the Wiegand bitstreams via the ESPKey web interface. Decode each captured bitstream using the 26-bit H10301 format (extract facility code and card number, verify parity). (b) Replay one captured credential using the ESPKey replay function and verify that the controller grants access. (c) Assess the attack: document the total time from physical access to reader wiring to first credential capture, the equipment cost, and the detection difficulty. (d) If OSDP v2 equipment is available: reconfigure the reader-controller link to use OSDP v2 with SCP (AES-128 encrypted channel). Verify that the ESPKey captures only encrypted RS-485 data that cannot be decoded or replayed. Deliverable: captured Wiegand bitstreams with decoded FC/CN, replay evidence, attack timeline, and OSDP v2 comparison.

**Exercise 17.1-3 — USB HID injection and defense.**
(a) Write a DuckyScript payload for Hak5 USB Rubber Ducky (or Flipper Zero BadUSB) that: opens a PowerShell window (hidden), downloads a benign marker file from a lab HTTP server, and writes a timestamped entry to a log file proving execution. Test on a Windows 10/11 target and measure the time from USB insertion to payload completion. (b) Deploy USBGuard on a Linux target: generate a baseline policy from currently connected devices, then add a rule rejecting all USB HID class devices (`reject with-interface equals { 03:*:* }`). Insert the Rubber Ducky — verify it is blocked. Verify that an already-whitelisted keyboard continues to function. (c) On Windows: configure a Group Policy to restrict USB device installation by device setup class GUID (block HID devices from unknown vendors). Test the Rubber Ducky — document whether the GP restriction blocks it. Deliverable: DuckyScript payload, execution timing, USBGuard policy file, GPO configuration, and test results for both platforms.

**Exercise 17.1-4 — Cold boot attack simulation.**
In an isolated lab VM (Linux, LUKS-encrypted root): (a) Mount the encrypted filesystem and load a test AES-256 key into a running process (e.g., `openssl enc -aes-256-cbc` with a known key). (b) Take a memory snapshot of the VM (simulating DRAM acquisition after cooling). (c) Search the memory dump for AES key schedules using `aeskeyfind`. Verify that the recovered key matches the known test key. (d) Enable AMD SEV (or Intel TME if available in the hypervisor) on the VM. Repeat the memory snapshot and `aeskeyfind` search — verify that the encrypted memory yields no recognizable key schedules. (e) Document the countermeasure effectiveness and discuss the residual risk window (keys in CPU registers vs. DRAM). Deliverable: `aeskeyfind` output (with and without memory encryption), key comparison verification, and a countermeasure effectiveness assessment.

**Exercise 17.1-5 — Physical penetration test reconnaissance.**
Design a physical penetration test reconnaissance plan for a hypothetical corporate office building with: badge-access main entrance, guard desk, camera system, loading dock, and a server room behind a mantrap. (a) Identify at minimum five CPTED weaknesses to assess during reconnaissance (blind spots, uncontrolled entrances, tailgating opportunities, camera coverage gaps, exposed Wiegand wiring). (b) Plan three social engineering pretexts with required props, target personnel, and success criteria. (c) Identify the technical attack chain: long-range RFID skimming → badge cloning → tailgating or badge-in → Wiegand interception for server room access → network implant deployment. (d) Define the rules of engagement boundaries and evidence preservation requirements (photographs, timestamps, chain of custody for any captured credentials). Deliverable: reconnaissance plan, pretext profiles, technical attack chain diagram, and RoE document.

---

## 21. Readings and References

- Grand, Joe, "JTAGulator: Assisted Discovery of On-Chip Debug Interfaces," http://www.grandideastudio.com/jtagulator/ (retrieved: 2026-05-29)
- Hak5, "USB Rubber Ducky Documentation," https://docs.hak5.org/hak5-usb-rubber-ducky/ (retrieved: 2026-05-29)
- Proxmark3 Community, "Proxmark3 RRG/Iceman Documentation," https://github.com/RfidResearchGroup/proxmark3 (retrieved: 2026-05-29)
- Flipper Devices, "Flipper Zero Documentation," https://docs.flipper.net/ (retrieved: 2026-05-29)
- Garcia, Flavio D. et al., "Dismantling MIFARE Classic," USENIX Security 2008
- Courtois, Nicolas T., "The Dark Side of Security by Obscurity — and Cloning MIFARE Classic Rail/Building Passes," IACR ePrint 2009/137
- Vuagnoux, Martin and Pasini, Sylvain, "Compromising Electromagnetic Emanations of Wired and Wireless Keyboards," USENIX Security 2009
- Van Eck, Wim, "Electromagnetic Radiation from Video Display Units: An Eavesdropping Risk?," Computers & Security, 1985
- Halderman, J. Alex et al., "Lest We Remember: Cold Boot Attacks on Encryption Keys," USENIX Security 2008
- SIA, "OSDP v2 Open Supervised Device Protocol," https://www.securityindustry.org/industry-standards/open-supervised-device-protocol/ (retrieved: 2026-05-29)
- ICD 705, "Technical Specifications for Construction and Management of SCIFs," IC Tech Spec for ICD/ICS 705
- NATO SDIP-27, "NATO TEMPEST Standards (AMSG 720B / 788A / 784)"
- Brown, Fran (Bishop Fox), "Long-Range RFID Badge Cloning," DEF CON 21, 2013
- Baseggio, Mark and Evenchick, Eric, "BLEKey: Breaking Access Controls with BLE," Black Hat 2015

---

## 22. Cross-References (tabular)

| Domain/Chapter | Topic | Relationship to This Chapter |
|---|---|---|
| Domain 7B (Hardware Vulnerabilities) | Plundervolt, Rowhammer, BadUSB | USB HID trust model (§7) connects to BadUSB research; power analysis/EM analysis underlie PLATYPUS and Plundervolt |
| Domain 12 (Reverse Engineering) | Firmware RE, debug interfaces | JTAG/SWD/UART (§17C–17D) are hardware debug interfaces for Domain 12B dynamic analysis; network implants discoverable via Domain 12A traffic analysis |
| Domain 13 (Cryptography) | AES internals, FDE key material | DPA/CPA targets AES (Domain 13A §1.5); cold boot attacks (§10) target in-memory FDE keys; TEMPEST attacks target cleartext signal processing |
| Domain 17B (Fault Injection) | Voltage/clock/EM/laser glitching, DFA | Companion chapter covering ChipWhisperer methodology, crowbar design, secure-boot bypass case studies, and countermeasure evaluation |
| Domain 17C (PCB RE & Chip Analysis) | PCB delayering, decapsulation, memory extraction | Companion chapter covering X-ray CT, chip decapsulation chemistry, SEM/FIB/TEM, NAND/eMMC extraction, and hardware Trojans |
| Domain 17D (Debug & Secure Boot) | JTAG/SWD, CoreSight, secure boot bypass | Companion chapter covering IEEE 1149.1 TAP, ARM CoreSight debug auth, UEFI/Boot Guard/HABv4 architectures, and checkm8/RDP/APPROTECT bypass |
...
| Term | Definition |
|---|---|
| **CPTED** | Crime Prevention Through Environmental Design — applying environmental design principles (natural surveillance, access control, territorial reinforcement, maintenance) to reduce criminal opportunity |
| **Mantrap** | Small vestibule with two interlocked doors where only one can be open at a time; occupancy sensors enforce single-person entry to prevent tailgating |
| **Wiegand Protocol** | Legacy wiring standard between card readers and access controllers; transmits credentials in plaintext with no encryption, authentication, or replay protection |
| **OSDP v2** | Open Supervised Device Protocol version 2 — modern reader-to-controller protocol with AES-128 encrypted secure channel, bidirectional communication, and tamper supervision |
| **ESPKey** | Open-source ESP8266/ESP32 device for inline Wiegand interception and replay, powered parasitically from the reader's supply |
| **T5577** | Universal 125 kHz writable RFID tag that emulates HID Prox, EM4100, AWID, and other LF protocols by reconfiguring its modulation registers |
| **Crypto-1** | Broken proprietary cipher used in MIFARE Classic cards; key recovery via darkside/nested/hardnested attacks takes seconds with Proxmark3 |
| **DuckyScript** | Simple scripting language for USB HID injection devices (Rubber Ducky, Flipper Zero BadUSB); compiled to binary payloads that type pre-scripted commands |
| **DRAM Remanence** | Physical property where DRAM cells retain charge after power removal; cooling extends retention from seconds to hours, enabling cold boot key extraction |
| **Van Eck Phreaking** | Reconstructing display content from electromagnetic emanations of video cables/monitors using a wideband receiver and signal processing |
| **Faraday Cage** | Conductive enclosure (copper mesh, steel panels) that attenuates electromagnetic fields; effectiveness measured in shielding effectiveness (SE) in dB |
| **SCIF** | Sensitive Compartmented Information Facility — accredited space for processing SCI, meeting ICD 705 construction, access control, IDS, and TEMPEST requirements |
| **Anti-Passback** | Access control mechanism preventing a badge from re-entering a zone without first being recorded exiting, defeating badge-sharing attacks |
| **PCILeech** | FPGA-based DMA attack tool using PCIe/Thunderbolt for direct physical memory read/write, bypassing CPU and OS — countermeasure: IOMMU (VT-d/AMD-Vi) |

---

## Exercises

**Exercise 1 — RFID/NFC Badge Cloning and Wiegand Interception.**
Using a Proxmark3 RDV4 (or Iceman firmware) and a Flipper Zero in a controlled lab: (a) Clone a 125 kHz HID ProxCard II to a T5577 blank using `lf hid read` and `lf hid clone`. Verify the clone reads correctly on a test reader. (b) Perform a MIFARE Classic 1K key recovery using `hf mf autopwn`, dump all sectors, and write the dump to a Gen2/CUID Magic card. Document the Crypto-1 attack sequence (darkside → nested → hardnested). (c) Read an iCLASS Standard card using the published HID legacy master key (`hf iclass dump --ki 0`). (d) Set up an ESPKey on a test Wiegand bus (DATA0/DATA1 between a reader and an access controller). Capture five credential presentations, decode each as H10301 26-bit format (facility code + card number), and replay one captured credential. (e) Propose a migration plan from Wiegand to OSDP v2 with AES-128 SCP, including cost estimate and deployment phases.

**Exercise 2 — USB HID Injection and Defense.**
(a) Develop a DuckyScript payload for the USB Rubber Ducky (or Flipper Zero BadUSB) that opens a PowerShell reverse shell on Windows in under 3 seconds. Test against a lab Windows 10/11 workstation. (b) Develop a second payload that exfiltrates the Wi-Fi saved profiles (`netsh wlan show profiles` + `key=clear`) to a local file. (c) Deploy USBGuard on a Linux workstation: generate the initial policy from connected devices, add a rule rejecting all USB HID class devices (`with-interface equals { 03:*:* }`), and verify that the Rubber Ducky is blocked. (d) On Windows, configure Group Policy to restrict USB storage devices by class GUID and test that a standard USB keyboard still works while a BadUSB device is blocked. (e) Conduct a physical social engineering exercise: leave a BadUSB device labeled "Employee Bonus Q4" in a parking lot and monitor (in a controlled environment) whether test participants plug it in. Document the success rate and propose awareness training improvements.

**Exercise 3 — Cold Boot Attack and Key Extraction.**
In a controlled lab with a sacrificial laptop running LUKS full-disk encryption: (a) With the laptop powered on and the LUKS volume mounted, cool the DRAM modules using inverted compressed air spray. (b) Hard-reboot the laptop and immediately boot from a USB drive running a memory-imaging tool (e.g., a custom Linux initrd that dumps physical memory). (c) Search the memory dump for AES key schedules using `aeskeyfind`. (d) Attempt to open the LUKS volume with the recovered master key using `cryptsetup luksOpen --master-key-file`. (e) Document the countermeasure effectiveness: repeat the exercise with AMD SME enabled (if supported) and verify that the memory dump yields only encrypted data. Evaluate TPM + PIN as an additional mitigation.

**Exercise 4 — TEMPEST Emanation Assessment.**
(a) Set up a HackRF One or RTL-SDR with a directional antenna aimed at a test monitor displaying a known pattern (high-contrast black/white text). (b) Using GNURadio, build a flowgraph that receives the wideband signal, applies AM demodulation, and attempts video sync recovery to reconstruct the display content. Document the signal-to-noise ratio at distances of 1 m, 3 m, and 10 m. (c) Measure the shielding effectiveness of a test Faraday enclosure (copper mesh or aluminum foil) by comparing received signal strength with and without the enclosure. (d) Assess the keyboard electromagnetic emanation: capture the EM signature of individual keystrokes using a near-field H-probe and correlate with the USB HID data. (e) Propose a TEMPEST mitigation plan for a hypothetical SCIF, specifying NATO SDIP-27 zone classification, shielding materials, power-line filtering, and waveguide ventilation.

**Exercise 5 — Physical Penetration Test: Social Engineering and Access Control Bypass.**
Design and execute (in an authorized engagement) a physical penetration test against a corporate facility. (a) Reconnaissance: photograph access points, identify camera models and coverage gaps, map badge reader locations, and observe employee entry patterns during peak hours. (b) Pretext development: prepare an IT technician pretext with appropriate props (polo shirt, laptop bag, cable tester, vendor badge). (c) Attempt tailgating through a badge-access door during morning rush. Document the success/failure and employee response. (d) If badge access is required, attempt long-range RFID skimming of a 125 kHz badge using a Proxmark3 with an extended-range antenna at belt-clip distance. (e) Produce a physical security assessment report documenting all findings, rated by severity (CPTED gaps, access control weaknesses, surveillance blind spots), with remediation recommendations.
...
2. NewAE Technology. "ChipWhisperer Documentation." <https://chipwhisperer.readthedocs.io/en/latest/> (retrieved: 2026-05-29).
3. NewAE Technology. "Introduction to Side-Channel Power Analysis." <https://learn.chipwhisperer.io/courses/introduction-to-side-channel-analysis> (retrieved: 2026-05-29).
4. Wrongbaud. "Hardware Debugging for Reverse Engineers Part 1: SWD, OpenOCD." <https://wrongbaud.github.io/posts/stm-xbox-jtag/> (retrieved: 2026-05-29).
5. Memfault. "Diving into JTAG — Security (Part 6)." <https://interrupt.memfault.com/blog/diving-into-jtag-part-6> (retrieved: 2026-05-29).
6. HardBreak Wiki. "SWD." <https://www.hardbreak.wiki/hardware-hacking/interface-interaction/jtag-swd/swd> (retrieved: 2026-05-29).
7. HackTricks. "JTAG." <https://hacktricks.wiki/en/todo/hardware-hacking/jtag.html> (retrieved: 2026-05-29).
8. swisskyrepo. "HardwareAllTheThings — Side-Channel / Fault Injection." <https://github.com/swisskyrepo/HardwareAllTheThings/blob/main/docs/side-channel/fault-injection.md> (retrieved: 2026-05-29).
9. Synacktiv. "How to Voltage Fault Injection." <https://www.synacktiv.com/en/publications/how-to-voltage-fault-injection> (retrieved: 2026-05-29).
10. Embedded Artistry. "nRF52 Security Vulnerability: APPROTECT Bypass." <https://embeddedartistry.com/fieldatlas/nrf52-security-vulnerability-approtect-bypass/> (retrieved: 2026-05-29).
...
12. NewAE Technology. "Power Analysis 101 — Online Course." <https://learn.chipwhisperer.io/courses/power-analysis-101> (retrieved: 2026-05-29).
...
| Module | Relationship |
|--------|-------------|
| Domain 17B — Fault Injection | Extends this module with voltage glitching, EMFI, laser FI, DFA on AES/RSA, and secure-boot bypass via ChipWhisperer/PicoEMP |
| Domain 17C — PCB RE and Chip Analysis | PCB reverse engineering, memory extraction (flashrom, chip-off), die-level analysis with SEM/FIB, and counterfeit IC detection |
| Domain 17D — Debug Interfaces and Secure Boot | JTAG/SWD exploitation with OpenOCD/pyOCD, CoreSight debug architecture, secure-boot chain-of-trust models and bypass |
| Domain 12 — Reverse Engineering | Firmware analysis (Ghidra, binwalk) of images extracted via cold boot, chip-off, or debug port access |
| Domain 15 — Mobile Security | Mobile device physical attacks: evil maid against FDE, USB HID injection, DMA via Thunderbolt, secure-boot bypass on mobile SoCs |
| Domain 16A — ICS/OT Security | Physical security of ICS field devices, tamper protection for PLCs/RTUs, and supply chain integrity for industrial hardware |
