...
fase: "Domain 23 — Social Engineering & OSINT"
modulo: "23.2"
titolo: "Advanced OSINT Tradecraft, Counter-Intelligence, and Social Engineering Defense"
versione: "Maltego 4.x / FIDO2 / C2PA 1.x / ProjectDiscovery 2024"
livello: "Advanced"
prerequisiti:
  - "Domain 23 Chapter 23A — Social Engineering and OSINT fundamentals"
  - "Domain 9 Chapter 9A — DNS and TLS fundamentals"
  - "Domain 8 Chapter 8A — Web Security and OAuth flows"
obiettivi:
  - "Develop custom Maltego TRX transforms for passive-DNS pivoting and infrastructure clustering"
  - "Detect coordinated inauthentic behavior (bot networks, influence operations) using co-amplification graph analysis and Louvain community detection"
  - "Conduct dark-web intelligence collection on Tor hidden services using OnionScan, maintaining strict researcher OPSEC"
  - "Deploy a continuous external attack-surface management pipeline using the ProjectDiscovery toolchain (subfinder, httpx, nuclei, katana)"
  - "Architect a social-engineering defense program integrating FIDO2/WebAuthn, email-gateway sandboxing, phishing simulation, and insider-threat behavioral analytics"
tag: [osint, maltego, socmint, dark-web, attack-surface, fido2, webauthn, influence-operations, deepfake-detection, counter-osint]
---

# Domain 23, Chapter 23B — Advanced OSINT Tradecraft, Counter-Intelligence, and Social Engineering Defense

> **Learning objectives.** After completing this chapter, the student will be able to: (1) build and deploy custom Maltego transforms using the Python TRX library, integrating passive-DNS, WHOIS-history, and TLS-certificate intelligence sources; (2) conduct social-media intelligence (SOCMINT) collection under post-2023 API restrictions using browser-automation anti-detection patterns; (3) perform infrastructure pivoting through passive-DNS temporal correlation, WHOIS registrant clustering, and JA3/JA4/JARM TLS fingerprinting; (4) operate a continuous attack-surface management program using ProjectDiscovery, commercial ASM platforms, and exposed-credential monitoring; (5) design enterprise social-engineering defense architecture — phishing-resistant authentication (FIDO2/passkeys), email-gateway detonation, deepfake detection, and SE-aware vendor risk assessment.
...
1. **Custom Maltego transform development.** Write a Python TRX transform that takes a domain entity as input, queries the SecurityTrails API for historical A records, and returns IPv4Address entities annotated with first-seen/last-seen dates and link labels showing the temporal range. Register the transform with a local iTDS server. Then chain it with a reverse-WHOIS transform: for each historical IP, find co-hosted domains and identify clusters sharing the same registrant email. Visualize the resulting graph and apply Maltego's centrality analysis to identify hub nodes.

2. **SpiderFoot attack-surface discovery.** Run a SpiderFoot scan (`spiderfoot -s target.com -m all`) against an authorized target domain. Compare the results against a manual subfinder+httpx+nuclei pipeline on the same target. Document: which assets SpiderFoot discovered that the manual pipeline missed (and vice versa), the false-positive rate of each approach, and the total scan time. Produce a consolidated attack-surface inventory with risk ratings.

3. **Deepfake vishing detection lab.** Using an open-source voice-cloning tool (e.g., RVC or Bark), generate a deepfake audio sample of a public figure from a 10-second source clip. Then apply three detection methods: (a) spectral analysis looking for artifacts in the 4-8 kHz range typical of neural vocoders, (b) temporal consistency analysis checking for unnatural pause patterns and breathing, (c) C2PA/Content Credentials metadata verification. Document detection accuracy for each method and propose an organizational defense workflow for vishing calls from spoofed executive voices.

4. **Coordinated inauthentic behavior detection.** Collect 10,000 public posts from a social media platform (using the platform's official API or a research dataset). Build a co-amplification graph using the Python code from section 1.2: nodes = accounts, edges = co-sharing of the same URL within a 60-second window. Apply Louvain community detection and identify clusters of 5+ accounts. For each cluster, compute: average account age, posting-frequency variance, content-originality score (Jaccard similarity between posts), and profile-completion rate. Classify clusters as likely organic or likely coordinated.

5. **FIDO2/WebAuthn phishing-resistance validation.** Set up an Evilginx2 instance targeting a test Microsoft 365 tenant. Configure two test accounts: one with TOTP MFA and one with a FIDO2 security key (YubiKey or software authenticator). Attempt credential theft against both accounts through the Evilginx2 proxy. Document: the TOTP account's session token is captured successfully; the FIDO2 account's authentication fails because the authenticator rejects the phishing domain's origin. Produce a technical brief explaining the cryptographic mechanism (origin binding in the WebAuthn assertion) that prevents the attack.

---

## Readings and References

- Maltego Technologies, "Maltego TRX Transform Development Guide." https://docs.maltego.com/support/solutions/articles/15000034557 (retrieved: 2026-05-29)
- ProjectDiscovery, "Nuclei: Vulnerability Scanner with YAML Templates." https://github.com/projectdiscovery/nuclei (retrieved: 2026-05-29)
- CrowdStrike, "Falcon Surface (External Attack Surface Management)." https://www.crowdstrike.com/products/exposure-management/falcon-surface-easm/ (retrieved: 2026-05-29)
- W3C, "Web Authentication: An API for accessing Public Key Credentials (WebAuthn Level 3)," 2025. https://www.w3.org/TR/webauthn-3/ (retrieved: 2026-05-29)
- C2PA (Coalition for Content Provenance and Authenticity), "C2PA Specification v1.4," 2024. https://c2pa.org/specifications/specifications/1.4/index.html (retrieved: 2026-05-29)
- Graphika, "Spamouflage Network Analysis: Chinese State-Linked Influence Operation," 2023. https://graphika.com/ (retrieved: 2026-05-29)
- Stanford Internet Observatory, "Coordinated Inauthentic Behavior Detection Methodologies." https://cyber.fsi.stanford.edu/io (retrieved: 2026-05-29)
- Jericho Security, "Deepfake Phishing: The AI-Powered Social Engineering Threat in 2025." https://www.jerichosecurity.com/blog/deepfake-phishing-the-ai-powered-social-engineering-threat-putting-cisos-on-high-alert-in-2025 (retrieved: 2026-05-29)
- Axis Intelligence, "Top 10 OSINT Tools in 2025 Cyber Analysts Trust." https://axis-intelligence.com/top-10-osint-tools-in-2025-review/ (retrieved: 2026-05-29)
- SpiderFoot, "SpiderFoot: Automated OSINT Collection." https://github.com/smicallef/spiderfoot (retrieved: 2026-05-29)
- Flashpoint, "Threat Intelligence and Dark Web Monitoring." https://flashpoint.io/ (retrieved: 2026-05-29)
...
|---|---|---|
| Social engineering fundamentals (pretexting, phishing, vishing) | Domain 23, Chapter 23A §1-5 | This chapter extends 23A with advanced tradecraft, defense architecture, and detection |
| DNS, TLS, and Certificate Transparency | Domain 9, Chapter 9A §3 | CT-log monitoring, JA3/JA4/JARM fingerprinting, passive DNS |
| Web security and OAuth consent flows | Domain 8, Chapters 8A-8B | Consent phishing, GraphQL introspection, API exposure discovery |
| Malware C2 frameworks and TLS fingerprinting | Domain 11, Chapter 11A §4-5 | JARM identifies Cobalt Strike team servers; JA3 fingerprints C2 beacons |
| Cloud security and Kubernetes | Domain 10, Chapter 10B §2 | Exposed S3 buckets, etcd instances, Kubernetes dashboards in ASM |
| Identity and access management (FIDO2, MFA) | Domain 14, Chapter 14B §3 | FIDO2/WebAuthn as phishing-resistant authentication; Conditional Access |

---

## Glossary

- **ASM (Attack Surface Management):** Continuous discovery, inventory, and monitoring of an organization's externally-visible assets and their security posture.
- **Botometer:** A machine-learning classifier scoring social media accounts on bot-likelihood based on behavioral, network, and content features.
- **C2PA (Coalition for Content Provenance and Authenticity):** A standard for embedding cryptographic provenance metadata in media files to verify authenticity and detect deepfakes.
- **CIB (Coordinated Inauthentic Behavior):** Organized manipulation of social media platforms using networks of fake accounts to amplify, suppress, or distort information.
- **CertStream:** A real-time websocket feed of every certificate logged to Certificate Transparency logs, enabling instant detection of suspicious issuances.
- **DNSDB (Farsight):** The largest passive DNS database, providing historical forward and inverse DNS resolution data for infrastructure pivoting.
- **FIDO2:** An authentication standard combining the WebAuthn browser API with the CTAP authenticator protocol, providing phishing-resistant login via origin binding.
- **IAB (Initial Access Broker):** A threat actor who gains unauthorized access to corporate networks and sells that access to ransomware operators or other attackers.
- **JA3/JA4:** Passive TLS client fingerprinting techniques that identify the software initiating TLS connections based on ClientHello message parameters.
- **JARM:** An active TLS server fingerprinting technique that sends crafted probes and fingerprints the server's responses to identify the TLS implementation.
- **Louvain algorithm:** A community-detection algorithm for graphs that maximizes modularity, used to identify clusters of coordinated accounts in social network analysis.
- **Maltego TRX:** The Python transform development library for building custom Maltego transforms that integrate arbitrary data sources into the graph-analysis workflow.
- **Passkeys:** FIDO2 credentials stored in platform authenticators (OS keychain, cloud sync) that replace passwords with public-key cryptography.
- **Passive DNS (pDNS):** Historical DNS resolution data collected by sensors at recursive resolvers, providing temporal records of domain-to-IP mappings.
- **SOCMINT (Social Media Intelligence):** Intelligence derived from social media platforms — profile data, posting patterns, network connections, and content analysis.
