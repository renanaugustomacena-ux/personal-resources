...
fase: "Domain 9 — Network Security"
modulo: "9.1"
titolo: "TCP/IP, DNS, and TLS Security"
versione: "TLS 1.3 (RFC 8446), DNSSEC (RFC 4033–4035), QUIC (RFC 9000), ML-KEM (FIPS 203)"
livello: "Advanced"
prerequisiti:
  - "TCP/IP networking fundamentals (three-way handshake, IP addressing, routing)"
  - "DNS fundamentals (query types, recursive vs authoritative, record types)"
  - "Basic cryptography (symmetric vs asymmetric, hash functions, digital signatures)"
  - "Linux CLI proficiency (Scapy, Wireshark, tcpdump)"
obiettivi:
  - "Execute and detect TCP stack attacks (SYN flooding, RST injection, session hijacking, idle scan) using Scapy and hping3 with appropriate kernel-level mitigations"
  - "Demonstrate DNS poisoning via the Kaminsky attack, DNS rebinding, and DNS tunneling detection using Suricata and Zeek entropy-based analysis"
  - "Analyze TLS 1.2 vs 1.3 handshake security properties and exploit known weaknesses (0-RTT replay, BEAST, Lucky13, Bleichenbacher/ROBOT, CRIME/BREACH)"
  - "Perform MitM attacks combining ARP poisoning with SSL stripping and evaluate HSTS and certificate pinning as countermeasures"
  - "Compute and apply JA3/JA4+ TLS fingerprints for malicious client detection in SIEM/IDS workflows"
tag: [security, tcp, ip, dns, tls, ssl, dnssec, quic, mitm, syn-flood, kaminsky, certificate-transparency, ja3, post-quantum]
---
...
> 1. Craft and analyze TCP stack attacks (SYN flood, RST injection, session hijacking, idle scan, Sockstress) using Scapy and hping3, and deploy kernel-level mitigations (SYN cookies, challenge ACK hardening, `tcp_timestamps` randomization).
> 2. Execute DNS poisoning (Kaminsky attack methodology), DNS rebinding with Singularity, and DNS tunneling with iodine/dnscat2, then detect each using Suricata rules and Zeek entropy-based scripts.
> 3. Compare TLS 1.2 and 1.3 handshake security properties, exploit padding oracles (Lucky13) and compression oracles (BREACH), and assess servers with testssl.sh for downgrade vulnerabilities and ROBOT.
> 4. Perform MITM attacks combining ARP poisoning with SSL stripping using Bettercap, and validate HSTS preload, certificate pinning, and Certificate Transparency as countermeasures.
> 5. Compute JA3/JA4+ TLS fingerprints from packet captures and integrate them into Zeek/Suricata detection workflows for identifying malicious TLS clients and C2 infrastructure.
...
> **Lab environment.** See `tutorials/tutorial_domain9_ch9A_tcp_dns_tls_lab.md` for full setup instructions, VM topology, and step-by-step walkthroughs.

**Exercise 1 — DNS Cache Poisoning (Kaminsky Attack Simulation)**

Set up a local recursive resolver (Unbound or BIND) in a lab VM. From an attacker VM, write a Scapy script that triggers queries for random subdomains of a target zone and floods the resolver with forged responses containing an authority-section delegation to an attacker-controlled nameserver. Measure the success rate with and without source port randomization enabled on the resolver (`sysctl -w net.ipv4.ip_local_port_range="32768 60999"`). Then enable 0x20 encoding on the resolver and demonstrate that the attack success rate drops to near zero. Analyze the traffic in Wireshark, filtering for `dns.flags.response == 1 && dns.count.auth_rr > 0`.

**Exercise 2 — TLS Interception and HSTS Bypass Assessment**

Using Bettercap on a lab network, perform ARP spoofing and SSL stripping (`set http.proxy.sslstrip true; http.proxy on`). Verify that HTTP credentials are captured in cleartext for a site without HSTS. Then configure the target site's server with `Strict-Transport-Security: max-age=31536000; includeSubDomains; preload` and demonstrate that the stripping fails for a client that has previously visited the site (HSTS cached). Test with `testssl.sh --hsts <target>` and `curl -v https://<target> | grep -i strict`. Document the difference between first-visit vulnerability (no HSTS preload) and preloaded HSTS protection.

**Exercise 3 — JA3/JA4+ TLS Fingerprinting for C2 Detection**

Capture TLS ClientHello packets from three different clients (Chrome, Firefox, curl) connecting to the same server. Extract the JA3 hash from each using Zeek (`zeek -r capture.pcap policy/protocols/ssl/ja3.zeek; cat ja3.log`). Then generate traffic from a known C2 framework (Cobalt Strike or Sliver in a sandboxed lab) and compute its JA3 hash. Write a Suricata rule that alerts on the C2's JA3 hash. Test both a matching and non-matching TLS connection. Compare JA3 stability across different OS versions and document when JA3 hashes change (e.g., after browser updates).

**Exercise 4 — TCP RST Injection and Challenge ACK Side-Channel (CVE-2016-5696)**

In a lab with two hosts maintaining a long-lived TCP connection (e.g., a persistent HTTP/1.1 keep-alive), use Scapy to inject RST packets with guessed sequence numbers from an off-path position. Measure the success rate across different receive-window sizes. Then reproduce the CVE-2016-5696 side channel: send RSTs to the connection and observe the global challenge ACK rate via `/proc/net/snmp | grep TcpExt`. Demonstrate how the rate-limited counter leaks connection existence. Apply the mitigation (`sysctl -w net.ipv4.tcp_challenge_ack_limit=2147483647`) and confirm the side channel is eliminated.

**Exercise 5 — DNS Tunneling Detection with Entropy Analysis**

Set up a DNS tunnel using iodine (server: `iodined -f -c -P pass 10.0.0.1 tunnel.lab.local`, client: `iodine -f -P pass tunnel.lab.local`). Generate traffic through the tunnel (e.g., SSH session or HTTP fetch). Capture DNS traffic with Wireshark and analyze: (a) average query label length, (b) Shannon entropy of the longest label, (c) query type distribution (NULL/TXT vs A/AAAA). Write a Zeek script that alerts when a domain's longest label exceeds 30 characters and entropy exceeds 3.5 bits/char. Compare the entropy signature of tunnel traffic against normal browsing DNS traffic and document the detection thresholds.
...
**RFCs and Standards:**
- RFC 4987 — TCP SYN Flooding Attacks and Common Mitigations. (retrieved: 2026-05-29)
- RFC 5961 — Improving TCP's Robustness to Blind In-Window Attacks. (retrieved: 2026-05-29)
- RFC 6528 — Defending Against Sequence Number Attacks. (retrieved: 2026-05-29)
- RFC 8446 — The Transport Layer Security (TLS) Protocol Version 1.3. (retrieved: 2026-05-29)
- RFC 9000 — QUIC: A UDP-Based Multiplexed and Secure Transport. (retrieved: 2026-05-29)
- RFC 7858 — Specification for DNS over Transport Layer Security (DoT). (retrieved: 2026-05-29)
- RFC 8484 — DNS Queries over HTTPS (DoH). (retrieved: 2026-05-29)
- NIST FIPS 203 — Module-Lattice-Based Key-Encapsulation Mechanism (ML-KEM). (retrieved: 2026-05-29)

**MITRE ATT&CK:**
- T1557 — Adversary-in-the-Middle: https://attack.mitre.org/techniques/T1557/ (retrieved: 2026-05-29)
- T1557.001 — ARP Cache Poisoning / Name Resolution Poisoning: https://attack.mitre.org/techniques/T1557/001/ (retrieved: 2026-05-29)
- T1071.004 — Application Layer Protocol: DNS: https://attack.mitre.org/techniques/T1071/004/ (retrieved: 2026-05-29)
- T1572 — Protocol Tunneling (DNS tunneling): https://attack.mitre.org/techniques/T1572/ (retrieved: 2026-05-29)

**CVEs Referenced:**
- CVE-2016-5696 — TCP side-channel via challenge ACK rate limiting (CVSS 4.8, CWE-200). (retrieved: 2026-05-29)
- CVE-2011-3389 — BEAST: Browser Exploit Against SSL/TLS (CWE-326). (retrieved: 2026-05-29)
- CVE-2013-0169 — Lucky13: TLS CBC padding oracle (CWE-310). (retrieved: 2026-05-29)
- CVE-2017-13099 — ROBOT: Return Of Bleichenbacher's Oracle Threat (CWE-203). (retrieved: 2026-05-29)
- CVE-2012-4929 — CRIME: TLS compression oracle (CWE-310). (retrieved: 2026-05-29)
- CVE-2009-3555 — TLS renegotiation prefix injection (CWE-300). (retrieved: 2026-05-29)
- CVE-2015-4000 — Logjam: DHE export downgrade (CVSS 3.7, CWE-310). (retrieved: 2026-05-29)
- CVE-2021-3618 — ALPACA: Application Layer Protocol Confusion (CWE-295). (retrieved: 2026-05-29)
- CVE-2025-1094 — PostgreSQL SQL injection exploited in BeyondTrust/US Treasury breach chain (CVSS 8.1). (retrieved: 2026-05-29)

**Tools and Research:**
- testssl.sh — TLS/SSL cipher, protocol, and vulnerability scanner: https://testssl.sh/ (retrieved: 2026-05-29)
- Bettercap: https://www.bettercap.org/ (retrieved: 2026-05-29)
- Singularity of Origin (DNS rebinding): https://github.com/nccgroup/singularity (retrieved: 2026-05-29)
- iodine (IP-over-DNS): https://code.kryo.se/iodine/ (retrieved: 2026-05-29)
- dnscat2 (C2 over DNS): https://github.com/iagox86/dnscat2 (retrieved: 2026-05-29)
- Salesforce JA3 fingerprint database: https://github.com/salesforce/ja3 (retrieved: 2026-05-29)
- NCSC — APT28 exploit routers for DNS hijacking (2025): https://www.ncsc.gov.uk/news/apt28-exploit-routers-to-enable-dns-hijacking-operations (retrieved: 2026-05-29)
...
|-------|----------------|---------|-------------|
| ARP poisoning as L2 prerequisite for TCP/DNS/TLS MitM | Domain 9, Chapter 9B | §1.1 | L2 ARP spoofing provides the on-path position required for RST injection, DNS poisoning, and SSL stripping |
| DNS rebinding chaining with browser SOP | Domain 8, Chapter 8A | §1 | DNS rebinding bypasses the SOP by re-resolving a domain to an internal IP, enabling JavaScript access to internal services |
| TLS padding/compression oracles as timing side channels | Domain 7, Chapter 7A | §7 | Lucky13 and CRIME/BREACH are classical timing side channels analogous to cache-timing attacks |
| Kernel networking stack gated by seccomp and namespaces | Domain 2, Chapters 2B–2C | §§3, 3.3 | TCP SYN cookies, IPsec, and QUIC interact with the kernel packet path in netfilter; network namespaces isolate per-process |
| TLS interception intersecting with enterprise DLP/monitoring | Domain 9, Chapter 9C | §2 | Enterprise TLS proxies share infrastructure with threat detection; DNS tunneling is a primary exfiltration vector |
| Post-quantum TLS (ML-KEM) and cryptographic fundamentals | Domain 13, Chapters 13A–13B | §§1–3 | Hybrid X25519Kyber768 key exchange depends on asymmetric/symmetric cryptographic primitives and PKI trust models |
...
| Term | Definition |
|------|-----------|
| **SYN Cookie** | A kernel defense against SYN floods that encodes connection state (MSS, timestamp) into the ISN of the SYN-ACK, avoiding memory allocation until the handshake completes |
| **ISN** | Initial Sequence Number — the 32-bit value in the first SYN packet; modern kernels generate ISNs using SipHash with a secret key to prevent prediction |
| **Kaminsky Attack** | DNS cache poisoning technique that queries random subdomains to avoid TTL caching limits, enabling continuous poisoning attempts with forged authority-section delegations |
| **DNSSEC** | DNS Security Extensions — cryptographic signing of DNS records using ZSK/KSK key pairs, forming a chain of trust from the root; provides authenticated denial of existence via NSEC/NSEC3 |
| **0x20 Encoding** | DNS randomization technique that randomly capitalizes letters in query names and requires matching capitalization in responses, adding entropy to defeat cache poisoning |
| **HSTS** | HTTP Strict Transport Security — header instructing browsers to enforce HTTPS and refuse plaintext HTTP, with `preload` enabling inclusion in browser-hardcoded lists to close first-visit vulnerabilities |
| **JA3/JA4+** | TLS fingerprinting methods that hash ClientHello parameters (version, cipher suites, extensions, curves) to identify client implementations; used for malware and C2 detection |
| **ALPACA** | Application Layer Protocol Confusion Attack — exploits shared TLS certificates across protocols (HTTPS/SMTP/FTP) to redirect connections to unintended services |
| **STEK** | Session Ticket Encryption Key — the server-side symmetric key used to encrypt TLS session tickets; compromise breaks forward secrecy for all resumed sessions using that key |
| **ROBOT** | Return Of Bleichenbacher's Oracle Threat — a 2017 rediscovery showing that RSA PKCS#1 v1.5 padding oracle attacks remained viable against major TLS implementations 19 years after the original disclosure |
| **Certificate Transparency** | A system requiring CAs to submit all issued certificates to append-only Merkle-tree logs; browsers require Signed Certificate Timestamps (SCTs), enabling domain owners to detect unauthorized certificates |
| **DNS Rebinding** | Attack where an attacker-controlled domain alternates DNS responses between the attacker's IP and an internal IP, exploiting the browser's same-origin trust to access internal services |
| **QUIC** | A UDP-based transport protocol integrating TLS 1.3, providing encrypted transport with connection migration via connection IDs rather than the 4-tuple |
| **TFO** | TCP Fast Open — allows data in the SYN packet using a server-issued cookie, eliminating one RTT for repeat connections but introducing replay risks for non-idempotent operations |
