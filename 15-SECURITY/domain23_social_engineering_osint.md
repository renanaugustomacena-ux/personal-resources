...
modulo: "23.1"
titolo: "Social Engineering and Open-Source Intelligence"
versione: "MITRE ATT&CK 15.x / PTES / Gophish 0.12.x"
livello: "Advanced"
prerequisiti:
  - "Domain 8 — Web Application Security (XSS, OAuth, SSRF)"
  - "Domain 14 — Active Directory and M365 Security (Conditional Access, MFA)"
  - "Domain 11 — Malware Analysis and C2 Frameworks"
obiettivi:
  - "Design and execute a multi-vector social engineering campaign (phishing, vishing, physical) within authorized red-team rules of engagement"
  - "Build a comprehensive OSINT target profile using Maltego, SpiderFoot, theHarvester, Sherlock, and breach-data analysis tools"
  - "Enumerate an organization's external attack surface through passive DNS, CT-log, Shodan, and Google dorking techniques"
  - "Deploy and operate Gophish phishing simulations, measure KPIs (click rate, credential submission, report rate), and design progressive-difficulty campaigns"
  - "Implement counter-OSINT defenses: metadata stripping, data-broker opt-out, exposure monitoring, and canary-token deployment"
tag: [social-engineering, osint, phishing, vishing, pretexting, recon, maltego, spiderfoot, gophish, counter-osint]
---

# Domain 23 — Social Engineering and Open-Source Intelligence

> **Learning objectives.** After completing this chapter, the student will be able to: (1) apply Cialdini's influence principles and cognitive-bias exploitation to construct believable pretexts for phishing, vishing, and physical social engineering engagements; (2) execute a full OSINT intelligence cycle — from requirements definition through passive/active collection, processing, analysis, and dissemination — using industry-standard tools; (3) enumerate email addresses, subdomains, exposed credentials, source-code secrets, and technology stacks for a target organization; (4) build and operate phishing simulation infrastructure (Gophish, Evilginx2) for authorized security-awareness testing, including MFA-bypass scenarios; (5) design an organizational counter-OSINT program encompassing digital footprint reduction, metadata scrubbing, and exposure monitoring.
...
1. **OSINT persona building.** Create a complete sock-puppet persona for an authorized red-team engagement: generate an AI profile photo (verify it is not reverse-searchable via Yandex and TinEye), establish a consistent backstory using fakenamegenerator.com, create aged accounts on three social platforms using dedicated infrastructure (VPN exit matching the persona's claimed location, dedicated browser profile, virtual phone number). Document the OPSEC measures taken at each step. Then use your own organization as the target: run Sherlock, theHarvester, and Amass against it and compile a target-profile dossier.

2. **Maltego reconnaissance graph.** Using Maltego CE (or a trial of Maltego Pro), build a reconnaissance graph for an authorized target domain. Start with the domain entity, run DNS transforms (subdomains, MX, NS), pivot to IP addresses (reverse DNS, WHOIS), pivot to registrant emails (reverse WHOIS), and expand to linked domains. Apply centrality analysis to identify the most-connected infrastructure node. Export the graph and write a 1-page intelligence summary identifying the organization's external attack surface, key personnel, and technology stack.

3. **Phishing campaign detection engineering.** Deploy a Gophish instance and configure a phishing simulation against a test group. Craft three email templates of increasing sophistication: (a) generic "password reset" with a typosquatted domain, (b) spear-phish referencing real internal project names gathered via OSINT, (c) QR-code-based quishing with no clickable URL. Measure click rate, credential-submission rate, and report rate for each template. Write Sigma detection rules for each delivery method targeting proxy logs (new domain age, credential-harvest path patterns) and email-gateway logs (QR-code attachment without text URL).

4. **Physical social engineering assessment.** Plan a physical SE engagement for an authorized target facility. Document: pretext development (role, scenario, supporting artifacts), badge-cloning feasibility assessment (identify card technology — 125 kHz vs. 13.56 MHz — using a Proxmark3 read), tailgating/piggybacking opportunities (observe access-control points during peak hours), and USB-drop placement strategy. Execute the engagement (with written authorization and de-escalation procedures), document findings with CVSS scores, and provide remediation recommendations.

5. **Counter-OSINT exposure audit.** Conduct a full counter-OSINT assessment of your own organization (or a personal digital footprint): run dnstwist for typosquatting domains, check all executive email addresses against HIBP and DeHashed, enumerate metadata from publicly-available documents using metagoofil/exiftool, search GitHub for leaked credentials using trufflehog, and monitor CT logs for unauthorized certificate issuance. Produce a risk-ranked findings report with immediate remediation actions.

---

## Readings and References

- Cialdini, R.B., *Influence: The Psychology of Persuasion*, Revised Edition, Harper Business, 2021. (retrieved: 2026-05-29)
- CrowdStrike, "2025 Global Threat Report: How GenAI Powers Social Engineering." https://www.crowdstrike.com/en-us/resources/articles/crowdstrike-2025-global-threat-report-genai-powers-social-engineering/ (retrieved: 2026-05-29)
- Right-Hand Cybersecurity, "The State of Deep Fake Vishing Attacks in 2025." https://right-hand.ai/blog/deep-fake-vishing-attacks-2025/ (retrieved: 2026-05-29)
- StationX, "Social Engineering Statistics 2026: 100+ Facts & Trends." https://app.stationx.net/articles/social-engineering-statistics (retrieved: 2026-05-29)
- MITRE ATT&CK, "Phishing (T1566)." https://attack.mitre.org/techniques/T1566/ (retrieved: 2026-05-29)
- Gophish, "Open-Source Phishing Framework, v0.12.1." https://getgophish.com/ (retrieved: 2026-05-29)
- kgretzky, "Evilginx2: Standalone man-in-the-middle attack framework." https://github.com/kgretzky/evilginx2 (retrieved: 2026-05-29)
- OSINT Framework. https://osintframework.com/ (retrieved: 2026-05-29)
- ProjectDiscovery, "subfinder: Fast passive subdomain enumeration." https://github.com/projectdiscovery/subfinder (retrieved: 2026-05-29)
- PTES, "Penetration Testing Execution Standard — Social Engineering Section." http://www.pentest-standard.org/index.php/Social_Engineering (retrieved: 2026-05-29)
...
|---|---|---|
| Web payloads delivered via phishing (XSS, credential harvesting) | Domain 8, Chapters 8A-8B | Spear-phishing delivers web-based payloads; SSRF targeted via phishing links |
| Microsoft 365, Conditional Access, MFA bypass | Domain 14, Chapter 14B §3-5 | BEC targets M365; consent phishing exploits OAuth; MFA fatigue bypasses Conditional Access |
| Malware delivery via USB attacks and reverse shells | Domain 11, Chapter 11A | Rubber Ducky/Bash Bunny deliver C2 implants; Evilginx2 captures session tokens |
| SIM swapping and mobile network identity | Domain 15, Chapter 15B §3 | SIM swap exploits carrier subscriber management, not the radio protocol |
| Badge cloning and RFID exploitation | Domain 20, §3 | 125 kHz Proxmark3 cloning; upgrade path to 13.56 MHz with mutual authentication |
| Certificate Transparency and DNS security | Domain 9, Chapter 9A §3 | CT-log monitoring for typosquatting; SPF/DKIM/DMARC cryptographic signing |

---

## Glossary

- **BEC (Business Email Compromise):** A social engineering attack where the attacker impersonates or compromises a business email account to initiate fraudulent wire transfers or data theft.
- **Callback phishing (BazarCall):** A phishing technique where the email contains only a phone number; the victim calls and is socially engineered into installing malware.
- **Cialdini's principles:** Seven psychological principles of influence (reciprocity, commitment, social proof, authority, liking, scarcity, unity) exploited in social engineering.
- **DMARC (Domain-based Message Authentication, Reporting, and Conformance):** An email authentication protocol that ties SPF and DKIM together and specifies policy for failed messages.
- **Evilginx2:** A man-in-the-middle phishing framework that captures session tokens in real-time, bypassing TOTP and push MFA (but not FIDO2/WebAuthn).
- **FIDO2/WebAuthn:** A phishing-resistant authentication standard where the authenticator verifies the origin domain, preventing credential theft by proxy phishing.
- **Gophish:** An open-source phishing simulation platform for authorized security-awareness testing, tracking email opens, link clicks, and credential submissions.
- **HTML smuggling:** A payload delivery technique embedding Base64-encoded files in HTML that are assembled client-side via JavaScript, bypassing email-gateway scanning.
- **MFA fatigue (push bombing):** Sending repeated MFA push notifications to a target until they approve one out of annoyance or confusion.
- **OSINT (Open-Source Intelligence):** Intelligence derived from publicly available information — social media, DNS records, CT logs, breach databases, public documents.
- **Pretexting:** Creating a fabricated scenario to manipulate a target into providing information or access they would not normally give.
- **Quishing:** QR-code-based phishing where the phishing URL is encoded in a QR code image, bypassing text-based URL scanning in email gateways.
- **SIM swapping:** Fraudulently transferring a victim's phone number to an attacker-controlled SIM card to intercept SMS-based 2FA codes.
- **Sock puppet:** A fictitious online identity created for OSINT collection, community infiltration, or social engineering operations.
- **theHarvester:** An OSINT tool that aggregates email addresses, subdomains, hostnames, and names from multiple public sources for a target domain.
