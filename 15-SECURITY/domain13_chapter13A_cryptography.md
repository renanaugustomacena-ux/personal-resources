...
modulo: "13.1"
titolo: "Symmetric, Asymmetric, and Post-Quantum Cryptography"
versione: "OpenSSL 3.5 / FIPS 203-205 (2024-08-13) / liboqs 0.12 / hashcat 6.2"
livello: "Advanced"
prerequisiti:
  - "Modular arithmetic, finite fields (GF(2^n)), and group theory fundamentals"
  - "Domain 9, Chapter 9A §3 — TLS protocol mechanics"
  - "Domain 7, Chapter 7A §7 — Cache side-channel primitives (FLUSH+RELOAD, PRIME+PROBE)"
  - "Domain 7, Chapter 7B §3.1 — Power analysis (SPA/DPA) fundamentals"
  - "Familiarity with Python, SageMath, or equivalent for cryptanalysis scripting"
obiettivi:
  - "Exploit block cipher mode weaknesses — execute CBC bit-flipping, padding oracle (Vaudenay), and GCM nonce-reuse attacks — and implement AEAD-based hardening that eliminates each attack class"
  - "Attack RSA implementations via Bleichenbacher oracle, Coppersmith small-roots, Hastad broadcast, and Wiener/Boneh-Durfee methods; recover ECDSA private keys from nonce reuse and biased-nonce lattice attacks"
  - "Evaluate post-quantum algorithm trade-offs (ML-KEM, ML-DSA, SLH-DSA, Falcon, Classic McEliece), deploy hybrid key exchange (X25519Kyber768) in TLS 1.3, and plan CNSA 2.0 migration timelines"
  - "Detect and exploit RNG failures (Debian CVE-2008-0166, Dual_EC_DRBG, Android SecureRandom) and implement cryptographic PRNG validation across language ecosystems"
  - "Audit TLS deployments for protocol-level vulnerabilities (BEAST, CRIME, BREACH, POODLE, DROWN, ROBOT, Raccoon) using testssl.sh, sslyze, and nmap — and produce hardened nginx/Apache configurations"
tag: [cryptography, AES, RSA, ECC, post-quantum, TLS, side-channel, padding-oracle, GCM, key-management, PKI, password-hashing, AEAD, ML-KEM, ML-DSA]
---
...
> 1. Exploit CBC bit-flipping, padding oracle (Vaudenay/padbuster), and GCM nonce-reuse attacks against real TLS deployments; then harden configurations to AEAD-only cipher suites with correct nonce management.
> 2. Execute RSA attacks (Bleichenbacher PKCS#1 v1.5 oracle, e=3 forgery, Coppersmith small-roots, Hastad broadcast, Wiener continued-fraction) and ECC attacks (ECDSA nonce reuse, biased-nonce lattice, invalid curve) using RsaCtfTool, SageMath, and TLS-Attacker.
> 3. Compare post-quantum algorithms (ML-KEM/Kyber, ML-DSA/Dilithium, SLH-DSA/SPHINCS+, Falcon, Classic McEliece) on key/signature size, performance, and security assumptions; deploy hybrid X25519Kyber768 in TLS 1.3 and plan a CNSA 2.0 migration timeline.
> 4. Identify and exploit RNG failures across platforms (Debian weak keys, Dual_EC_DRBG backdoor, Android SecureRandom bias); validate entropy sources; and enforce cryptographic PRNG usage per language ecosystem.
> 5. Audit production TLS stacks for all named protocol attacks (BEAST, CRIME, BREACH, POODLE, DROWN, ROBOT, Raccoon, Heartbleed) using testssl.sh, sslyze, and nmap scripts; write Sigma/Suricata detection rules for weak cipher negotiation and downgrade attempts.
...
### Exercise 15.1 — Padding Oracle Attack Against a CBC-Mode Web Application

Deploy a deliberately vulnerable web application (e.g., `PentesterLab Padding Oracle` or a custom Flask app that encrypts session tokens with AES-128-CBC and returns distinct HTTP status codes for valid vs. invalid padding):

1. Confirm the padding oracle by sending the legitimate encrypted token (expect 200), then flipping the last byte of the ciphertext (expect 500 or a different error). Document the oracle signal (HTTP status, response body, timing).
2. Use `padbuster` to decrypt the session token block-by-block. Record the number of HTTP requests required and the wall-clock time. Verify the recovered plaintext matches the expected session data.
3. Use `padbuster` in encrypt mode to forge a new session token with `role=admin`. Submit the forged token and confirm privilege escalation. Write a Sigma rule that detects the attack based on rapid 4xx/5xx responses to the token-bearing endpoint.
4. Harden the application: switch from CBC to AES-256-GCM. Verify that `padbuster` can no longer extract information (no distinguishable oracle). Document the code changes required.

### Exercise 15.2 — GCM Nonce Reuse Exploitation and Key Recovery

Using a lab environment with a custom TLS server configured to reuse GCM nonces (intentionally misconfigured `--nonce-mode=static`):

1. Capture two TLS records encrypted under the same nonce using Wireshark. XOR the two ciphertexts to obtain the XOR of the plaintexts. If one plaintext is known (e.g., an HTTP response header), recover the other.
2. Using the `nonce-disrespecting-adversaries` tool, recover the GHASH authentication key H from the two ciphertext/tag pairs. Verify the recovered H by computing a valid authentication tag for a new message.
3. Forge a complete authenticated message with arbitrary content using the recovered H. Submit it to the server and verify acceptance. Document the full attack chain from nonce reuse to authenticated message forgery.
4. Reconfigure the server to use TLS 1.3 (which enforces unique-per-record nonces via XOR with sequence number). Verify that nonce reuse is no longer possible.

### Exercise 15.3 — RSA Bleichenbacher Oracle and ROBOT Scanner Validation

Against a lab TLS server supporting RSA key exchange (static RSA, not ECDHE):

1. Run the ROBOT scanner (`robot-detect.py`) against the server. Interpret the output: identify whether the oracle is based on different alert codes, timing differences, or connection behavior.
2. If the server is vulnerable, use TLS-Attacker's Bleichenbacher workflow to decrypt a captured RSA-encrypted premaster secret. Document the number of oracle queries required and the time to decryption.
3. Harden the server by disabling all `kRSA` cipher suites (keeping only ECDHE). Re-run the ROBOT scanner and confirm the server is no longer vulnerable. Enforce TLS 1.3 minimum protocol version.

### Exercise 15.4 — TLS Downgrade Attack Detection and Certificate Pinning Bypass

Using `testssl.sh` and manual `openssl s_client` probes against a production-representative TLS server:

1. Test for all named TLS attacks: `testssl.sh --full target.com:443`. Parse the output and document each finding with its CVE, severity, and required remediation.
2. Attempt a TLS downgrade: use `openssl s_client -ssl3`, `-tls1`, `-tls1_1` to test for protocol downgrade acceptance. Test for POODLE and BEAST by verifying SSL 3.0 and TLS 1.0 CBC cipher availability. Document which legacy protocols the server supports.
3. Deploy certificate pinning in a mobile test application (Android — using OkHttp `CertificatePinner`). Confirm that the app rejects connections when the server presents a different certificate. Use Frida with `ssl-pinning-bypass.js` to defeat the pinning at runtime. Document the Frida hook that replaces the certificate validation logic.
4. Evaluate the pinning bypass risk and recommend alternatives to certificate pinning (Certificate Transparency monitoring, DANE/TLSA records).

### Exercise 15.5 — Post-Quantum Hybrid Key Exchange Deployment and Verification

Using OpenSSL 3.5+ with the OQS provider (or a Cloudflare/AWS endpoint that supports X25519Kyber768):

1. Generate an ML-KEM-768 key pair with `openssl genpkey -algorithm mlkem768`. Examine the key sizes (public key, private key) and compare against classical RSA-2048 and ECDSA P-256 keys.
2. Configure an nginx test server with `ssl_ecdh_curve x25519_kyber768:X25519:secp384r1`. Connect with `openssl s_client -groups x25519_kyber768` and verify the handshake completes with hybrid key exchange. Capture the handshake in Wireshark and identify the key_share extension carrying the ML-KEM encapsulation.
3. Benchmark the handshake latency and throughput of hybrid X25519Kyber768 vs. classical X25519 using `openssl s_time`. Document the overhead introduced by the post-quantum component.
4. Generate an ML-DSA-65 signing key and sign a test artifact. Verify the signature. Compare signature size and verification time against Ed25519. Produce a migration readiness assessment referencing the CNSA 2.0 timeline.
...
- NIST FIPS 203 — ML-KEM (Module-Lattice-Based Key-Encapsulation Mechanism). Published 2024-08-13. <https://csrc.nist.gov/pubs/fips/203/final>
- NIST FIPS 204 — ML-DSA (Module-Lattice-Based Digital Signature Algorithm). Published 2024-08-13. <https://csrc.nist.gov/pubs/fips/204/final>
- NIST FIPS 205 — SLH-DSA (Stateless Hash-Based Digital Signature Algorithm). Published 2024-08-13. <https://csrc.nist.gov/pubs/fips/205/final>
...
- NSA CNSA 2.0 — Commercial National Security Algorithm Suite 2.0 (September 2022). <https://media.defense.gov/2022/Sep/07/2003071834/-1/-1/0/CSA_CNSA_2.0_ALGORITHMS_.PDF>
- RFC 8446 — The Transport Layer Security (TLS) Protocol Version 1.3.
- RFC 7693 — The BLAKE2 Cryptographic Hash and Message Authentication Code (MAC).
- RFC 6979 — Deterministic Usage of the Digital Signature Algorithm (DSA) and Elliptic Curve Digital Signature Algorithm (ECDSA).
- RFC 9580 — OpenPGP (LibrePGP).

### Tools and Frameworks

- testssl.sh — TLS/SSL Testing Tool. <https://testssl.sh/>
- sslyze — Fast and Full-Featured SSL/TLS Scanner. <https://github.com/nabla-c0d3/sslyze>
- RsaCtfTool — RSA Attack Toolkit. <https://github.com/RsaCtfTool/RsaCtfTool>
- hashcat — Advanced Password Recovery. <https://hashcat.net/hashcat/>
- John the Ripper — Password Cracker. <https://www.openwall.com/john/>
- nonce-disrespecting-adversaries — GCM Nonce Reuse Exploitation. <https://github.com/nonce-disrespect/nonce-disrespect>
- TLS-Attacker — TLS Protocol Analysis Framework. <https://github.com/tls-attacker/TLS-Attacker>
- ROBOT Scanner — Bleichenbacher Oracle Detection. <https://github.com/robotattackorg/robot-detect>
- ChipWhisperer — Open-Source Side-Channel Analysis Platform. <https://github.com/newaetech/chipwhisperer>
- phoenixAES — DFA Key Recovery Tool. <https://github.com/SideChannelMarvels/JeanGrey>
- roca-detect — ROCA Vulnerable Key Detection. <https://github.com/crocs-muni/roca>
- Open Quantum Safe (liboqs) — PQC Library. <https://openquantumsafe.org/>
- oqs-provider — OpenSSL 3.x PQC Provider. <https://github.com/open-quantum-safe/oqs-provider>
- padbuster — Padding Oracle Exploitation Tool. <https://github.com/AonCyberLabs/PadBuster>
- HashPump — Length Extension Attack Tool. <https://github.com/bwall/HashPump>
...
- Sotirov, A. et al. "MD5 Considered Harmful Today: Creating a Rogue CA Certificate." 2008.

### CVEs Referenced

- CVE-2014-0160 — Heartbleed: OpenSSL TLS Heartbeat buffer over-read. CVSS 7.5.
- CVE-2017-15361 — ROCA: Infineon RSA key generation weakness. CVSS 5.9.
- CVE-2019-15809 — Minerva: ECDSA timing side-channel on Athena IDProtect. CVSS 5.9.
- CVE-2020-1968 — Raccoon: TLS-DH timing side-channel. CVSS 3.7.
- CVE-2008-0166 — Debian OpenSSL PRNG entropy reduction. CVSS 7.5.
- CVE-2015-7755 — Juniper ScreenOS unauthorized code (Dual_EC_DRBG). CVSS 10.0.
- CVE-2017-13077 — KRACK: WPA2 key reinstallation attack. CVSS 6.8.
- CVE-2025-59438 — Mbed TLS CBC-PKCS7 padding oracle via timing. CVSS pending. <https://mbed-tls.readthedocs.io/en/latest/security-advisories/mbedtls-security-advisory-2025-10-invalid-padding-error/>
...
|---------|---------------|-------------------|-------------|
| §1 Symmetric Cryptography (AES, GCM) | Domain 9 | Chapter 9A §3.1 (TLS cipher suites) | GCM and ChaCha20-Poly1305 are the AEAD cipher suites in TLS 1.3; mode vulnerabilities directly affect TLS security |
| §1.6 AES Side-Channels | Domain 7 | Chapter 7A §7 (Cache side-channels) | FLUSH+RELOAD and PRIME+PROBE on AES T-tables use the cache-attack primitives from Domain 7 |
| §2 RSA/ECC Attacks | Chapter 13B | §2.1 ROBOT / §2.2 DROWN | Protocol-level RSA attacks (ROBOT, DROWN) exploit the Bleichenbacher oracle described here |
| §4 Post-Quantum Cryptography | Domain 9 | Chapter 9A §3.1 (TLS key exchange) | Hybrid X25519Kyber768 is deployed in TLS 1.3 via the key_share extension |
| §2.5 Password Hashing | Domain 14 | Chapter 14A §1 (Kerberoasting / AS-REP) | hashcat modes for Kerberoast (13100), AS-REP (18200), NTLM (1000) use the hash formats from §2.4 |
| §11 Key Management / PKI | Domain 10 | Chapter 10A (Cloud security) | Cloud KMS (AWS KMS, Azure Key Vault, GCP Cloud KMS) implements the envelope encryption pattern from §11.5 |
...
| Term | Definition |
|------|-----------|
| **AEAD (Authenticated Encryption with Associated Data)** | An encryption scheme that simultaneously provides confidentiality, integrity, and authenticity — e.g., AES-GCM, ChaCha20-Poly1305, AES-CCM. |
| **CBC (Cipher Block Chaining)** | A block cipher mode where each plaintext block is XORed with the previous ciphertext block before encryption; vulnerable to bit-flipping and padding oracle attacks. |
| **GCM (Galois/Counter Mode)** | An AEAD mode combining CTR-mode encryption with GHASH polynomial authentication; catastrophically broken by nonce reuse. |
| **GHASH** | The universal hash function used in GCM for authentication, based on multiplication in GF(2^128); the authentication key H is derived from the encryption key. |
| **PKCS#7 Padding** | A padding scheme where the last N bytes of a block are each set to the value N; used in CBC mode to ensure the plaintext is a multiple of the block size. |
| **Padding Oracle** | An information leak where a system reveals whether decrypted ciphertext has valid padding, enabling byte-by-byte plaintext recovery (Vaudenay attack). |
| **OAEP (Optimal Asymmetric Encryption Padding)** | An RSA encryption padding scheme (PKCS#1 v2) that uses random padding to prevent Bleichenbacher-style chosen-ciphertext attacks. |
| **KEM (Key Encapsulation Mechanism)** | A public-key primitive where the sender encapsulates a shared secret that only the private-key holder can decapsulate; the post-quantum replacement for DH/ECDH key exchange. |
| **ML-KEM (Module-Lattice-Based KEM)** | NIST FIPS 203 — the primary post-quantum key encapsulation standard (formerly Kyber), based on the Module Learning With Errors problem. |
| **ML-DSA (Module-Lattice-Based Digital Signature Algorithm)** | NIST FIPS 204 — the primary post-quantum digital signature standard (formerly Dilithium), using the Fiat-Shamir with Aborts paradigm. |
| **SLH-DSA** | NIST FIPS 205 — a stateless hash-based signature scheme (formerly SPHINCS+); the most conservative PQC signature choice, depending only on hash function security. |
| **DPA (Differential Power Analysis)** | A side-channel attack that statistically correlates power consumption traces with hypothetical intermediate cryptographic values to recover key material. |
| **HSM (Hardware Security Module)** | A tamper-resistant hardware device that performs cryptographic operations and protects key material; keys generated inside an HSM never leave in plaintext. |
| **HMAC** | Hash-based Message Authentication Code — H((K XOR opad) || H((K XOR ipad) || m)); secure even when the underlying hash has broken collision resistance. |
| **Birthday Bound** | The expected number of evaluations (~2^(n/2) for n-bit output) before a collision occurs in a hash function or a repeated random value (nonce, IV). |
