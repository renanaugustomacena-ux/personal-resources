...
fase: "Domain 13 — Cryptography"
modulo: "13.2"
titolo: "Protocol Attacks"
versione: "TLS 1.3 (RFC 8446) / SSH OpenSSH 9.9 / QUIC RFC 9000 / Kerberos RFC 4120"
livello: "Advanced"
prerequisiti:
  - "Domain 13, Chapter 13A — Symmetric, asymmetric, and post-quantum cryptography"
  - "Domain 9, Chapter 9A — Network protocols and TLS mechanics"
  - "Domain 14, Chapter 14A — Active Directory and Kerberos architecture"
  - "TCP/IP stack internals (ARP, DNS, BGP, DHCP, NTP at packet level)"
  - "Familiarity with Wireshark, Scapy, Impacket, and network capture analysis"
obiettivi:
  - "Execute and detect key exchange attacks (Logjam, FREAK, small subgroup confinement) and TLS protocol attacks (ROBOT, DROWN, POODLE, BEAST, ALPACA, Raccoon) with full exploitation chains and hardening procedures"
  - "Forge and detect Kerberos tickets (Golden, Silver, Diamond, Sapphire) and abuse delegation (S4U2Self/S4U2Proxy, unconstrained delegation) — with Sigma/KQL detection rules and Active Directory hardening"
  - "Perform NTLM relay attacks via PrinterBug, PetitPotam, and Responder — relay to SMB, LDAP, and ADCS — and implement EPA, SMB signing, and LDAP channel binding as countermeasures"
  - "Exploit DNS infrastructure via Kaminsky cache poisoning, DNS rebinding, DNS tunneling, and amplification attacks — deploy DNSSEC, source-port randomization, and entropy-based tunneling detection"
  - "Attack and harden Layer 2/3 protocols: ARP spoofing with DAI, VLAN hopping with DTP disablement, BGP prefix hijacking with RPKI/ROA, DHCP starvation with snooping, and 802.1X bypass with EAP-TLS enforcement"
tag: [protocol-attacks, TLS, Kerberos, NTLM, DNS, ARP, BGP, VLAN, SSH, HTTP2, QUIC, 802.1X, DHCP, NTP, SMTP, LDAP-injection]
---
...
> 1. Execute TLS protocol attacks (ROBOT, DROWN, POODLE, BEAST, ALPACA, Raccoon, SWEET32) end-to-end — from vulnerability scanning through exploitation — and deploy protocol-level mitigations (TLS 1.3 enforcement, ECDHE-only cipher suites, HSTS preloading).
> 2. Forge Kerberos tickets (Golden, Silver, Diamond, Sapphire), abuse delegation primitives (S4U2Self/S4U2Proxy, unconstrained delegation), and perform Kerberoasting and AS-REP Roasting — then detect each attack via Windows Event Log correlation, Sigma rules, and KQL queries.
> 3. Conduct NTLM relay attacks (Responder, ntlmrelayx) via coercion triggers (PrinterBug, PetitPotam) and relay to SMB, LDAP, and ADCS targets — then harden with SMB signing, LDAP channel binding, EPA, and NTLM disablement.
> 4. Exploit DNS infrastructure (Kaminsky cache poisoning, rebinding, tunneling, amplification) and implement layered defenses (DNSSEC validation, source-port randomization, entropy-based tunneling detection, BCP38 ingress filtering).
> 5. Attack and defend Layer 2/3 infrastructure: ARP spoofing with DAI countermeasures, VLAN hopping via DTP negotiation with nonegotiate hardening, BGP prefix hijacking with RPKI/ROA validation, and 802.1X bypass with EAP-TLS enforcement.
...
### Exercise 18.1 — TLS Downgrade Attack Simulation and Detection

Using a lab server running nginx with a deliberately permissive TLS configuration (SSLv3 + TLS 1.0/1.1/1.2/1.3, all cipher suites including CBC and EXPORT):

1. Run `testssl.sh --full` against the server. Catalog every finding: deprecated protocols, weak ciphers, named vulnerabilities (POODLE, BEAST, SWEET32, DROWN, ROBOT, Raccoon). Map each finding to its CVE and MITRE ATT&CK technique ID.
2. Attempt active exploitation of POODLE: configure the server to accept SSL 3.0, use `openssl s_client -ssl3` to force a downgrade, and confirm the connection succeeds with a CBC cipher. Write a Suricata rule that detects SSL 3.0 negotiation attempts.
3. Harden the server iteratively: disable SSLv3, disable TLS 1.0/1.1, remove all CBC and EXPORT cipher suites, enforce ECDHE-only key exchange, and enable HSTS with preloading. After each change, re-run `testssl.sh` and verify the vulnerability is eliminated. Produce a final configuration matching the Mozilla "Modern" profile.
4. Deploy `TLS_FALLBACK_SCSV` (RFC 7507) and verify that a client attempting a downgrade from TLS 1.3 to TLS 1.2 receives an `inappropriate_fallback` alert.

### Exercise 18.2 — Padding Oracle Attack on a Deployed Endpoint

Deploy a vulnerable web application that encrypts session data with AES-128-CBC (e.g., using the `PentesterLab Padding Oracle` exercise or a custom Flask/Django app):

1. Identify the padding oracle by testing with a valid ciphertext (200 OK) and a corrupted ciphertext (500 error). Measure the timing difference between valid-padding and invalid-padding responses using `curl` with `time`.
2. Use `padbuster` to decrypt the session cookie. Document the recovered plaintext, the total number of requests, and the average requests per byte.
3. Use `padbuster -encrypt` to forge a session cookie with elevated privileges (e.g., `admin=true`). Verify the server accepts the forged cookie.
4. Fix the application by migrating to AES-256-GCM. Re-test and confirm the padding oracle no longer exists. Write a KQL query that would detect the brute-force pattern in web server logs (high-frequency 4xx/5xx responses from a single source IP to a session-handling endpoint).

### Exercise 18.3 — Certificate Pinning Bypass on Android with Frida

Using an Android emulator or rooted device with a target application that implements certificate pinning (e.g., via OkHttp `CertificatePinner` or the Android Network Security Config):

1. Configure a MitM proxy (Burp Suite or mitmproxy) and install its CA certificate on the device. Confirm that the target app rejects the proxy's certificate with a pinning violation error.
2. Use Frida with a universal SSL pinning bypass script (e.g., `fridascript/ssl-pinning-bypass.js` or `objection explore --startup-command "android sslpinning disable"`). Attach to the running app and confirm that HTTPS traffic is now visible in the proxy.
3. Identify the specific pinning implementation by searching the APK for `CertificatePinner`, `X509TrustManager`, or `network_security_config.xml`. Write a targeted Frida hook that replaces only the relevant trust manager's `checkServerTrusted` method. Document the hook code and its effect.
4. Discuss the limitations of certificate pinning as a security control. Evaluate alternatives: Certificate Transparency monitoring, DANE/TLSA DNS records, and runtime attestation.

### Exercise 18.4 — Kerberos Golden Ticket Forge, Detection, and Remediation

In a Windows Active Directory lab environment (domain controller + workstation):

1. Compromise the KRBTGT hash using DCSync (with Impacket `secretsdump.py` or Mimikatz `lsadump::dcsync /user:krbtgt`). Document the extracted hash.
2. Forge a Golden Ticket for a non-existent user with Domain Admin privileges using Mimikatz (`kerberos::golden`) or Impacket (`ticketer.py`). Use the forged TGT to access the domain controller via `psexec`. Confirm access and document the TGT lifetime.
3. On the domain controller, examine the Windows Security Event Log for anomalies: TGTs with abnormal lifetimes (Event ID 4769), TGTs for non-existent users, and TGTs issued without corresponding AS-REQ events (Event ID 4768). Write a Sigma rule targeting these indicators.
4. Remediate: perform a double KRBTGT password rotation (reset twice with at least 12 hours between resets to allow replication). Verify that the old Golden Ticket no longer works. Implement detection via Microsoft Defender for Identity or a SIEM correlation rule.

### Exercise 18.5 — DNS Tunneling Detection via Entropy Analysis

Set up a DNS tunneling lab with `iodine` (IP-over-DNS) or `dnscat2` (encrypted C2-over-DNS):

1. Configure the server on an authoritative nameserver for a test domain. Connect the client from an internal network. Generate traffic through the tunnel (e.g., SSH session or file transfer).
2. Capture DNS traffic on the internal resolver with `tcpdump`. Analyze the captures: compute Shannon entropy of the subdomain labels for tunnel traffic vs. normal DNS queries. Document the entropy distribution difference (normal: ~3.5–4.0 bits/char; tunnel: ~4.5–5.0+ bits/char).
3. Implement the Python entropy analysis script from §6.3. Feed it the captured DNS queries and verify it correctly classifies tunnel queries as suspicious (entropy > 4.2, label length > 20, subdomain > 50 chars).
4. Deploy Suricata rules from §6.3 on the internal network. Verify that the rules fire on tunnel traffic. Harden the resolver: force all DNS through the internal resolver, block external DNS (port 53) at the firewall, and configure `unbound` with private-address blocking to prevent DNS rebinding.
...
- RFC 5575 — BGP Flow Specification Rules. <https://www.rfc-editor.org/rfc/rfc5575>
- RFC 8461 — SMTP MTA Strict Transport Security (MTA-STS). <https://www.rfc-editor.org/rfc/rfc8461>

### Tools and Frameworks

- testssl.sh — TLS/SSL Testing Tool. <https://testssl.sh/>
- Impacket — Network Protocol Tool Suite. <https://github.com/fortra/impacket>
- Mimikatz — Windows Credential Extraction. <https://github.com/gentilkiwi/mimikatz>
- Responder — LLMNR/NBNS/mDNS Poisoner. <https://github.com/lgandx/Responder>
- Rubeus — Kerberos Abuse Toolkit. <https://github.com/GhostPack/Rubeus>
- Bettercap — Network Attack and Monitoring Framework. <https://www.bettercap.org/>
- iodine — IP-over-DNS Tunnel. <https://github.com/yarrick/iodine>
- dnscat2 — Encrypted DNS C2 Channel. <https://github.com/iagox86/dnscat2>
- Singularity of Origin — DNS Rebinding Framework. <https://github.com/nccgroup/singularity>
- Routinator — RPKI Relying Party Software. <https://github.com/NLnetLabs/routinator>
- ssh-audit — SSH Server/Client Auditing. <https://github.com/jtesta/ssh-audit>
- Yersinia — Layer 2 Attack Framework. <https://github.com/tomac/yersinia>
- hostapd-mana — Rogue AP with EAP Capture. <https://github.com/sensepost/hostapd-mana>
- PetitPotam — NTLM Relay Coercion via EFS RPC. <https://github.com/topotam/PetitPotam>
- pybgpstream — BGP Monitoring API (CAIDA). <https://bgpstream.caida.org/>

### Research Papers and Advisories

- Kaminsky, D. "Black Ops of DNS — It's the End of the Cache as We Know It." Black Hat USA, 2008.
- Adrian, D. et al. "Imperfect Forward Secrecy: How Diffie-Hellman Fails in Practice." ACM CCS 2015. (Logjam.)
- Bock, M. et al. "Return Of Bleichenbacher's Oracle Threat (ROBOT)." USENIX Security 2018.
- Aviram, N. et al. "DROWN: Breaking TLS Using SSLv2." USENIX Security 2016.
- Baeumer, M. et al. "Terrapin Attack: Breaking SSH Channel Integrity By Sequence Number Manipulation." USENIX Security 2024.
- Cloudflare, Google, AWS. "HTTP/2 Rapid Reset: Deconstructing the Record-Breaking Attack." October 2023. (CVE-2023-44487.)
- DeHaan, A. et al. "Blast-RADIUS: Forgery Attack on RADIUS/UDP." July 2024. (CVE-2024-3596.)

### CVEs Referenced

- CVE-2015-4000 — Logjam: TLS DHE export cipher downgrade. CVSS 3.7.
- CVE-2016-0800 — DROWN: SSLv2 cross-protocol attack on TLS. CVSS 5.9.
- CVE-2014-3566 — POODLE: SSL 3.0 CBC padding oracle. CVSS 3.4.
- CVE-2011-3389 — BEAST: TLS 1.0 predictable IV attack. CVSS 4.3.
- CVE-2023-44487 — HTTP/2 Rapid Reset DDoS attack. CVSS 7.5.
- CVE-2023-48795 — Terrapin: SSH sequence number manipulation. CVSS 5.9. <https://terrapin-attack.com/>
- CVE-2024-3596 — BlastRADIUS: RADIUS protocol forgery via MD5 collision. CVSS 9.0. <https://www.blastradius.fail/>
- CVE-2025-59438 — Mbed TLS CBC-PKCS7 padding oracle via timing. <https://mbed-tls.readthedocs.io/en/latest/security-advisories/mbedtls-security-advisory-2025-10-invalid-padding-error/>
- CVE-2009-3555 — TLS renegotiation MitM injection. CVSS 5.8.
- CVE-2008-1447 — Kaminsky DNS cache poisoning. CVSS 5.0.
- CVE-2021-36942 — PetitPotam: Windows EFS NTLM relay coercion. CVSS 5.3.
...
|---------|---------------|-------------------|-------------|
| §1–§2 TLS / Key Exchange Attacks | Chapter 13A | §1–§2 (Symmetric/Asymmetric crypto) | Protocol attacks exploit the cryptographic primitives (CBC padding, RSA PKCS#1, DH) described in 13A |
| §4 Kerberos Attacks | Domain 14 | Chapter 14A §1 (AD attack paths) | Kerberoasting, AS-REP Roasting, delegation abuse are the credential-theft primitives for AD lateral movement |
| §5 NTLM Relay | Domain 14 | Chapter 14A §2 (Credential relay) | NTLM relay to LDAP/SMB/ADCS enables the AD escalation paths (RBCD, certificate theft) in Domain 14 |
| §6 DNS Attacks | Domain 9 | Chapter 9A §2 (DNS security) | DNS cache poisoning, rebinding, and tunneling expand the DNS security model from Domain 9 with offensive techniques |
| §7 ARP / §15 VLAN / §9 DHCP | Domain 9 | Chapter 9B §1 (Layer 2 security) | Layer 2 attacks (ARP spoofing, VLAN hopping, DHCP starvation) target the switching infrastructure from Domain 9 |
| §13 SSH / Terrapin | Domain 7 | Chapter 7A §4 (Host hardening) | SSH hardening (sshd_config, algorithm selection) implements the host-level controls from Domain 7 |
...
| Term | Definition |
|------|-----------|
| **TGT (Ticket-Granting Ticket)** | A Kerberos ticket issued by the KDC after initial authentication, encrypted with the KRBTGT hash; presented to obtain service tickets. |
| **Golden Ticket** | A forged TGT created with the KRBTGT hash, granting the attacker arbitrary access to any service in the domain with any identity. |
| **Silver Ticket** | A forged service ticket created with a service account's hash, granting access to a specific service without touching the KDC. |
| **NTLM Relay** | An attack where the attacker forwards a victim's NTLM authentication to a different service, authenticating as the victim without knowing the password. |
| **PetitPotam** | A coercion technique (CVE-2021-36942) that triggers Windows EFS RPC to authenticate to an attacker-controlled server, enabling NTLM relay. |
| **Terrapin Attack** | CVE-2023-48795 — an SSH prefix truncation attack that manipulates sequence numbers during the handshake to disable security extensions. |
| **BlastRADIUS** | CVE-2024-3596 — a RADIUS protocol forgery attack exploiting MD5 chosen-prefix collisions in the Response Authenticator to modify Access-Accept/Reject packets. |
| **RPKI (Resource Public Key Infrastructure)** | A hierarchical PKI that binds IP prefixes to authorized origin ASes via signed ROA objects, preventing BGP prefix hijacking. |
| **ROA (Route Origin Authorization)** | A signed RPKI object stating which AS is authorized to originate a specific IP prefix, validated by relying-party validators (Routinator, rpki-client). |
| **DAI (Dynamic ARP Inspection)** | A Cisco switch feature that validates ARP packets against the DHCP snooping binding table, preventing ARP spoofing attacks. |
| **HSTS (HTTP Strict Transport Security)** | A response header that instructs browsers to only access the site via HTTPS, preventing SSL stripping and downgrade attacks. |
| **DTP (Dynamic Trunking Protocol)** | A Cisco proprietary protocol that automatically negotiates trunk links between switches; exploitable for VLAN hopping when not disabled on access ports. |
| **Padding Oracle** | An information leak where a decryption endpoint reveals whether ciphertext has valid padding, enabling byte-by-byte plaintext recovery without the key. |
| **DNS Rebinding** | An attack where a malicious DNS server alternates between external and internal IP addresses for the same domain, bypassing the browser's same-origin policy. |
| **Shannon Entropy** | A measure of information density (bits per symbol); used in DNS tunneling detection where tunnel traffic exhibits higher entropy (~4.5+) than normal queries (~3.5). |
