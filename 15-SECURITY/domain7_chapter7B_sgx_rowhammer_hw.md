...
fase: "Domain 7 — Hardware Attacks & TEEs"
modulo: "7.2"
titolo: "SGX Security, Rowhammer, and Hardware Vulnerability Taxonomy"
versione: "Intel SGX (Xeon only, deprecated client), Intel TDX (Sapphire Rapids+), AMD SEV-SNP (EPYC 9004), DDR4/DDR5, ARMv9 CCA"
livello: "Advanced"
prerequisiti:
  - "Chapter 7A — Speculative execution model, cache side channels, timing primitives"
  - "Domain 2 — Page tables, memory management, virtual memory layout"
  - "Domain 5 — Kernel exploitation techniques and SLUB allocator internals"
  - "Understanding of DRAM architecture (banks, rows, refresh cycles)"
  - "Familiarity with attestation concepts (PKI, remote verification, trusted computing base)"
obiettivi:
  - "Trace the SGX enclave lifecycle (ECREATE→EINIT→EENTER→EEXIT) and identify side-channel exposure at each stage"
  - "Verify DCAP attestation chains and evaluate TCB recovery impact on production SGX/TDX deployments"
  - "Execute double-sided Rowhammer with clflush-based cache bypass and reverse-engineer DRAM bank addressing via timing probes"
  - "Analyze TRR evasion techniques (TRRespass, Blacksmith, Phoenix CVE-2025-6202) and assess DDR5 ODECC limitations"
  - "Evaluate the TEE.fail DDR5 memory-bus interposition attack and its implications for SGX, TDX, and SEV-SNP confidential computing"
tag: [security, SGX, TDX, SEV-SNP, rowhammer, DRAM, DDR5, side-channel, Foreshadow, MDS, Plundervolt, AEPIC, TRR, Blacksmith, Phoenix, TEE, attestation, hardware-security]
---

# Domain 7, Chapter 7B — SGX Security, Rowhammer, and Hardware Vulnerability Taxonomy

> **Learning objectives.** After completing this chapter you will be able to: (1) trace the full DCAP attestation flow (EREPORT → QE signing → collateral retrieval → quote verification) and identify failure modes that permit forged attestation; (2) implement double-sided Rowhammer with `clflush`-based cache bypass, reverse-engineer DRAM bank addressing via timing probes, and assess TRR evasion using many-sided patterns; (3) evaluate the Phoenix (CVE-2025-6202) attack against DDR5 ODECC and explain why on-die ECC does not prevent Rowhammer exploitation; (4) analyze the TEE.fail DDR5 memory-bus interposition attack and assess its impact on SGX, TDX, and SEV-SNP confidential computing deployments; (5) design a hardware-security monitoring strategy using EDAC trending, performance-counter anomaly detection, and TEE attestation failure alerting.
...
Hands-on exercises for this chapter. For guided lab walkthroughs see `tutorials/tutorial_domain7_ch7B_sgx_rowhammer_lab.md`.

1. **Rowhammer detection and exploitation.** On a DDR4 test system (non-production): (a) compile and run the double-sided Rowhammer code from §2.2, targeting a known bank/row derived from timing-based DRAM address probing (§2.3); (b) use `memtest86+` or Google's `rowhammer-test` to identify vulnerable rows on your specific DIMM; (c) measure the number of activations required for the first bit flip and compare against the theoretical refresh-interval window (64ms / tRC); (d) implement the many-sided pattern from §2.4 with 8 aggressor rows and verify that it induces flips on TRR-enabled DIMMs where double-sided fails; (e) check your system's EDAC (Error Detection and Correction) logs via `edac-util -s` and `/sys/devices/system/edac/mc*/` to determine whether the flips are being detected by ECC. Document all results with UTC timestamps.

2. **SGX attestation verification (DCAP).** Using Intel's DCAP sample code and the SGX SDK: (a) build and run a simple enclave that generates a local report via `sgx_create_report`; (b) send the report to the Quoting Enclave and obtain a quote; (c) implement a verifier that retrieves PCK certificates and TCB Info from Intel's PCS (or a local PCCS cache); (d) verify the quote's ECDSA signature, PCK certificate chain, and TCB level; (e) intentionally modify one byte of the quote and confirm that verification fails. If SGX hardware is unavailable, use the SGX simulation mode and document which verification steps are skipped in simulation. Assess what happens when the platform's TCB version is older than the verifier's minimum requirement.

3. **TRR evasion analysis with Blacksmith patterns.** (a) Study the Blacksmith open-source fuzzer (https://github.com/comsec-ethz/blacksmith) and document its pattern generation algorithm (exploration phase: random non-uniform patterns; exploitation phase: mutation of flip-inducing patterns). (b) On a DDR4 test system, run Blacksmith with the default configuration for 30 minutes and record: number of patterns tested, number of flip-inducing patterns found, minimum activation count per pattern, and which aggressor frequencies were most effective. (c) Compare the Blacksmith results against a simple double-sided hammer on the same DIMM. (d) If DDR5 is available, attempt the Phoenix (CVE-2025-6202) synchronization pattern and document whether ODECC prevents visible flips at the application level. (e) Calculate the expected ODECC correction rate given the observed flip density and explain why ODECC's write-time-only correction allows sustained exploitation.

4. **DRAM address reverse-engineering.** (a) Implement the DRAMA timing-based bank detection from §2.3: allocate a 256MB region with huge pages, measure pairwise access latencies for 1000 address pairs, and cluster addresses by bank based on row-conflict timing (>300 cycles = same bank, different row). (b) Derive the XOR-based bank-index function by testing individual address bits: for each bit position 6-20, compare the bank classification with and without the bit set. (c) Validate your derived mapping by predicting which address pairs will exhibit row conflicts and measuring the prediction accuracy. (d) Use `/proc/PID/pagemap` (as root) to obtain physical addresses and verify your virtual-to-DRAM mapping. (e) Document the complete channel/rank/bank/row mapping function for your specific CPU and memory controller.

5. **TEE threat model comparison and hardening assessment.** (a) Create a comparison table of SGX, TDX, and SEV-SNP covering: isolation granularity, memory encryption scheme, integrity protection, attestation model, threat model exclusions, and known attack classes (controlled-channel, MDS, L1TF, Plundervolt, TEE.fail). (b) For each TEE, identify which attacks from this chapter are applicable and which are mitigated by the architecture. (c) Assess the TEE.fail DDR5 memory-bus interposition attack: document the hardware cost (<$1000), the attack procedure (interposer insertion, DIMM downclocking to 3200 MT/s, logic analyzer capture), and which TEE properties it violates (confidentiality, integrity, or both). (d) Design a detection strategy for physical memory-bus interposition (e.g., performance monitoring for unexpected memory latency increases, attestation-based integrity checks, anti-tamper chassis sensors). (e) Write a one-page recommendation for a production confidential-computing deployment choosing between TDX and SEV-SNP, citing the specific attack surface differences.
...
1. Kim, Y. et al. "Flipping Bits in Memory Without Accessing Them: An Experimental Study of DRAM Disturbance Errors." ISCA 2014. Original Rowhammer paper. https://users.ece.cmu.edu/~yoonguk/papers/kim-isca14.pdf
2. Jattke, P. et al. "Blacksmith: Scalable Rowhammering in the Frequency Domain." S&P 2022. Non-uniform TRR bypass. https://comsec.ethz.ch/research/dram/blacksmith/
3. Lipp, M. et al. "Phoenix: Rowhammer Attacks on DDR5 with Self-Correcting Synchronization." S&P 2026. CVE-2025-6202. https://comsec.ethz.ch/research/dram/phoenix/
4. Jattke, P. et al. "TRRespass: Exploiting the Many Sides of Target Row Refresh." S&P 2020. Many-sided TRR evasion. https://www.vusec.net/projects/trrespass/
5. Trippel, C. et al. "Half-Double: Hammering From the Next Row Over." 2021. Distance-two Rowhammer. https://github.com/google/hammer-kit
6. Xu, Y. et al. "Controlled-Channel Attacks: Deterministic Side Channels for Untrusted Operating Systems." S&P 2015. SGX page-fault side channel. https://ieeexplore.ieee.org/document/7163052
7. Van Bulck, J. et al. "Foreshadow: Extracting the Keys to the Intel SGX Kingdom." USENIX Security 2018. CVE-2018-3615. https://foreshadowattack.eu/
8. Van Bulck, J. et al. "SGX-Step: A Practical Attack Framework for Precise Enclave Execution Control." SysTEX 2017. https://github.com/jovanbulck/sgx-step
9. Schwarz, M. et al. "ZombieLoad: Cross-Privilege-Boundary Data Sampling." CCS 2019. CVE-2019-11091. https://zombieloadattack.com/
10. Murdock, K. et al. "Plundervolt: Software-based Fault Injection Attacks against Intel SGX." S&P 2020. CVE-2019-11157. https://plundervolt.com/
11. Buhren, R. et al. "TEE.fail: Breaking Trusted Execution Environments via DDR5 Memory Bus Interposition." 2025. https://tee.fail/
12. CVE-2025-6202 — Phoenix Rowhammer DDR5, SK Hynix modules. https://nvd.nist.gov/vuln/detail/CVE-2025-6202
13. CVE-2022-21233 — AEPIC Leak, architectural APIC MMIO data leakage. https://nvd.nist.gov/vuln/detail/CVE-2022-21233
14. Intel DCAP documentation and sample code. https://github.com/intel/SGXDataCenterAttestationPrimitives
15. MITRE ATT&CK T1068 — Exploitation for Privilege Escalation (Rowhammer PTE flip → root). https://attack.mitre.org/techniques/T1068/
16. MITRE ATT&CK T1203 — Exploitation for Client Execution (browser-based Rowhammer via SMASH). https://attack.mitre.org/techniques/T1203/
17. Red Hat — DRAM-Related Faults advisory (Rowhammer, ZenHammer, TRRespass, Blacksmith, Half-Double). https://access.redhat.com/articles/1377393
18. Google Project Zero — Rowhammer research and hammer-kit tools. https://blog.google/security/supporting-rowhammer-research-to/
19. Microsoft Azure — SGX DCsv2 retirement announcement (June 2025, retirement by June 2026). https://azure.microsoft.com/en-us/updates/
20. Pessl, P. et al. "DRAMA: Exploiting DRAM Addressing for Cross-CPU Attacks." USENIX Security 2016. Timing-based DRAM address reverse-engineering. https://www.usenix.org/conference/usenixsecurity16/technical-sessions/presentation/pessl
...
|---|---|---|
| Domain 7, Ch 7A (Spectre, cache side channels) | §1.4.2 cache-timing on SGX, §1.4.3 L1TF/Foreshadow | FLUSH+RELOAD and PRIME+PROBE primitives from Ch 7A are the extraction mechanisms for SGX enclave data |
| Domain 6, Ch 6A (ASLR/DEP/CFI bypass) | §2.6 PTE bit-flip exploitation | Rowhammer PTE corruption achieves ASLR/DEP bypass at the hardware level without software vulnerabilities |
| Domain 6, Ch 6B (kernel mitigation bypass) | §2.6 Rowhammer → kernel page-table manipulation | PTE bit-flips from Rowhammer provide an alternative to software-based kernel write primitives for privilege escalation |
| Domain 5, Ch 5A (kernel exploitation) | §2.7 exploitation primitives | Rowhammer-induced bit flips create arbitrary read/write primitives that feed the same kernel exploit chains as software UAF/overflow |
| Domain 2, Ch 2A (page tables, memory management) | §1.1 EPC architecture, §2.1 DRAM organization | Page-table structure defines which PTE bits are security-critical for Rowhammer targeting; DRAM hierarchy maps to the physical attack surface |
| Domain 2, Ch 2B–2C (seccomp, capabilities, LSMs) | §1.3 attestation, §1.5 SGX defense checklist | Attestation verifies TEE integrity; seccomp/LSM confinement limits damage from TEE bypass in containerized deployments |
...
| Term | Definition |
|---|---|
| **SGX** | Software Guard Extensions — Intel hardware enclaves providing encrypted, integrity-protected memory regions isolated from OS/hypervisor |
| **EPC** | Enclave Page Cache — hardware-reserved memory region holding encrypted enclave pages, managed by EPCM and protected by MEE |
| **DCAP** | Data Center Attestation Primitives — SGX attestation model using ECDSA and local verification without runtime dependency on Intel IAS |
| **MRENCLAVE** | SHA-256 measurement of an enclave's initial code and data pages, used for identity-based attestation and sealing |
| **Rowhammer** | DRAM disturbance attack exploiting electromagnetic coupling between wordlines to flip bits in adjacent rows via repeated row activation |
| **TRR** | Target Row Refresh — DRAM-internal mitigation proactively refreshing rows adjacent to frequently-activated aggressors; bypassed by many-sided patterns |
| **ODECC** | On-Die Error Correction Code — DDR5-internal single-bit ECC per 128-bit access; does not prevent Rowhammer because corrections occur only at write/refresh time |
| **Blacksmith** | Frequency-domain Rowhammer fuzzer discovering non-uniform access patterns that bypass all known TRR implementations |
| **Phoenix** | CVE-2025-6202 — Rowhammer attack with self-correcting synchronization bypassing DDR5 RFM and ODECC on SK Hynix modules in 109 seconds |
| **TEE.fail** | 2025 attack demonstrating DDR5 memory-bus interposition using a sub-$1000 hardware interposer to break SGX, TDX, and SEV-SNP confidentiality |
| **L1TF / Foreshadow** | Speculative L1D cache read through non-present PTEs; extracts decrypted SGX enclave data (CVE-2018-3615) |
| **MDS** | Microarchitectural Data Sampling — class of attacks (ZombieLoad, RIDL, Fallout) reading stale data from CPU internal buffers (LFB, load port, store buffer) |
| **SEV-SNP** | AMD Secure Encrypted Virtualization with Secure Nested Paging — VM-level confidential computing with integrity protection via Reverse Map Table |
| **TDX** | Intel Trust Domain Extensions — VM-level confidential computing successor to SGX, providing hardware-isolated Trust Domains on Sapphire Rapids+ Xeon |
| **Plundervolt** | CVE-2019-11157 — software-induced voltage-glitching attack corrupting SGX enclave computations for AES key extraction via differential fault analysis |
