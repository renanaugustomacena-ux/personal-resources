...
fase: "Domain 16 — ICS/OT Security"
modulo: "16.1"
titolo: "Industrial Control Systems and OT Security"
versione: "IEC 62443 Ed.4, Purdue Model (ISA-95), NERC CIP v7, NIST SP 800-82 Rev.3, MITRE ATT&CK for ICS v16"
livello: "Advanced"
prerequisiti:
  - "Networking fundamentals (TCP/IP, Ethernet, VLANs, firewalls)"
  - "Familiarity with IDS/IPS and Suricata/Zeek rule syntax"
  - "Basic understanding of PLC/RTU function in process control"
  - "Knowledge of asymmetric cryptography and TLS"
  - "Linux CLI proficiency for packet-capture and scripting tasks"
obiettivi:
  - "Analyze ICS protocol weaknesses (Modbus, DNP3, IEC 61850, OPC UA, EtherNet/IP) and construct detection signatures for unauthorized commands"
  - "Exploit and defend PLC platforms (Siemens S7, Schneider Modicon, Rockwell Logix, CODESYS V3) using real protocol libraries"
  - "Reconstruct the attack chains of TRITON, INDUSTROYER, PIPEDREAM, Stuxnet, and FrostyGoop with timeline and MITRE ATT&CK ICS mapping"
  - "Design a Purdue-model network architecture with IEC 62443 zone/conduit segmentation, data diodes, and jump-host controls"
  - "Perform ICS incident response including PLC forensic acquisition, historian-based timeline reconstruction, and safe-shutdown coordination"
tag: [ics, ot-security, scada, plc, modbus, dnp3, iec-61850, opc-ua, purdue-model, iec-62443, nerc-cip, triton, industroyer, pipedream, stuxnet]
---

# Domain 16 — Industrial Control Systems and OT Security

> **Learning Objectives.** After completing this module the student will be able to: (1) dissect SCADA/ICS protocol frames (Modbus TCP, DNP3 SAv5, IEC 61850 GOOSE/MMS, OPC UA, EtherNet/IP CIP) and identify unauthenticated command vectors; (2) write Suricata and Zeek detection rules for unauthorized function-code usage across ICS protocols; (3) exploit PLC platforms (Siemens S7, Schneider Modicon, Rockwell Logix, CODESYS V3) using Snap7, pycomm3, and Metasploit ICS modules in an authorized lab environment; (4) reconstruct full attack chains for TRITON, INDUSTROYER, PIPEDREAM, Stuxnet, BlackEnergy, and FrostyGoop with ATT&CK for ICS technique mapping; (5) design a Purdue-model network architecture with IEC 62443 zone/conduit segmentation, industrial DMZ, data diodes, and jump-host controls meeting SL-3 requirements.
...
**Exercise 16.1-1 — Modbus TCP traffic analysis and intrusion detection.**
Obtain the Modbus TCP pcap from the ICS-PCAP repository (or capture traffic against a GRFICSv2 virtual PLC). In Wireshark, apply the `modbus` display filter. (a) Enumerate all unique unit IDs and function codes observed. (b) Identify any FC05/FC06/FC15/FC16 write operations and record the source IP, destination IP, register address, and written value. (c) Write three Suricata rules: one alerting on FC16 (Write Multiple Registers) from any source outside the SCADA master subnet, one alerting on FC08 (Diagnostics) sub-function 01 (Restart Communications), and one alerting on FC43 (Read Device Identification) from any source. Validate all three rules against the pcap using `suricata -r capture.pcap -S custom.rules -l ./logs`. Deliverable: the three Suricata rules, a table of detected write operations, and an analyst note on which writes appear operationally anomalous.

**Exercise 16.1-2 — DNP3 Secure Authentication v5 evaluation.**
Using the DNP3 simulator from the OpenDNP3 project (or a physical DNP3 outstation), configure a master-outstation pair: (a) first without SA — capture a Direct Operate (FC 0x05) command in Wireshark and demonstrate that replaying the captured TCP segment reopens the commanded output; (b) then enable SAv5 on both master and outstation — capture the challenge-response exchange and verify that the replayed command is rejected (the outstation issues a new challenge). Document the HMAC-SHA-256 calculation visible in the authentication response. (c) Assess the residual risk: SAv5 authenticates commands but does not encrypt data — demonstrate that a passive observer can still read the commanded value from the unencrypted ASDU. Deliverable: two pcap files (without SA, with SAv5), a comparison table of security properties, and a risk-assessment paragraph.

**Exercise 16.1-3 — OPC UA security policy hardening.**
Deploy an open-source OPC UA server (e.g., open62541 or python-opcua). (a) Start the server with `SecurityPolicy#None` enabled. Using an OPC UA client (UaExpert or `asyncua` Python), connect with no security and browse the full address space. Document all readable nodes and their current values. (b) Reconfigure the server to enforce `Aes256_Sha256_RsaPss` with `SignAndEncrypt` mode and certificate-based mutual authentication. Generate server and client X.509 certificates using OpenSSL. Verify that the `None`-policy client is rejected. (c) Attempt a man-in-the-middle by ARP-poisoning the client-server link (Ettercap or arpspoof in an isolated lab) — confirm that TLS integrity protection causes the connection to fail with a `BadSecurityChecksFailed` status. Deliverable: server configuration files (before and after hardening), screenshots of successful and rejected connections, and the MITM attempt log.

**Exercise 16.1-4 — Purdue model network segmentation design.**
For a hypothetical chemical processing plant with: 12 PLCs (Siemens S7-1500), 4 HMI workstations (WinCC), 2 engineering workstations (TIA Portal), 1 OSIsoft PI historian, 1 Triconex SIS, and corporate IT connectivity: (a) Design a Purdue-model network diagram with explicit zones (L0–L5), the industrial DMZ (L3.5), and all firewall boundaries. Assign each asset to a Purdue level. (b) Define IEC 62443 zones and conduits: assign a Target Security Level (SL-T) to each zone, specify allowed protocols and ports on each conduit, and document the authentication requirements per conduit. (c) Specify where data diodes vs. firewalls are appropriate. (d) Design the remote-access architecture for vendor support (PAM-based, session-recorded, time-limited). Deliverable: a network diagram (Visio, draw.io, or equivalent), a zone/conduit matrix table, and a 1-page justification for each SL-T assignment.

**Exercise 16.1-5 — ICS attack chain reconstruction: INDUSTROYER.**
Using the ESET and Dragos public reports on INDUSTROYER (2016): (a) Build a full attack timeline from initial compromise to grid impact, with UTC timestamps where available. (b) Map each phase to MITRE ATT&CK for ICS techniques (provide technique ID, tactic, and a 1-sentence justification). (c) For the IEC 104 module: reconstruct the exact IEC 104 command sequence (STARTDT → Station Interrogation → Select-Before-Operate → Single Command) and write Suricata rules that would detect the automated rapid-fire command pattern (>10 Type 45/46 commands in <5 seconds from a single source). (d) Identify the three most effective defensive controls that would have disrupted the attack at different stages. Deliverable: the timeline, ATT&CK mapping table, Suricata rules, and a defense-in-depth recommendation.

---

## 13. Readings and References

- CISA, "ICS Advisories," https://www.cisa.gov/news-events/ics-advisories (retrieved: 2026-05-29)
- MITRE, "ATT&CK for ICS Matrix," https://attack.mitre.org/matrices/ics/ (retrieved: 2026-05-29)
- ISA, "ISA/IEC 62443 Series of Standards," https://www.isa.org/standards-and-publications/isa-standards/isa-iec-62443-series-of-standards (retrieved: 2026-05-29)
- NIST, "SP 800-82 Rev. 3: Guide to Operational Technology (OT) Security," https://csrc.nist.gov/pubs/sp/800/82/r3/final (retrieved: 2026-05-29)
- Dragos, "PIPEDREAM/INCONTROLLER ICS Attack Framework," https://www.dragos.com/blog/industry-news/chernovite-pipedream-malware-targeting-industrial-control-systems/ (retrieved: 2026-05-29)
- ESET, "Industroyer: A new threat for industrial control systems," https://www.welivesecurity.com/2017/06/12/industroyer-biggest-threat-industrial-control-systems-since-stuxnet/ (retrieved: 2026-05-29)
- FireEye/Mandiant, "TRITON Attribution: Russian Government-Owned Lab," https://cloud.google.com/blog/topics/threat-intelligence/triton-attribution-russian-government-owned-lab (retrieved: 2026-05-29)
- SOCRadar, "CISA ICS Advisories Recap for 2025," https://socradar.io/blog/cisa-industrial-control-systems-ics-advisories-2025/ (retrieved: 2026-05-29)
- Infosecurity Magazine, "Industrial Control System Vulnerabilities Hit Record Highs," https://www.infosecurity-magazine.com/news/industrial-control-system-vulns/ (retrieved: 2026-05-29)
- ISA, "ISA-TR62443-2-2-2025: IACS Security Protection Scheme," https://www.isa.org/news-press-releases/2025/december/update-to-isa-iec-62443-series-includes-guidance-o (retrieved: 2026-05-29)
...
- Assante, Michael and Lee, Robert M., "The Industrial Control System Cyber Kill Chain," SANS ICS, 2015
- Zetter, Kim, *Countdown to Zero Day: Stuxnet and the Launch of the World's First Digital Weapon*, Crown, 2014

---

## 14. Cross-References (tabular)

| Domain/Chapter | Topic | Relationship to This Chapter |
|---|---|---|
| Domain 9 (Network Security) | TCP/IP, TLS, ARP, network IDS/IPS | ICS protocols run over TCP/IP; TLS wrapping (Modbus/TCP Security, OPC UA, CIP Security) uses Domain 9 TLS stack; ARP spoofing on flat OT networks |
| Domain 11 (Malware Analysis) | Malware RE, C2 frameworks | TRITON, INDUSTROYER, PIPEDREAM, Stuxnet are analyzed with Domain 11 RE techniques; BlackEnergy C2 uses standard IT-side patterns |
| Domain 14 (AD/Windows) | Credential theft, lateral movement | OT workstation compromise follows Domain 14 patterns (Mimikatz, PtH, Kerberoast); OT AD forest should be separate from corporate |
| Domain 16B (ICS Attacks & Defense) | Deep attack analysis, detection rules, IR | Companion chapter with deep-dive TRITON/INDUSTROYER/PIPEDREAM internals, Suricata/Sigma/YARA rules, PLC forensics, and OT threat hunting playbooks |
| Domain 17 (Physical/Hardware) | PLC hardware security, FI bypass | S7-1500 firmware decryption used voltage glitching (17B) and PCB analysis (17C); Stuxnet USB delivery is a physical-layer vector |
| Domain 27 (Defensive Architecture) | Defense-in-depth, SIEM/EDR, zero-trust | ICS network architecture implements Domain 27 principles adapted to OT constraints; ICS detection complements IT SIEM with protocol-aware baselines |
...
| Term | Definition |
|---|---|
| **SCADA** | Supervisory Control and Data Acquisition — centralized system that monitors and controls geographically distributed industrial processes via RTUs and PLCs |
| **PLC** | Programmable Logic Controller — ruggedized computer that executes real-time control logic, reading sensor inputs and driving actuator outputs |
| **RTU** | Remote Terminal Unit — field device that interfaces sensors/actuators to a SCADA master via telemetry protocols (DNP3, IEC 104) |
| **HMI** | Human-Machine Interface — operator workstation displaying process graphics, alarms, and control widgets for SCADA interaction |
| **Purdue Model** | Hierarchical reference architecture (ISA-95/IEC 62443) defining five levels (L0–L5) of OT/IT network segmentation with an industrial DMZ |
| **Data Diode** | Hardware-enforced unidirectional network gateway using a fiber-optic transmitter with no return path, providing physics-based isolation |
| **IEC 62443** | International standard series for industrial automation and control system (IACS) security defining zones, conduits, security levels, and lifecycle requirements |
| **NERC CIP** | North American Electric Reliability Corporation Critical Infrastructure Protection standards — mandatory cybersecurity requirements for the bulk electric system |
| **Safety Instrumented System (SIS)** | Independent automated system that monitors process variables and triggers emergency shutdowns when safety limits are exceeded |
| **Function Code (FC)** | Numeric identifier in Modbus (and similar protocols) specifying the operation type (read coils, write registers, diagnostics) |
| **GOOSE** | Generic Object-Oriented Substation Event — IEC 61850 multicast Layer 2 protocol for fast event notification between IEDs in substations |
| **OPC UA** | Open Platform Communications Unified Architecture — modern industrial interoperability standard with built-in security model (sign, encrypt, certificate auth) |
| **ATT&CK for ICS** | MITRE framework taxonomy of adversary techniques specific to industrial control systems, organized across 12 tactics with ICS-specific assets and mitigations |
| **Unsolicited Response** | DNP3 mechanism where an outstation pushes event data to the master without being polled — exploitable for data injection if unauthenticated |
| **Zone/Conduit** | IEC 62443 model: a zone groups assets sharing a security level; a conduit is the communication channel between zones with defined security controls |

---

## Exercises

**Exercise 1 — Modbus TCP Traffic Analysis in Wireshark.**
Set up a GRFICSv2 virtual ICS environment (chemical plant simulation with OpenPLC). Capture Modbus TCP traffic between the SCADA master and the PLC on port 502 using Wireshark with the Modbus dissector enabled. (a) Identify all Modbus function codes in the capture and classify each as read, write, or diagnostic. (b) Isolate any FC05/FC06/FC15/FC16 write commands and determine the target register addresses and values. (c) Craft a Suricata rule that alerts on FC16 (Write Multiple Registers) from any source IP other than the SCADA master. (d) Replay a captured FC06 (Write Single Register) packet using Scapy and observe the PLC response. Document the packet structure (Transaction ID, Protocol ID, Unit ID, FC, register address, value) with annotated screenshots.

**Exercise 2 — PLC Ladder Logic Exploitation on OpenPLC.**
Using GRFICSv2 or a standalone OpenPLC Runtime on a Raspberry Pi, deploy a simple temperature-control ladder logic program. (a) Connect to the PLC via Modbus TCP using `pymodbus` and read the current setpoint register. (b) Write a modified setpoint value using FC06 and observe the process response on the HMI. (c) Upload a modified ladder logic program (via the OpenPLC web interface) that disables the high-temperature safety interlock. (d) Demonstrate the Stuxnet-style concealment concept: write a Python script that intercepts PLC register reads and replaces real values with pre-recorded normal values. Map each step to the MITRE ATT&CK for ICS matrix (T0855 Unauthorized Command Message, T0836 Modify Parameter, T0821 Modify Controller Tasking).

**Exercise 3 — ICS Network Segmentation Audit per IEC 62443.**
Given a network diagram of a hypothetical water treatment facility with a flat OT network, perform a zone/conduit decomposition per IEC 62443-3-2. (a) Define at least five security zones (safety system, basic process control, HMI/supervision, site operations, DMZ) and assign a Target Security Level (SL-T 1 through 4) to each. (b) Define the conduits between each zone pair, specifying allowed protocols, authentication requirements, and monitoring controls. (c) Design the Industrial DMZ with data diode placement for historian replication, jump-host configuration for engineering access, and firewall rules for each conduit. (d) Produce a Purdue-model network diagram with VLAN assignments, firewall rule tables, and a gap analysis. (e) Estimate remediation cost and timeline, identifying operational constraints.

**Exercise 4 — Shodan/Censys ICS Asset Discovery.**
Conduct an authorized passive reconnaissance exercise against publicly exposed ICS devices. (a) Using Shodan, search for Modbus (`port:502`), DNP3 (`port:20000`), BACnet (`port:47808`), EtherNet/IP (`port:44818`), and OPC UA (`port:4840`) devices. Record count, geographic distribution, and top vendors. (b) Using Censys, search for IEC 104 (`services.port:2404`) and S7comm (`services.port:102`) devices. Compare with Shodan. (c) For five selected devices (from a lab you control, or banner data only), analyze exposed information: device type, firmware version, vendor. (d) Map each finding to MITRE ATT&CK for ICS techniques (T0846, T0888). (e) Write a one-page risk assessment with specific remediation actions.

**Exercise 5 — OT Incident Response Tabletop Exercise.**
Design and facilitate a 90-minute tabletop exercise simulating a FrostyGoop-style Modbus TCP attack against a district heating system. (a) Develop scenario injects: initial detection, escalation, containment decision, and recovery. (b) Define roles: OT SOC analyst, plant operations manager, process engineer, IT incident commander. (c) For each inject, define expected decisions and process-safety implications. (d) Include a historian-based timeline reconstruction exercise with simulated CSV data. (e) Produce an after-action report template documenting lessons learned, gaps, and remediation actions.

---

## Readings and References

1. MITRE. "ATT&CK for ICS Matrix." <https://attack.mitre.org/matrices/ics/> (retrieved: 2026-05-29).
2. MITRE. "ATT&CK for ICS — Techniques." <https://attack.mitre.org/techniques/ics/> (retrieved: 2026-05-29).
3. CISA. "ICS Advisories." <https://www.cisa.gov/news-events/ics-advisories> (retrieved: 2026-05-29).
4. CISA. "Industrial Control Systems." <https://www.cisa.gov/topics/industrial-control-systems> (retrieved: 2026-05-29).
5. NIST. "SP 800-82 Rev. 3 — Guide to Operational Technology (OT) Security." <https://csrc.nist.gov/pubs/sp/800/82/r3/final> (retrieved: 2026-05-29).
6. Dragos. "MITRE ATT&CK for ICS Framework." <https://www.dragos.com/mitre-attack-for-ics> (retrieved: 2026-05-29).
7. Dragos. "Understanding ISA/IEC 62443: A Guide for OT Security Teams." <https://www.dragos.com/blog/isa-iec-62443-concepts> (retrieved: 2026-05-29).
8. Fortiphyd Logic. "GRFICSv2 — Graphical Realism Framework for Industrial Control Simulation." <https://github.com/Fortiphyd/GRFICSv2> (retrieved: 2026-05-29).
9. Elisity. "IEC 62443 in 2025: Network Segmentation Requirements and Changes." <https://www.elisity.com/blog/iec-62443-in-2025-network-segmentation-requirements-and-changes> (retrieved: 2026-05-29).
10. SANS. "What's the Scoop on FrostyGoop." <https://www.sans.org/blog/whats-the-scoop-on-frostygoop-the-latest-ics-malware-and-ics-controls-considerations> (retrieved: 2026-05-29).
11. Deniz Halil. "Critical Infrastructure Discovery with Shodan Dorks." <https://denizhalil.com/2025/06/23/critical-infrastructure-discovery-shodan-dorks/> (retrieved: 2026-05-29).
12. SOCRadar. "CISA ICS Advisories Recap for 2025." <https://socradar.io/blog/cisa-industrial-control-systems-ics-advisories-2025/> (retrieved: 2026-05-29).
...
| Module | Relationship |
|--------|-------------|
| Domain 16, Chapter 16B — ICS Attack Case Studies and Defense | Extends this module with deep analysis of TRITON, INDUSTROYER, PIPEDREAM, FrostyGoop and ICS-specific detection rules |
| Domain 12 — Reverse Engineering | Firmware RE skills (Ghidra, IDA) applied to PLC program analysis and ICS malware sample analysis |
| Domain 13 — Cryptography | Cryptographic protocol weaknesses in ICS: OPC UA security policies, DNP3 SAv5 key management, IEC 62351 signatures |
| Domain 7B — Hardware-Level Vulnerabilities | Rowhammer and Plundervolt techniques applicable to ICS workstation compromise; DMA attacks on engineering workstations |
| Domain 17A — Physical and Hardware Security | Physical security of ICS field devices, tamper-evident packaging for RTUs/PLCs, supply chain interdiction |
| Domain 20 — Wireless Security | Wireless ICS protocols (WirelessHART, ISA100.11a, LoRaWAN for IIoT) and RF attacks against wireless field instruments |
