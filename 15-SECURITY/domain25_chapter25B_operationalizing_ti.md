...
fase: "Domain 25 — Threat Intelligence and Adversary Tracking"
modulo: "25.2"
titolo: "Operationalizing Threat Intelligence: From Indicators to Adversary Tracking at Enterprise Scale"
versione: "MISP 2.4.x / OpenCTI v7 / pySigma 0.11 / STIX 2.1 Errata 01 / Suricata 7.x"
livello: "Advanced"
prerequisiti:
  - "Domain 25 Chapter 25A (TI frameworks, Diamond Model, ATT&CK, STIX/TAXII, MISP fundamentals)"
  - "Domain 11 (malware analysis — understanding malware families referenced in TI)"
  - "Domain 24 (DFIR — incident response workflows that consume TI)"
  - "Domain 27A (detection engineering fundamentals — SIEM/EDR architecture)"
  - "Python programming for API integration and data processing"
obiettivi:
  - "Build and operate a TI-to-detection pipeline that ingests, validates, enriches, and deploys indicators as Sigma rules, YARA rules, and Suricata signatures"
  - "Implement indicator lifecycle management with exponential decay models, MISP sighting-based renewal, and automated retirement at configurable confidence thresholds"
  - "Track adversary infrastructure using passive DNS pivoting (DNSDB), JARM fingerprinting, certificate transparency monitoring, and WHOIS/RDAP registrant analysis"
  - "Apply attribution methodology combining technical pillars (code reuse, infrastructure overlap, operational patterns) with geopolitical context and false-flag analysis"
  - "Design and measure a TI program using PIR prioritization, ATT&CK coverage mapping, feed quality scoring, and stakeholder satisfaction metrics"
tag: [security, threat-intelligence, ti-pipeline, indicator-management, infrastructure-tracking, attribution, misp, opencti, passive-dns, jarm, detection-engineering]
---
...
>
> 1. Build and operate a TI-to-detection pipeline that ingests, validates, enriches, and deploys indicators as Sigma rules, YARA rules, and Suricata signatures.
> 2. Implement indicator lifecycle management with exponential decay models, MISP sighting-based renewal, and automated retirement at configurable confidence thresholds.
> 3. Track adversary infrastructure using passive DNS pivoting (DNSDB), JARM fingerprinting, certificate transparency monitoring, and WHOIS/RDAP registrant analysis.
> 4. Apply attribution methodology combining technical pillars (code reuse, infrastructure overlap, operational patterns) with geopolitical context and false-flag analysis.
> 5. Design and measure a TI program using PIR prioritization, ATT&CK coverage mapping, feed quality scoring, and stakeholder satisfaction metrics.
...
1. **STIX-to-Sigma automated pipeline.** Using the Python script from §1.6 as a starting point, extend the `stix_indicator_to_sigma` function to handle URL-based indicators (generating proxy-log Sigma rules) and file-hash indicators (generating Sysmon Event ID 1 `Hashes` field rules). Ingest a STIX 2.1 bundle containing 10 mixed-type indicators from a TAXII 2.1 test server, generate Sigma YAML for each, and compile the Sigma rules to Splunk SPL using pySigma. Validate each generated rule against a test log dataset containing both true-positive and benign entries.

2. **Indicator decay model implementation.** Using the exponential decay code from §2.7, build a scheduled Python script that connects to a MISP instance via PyMISP, iterates over all active `to_ids=True` attributes, computes the current decayed confidence using type-specific half-lives (IP: 30 days, domain: 90 days, hash: 365 days), and retires attributes that fall below the 15-point threshold by setting `to_ids=False` and adding a `retired-by-decay` tag. Run the script against a test MISP instance with 500+ attributes of varying ages and document the retirement rate by indicator type.

3. **Passive DNS infrastructure pivot.** Starting from a single confirmed C2 domain (provided), execute a full infrastructure-expansion workflow: (a) forward pDNS query (DNSDB or SecurityTrails) to identify all resolved IPs, (b) reverse pDNS on each IP to find co-hosted domains, (c) WHOIS/RDAP lookup on candidate domains to identify registrant patterns, (d) crt.sh certificate transparency query to find related certificates, (e) JARM scan on identified IPs to fingerprint TLS server implementations. Document the resulting infrastructure cluster as a graph (nodes = domains/IPs, edges = relationships) and identify at least three high-confidence adversary-controlled domains not in the original intelligence.

4. **Feed quality audit.** Using the feed quality scoring code from §2.9, compute composite quality scores for three TI feeds your organization consumes (or provided test data): measure precision (true positives / total alerts), uniqueness ratio (indicators not duplicated by other feeds), timeliness (average days from wild observation to feed publication), and cost per actionable indicator. Rank the feeds, identify the lowest-value feed, and draft a recommendation memo to TI program leadership with data-driven justification for renewal or cancellation.

5. **YARA retrohunt and Sigma deployment.** Write a YARA rule targeting a specific malware family based on a provided malware analysis report (unique strings, code patterns, PE metadata). Test the rule via VirusTotal Intelligence retrohunt (or a local malware corpus) to verify detection coverage and false-positive rate. Simultaneously, write two Sigma rules targeting the malware's endpoint behaviors (process creation with specific command-line patterns, file creation in specific directories). Deploy the Sigma rules to a test Splunk instance via pySigma, validate against synthetic log data, and commit all rules to a Git repository following detection-as-code practices.

---

## Readings and References

- OpenCTI documentation — platform architecture and GraphQL API. <https://docs.opencti.io/latest/> (retrieved: 2026-05-29)
- MISP documentation — decay models and indicator lifecycle. <https://www.misp-project.org/documentation/> (retrieved: 2026-05-29)
- Farsight DNSDB — passive DNS database and API reference. <https://www.farsightsecurity.com/solutions/dnsdb/> (retrieved: 2026-05-29)
- Salesforce — JARM fingerprinting tool and research. <https://github.com/salesforce/jarm> (retrieved: 2026-05-29)
- Certificate Transparency — RFC 6962. <https://www.rfc-editor.org/rfc/rfc6962> (retrieved: 2026-05-29)
- Certstream — real-time CT log monitoring. <https://certstream.calidog.io/> (retrieved: 2026-05-29)
- SigmaHQ — community Sigma rules and pySigma framework. <https://github.com/SigmaHQ/sigma> (retrieved: 2026-05-29)
- pySigma documentation — Sigma rule compilation backends. <https://sigmahq.pysigma.org/> (retrieved: 2026-05-29)
- Bianco, David. *The Pyramid of Pain*. 2013. <https://detect-respond.blogspot.com/2013/03/the-pyramid-of-pain.html> (retrieved: 2026-05-29)
- FIRST — Traffic Light Protocol 2.0. <https://www.first.org/tlp/> (retrieved: 2026-05-29)
...
|---|---|---|
| Domain 11 — Malware Analysis | YARA rules encode malware characteristics; C2 internals inform network detection rules and JARM fingerprinting | §1.4, §1.5, §3.2 |
| Domain 23B — OSINT | OSINT feeds (dark web, social media, Shodan/Censys) provide raw indicators for the TI pipeline | §1.1, §3.1–§3.6 |
| Domain 24 — DFIR | IR produces TI (indicators, TTPs); TI guides IR (actor profiles, campaign context); hunt tools from Ch. 24A | §1.1, §8.1–§8.3 |
| Domain 25A — TI Fundamentals | Extends TI frameworks into pipeline engineering, infrastructure tracking, attribution, and program measurement | §1–§8 |
| Domain 27A — Detection Engineering | Detection-as-code pipeline deploys TI-driven Sigma/YARA rules; ATT&CK coverage mapping measures effectiveness | §1.3, §1.6, §7.4 |
| Domain 19 — Supply Chain | Infrastructure tracking identifies adversary staging for supply chain compromise; attribution connects incidents | §3.1–§3.7, §4 |

---

## Glossary

- **Admiralty Scale:** Six-level source reliability (A–F) and six-level information credibility (1–6) framework adapted from military intelligence for TI confidence scoring.
- **Certstream:** Open-source tool providing a real-time WebSocket feed of newly-issued SSL/TLS certificates from Certificate Transparency logs.
- **Decay model (exponential):** Mathematical model `confidence(t) = initial * e^(-lambda*t)` used to automatically reduce indicator confidence over time based on type-specific half-lives.
- **Detection-as-code:** Practice of managing detection rules (Sigma, YARA, Suricata) as version-controlled software artifacts with CI/CD testing and deployment pipelines.
- **DNSDB:** Farsight Security's passive DNS database containing trillions of historical DNS resolution records, supporting forward (rrset) and reverse (rdata) queries.
- **Fast-flux:** DNS evasion technique rapidly rotating A records (and optionally NS records in double-flux) across compromised hosts to resist infrastructure takedown.
- **JARM:** TLS server fingerprinting technique (Salesforce) that hashes responses to ten crafted Client Hello probes, identifying specific TLS implementations and C2 frameworks.
- **NSRL (National Software Reference Library):** NIST-maintained database of cryptographic hashes for known legitimate software, used to filter false-positive file-hash indicators.
- **Passive DNS (pDNS):** Historical DNS resolution data collected by sensors across the internet, preserving domain-to-IP mappings over time for infrastructure tracking.
- **PIR (Priority Intelligence Requirement):** Formally-defined question that guides TI collection based on stakeholder needs, assessed by relevance, feasibility, and actionability.
- **Pyramid of Pain:** David Bianco's framework ranking indicator types by adversary cost to change, from trivial (hashes) to very high (TTPs).
- **pySigma:** Python framework for compiling platform-agnostic Sigma rules to SIEM-specific query languages (Splunk SPL, Elastic KQL, Sentinel KQL, etc.).
- **Safe-linking (glibc):** Pointer mangling mitigation in glibc 2.32+ that XOR-encrypts tcache `next` pointers, relevant when validating hash-based indicators against memory forensics artifacts.
- **Sighting (STIX):** STIX Relationship Object recording when and where an indicator was observed, used to reset indicator decay timers and validate continued relevance.
