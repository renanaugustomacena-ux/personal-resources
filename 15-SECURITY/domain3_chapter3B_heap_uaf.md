...
fase: "Domain 3 — Userspace Memory Corruption"
modulo: "3.2"
titolo: "Heap Exploitation: ptmalloc2, UAF, and Type Confusion"
versione: "glibc 2.39+ (ptmalloc2), GCC 14 / Clang 18, pwndbg / pwntools 4.x"
livello: "Advanced"
prerequisiti:
  - "Domain 3 Chapter 3A (stack layout, integer errors as root causes)"
  - "Domain 2 Chapter 2A (mmap, brk, page faults)"
  - "C dynamic memory (malloc, free, realloc, calloc)"
  - "Data structures (linked lists, doubly-linked lists, hash tables)"
  - "GDB with heap inspection plugins (pwndbg, gef)"
obiettivi:
  - "Map the ptmalloc2 allocator internals: malloc_chunk layout, arena management, fastbins, tcache, and unsorted bin"
  - "Demonstrate tcache poisoning, fastbin dup, and House of Orange exploitation techniques with pwntools"
  - "Exploit use-after-free and double-free vulnerabilities through controlled heap shaping and spray"
  - "Detect heap corruption artifacts in crash dumps and core files using GDB heap commands and forensic analysis"
  - "Evaluate allocator hardening mechanisms (safe-linking, tcache key, MTE, PartitionAlloc, Scudo) and their bypass conditions"
tag: [heap, ptmalloc2, tcache, uaf, double-free, type-confusion, vtable, exploitation, allocator-hardening]
---
...
1. **Tcache Poisoning Lab.** Write a vulnerable program with a UAF on a tcache-sized chunk (0x20-0x410 range). Using pwntools and pwndbg, demonstrate tcache poisoning: free the chunk, overwrite its `fd` pointer (accounting for safe-linking XOR on glibc 2.35+), allocate twice to get an overlapping allocation at a controlled address (e.g., `__free_hook` on glibc < 2.34, or `_IO_list_all` on glibc 2.34+). Document the safe-linking bypass. Reference: `tutorials/tutorial_domain3_ch3B_heap_uaf_lab.md`.

2. **Heap Forensics with GDB.** Load a core dump from a crashed program into GDB with pwndbg. Use `heap`, `bins`, `tcachebins`, `fastbins`, `unsortedbin` commands to reconstruct the heap state at crash time. Identify the corrupted chunk, trace the corruption backwards through the bin chains, and determine whether the crash was caused by a double-free, overflow, or UAF.

3. **House of Orange Exploitation.** On glibc 2.23 (use a Docker container with Ubuntu 16.04), implement the House of Orange technique: (a) overflow the top chunk to shrink it, (b) trigger `sysmalloc` to free the old top chunk into the unsorted bin, (c) corrupt the unsorted bin entry to overlap with `_IO_list_all`, (d) craft a fake `_IO_FILE` structure that calls `system("/bin/sh")` via the vtable. Document which glibc mitigations (vtable validation, safe-linking) block this on glibc 2.39+.

4. **Allocator Comparison Lab.** Compile the same UAF-vulnerable program against four allocators: ptmalloc2 (default glibc), jemalloc, mimalloc, and Scudo. For each, attempt the same exploitation technique (tcache/thread-cache poisoning or freelist corruption). Document which allocators prevent exploitation and why (quarantine, randomization, guard pages, type segregation).

5. **C++ Vtable Corruption and CFI.** Write a C++ program with a virtual class hierarchy and a UAF that corrupts a vtable pointer. Demonstrate vtable hijacking to redirect a virtual call. Then recompile with Clang's `-fsanitize=cfi-vcall` and verify the CFI check detects the corrupted vtable at runtime. Analyze the generated CFI type-check code in the disassembly.

---

## Readings and References

- Azeria Labs, "Heap Exploitation Part 1: Understanding the Glibc Heap Implementation": https://azeria-labs.com/heap-exploitation-part-1-understanding-the-glibc-heap-implementation/ (retrieved: 2026-05-29)
- glibc malloc internals — `malloc/malloc.c`: https://sourceware.org/git/?p=glibc.git;a=blob;f=malloc/malloc.c (retrieved: 2026-05-29)
- "how2heap" — shellphish educational heap exploitation examples: https://github.com/shellphish/how2heap (retrieved: 2026-05-29)
- Safe-Linking — "Protecting the Heap" (Check Point Research): https://research.checkpoint.com/2020/safe-linking-eliminating-a-20-year-old-malloc-exploit-primitive/ (retrieved: 2026-05-29)
- HackTricks — Heap Overflow: https://hacktricks.wiki/en/binary-exploitation/libc-heap/heap-overflow.html (retrieved: 2026-05-29)
- MITRE CWE-416 — Use After Free: https://cwe.mitre.org/data/definitions/416.html (retrieved: 2026-05-29)
- MITRE CWE-415 — Double Free: https://cwe.mitre.org/data/definitions/415.html (retrieved: 2026-05-29)
- MITRE CWE-122 — Heap-based Buffer Overflow: https://cwe.mitre.org/data/definitions/122.html (retrieved: 2026-05-29)
- MITRE ATT&CK T1203 — Exploitation for Client Execution: https://attack.mitre.org/techniques/T1203/ (retrieved: 2026-05-29)
- Google Project Zero — "Automatic Heap Layout Manipulation for Exploitation": https://arxiv.org/pdf/1804.08470 (retrieved: 2026-05-29)
- Scudo Hardened Allocator documentation: https://llvm.org/docs/ScudoHardenedAllocator.html (retrieved: 2026-05-29)
- ARM Memory Tagging Extension (MTE) for heap safety: https://developer.arm.com/documentation/102925/latest (retrieved: 2026-05-29)
- picoCTF 2026 heap exploitation challenges: https://picoctfsolutions.com/posts/heap-exploitation-ctf (retrieved: 2026-05-29)
...
|---------|-------|-------------|
| `domain3_chapter3A_stack_format_integer.md` | Integer overflow root cause, format string writes | Integer errors cause undersized allocations; format writes target heap metadata |
| `domain1_chapter1A_elf_foundations.md` | GOT entries, _IO_FILE vtables, full RELRO | GOT and _IO_FILE are heap-corruption-to-code-execution targets; RELRO protects GOT |
| `domain1_chapter1B_elf_advanced.md` | .fini_array, constructor ordering | .fini_array overwrite is alternative code execution target from heap corruption |
| `domain2_chapter2A_process_memory.md` | mmap/brk heap placement, MADV_DONTNEED, heap shaping | Heap shaping via mmap/munmap complements allocator-level exploitation |
| `domain4_code_reuse_attacks.md` | ROP/JOP payloads, CFI vtable protection | Heap exploitation provides the corruption primitive; code reuse provides the payload |
| `domain5_chapter5A_kernel_exploitation.md` | SLUB allocator, cross-cache attacks, msg_msg | Kernel SLUB exploitation mirrors ptmalloc2 patterns with different named techniques |

---

## Glossary

- **ptmalloc2:** The default userspace memory allocator in glibc, managing chunks via arenas, bins, and thread-local caches (tcache).
- **Tcache (Thread Cache):** Per-thread singly-linked freelist in glibc (since 2.26) holding recently freed chunks for fast reallocation without locking.
- **Fastbin:** Per-arena singly-linked freelist for small chunks (up to 0x80 bytes on 64-bit); LIFO, no coalescing, single-threaded access via arena lock.
- **Unsorted Bin:** Doubly-linked list serving as a staging area for all freed chunks before they are sorted into small or large bins.
- **Safe-Linking:** glibc 2.32+ mitigation XOR-encoding freelist pointers with a per-thread secret, requiring a heap address leak to bypass.
- **Use-After-Free (UAF):** Vulnerability where a program accesses memory through a pointer to a freed allocation, potentially reading/writing attacker-controlled data.
- **Double-Free:** Bug where `free()` is called twice on the same pointer, corrupting the freelist and enabling allocation overlap.
- **House of Orange:** Named exploitation technique corrupting the top chunk to trigger `sysmalloc`, freeing it into the unsorted bin for `_IO_FILE` overlap.
- **Tcache Poisoning:** Overwriting a tcache entry's `fd` pointer to redirect the next allocation to an attacker-controlled address.
- **Fastbin Dup:** Double-free in a fastbin freelist creating a cycle that returns overlapping allocations.
- **FSOP (File Stream Oriented Programming):** Exploitation technique hijacking `_IO_FILE` structures to achieve code execution via corrupted vtable pointers.
- **Type Confusion:** Vulnerability where an object of one type is accessed as a different type, enabling vtable corruption or field misinterpretation.
- **Vtable Verification:** glibc mitigation checking that `_IO_FILE` vtable pointers fall within the legitimate `__libc_IO_vtables` section.
- **PartitionAlloc:** Chromium's hardened allocator segregating allocations by type to prevent cross-type UAF exploitation.
- **MTE (Memory Tagging Extension):** ARM hardware feature assigning 4-bit tags to memory granules and pointers; mismatches fault on access.
