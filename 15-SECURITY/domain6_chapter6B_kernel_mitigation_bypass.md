...
fase: "Domain 6 — Kernel Mitigation Bypass"
modulo: "6.2"
titolo: "Kernel Mitigation Bypass and Combined Exploit Primitive Chains"
versione: "Linux 6.x, Windows 11 24H2/Server 2025, ARMv8.6-A+, Intel CET, AMD Zen 4/5"
livello: "Advanced"
prerequisiti:
  - "Domain 6, Chapter 6A — Userspace mitigation bypass (ASLR, DEP, canary, RELRO, CFI)"
  - "Domain 4, Chapter 4B — Hardware-enforced CFI (CET, PAC, MTE) in kernel context"
  - "Domain 5 — Kernel exploitation (SLUB heap spray, cross-cache attacks, elastic objects)"
  - "Domain 2 — Page tables, address space layout, syscall dispatch, seccomp, capabilities, namespaces, LSMs"
  - "Domain 3 — Memory corruption primitives (heap overflow, UAF, type confusion)"
obiettivi:
  - "Defeat KASLR via EntryBleed, prefetch timing, branch-predictor residuals, and eBPF verifier leaks on modern 6.x kernels"
  - "Construct SMEP/SMAP bypass chains using kernel ROP, physmap spray, CR4 pinning circumvention, and PTE manipulation"
  - "Execute cross-cache attacks against SLUB freelist isolation using elastic objects (msg_msg, pipe_buffer, sk_buff, seq_operations)"
  - "Perform data-only privilege escalation via struct cred manipulation, modprobe_path overwrite, and TIF_SECCOMP flag clearing"
  - "Evaluate Windows kernel mitigations (HVCI, VBS, KDP, PatchGuard) and their bypass surface through data-only attacks"
tag: [security, kernel, KASLR, SMEP, SMAP, KPTI, cross-cache, elastic-objects, privilege-escalation, cred, modprobe_path, seccomp-bypass, VBS, HVCI, PatchGuard]
---

# Domain 6, Chapter 6B — Kernel Mitigation Bypass and Combined Exploit Primitive Chains

> **Learning objectives.** After completing this chapter you will be able to: (1) defeat KASLR on a modern Linux 6.x kernel using at least three independent techniques (direct info leak, EntryBleed side-channel, eBPF verifier bypass); (2) construct a complete kernel ROP chain that calls `commit_creds(prepare_kernel_cred(0))` and returns cleanly via the KPTI trampoline; (3) execute a cross-cache SLUB attack using `msg_msg` elastic objects to convert a UAF into an arbitrary kernel read/write primitive; (4) perform data-only privilege escalation by directly modifying `struct cred` fields and clearing `TIF_SECCOMP`, bypassing kernel CFI entirely; (5) compare Linux and Windows kernel mitigation architectures (KPTI vs. KVA Shadow, SMEP/SMAP vs. HVCI/VBS, seccomp vs. WDAC) and identify the bypass surface for each.
...
Hands-on exercises for this chapter. For guided lab walkthroughs see `tutorials/tutorial_domain6_ch6B_kernel_bypass_lab.md`.

1. **KASLR defeat via three independent methods.** On a Linux 6.x VM with `kptr_restrict=1` and `dmesg_restrict=1`: (a) write a C program that uses prefetch-timing side-channel probing to scan the kernel text region at 2MB alignment and identify the kernel base address; (b) use an eBPF program (if `unprivileged_bpf_disabled=0`) to leak a kernel code pointer from a BPF helper return value; (c) search `/sys/kernel/debug/` (as root) for subsystem entries that expose raw kernel pointers via `%p` or `%px`. Compare the three results and measure the time-to-leak for each method. Document which `sysctl` settings would block each technique.

2. **Kernel ROP chain: commit_creds + KPTI trampoline.** Using a provided vulnerable kernel module with a stack buffer overflow, construct a complete kernel ROP chain that: (a) calls `prepare_kernel_cred(NULL)` to create a root credential, (b) transfers the return value (`rax`) to `rdi` via a `mov rdi, rax` gadget (document the gadget search process using ROPgadget on `vmlinux`), (c) calls `commit_creds`, and (d) returns to userspace via `swapgs_restore_regs_and_return_to_usermode + 22`. Save and restore user-space register state (`CS`, `SS`, `RFLAGS`, `RSP`, `RIP`) before triggering the exploit. Verify privilege escalation with `getuid() == 0` in the landing function.

3. **Cross-cache attack with msg_msg elastic objects.** Using a provided kernel module that exposes a UAF on a `kmalloc-256` object: (a) groom the SLUB heap by allocating `OBJS_PER_SLAB * 4` objects, (b) free surrounding objects to drain the target slab page back to the buddy allocator, (c) spray `msg_msg` objects (body size = 208 bytes for `kmalloc-256` targeting) to reclaim the freed page, (d) use the dangling UAF pointer to read the `msg_msg` header and leak kernel heap pointers (`m_list.next/prev`), (e) corrupt `m_ts` to enable OOB read. Parse `/proc/slabinfo` to determine `objs_per_slab` and `pages_per_slab` for your target cache. Measure reclaim reliability over 50 attempts.

4. **Data-only privilege escalation: modprobe_path overwrite.** Given an arbitrary kernel write primitive (provided via a vulnerable module), overwrite `modprobe_path` from its default `/sbin/modprobe` to `/tmp/pwn`. Create `/tmp/pwn` as a script that copies `/bin/sh` to `/tmp/rootsh` with SUID root. Trigger the modprobe execution by running a file with an unrecognized binary format header (`\xff\xff\xff\xff`). Verify that `/tmp/rootsh` exists with SUID root and obtain a root shell. Explain why this technique bypasses kernel CFI and why it does not require kernel code execution.

5. **Credential structure manipulation and seccomp bypass.** Using the same arbitrary write primitive: (a) locate `current->cred` by reading the per-CPU `current_task` variable (compute the GS-base offset for your kernel version), (b) overwrite `uid`, `euid`, `suid`, `fsuid` fields to 0 in `struct cred`, (c) clear the `TIF_SECCOMP` flag in `current->thread_info.flags`, (d) verify with `id` that you are root and with `prctl(PR_GET_SECCOMP)` that seccomp is disabled. Document the exact offsets within `task_struct` and `cred` for your kernel build (use `pahole` or `/proc/config.gz` + `offsetof` analysis).
...
1. Horn, J. "Exploiting the Linux kernel via packet sockets." Google Project Zero, 2017. Cross-cache heap exploitation techniques. https://googleprojectzero.blogspot.com/2017/05/exploiting-linux-kernel-via-packet.html
2. Will. "EntryBleed: A Universal KASLR Bypass against KPTI on Linux." 2022. CVE-2022-4543. https://www.willsroot.io/2022/12/entrybleed.html
3. Gruss, D. et al. "Prefetch Side-Channel Attacks: Bypassing SMAP and Kernel ASLR." CCS 2016. https://dl.acm.org/doi/10.1145/2976749.2978356
4. Evtyushkin, D. et al. "Jump Over ASLR: Attacking Branch Predictors to Bypass ASLR." MICRO 2016. BTB-based KASLR bypass. https://ieeexplore.ieee.org/document/7783743
5. CVE-2024-1086 — Netfilter nf_tables double-free, page-level cross-cache PTE manipulation. https://nvd.nist.gov/vuln/detail/CVE-2024-1086
6. CVE-2022-0847 — Dirty Pipe, page-cache corruption without memory corruption. https://nvd.nist.gov/vuln/detail/CVE-2022-0847
7. CVE-2023-2163 — eBPF verifier precision tracking bypass. https://nvd.nist.gov/vuln/detail/CVE-2023-2163
8. CVE-2025-38617 — Linux Packet Sockets privilege escalation. https://nvd.nist.gov/vuln/detail/CVE-2025-38617
9. MITRE ATT&CK T1068 — Exploitation for Privilege Escalation. https://attack.mitre.org/techniques/T1068/
10. MITRE ATT&CK T1014 — Rootkit (kernel-level persistence via cred/LSM manipulation). https://attack.mitre.org/techniques/T1014/
11. MITRE ATT&CK T1574 — Hijack Execution Flow (modprobe_path, core_pattern). https://attack.mitre.org/techniques/T1574/
12. xairy/linux-kernel-exploitation — curated kernel exploitation resource list. https://github.com/xairy/linux-kernel-exploitation
13. hackyboiz. "Bypassing Windows Kernel Mitigations: Part 0 — Deep Dive into KASLR Leaks Restriction." 2025. https://hackyboiz.github.io/2025/04/13/l0ch/bypassing-kernel-mitigation-part0/en/
14. Linux Kernel Runtime Guard (LKRG) — runtime integrity checker for kernel code and credential structures. https://lkrg.org/
15. ROPgadget — gadget finder for kernel ROP chain construction from vmlinux. https://github.com/JonathanSalwan/ROPgadget
...
|---|---|---|
| Domain 6, Ch 6A (userspace mitigation bypass) | §1 KASLR bypass, §2–3 SMEP/SMAP bypass | Userspace bypass techniques (ASLR leak, DEP ROP chain) are prerequisites; kernel bypasses extend them to ring 0 |
| Domain 4, Ch 4B (hardware CFI: CET, PAC, MTE) | §2.3 CR4 pinning, §3.7 kernel ROP, §9.5 kernel CET IBT | Kernel CET IBT constrains gadget sets; data-only attacks (§6.2, §8.4) specifically circumvent hardware CFI |
| Domain 5, Ch 5A–5B (kernel exploitation, subsystems) | §8 combined exploit chains, §8.2–8.3 cross-cache/elastic objects | SLUB heap internals, elastic object spray, and subsystem attack surfaces from Domain 5 are the building blocks |
| Domain 7, Ch 7A (Spectre, side channels) | §1.2 EntryBleed, §1.3 prefetch timing, §1.4 branch predictor | Speculative-execution side channels provide KASLR bypass without memory-corruption prerequisites |
| Domain 2, Ch 2A–2C (page tables, seccomp, capabilities, LSMs) | §4 KPTI, §6 MAC bypass, §7 seccomp bypass | Page-table structure, seccomp enforcement, and LSM hook architecture define the defense surface attacked here |
| Domain 3, Ch 3A–3B (memory corruption primitives) | §8.1 canonical pipeline, §8.2 cross-cache | Heap overflow, UAF, and type-confusion from Domain 3 are the initial corruption primitives that start every kernel exploit chain |
...
| Term | Definition |
|---|---|
| **KASLR** | Kernel Address Space Layout Randomization — boot-time randomization of kernel text, module, and physmap base addresses (~9 bits entropy for text on x86_64) |
| **SMEP** | Supervisor Mode Execution Prevention — CPU feature (CR4 bit 20) blocking kernel execution of user-space pages |
| **SMAP** | Supervisor Mode Access Prevention — CPU feature (CR4 bit 21) blocking kernel read/write of user-space pages; disabled via STAC/CLAC |
| **KPTI** | Kernel Page Table Isolation — separate user/kernel page tables mitigating Meltdown and hardening KASLR against prefetch side-channels |
| **Physmap** | Direct mapping of all physical memory into kernel virtual address space; primary SMAP bypass vector via physmap spray |
| **Cross-cache attack** | SLUB exploitation technique draining a slab page to the buddy allocator and reclaiming it into a different cache for type-confusion |
| **Elastic object** | Kernel object with user-controlled allocation size (e.g., `msg_msg`, `pipe_buffer`, `sk_buff`) enabling targeted slab-cache placement |
| **commit_creds** | Kernel function replacing the current task's credentials; `commit_creds(prepare_kernel_cred(NULL))` is the canonical privilege-escalation primitive |
| **modprobe_path** | Kernel global string (`/sbin/modprobe`) executed as root when an unknown binary format is encountered; overwritable for data-only privilege escalation |
| **TIF_SECCOMP** | Thread-info flag enabling seccomp filter evaluation on syscall entry; clearing it disables all seccomp restrictions for the task |
| **CR4 pinning** | Kernel hardening (Linux 5.3+) preventing runtime clearing of SMEP/SMAP bits in CR4 via `native_write_cr4` validation |
| **HVCI** | Hypervisor-Protected Code Integrity — Windows VBS feature enforcing kernel code-page integrity via the secure kernel hypervisor |
| **VBS** | Virtualization-Based Security — Windows architecture using a hypervisor to create an isolated secure kernel for credential and code-integrity protection |
| **PatchGuard / KPP** | Kernel Patch Protection — Windows mechanism detecting unauthorized modification of kernel code, SSDT, IDT, and GDT |
| **LKRG** | Linux Kernel Runtime Guard — out-of-tree module monitoring kernel code integrity and credential structure consistency at runtime |
