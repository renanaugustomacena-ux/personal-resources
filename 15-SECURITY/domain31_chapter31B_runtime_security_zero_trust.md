...
fase: "Domain 31 — Detection Engineering and SOC Architecture at Scale"
modulo: "31.2"
titolo: "Runtime Security: eBPF-Based Monitoring, Auditd at Scale, and Zero Trust Architecture"
versione: "Linux 5.7+ (eBPF LSM), Falco 0.38+, Tetragon 1.x, Cilium 1.15+, Tracee 0.20+, NIST SP 800-207"
livello: "Advanced"
prerequisiti:
  - "Linux kernel fundamentals: system calls, namespaces, cgroups, security modules"
  - "Container orchestration: Kubernetes pods, namespaces, NetworkPolicy, RBAC"
  - "SIEM/SOAR pipeline design (Domain 31 Chapter 31A)"
  - "Windows security controls: WDAC, ASR rules, Credential Guard"
  - "Cryptographic identity: X.509, mTLS, SPIFFE/SPIRE"
obiettivi:
  - "Analyze eBPF program types (kprobes, tracepoints, LSM hooks, XDP) and design kernel-level security enforcement policies using Tetragon TracingPolicy"
  - "Write custom Falco rules for container escape detection, fileless execution, reverse shell identification, and crypto-mining behavioral patterns"
  - "Design and deploy auditd rule sets for compliance-mandated monitoring (STIG/CIS/PCI-DSS) with performance tuning for high-throughput environments"
  - "Architect a zero trust access model implementing NIST SP 800-207 with identity-centric access control, microsegmentation (Kubernetes NetworkPolicy, Cilium, Calico), and continuous access evaluation"
  - "Implement supply chain runtime verification using cosign/Sigstore for image signing and Kyverno/OPA Gatekeeper admission controllers for policy enforcement"
tag: [security, ebpf, falco, tetragon, cilium, tracee, auditd, zero-trust, microsegmentation, seccomp, wdac, spiffe, runtime-security, container-security]
---
...
> 1. Explain eBPF verifier constraints, CO-RE portability mechanisms, and BPF map types, and assess the offensive abuse potential of eBPF-based rootkits.
> 2. Deploy Tetragon, Falco, and Tracee in a Kubernetes cluster and write custom detection policies for container escape, privilege escalation, and fileless execution.
> 3. Design tiered auditd rule sets balancing coverage and performance, implement IMA appraisal for binary authorization, and integrate audit telemetry with SIEM pipelines.
> 4. Implement zero trust microsegmentation using Kubernetes NetworkPolicy and Cilium L7 policies with default-deny posture and DNS-aware egress controls.
> 5. Configure WDAC application whitelisting, ASR rules, and Credential Guard on Windows endpoints, and map bypass techniques to detection rules.
...
1. **Falco custom rule development for container runtime detection.** Deploy Falco in a Kubernetes cluster (minikube or kind). Write custom Falco rules detecting: (a) reverse shell establishment via `dup2` redirecting stdio to a network socket, (b) access to the Kubernetes service account token at `/var/run/secrets/kubernetes.io/serviceaccount/token`, (c) execution of binaries not present in the original container image (drift detection), (d) outbound connections to common crypto-mining pool ports (3333, 4444, 5555), and (e) `ptrace` attachment from a container targeting a host-PID-namespace process. Validate each rule by executing the corresponding attack technique and confirm alert generation with zero false positives from normal application operation.

2. **Tetragon TracingPolicy for kernel-level enforcement.** Deploy Tetragon in a Kubernetes cluster. Write TracingPolicies that: (a) block execution of any binary not in a defined allowlist (`security_bprm_check` hook with `Sigkill` action), (b) detect container escape via `setns` to the host PID namespace, (c) monitor `commit_creds` calls indicating privilege escalation, (d) alert on kernel module loading (`init_module`/`finit_module`) outside maintenance windows, and (e) log all `connect` syscalls to external IP addresses from container workloads. Verify enforcement by attempting blocked actions and confirming process termination.

3. **Auditd rule deployment and SIEM integration.** On a Linux server: (a) deploy the comprehensive audit rule set from section 3.2, (b) configure Auditbeat to read from the kernel audit subsystem and ship events to Elasticsearch in ECS format, (c) make the rules immutable with `-e 2`, (d) write Sigma rules that detect: ptrace injection (`-k process_injection`), kernel module loading (`-k kernel_module`), and SSH key modification (`-k ssh_keys`), (e) execute each attack technique and validate detection in Kibana. Measure the performance impact of the full rule set vs. a targeted subset on a high-throughput web server.

4. **Zero trust microsegmentation with Cilium.** In a Kubernetes cluster with Cilium: (a) implement a default-deny NetworkPolicy for all namespaces, (b) create CiliumNetworkPolicies allowing only specific pod-to-pod communication paths with L7 HTTP method/path restrictions, (c) configure DNS-aware egress policies restricting outbound access to explicitly allowed FQDNs, (d) simulate a lateral movement attempt (compromised pod trying to access an unauthorized service) and verify policy denial, (e) use Hubble to visualize network flows and identify any unexpected communication paths. Document the policy set and verify no legitimate application traffic is blocked.

5. **Supply chain runtime verification with cosign and Kyverno.** Set up an image signing and admission control pipeline: (a) generate a cosign key pair and sign a container image, (b) deploy Kyverno with a ClusterPolicy requiring image signature verification for all pods in the `production` namespace, (c) attempt to deploy an unsigned image and verify admission rejection, (d) deploy a signed image and verify admission success, (e) add a Kyverno policy enforcing pod security baseline (non-root, read-only rootfs, drop all capabilities, no privilege escalation), (f) attempt to deploy a privileged container and verify rejection. Document the complete pipeline from build to deploy.

---

## Readings and References

- NIST. "SP 800-207: Zero Trust Architecture." https://csrc.nist.gov/pubs/sp/800/207/final (retrieved: 2026-05-29)
- Isovalent/Cilium. "Tetragon: eBPF-based Security Observability and Runtime Enforcement." https://tetragon.io/ (retrieved: 2026-05-29)
- Falco Project (CNCF). "Falco: Cloud Native Runtime Security." https://falco.org/ (retrieved: 2026-05-29)
- Falco Project. "Falco Rules Repository." https://github.com/falcosecurity/rules (retrieved: 2026-05-29)
- Aqua Security. "Tracee: eBPF-based Runtime Security." https://github.com/aquasecurity/tracee (retrieved: 2026-05-29)
- Cilium Project. "Cilium Documentation — Network Policies." https://docs.cilium.io/en/stable/security/ (retrieved: 2026-05-29)
- Sigstore. "cosign — Container Signing, Verification, and Storage." https://docs.sigstore.dev/cosign/overview/ (retrieved: 2026-05-29)
- Kyverno. "Kyverno: Kubernetes Native Policy Management." https://kyverno.io/docs/ (retrieved: 2026-05-29)
- OPA Gatekeeper. "Policy Controller for Kubernetes." https://open-policy-agent.github.io/gatekeeper/ (retrieved: 2026-05-29)
- SPIFFE/SPIRE. "Secure Production Identity Framework for Everyone." https://spiffe.io/ (retrieved: 2026-05-29)
- OneUpTime. "How to Implement Security Monitoring with eBPF (Falco, Tetragon)." https://oneuptime.com/blog/post/2026-01-07-ebpf-security-monitoring-falco-tetragon/view (retrieved: 2026-05-29)
- ARMO. "Best eBPF Security Solutions for Kubernetes & Runtime Protection." https://www.armosec.io/blog/best-ebpf-security-solutions-runtime-protection/ (retrieved: 2026-05-29)
...
|---|---|---|
| Domain 31 Chapter 31A | SIEM/SOAR pipeline design | Infrastructure consuming eBPF, auditd, and zero trust telemetry; Sigma rule deployment for runtime events |
| Domain 7 Chapter 7A | Container security fundamentals | Base container hardening (image scanning, least-privilege) that runtime detection monitors for drift |
| Domain 30 Chapter 30B | Credential theft mechanics | LSASS access and token manipulation detectable via eBPF LSM hooks and WDAC/ASR rules |
| Domain 13 Chapter 13A | Incident response lifecycle | IR framework adapted for ephemeral container evidence collection and cloud-native response |
| Domain 15 Chapter 15A | Digital forensics | Forensic methodology for volatile container evidence and eBPF program analysis |
| Domain 28 Chapter 28A | Cloud security architecture | Cloud-native zero trust patterns (VPC microsegmentation, IAM, workload identity) complementing Kubernetes-level controls |
...
| Term | Definition |
|---|---|
| **eBPF (extended Berkeley Packet Filter)** | Linux kernel technology enabling sandboxed programs to run at kernel level without modifying kernel source, providing system call interception, network packet processing, and security policy enforcement with minimal overhead. |
| **CO-RE (Compile Once, Run Everywhere)** | eBPF portability mechanism using BTF (BPF Type Format) metadata to relocate struct field offsets at program load time, enabling a single compiled eBPF program to run across different kernel versions. |
| **Tetragon** | Cilium/Isovalent eBPF-based runtime security tool providing kernel-level policy enforcement (process kill, syscall deny) directly from eBPF programs without user-space round-trip latency. |
| **Falco** | CNCF-graduated runtime detection tool using eBPF (or kernel module) to intercept system calls and evaluate them against YAML-based security rules, operating in detection-only mode. |
| **Tracee** | Aqua Security eBPF tool capturing 400+ kernel events with Rego (OPA) and Go-based detection modules, specialized in container escape CVE detection. |
| **TracingPolicy** | Tetragon's custom resource for defining eBPF-based kernel monitoring rules, specifying kprobe/tracepoint hooks, selector filters, and enforcement actions (Sigkill, Post). |
| **Seccomp (Secure Computing Mode)** | Linux kernel feature restricting which system calls a process can invoke via BPF filter programs, used in containers to reduce the kernel attack surface. |
| **LSM (Linux Security Module)** | Kernel framework providing security hook points (file access, process execution, network operations) used by SELinux, AppArmor, Landlock, and eBPF LSM programs for mandatory access control. |
| **Landlock** | Unprivileged Linux security module (kernel 5.13+) enabling processes to restrict their own filesystem and network access without root privileges, providing application self-sandboxing. |
| **SPIFFE/SPIRE** | Framework (SPIFFE) and runtime (SPIRE) for assigning cryptographic X.509 identities to workloads, enabling mutual TLS authentication between services without pre-shared secrets. |
| **Microsegmentation** | Network security approach enforcing access control at the individual workload level with default-deny policies, implemented via Kubernetes NetworkPolicy, Cilium, Calico, or cloud security groups. |
| **CAE (Continuous Access Evaluation)** | Azure AD feature enabling near-real-time token revocation by subscribing resource providers to identity events (user disabled, password changed), reducing the token replay window. |
| **IMA (Integrity Measurement Architecture)** | Linux kernel subsystem that measures (hashes) and optionally verifies signatures of files before access, extending measurements into TPM PCR registers for remote attestation. |
| **WDAC (Windows Defender Application Control)** | Windows kernel-mode code integrity policy restricting which drivers and applications can execute, superseding AppLocker for application whitelisting enforcement. |
| **ASR (Attack Surface Reduction)** | Microsoft Defender rules blocking specific attack techniques (LSASS credential stealing, Office macro child processes, WMI persistence) at the endpoint level. |
