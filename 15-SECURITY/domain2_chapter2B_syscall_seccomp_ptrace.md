...
modulo: "2.2"
titolo: "System Call Dispatch, Seccomp, and Process Interaction Primitives"
versione: "Linux 6.x, libseccomp 2.5+, io_uring 6.x, strace 6.x"
livello: "Advanced"
prerequisiti:
  - "Domain 2 Chapter 2A (page fault handler, VMA structures, process address space)"
  - "Domain 1 Chapter 1A (kernel execve path, auxiliary vector)"
  - "x86_64 assembly (syscall instruction, register conventions)"
  - "BPF fundamentals (bytecode, filter programs)"
  - "Linux process model (fork, clone, signals, ptrace basics)"
obiettivi:
  - "Trace the x86_64 syscall dispatch path from the syscall instruction through entry_SYSCALL_64 to the sys_call_table"
  - "Design and deploy seccomp-bpf filters that restrict system call access with SECCOMP_RET_KILL, ERRNO, TRACE, and USER_NOTIF"
  - "Exploit userfaultfd for TOCTOU race condition amplification and implement the sysctl-based mitigation"
  - "Analyze io_uring security implications including seccomp bypass and demonstrate ring-based vulnerability patterns"
  - "Use ptrace for process inspection and injection, and configure Yama LSM ptrace_scope for defense"
tag: [syscall, seccomp, ptrace, io-uring, userfaultfd, bpf, sandbox, process-injection, linux-security]
---
...
1. **Seccomp-BPF Filter Development.** Write a seccomp-bpf filter in C (using `libseccomp`) that allows only `read`, `write`, `exit`, `exit_group`, `sigreturn`, and `rt_sigreturn`, blocking everything else with `SECCOMP_RET_KILL_PROCESS`. Apply it to a simple echo server. Verify that `execve` and `mprotect(PROT_EXEC)` are blocked by attempting them from the sandboxed process. Reference: `tutorials/tutorial_domain2_ch2B_syscall_seccomp_lab.md`.

2. **Userfaultfd Race Window Exploitation.** Write a program that registers a `userfaultfd` handler on an anonymous mapping, then spawns two threads: one that triggers a page fault by reading the registered region, and a handler thread that pauses (sleeps) before resolving the fault via `UFFDIO_COPY`. Measure the race window duration. Then set `vm.unprivileged_userfaultfd=0` and verify the `userfaultfd` syscall fails with `EPERM`.

3. **Ptrace-Based Process Injection.** Using `ptrace(PTRACE_ATTACH)`, attach to a target process, inject shellcode into its writable memory via `PTRACE_POKEDATA`, modify `RIP` via `PTRACE_SETREGS` to point at the injected code, and resume. Verify execution. Then enable `kernel.yama.ptrace_scope=2` and confirm the attach fails. Document the Yama scope levels and their security implications.

4. **io_uring Seccomp Bypass Demonstration.** Write a program that installs a seccomp filter blocking `openat` and `read`, then uses `io_uring` to perform the same operations via submission queue entries. Verify whether the seccomp filter is enforced (behavior depends on kernel version and `io_uring` configuration). Document the kernel version boundary where io_uring seccomp enforcement was added.

5. **Syscall Dispatch Tracing.** Using `strace -c` and `perf trace`, profile a real application (e.g., `nginx` serving requests) and produce a syscall frequency histogram. Identify the top 10 syscalls by count and by time. Design a seccomp allowlist that permits only these syscalls plus essential process-lifecycle calls, and test the application under the filter.

---

## Readings and References

- Linux `seccomp(2)` man page: https://man7.org/linux/man-pages/man2/seccomp.2.html (retrieved: 2026-05-29)
- Linux kernel seccomp-BPF documentation: https://docs.kernel.org/userspace-api/seccomp_filter.html (retrieved: 2026-05-29)
- Linux `ptrace(2)` man page: https://man7.org/linux/man-pages/man2/ptrace.2.html (retrieved: 2026-05-29)
- io_uring security analysis — "io_uring: Linux Performance Boost or Security Headache?": https://www.upwind.io/feed/io_uring-linux-performance-boost-or-security-headache (retrieved: 2026-05-29)
- CVE-2026-31525 — Linux kernel BPF interpreter privilege escalation: https://www.sentinelone.com/vulnerability-database/cve-2026-31525/ (retrieved: 2026-05-29)
- MITRE ATT&CK T1055 — Process Injection: https://attack.mitre.org/techniques/T1055/ (retrieved: 2026-05-29)
- MITRE ATT&CK T1055.008 — Ptrace System Calls: https://attack.mitre.org/techniques/T1055/008/ (retrieved: 2026-05-29)
- libseccomp library documentation: https://github.com/seccomp/libseccomp (retrieved: 2026-05-29)
- "Programmable System Call Security with eBPF" (arXiv 2302.10366): https://arxiv.org/pdf/2302.10366 (retrieved: 2026-05-29)
- "Seccomp: Enhance Security for Linux Applications": https://www.armosec.io/blog/seccomp-internals-part-1/ (retrieved: 2026-05-29)
- Yama LSM documentation: https://docs.kernel.org/admin-guide/LSM/Yama.html (retrieved: 2026-05-29)
- Linux `userfaultfd(2)` man page: https://man7.org/linux/man-pages/man2/userfaultfd.2.html (retrieved: 2026-05-29)
...
|---------|-------|-------------|
| `domain2_chapter2A_process_memory.md` | Page fault handler, mmap/mprotect, VMA, KPTI | userfaultfd hooks page faults; mmap/mprotect are syscalls dispatched here |
| `domain2_chapter2C_caps_ns_lsm.md` | CAP_SYS_PTRACE, CAP_SYS_ADMIN, user namespaces, Yama | Capabilities gate ptrace/userfaultfd; namespaces affect ptrace UID checks |
| `domain1_chapter1A_elf_foundations.md` | execve path, pt_regs, register conventions | pt_regs is the register frame populated during execve; R10 replaces RCX |
| `domain5_chapter5A_kernel_exploitation.md` | Kernel heap exploitation, msg_msg spray, SLUB | io_uring UAF follows kernel heap exploitation methodology; userfaultfd TOCTOU |
| `domain3_chapter3A_stack_format_integer.md` | Stack overflow, sigreturn, SROP | Sigreturn dispatched via syscall mechanism; SROP crafts fake sigframes |
| `domain10_chapter10B_container_k8s_serverless.md` | Container seccomp profiles, Kubernetes pod security | Seccomp + Yama + namespaces define the container sandbox boundary |

---

## Glossary

- **Seccomp (Secure Computing):** Kernel mechanism for restricting a process's system call access via BPF filter programs.
- **BPF (Berkeley Packet Filter):** Bytecode virtual machine used for seccomp filters (cBPF) and modern tracing/security programs (eBPF).
- **io_uring:** High-performance asynchronous I/O interface using shared ring buffers between user and kernel space; bypasses seccomp on older kernels.
- **userfaultfd:** Mechanism allowing userspace to handle page faults, used legitimately for live migration and exploitatively for TOCTOU race amplification.
- **ptrace:** System call for process tracing and debugging, providing read/write access to another process's registers and memory.
- **Yama LSM:** Linux Security Module providing ptrace scope restrictions beyond standard UNIX permission checks.
- **TOCTOU (Time-of-Check-to-Time-of-Use):** Race condition class where a security check and the subsequent use of the checked value can be interleaved by an attacker.
- **pt_regs:** Kernel structure holding the saved user-mode register state on syscall entry, readable and writable via ptrace.
- **SECCOMP_RET_USER_NOTIF:** Seccomp action that forwards the syscall decision to a supervisor process via a notification file descriptor.
- **pidfd:** File descriptor referring to a process, providing race-free process management without PID reuse concerns.
- **process_vm_readv/writev:** System calls for direct cross-process memory transfer without ptrace attachment.
- **KPTI (Kernel Page Table Isolation):** Meltdown mitigation switching CR3 between user and kernel page tables on syscall entry/exit.
- **entry_SYSCALL_64:** x86_64 kernel syscall entry point loaded from IA32_LSTAR MSR, performing stack switch and register save.
- **sys_call_table:** Kernel array mapping syscall numbers to handler function pointers; the dispatch target after entry_SYSCALL_64.
