...
modulo: "22.3"
titolo: "Bitcoin, Ethereum, and Smart Contract Security"
versione: "Bitcoin Core 27.x / Solidity 0.8.x / EIP-1559 / EIP-4844"
livello: "Advanced"
prerequisiti:
  - "Domain 13 Chapter 13A §2.2 — Elliptic-Curve Cryptography (ECDSA, Schnorr)"
  - "Domain 22 Chapters 22A–B — Traditional Payment Security"
  - "Solidity/Vyper programming fundamentals"
obiettivi:
  - "Trace a Bitcoin transaction at the byte level through UTXO selection, Script evaluation, and SegWit witness construction"
  - "Classify and exploit the major smart-contract vulnerability classes — reentrancy, integer overflow, delegatecall injection, oracle manipulation — in a Foundry test environment"
  - "Execute a multi-tool smart-contract audit using Slither static analysis, Mythril symbolic execution, and Echidna property-based fuzzing"
  - "Reconstruct a flash-loan attack chain from Aave borrow through DEX price manipulation to profit extraction"
  - "Perform on-chain forensic analysis using transaction-graph traversal, mixer detection heuristics, and OFAC SDN screening"
tag: [bitcoin, ethereum, smart-contract, solidity, defi, flash-loan, reentrancy, slither, mythril, foundry, on-chain-forensics]
---

# Domain 22, Chapter 22C — Bitcoin, Ethereum, and Smart Contract Security

> **Learning objectives.** After completing this chapter, the student will be able to: (1) decode raw Bitcoin transactions at the byte level, evaluate Script opcodes, and explain the security model of P2PKH, P2SH, P2WPKH, P2WSH, and P2TR (Taproot); (2) identify, exploit, and remediate the top smart-contract vulnerability classes using industry-standard tooling; (3) conduct a structured smart-contract security audit combining Slither, Mythril, Foundry fuzzing, and manual code review; (4) analyze DeFi attack patterns — flash-loan exploit chains, governance attacks, oracle manipulation, and rug pulls — with reference to real-world incidents; (5) apply on-chain forensic techniques including transaction-graph analysis, mixer detection, and regulatory compliance screening.
...
1. **Smart contract auditing with Slither.** Clone the Damn Vulnerable DeFi v3 repository. Run `slither .` on each challenge contract and document every finding. For each HIGH/MEDIUM finding, determine whether it is a true positive or false positive. Then manually identify at least one vulnerability that Slither missed. Write a custom Slither detector (Python plugin) that catches the missed pattern.

2. **Flash loan exploit simulation in Foundry.** Using Foundry's `forge` framework, write a Solidity test that: (a) takes a flash loan from Aave V3 on a local fork, (b) manipulates a Uniswap V2 pool's spot price by executing a large swap, (c) exploits a vulnerable lending protocol that uses the manipulated spot price as its oracle, and (d) repays the flash loan with profit. Calculate the minimum flash-loan amount needed to move the price sufficiently, and document gas costs.

3. **Mythril symbolic execution.** Run `mythril analyze` against a contract containing a reentrancy vulnerability, an integer overflow (pre-Solidity 0.8), and an access-control flaw. Compare the results against Slither's output on the same contract. Document the detection overlap and unique findings from each tool. Measure execution time for contracts of increasing complexity (100, 500, 1000, 5000 SLOC).

4. **On-chain forensic investigation.** Using Etherscan and a local archive node (or a free RPC), trace the complete fund flow of the Euler Finance exploit (March 2023, ~$197M). Identify: the attacker's initial funding address, the flash-loan source, the vulnerable function call, the profit extraction path, and the addresses where funds were eventually returned. Construct a transaction-graph visualization.

5. **BIP-39/BIP-32 key derivation.** Implement BIP-39 mnemonic generation (128-bit entropy, 12 words) and BIP-32 hierarchical deterministic key derivation in Python using only the `hashlib` and `hmac` standard library modules plus `ecdsa`. Derive the first 5 Bitcoin addresses at path `m/84'/0'/0'/0/0` through `m/84'/0'/0'/0/4`. Verify your output against a reference implementation (Ian Coleman's BIP39 tool).

---

## Readings and References

- Atzei, N., Bartoletti, M., and Cimoli, T., "A Survey of Attacks on Ethereum Smart Contracts (SoK)," *POST 2017*. https://doi.org/10.1007/978-3-662-54455-6_8 (retrieved: 2026-05-29)
- Trail of Bits, "Slither: A Static Analysis Framework for Solidity." https://github.com/crytic/slither (retrieved: 2026-05-29)
- ConsenSys Diligence, "Mythril: Security Analysis Tool for EVM Bytecode." https://github.com/Consensys/mythril (retrieved: 2026-05-29)
- Foundry Book, "Foundry: Ethereum Development Toolchain." https://book.getfoundry.sh/ (retrieved: 2026-05-29)
- Trail of Bits, "Echidna: Smart Contract Fuzzer." https://github.com/crytic/echidna (retrieved: 2026-05-29)
- Hacken, "Smart Contract Auditing Tools 2026: A Reviewer's Stack." https://hacken.io/discover/audit-tools-review/ (retrieved: 2026-05-29)
- CoinLaw, "Smart Contract Security Risks and Audits Statistics 2026." https://coinlaw.io/smart-contract-security-risks-and-audits-statistics/ (retrieved: 2026-05-29)
- Nakamoto, S., "Bitcoin: A Peer-to-Peer Electronic Cash System," 2008. https://bitcoin.org/bitcoin.pdf (retrieved: 2026-05-29)
- Wood, G., "Ethereum Yellow Paper: A Formal Specification," 2024. https://ethereum.github.io/yellowpaper/paper.pdf (retrieved: 2026-05-29)
- FATF, "Updated Guidance for a Risk-Based Approach to Virtual Assets and VASPs," October 2021. https://www.fatf-gafi.org/en/publications/Fatfrecommendations/Guidance-rba-virtual-assets-2021.html (retrieved: 2026-05-29)
- OFAC, "Specially Designated Nationals and Blocked Persons List (SDN)." https://ofac.treasury.gov/specially-designated-nationals-and-blocked-persons-list-sdn-human-readable-lists (retrieved: 2026-05-29)
...
|---|---|---|
| Elliptic-curve cryptography, ECDSA, Schnorr | Domain 13, Chapter 13A §2.2 | Bitcoin/Ethereum transaction signing; Taproot Schnorr signatures |
| Zero-knowledge proofs, privacy coins, bridges | Domain 22, Chapter 22D | ZK rollups, Zcash/Monero privacy, cross-chain bridge exploits |
| Traditional payment security (EMV, ISO 8583) | Domain 22, Chapters 22A-22B | Contrasting centralized trust vs. decentralized consensus models |
| Web application security (SSRF, OAuth) | Domain 8, Chapters 8A-8B | Smart-contract reentrancy analogous to CSRF; oracle manipulation analogous to SSRF |
| Malware delivery and C2 | Domain 11, Chapter 11A | Cryptojacking malware uses standard delivery and persistence mechanisms |
| Identity and access management | Domain 14, Chapters 14A-14B | Wallet key management as distributed IAM; MPC threshold signatures |

---

## Glossary

- **ABI (Application Binary Interface):** The interface specification defining how to encode/decode function calls and data for EVM smart contracts.
- **BIP-32/39/44:** Bitcoin Improvement Proposals defining hierarchical deterministic wallets, mnemonic seed phrases, and multi-account derivation paths.
- **DEX (Decentralized Exchange):** An on-chain exchange protocol (Uniswap, SushiSwap) that enables token swaps without a centralized intermediary.
- **EIP-1559:** Ethereum transaction pricing mechanism introducing base fees (burned) and priority fees (tips to validators).
- **EVM (Ethereum Virtual Machine):** The stack-based virtual machine executing smart-contract bytecode on every Ethereum node.
- **Flash loan:** An uncollateralized loan that must be borrowed and repaid within a single atomic transaction; used legitimately for arbitrage and exploitatively for oracle manipulation.
- **MEV (Maximal Extractable Value):** The value that can be extracted by reordering, inserting, or censoring transactions within a block.
- **MPC (Multi-Party Computation):** A cryptographic protocol where multiple parties jointly compute a function (e.g., ECDSA signing) without any party learning the others' private inputs.
- **P2TR (Pay-to-Taproot):** Bitcoin output type combining Schnorr signatures with MAST for efficient and private script execution.
- **Reentrancy:** A smart-contract vulnerability where an external call re-enters the calling contract before state updates complete, enabling repeated withdrawals.
- **Rug pull:** A DeFi exit scam where project developers drain liquidity pools or mint unlimited tokens using hidden backdoor functions.
- **Slither:** A static-analysis framework for Solidity that detects 40+ vulnerability patterns through AST-level analysis.
- **UTXO (Unspent Transaction Output):** Bitcoin's accounting model where each transaction consumes previous outputs and creates new ones.
