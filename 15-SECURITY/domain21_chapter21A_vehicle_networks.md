...
fase: "Domain 21 — Automotive Security: In-Vehicle Networks"
modulo: "21.A"
titolo: "In-Vehicle Networks and Protocols"
versione: "ISO 11898 CAN 2.0B / CAN FD / ISO 14229 UDS / ISO 13400 DoIP / AUTOSAR R22-11 / SOME/IP / SecOC"
livello: "Advanced"
prerequisiti:
  - "Fundamentals of digital communication (bus topologies, serial protocols, error detection)"
  - "Linux networking and SocketCAN (ip link, candump, cansend)"
  - "Python scripting with python-can and scapy"
  - "Domain 9 — Network Security (TCP/IP, Ethernet, VLAN concepts)"
  - "Domain 13 — Applied cryptography (AES-CMAC, HMAC, symmetric key management)"
obiettivi:
  - "Analyze CAN 2.0B frame structure at the bit level including arbitration, error handling, and bit-stuffing mechanics"
  - "Reverse-engineer CAN message semantics from raw bus captures using SavvyCAN, python-can, and cantools with DBC correlation"
  - "Execute CAN bus attacks (ID enumeration, fuzzing, injection, priority DoS, bus-off) on a virtual or bench CAN network and measure their impact"
  - "Assess UDS SecurityAccess implementations for cryptographic weaknesses and demonstrate seed-key brute-force using udsoncan and caringcaribou"
  - "Evaluate SecOC deployment effectiveness including MAC truncation tradeoffs, freshness management, and key distribution attack surface"
tag: [can-bus, can-fd, can-xl, automotive-ethernet, uds, iso-tp, secoc, some-ip, doip, tsn, autosar, vehicle-security, ids, gateway]
---

# Domain 21, Chapter 21A — In-Vehicle Networks and Protocols

> **Learning objectives.** After completing this module the student will be able to: (1) explain CAN bus physical-layer operation including differential signaling, CSMA/CR arbitration, and TEC/REC error-confinement state machine at the bit level; (2) perform systematic CAN message reverse engineering from passive capture through correlation to DBC signal extraction using SavvyCAN and python-can; (3) implement and detect CAN bus attacks (injection, priority DoS, bus-off) on a SocketCAN virtual interface, documenting attack bandwidth requirements and IDS detection signatures; (4) exploit UDS SecurityAccess weaknesses via seed-key algorithm reversing and brute-force using caringcaribou and udsoncan; (5) assess SecOC deployment against MAC forgery, replay, and freshness-counter manipulation, calculating brute-force feasibility for truncated MACs.
...
**Exercise 1 — CAN Bus Injection and Priority DoS on Virtual CAN.**
Set up a virtual CAN interface on Linux (`ip link add dev vcan0 type vcan && ip link set up vcan0`). (a) Use `cangen vcan0 -g 10 -I 0x188 -D r -L 8` to simulate an engine-control ECU transmitting at 10 ms intervals. In a second terminal, run `cansniffer -c vcan0` to observe the baseline traffic. (b) Perform a targeted injection: use `python-can` to inject frames at ID 0x188 with crafted data bytes (simulating a fake RPM value of 8000 RPM) at 5 ms intervals. Observe in cansniffer how the injected frames interleave with the legitimate ECU's frames. Calculate the effective injection rate and the percentage of frames on the bus that are attacker-controlled. (c) Execute a priority DoS: flood the bus with `cangen vcan0 -g 0 -I 0x000 -D 0000000000000000 -L 8` and measure bus utilization with `canbusload vcan0@500000`. Document the impact on the legitimate ECU's ability to transmit. (d) Write specification-based IDS rules (as pseudocode or Python) that would detect both the injection and the DoS: frequency anomaly detection for (b) and bus-load threshold for (c).

**Exercise 2 — UDS Service Enumeration and SecurityAccess Fuzzing with caringcaribou.**
Using a virtual CAN setup with a simulated ECU (use `python-can` to implement a minimal UDS responder on vcan0 that handles DiagnosticSessionControl, SecurityAccess, and ReadDataByIdentifier). (a) Run `python -m caringcaribou uds discovery` to discover the simulated ECU's arbitration ID pair. (b) Enumerate supported UDS services with `python -m caringcaribou uds services <tx_id> <rx_id>`. (c) Fuzz the SecurityAccess service: `python -m caringcaribou uds security_access <tx_id> <rx_id>`. Implement a deliberately weak seed-key algorithm in the simulated ECU (XOR with a 2-byte static mask) and verify that caringcaribou's brute-force module recovers the key. (d) Modify the simulated ECU to use a 4-byte key with attempt-limiting (NRC 0x36 after 3 failed attempts, 10-second lockout). Re-run the brute-force and document the impact of attempt limiting on attack feasibility. Deliverable: comparison table of weak vs. hardened SecurityAccess configurations with time-to-compromise estimates.

**Exercise 3 — CAN Message Reverse Engineering with SavvyCAN.**
Obtain a CAN bus capture from a test vehicle or use the publicly available OpenDBC dataset for a supported vehicle platform. (a) Load the capture into SavvyCAN. (b) For each unique arbitration ID, record: transmission period, DLC, and data range per byte. (c) If a DBC file is available (from commaai/opendbc), load it in SavvyCAN and verify the signal decoding against known physical values (RPM, speed, steering angle). (d) If no DBC is available, perform manual correlation: use SavvyCAN's graphing feature to plot individual byte values over time while replaying the capture. Identify at least three signals (e.g., throttle position, brake status, steering angle) by correlating graph patterns with expected physical behavior. (e) Create a partial DBC file documenting the identified signals with start bit, length, byte order, scale, and offset. Validate the DBC by decoding a segment of the capture with `cantools` in Python and comparing decoded values against expected ranges.

**Exercise 4 — SOME/IP Service Discovery Monitoring and Spoofing with Scapy.**
Set up a vsomeip test environment on two Linux hosts connected via Ethernet. (a) Configure one host as a SOME/IP service provider (offering service 0x1000 on UDP port 30509) and the other as a consumer. Monitor the SOME/IP-SD multicast traffic (224.224.224.245:30490) with Wireshark's SOME/IP dissector. (b) From a third host (attacker), craft a SOME/IP-SD OfferService packet using Scapy's `automotive.someip` contribution layer, advertising service 0x1000 at the attacker's IP address. Send it to the multicast group. (c) Observe whether the consumer re-subscribes to the attacker's spoofed service. (d) If successful, implement a minimal SOME/IP server on the attacker host that responds to method calls with manipulated data. Document the end-to-end attack chain and propose mitigations (DTLS for SOME/IP, SOME/IP-SD authentication, PSFP ingress filtering).

**Exercise 5 — ISO-TP Fuzzing and DoIP Reconnaissance.**
(a) On a virtual CAN interface with a simulated UDS responder, use Scapy's `ISOTPSocket` to send malformed ISO-TP frames: (i) a First Frame with length field 0xFFF (4095 bytes) when the responder allocates only a 256-byte buffer; (ii) Consecutive Frames with out-of-sequence sequence numbers; (iii) Flow Control frames with FS=Wait (0x01) sent repeatedly to stall a transmitting ECU. Document the simulated ECU's response to each malformed input (crash, timeout, error frame, graceful rejection). (b) On a lab Ethernet network, use the `python-doipclient` library to send a Vehicle Identification Request (UDP broadcast on port 13400). Document all DoIP entities that respond, including VIN, logical address, and GID. Attempt a Routing Activation Request to each entity and document whether authentication is required (unauthenticated DoIP vs. DoIP-over-TLS on port 3496). Deliverable: ISO-TP fuzzing results matrix and DoIP attack surface inventory.
...
- ISO 13400-2:2019, "Road vehicles — Diagnostic communication over Internet Protocol (DoIP) — Part 2: Transport protocol and network layer services." International Organization for Standardization.
...
- AUTOSAR, "Specification of Secure Onboard Communication (SecOC)," AUTOSAR CP R22-11. <https://www.autosar.org/standards/classic-platform> (retrieved: 2026-05-29)
- AUTOSAR, "Specification of Intrusion Detection System Manager (IdsM)," AUTOSAR CP R22-11. <https://www.autosar.org/standards/classic-platform> (retrieved: 2026-05-29)
- Miller, C. and Valasek, C., "Remote Exploitation of an Unaltered Passenger Vehicle," Black Hat USA 2015. <https://www.blackhat.com/us-15/briefings.html>
- Cho, K.-T. and Shin, K. G., "Error Handling of In-Vehicle Networks Makes Them Vulnerable," ACM CCS 2016. <https://doi.org/10.1145/2976749.2978302> (bus-off attack)
- Cho, K.-T. and Shin, K. G., "Fingerprinting Electronic Control Units for Vehicle Intrusion Detection," USENIX Security 2016. <https://www.usenix.org/conference/usenixsecurity16/technical-sessions/presentation/cho> (clock-based CAN IDS)
- Hanselmann, M. et al., "CANet: An Unsupervised Intrusion Detection System for High Dimensional CAN Bus Data," IEEE Access, 2020. <https://doi.org/10.1109/ACCESS.2020.2995707>
- Wen, H. et al., "CAN-D: Automated CAN Message Decoder," USENIX Security 2020. <https://www.usenix.org/conference/usenixsecurity20>
- CVE-2015-5611 — Jeep Cherokee Uconnect: cellular-to-CAN remote exploitation. CVSS 9.8.
- CVE-2023-29389 — Toyota RAV4: CAN injection via headlight connector, no bus segmentation. CVSS 6.8.
- CVE-2023-33075 — Qualcomm SA8155P automotive SoC: memory corruption enabling IVI-to-CAN pivot. CVSS 9.8.
- Lufsec, "Why CAN Bus Defenses Still Fail in 2026," <https://blog.lufsec.com/why-can-bus-defenses-still-fail-in-2026/> (retrieved: 2026-05-29)
- commaai/opendbc, "Open CAN DBC Files for Vehicle Platforms," <https://github.com/commaai/opendbc> (retrieved: 2026-05-29)
- caringcaribou Project, "A Friendly Automotive Security Exploration Tool," <https://github.com/CaringCaribou/caringcaribou> (retrieved: 2026-05-29)
...
|---|---|---|---|
| Domain 9 — Network Security | §9A (TCP/IP, Ethernet) | Automotive Ethernet inherits IP-based attack vectors (ARP spoofing, DNS hijacking); DoIP is UDS over TCP/IP; TSN gPTP parallels NTP/PTP attacks | Bidirectional |
| Domain 12 — Reverse Engineering | §12B (firmware RE) | ECU firmware RE is prerequisite for reversing SecurityAccess algorithms and CAN message encoding; ARM/TriCore/RH850 targets | 21A → RE |
| Domain 13 — Cryptography | §13B (symmetric auth) | SecOC CMAC-AES-128, MAC truncation analysis, UDS Authentication PKI; side-channel attacks on AES key extraction | Bidirectional |
| Domain 17 — Physical Security | §17A–B (fault injection, JTAG) | ECU HSM security, fault injection bypassing SecureBoot, JTAG/SWD firmware extraction | 17 → 21A |
| Domain 18 — Hardware Security | §18 (ECU hardware) | CAN transceiver security, bus guardian ICs (NXP TJA1462), HSM key storage | Bidirectional |
| Chapter 21B — Vehicle Attacks | §21B (gateway bypass, OTA) | 21A provides network-level foundation; 21B extends to vehicle-level attacks, OTA security, sensor spoofing | 21A → 21B |
...
| Term | Definition |
|---|---|
| **CAN (Controller Area Network)** | Multi-master serial bus (ISO 11898) using differential signaling and CSMA/CR arbitration; dominant automotive in-vehicle network since the 1990s. |
| **Arbitration ID** | 11-bit (standard) or 29-bit (extended) identifier in the CAN frame header; determines message priority (lower ID = higher priority) and message type. |
| **CSMA/CR** | Carrier-Sense Multiple Access with Collision Resolution; CAN's deterministic medium access control where the lowest-ID transmitter always wins arbitration. |
| **TEC / REC** | Transmit Error Counter / Receive Error Counter; CAN error-confinement counters that transition nodes through Error Active → Error Passive → Bus Off states. |
| **Bus-off attack** | Targeted DoS exploiting TEC: attacker injects dominant bits during a specific ECU's recessive-bit transmissions to drive its TEC above 255, forcing disconnect. |
| **DBC (Database CAN)** | Standard file format mapping CAN arbitration IDs to signal definitions (name, start bit, length, byte order, scale, offset, unit). |
| **UDS (Unified Diagnostic Services)** | ISO 14229 diagnostic protocol providing standardized services (session control, security access, memory read/write, firmware transfer) over CAN/Ethernet. |
| **SecurityAccess (SID 0x27)** | UDS service implementing seed-key challenge-response authentication; the sole authentication gate for privileged diagnostic operations. |
| **ISO-TP (ISO 15765-2)** | Transport protocol providing segmentation and reassembly for multi-frame UDS messages over CAN's 8-byte payload limit. |
| **SecOC (Secure Onboard Communication)** | AUTOSAR module adding CMAC-AES-128 authentication and freshness counters to CAN/CAN FD frames; primary defense against CAN injection. |
| **DoIP (Diagnostics over IP)** | ISO 13400 protocol encapsulating UDS messages in TCP/IP for diagnostic communication over Automotive Ethernet. |
| **SOME/IP** | Scalable service-Oriented MiddlewarE over IP; dominant automotive service-oriented communication protocol in AUTOSAR Adaptive Platform. |
| **TSN (Time-Sensitive Networking)** | IEEE 802.1 profile providing deterministic, real-time Ethernet communication via time-aware shaping (802.1Qbv), precision timing (802.1AS), and per-stream policing (802.1Qci). |
| **CAN FD (Flexible Data-rate)** | CAN extension (ISO 11898-1:2015) supporting payloads up to 64 bytes and data-phase bit rates up to 8 Mbps while maintaining arbitration-phase backward compatibility. |
| **MACsec (IEEE 802.1AE)** | Link-layer encryption and authentication for Ethernet using AES-GCM; prevents sniffing and injection on Automotive Ethernet segments. |
