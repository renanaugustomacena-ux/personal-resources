...
modulo: "31.1"
titolo: "SIEM/SOAR Pipeline Design, Detection-as-Code, and Threat Intelligence Operationalization"
versione: "Sigma 1.1+, Splunk ES 8.x, Elastic Security 8.x, Microsoft Sentinel, Google SecOps (Chronicle), YARA-L 2.0"
livello: "Advanced"
prerequisiti:
  - "Log pipeline fundamentals: syslog, message queuing (Kafka), structured logging"
  - "Query languages: SPL basics, KQL basics, SQL"
  - "MITRE ATT&CK framework: tactics, techniques, sub-techniques, data sources"
  - "Windows and Linux event logging: Sysmon, auditd, PowerShell logging"
  - "Threat intelligence concepts: IOCs, TTPs, STIX/TAXII"
obiettivi:
  - "Design a conglomerate-scale log pipeline with Kafka-based ingestion, ECS/OCSF normalization, streaming enrichment, and hot/warm/cold tiered storage"
  - "Author portable Sigma detection rules using the complete modifier reference, correlation syntax, and CI/CD pipeline with unit testing against Mordor/EVTX datasets"
  - "Convert Sigma rules to SPL, KQL, EQL, and YARA-L and evaluate the tradeoffs between detection languages for sequence detection, statistical anomaly, and threshold-based rules"
  - "Build SOAR playbooks with enrichment-first triage, auto-remediation guardrails, and human-in-the-loop escalation for multi-subsidiary environments"
  - "Map detection coverage to MITRE ATT&CK, quantify gaps using composite scoring, and validate coverage through purple team exercises with Atomic Red Team and Caldera"
tag: [security, siem, soar, sigma, splunk, elastic, sentinel, chronicle, detection-as-code, threat-intelligence, mitre-attack, detection-engineering]
---
...
> 1. Architect a petabyte-scale SIEM pipeline with resilient ingestion (Kafka, WEF, agent-based collection), schema normalization (ECS, OCSF, CIM), and cost-optimized tiered storage.
> 2. Write production-grade Sigma rules with full metadata, cross-backend portability, correlation syntax for multi-event detections, and structured test cases.
> 3. Implement a Detection-as-Code CI/CD pipeline with GitHub Actions for validation, compilation to multiple backends, unit testing, and automated deployment.
> 4. Design SOAR playbooks for phishing triage, credential compromise response, and ransomware containment with appropriate automation/human-in-the-loop boundaries.
> 5. Operationalize threat intelligence through STIX/TAXII feeds, Diamond Model pivot-based hunting, and indicator lifecycle management with defined expiration policies.
...
1. **Sigma-to-Splunk/Elastic conversion pipeline.** Set up a Detection-as-Code CI/CD pipeline: (a) install `sigma-cli` with Splunk and Elasticsearch backends, (b) write a Sigma rule detecting LSASS access via comsvcs.dll (Event ID 1, CommandLine containing `comsvcs` and `MiniDump`), (c) convert the rule to Splunk SPL and Elastic KQL using `sigma convert`, (d) deploy both converted rules to a test Splunk instance and a test Elastic Security instance, (e) validate by executing the technique with Atomic Red Team (`Invoke-AtomicTest T1003.001`), and (f) integrate the workflow into a GitHub Actions CI pipeline that validates, compiles, and tests on every pull request.

2. **SOAR playbook development for phishing triage.** Design and implement a phishing triage playbook in a SOAR platform (Splunk SOAR, Shuffle, or TheHive/Cortex): (a) extract IOCs (sender, URLs, attachment hashes) from a reported phishing email, (b) enrich each IOC via VirusTotal and AbuseIPDB APIs, (c) compute a composite risk score based on enrichment results and sender domain age, (d) if score exceeds threshold: auto-quarantine the email across all inboxes (Exchange API), block sender domain at the email gateway, and isolate any host that clicked a URL (EDR API), (e) create a case ticket with full enrichment documentation. Test with 10 benign and 5 malicious email samples.

3. **ATT&CK coverage gap analysis and remediation.** Using the ATT&CK Navigator layer generation script from section 5.1: (a) generate a coverage heat map from your Sigma rule repository by extracting `attack.tXXXX` tags, (b) identify the top 10 techniques with zero detection coverage that are used by your threat model's priority threat actors, (c) for each gap, determine the required log source, write a Sigma rule, and add it to the CI pipeline, (d) validate three of the new rules using Atomic Red Team tests, and (e) produce a before/after ATT&CK Navigator comparison showing improved coverage.

4. **Alert tuning and false positive reduction.** Select the five noisiest detection rules in your SIEM (highest false positive rate). For each rule: (a) analyze 30 days of alert data to categorize true positives vs. false positives, (b) identify the common false positive patterns, (c) add targeted filter conditions to the Sigma rule (not broad exclusions that create blind spots), (d) redeploy via CI/CD, (e) measure the precision improvement over 7 days post-tuning, and (f) document each exclusion with a justification and an expiration review date.

5. **Threat intelligence operationalization pipeline.** Build a TI integration pipeline: (a) configure a TAXII 2.1 client to pull STIX indicators from a public feed (AlienVault OTX or CISA AIS), (b) normalize indicators into your SIEM's IOC format and load them into a lookup table, (c) write a streaming enrichment rule that matches incoming proxy/firewall events against the IOC table in real-time, (d) implement indicator lifecycle management: auto-expire IP indicators after 90 days and domain indicators after 12 months, (e) measure the match rate and false positive rate over 7 days, and (f) use the Diamond Model to pivot from one matched IOC to discover related infrastructure.

---

## Readings and References

- SigmaHQ. "Sigma Rule Repository." https://github.com/SigmaHQ/sigma (retrieved: 2026-05-29)
- SigmaHQ. "pySigma — Sigma Rule Processing Framework." https://github.com/SigmaHQ/pySigma (retrieved: 2026-05-29)
- Elastic. "Elastic Common Schema (ECS) Reference." https://www.elastic.co/guide/en/ecs/current/index.html (retrieved: 2026-05-29)
...
- Red Canary. "Atomic Red Team — MITRE ATT&CK Testing Framework." https://github.com/redcanaryco/atomic-red-team (retrieved: 2026-05-29)
- MITRE. "Caldera — Automated Adversary Emulation." https://github.com/mitre/caldera (retrieved: 2026-05-29)
- Splunk. "Splunk Enterprise Security Content Updates (ESCU)." https://research.splunk.com/ (retrieved: 2026-05-29)
- Elastic. "Elastic Detection Rules." https://github.com/elastic/detection-rules (retrieved: 2026-05-29)
- OASIS. "STIX 2.1 Specification." https://docs.oasis-open.org/cti/stix/v2.1/stix-v2.1.html (retrieved: 2026-05-29)
- OASIS. "TAXII 2.1 Specification." https://docs.oasis-open.org/cti/taxii/v2.1/taxii-v2.1.html (retrieved: 2026-05-29)
- TheHGTech. "Sigma Rules for SIEM Detection: Complete Guide [2026]." https://thehgtech.com/guides/sigma-rules-siem-detection.html (retrieved: 2026-05-29)
- GitHub. "Awesome Detection Engineering." https://github.com/infosecB/awesome-detection-engineering (retrieved: 2026-05-29)
...
|---|---|---|
| Domain 30 Chapter 30A | C2 framework detection | Network and host detection signatures (Sigma, Suricata, YARA) that this chapter's pipeline operationalizes |
| Domain 30 Chapter 30B | Credential theft detection | Authentication monitoring rules (Event ID 4662, 4769, 4768) deployed through Detection-as-Code pipelines |
| Domain 29 Chapter 29A | Ransomware kill chain | Detection requirements at each phase that map to Sigma rules managed in this chapter's CI/CD workflow |
| Domain 29 Chapter 29B | Campaign-specific detections | SPL/KQL queries for SolarWinds, MOVEit, and Change Healthcare triage integrated into SOAR playbooks |
| Domain 31 Chapter 31B | Runtime security (eBPF, auditd) | Telemetry sources (Falco, Tetragon, auditd) that feed into the SIEM pipeline designed in this chapter |
| Domain 11 Chapter 11B | EDR evasion techniques | Anti-evasion detections (ETW patching, AMSI bypass, log tampering) addressed in section 8 |
...
| Term | Definition |
|---|---|
| **Sigma** | Open-source, vendor-neutral detection rule format in YAML that compiles to platform-specific query languages (SPL, KQL, EQL, YARA-L) via pySigma backends. |
| **Detection-as-Code (DaC)** | Practice of managing detection rules with software engineering workflows: version control, code review, CI/CD pipelines, unit testing, and automated deployment. |
| **ECS (Elastic Common Schema)** | Hierarchical field namespace (`source.ip`, `user.name`, `process.name`) providing standardized field semantics for cross-source detection rule portability. |
| **OCSF (Open Cybersecurity Schema Framework)** | Vendor-neutral event schema using numbered classes (1001=File Activity, 3002=DNS Activity, 4001=Security Finding) with mandatory base attributes. |
| **CIM (Common Information Model)** | Splunk's data normalization layer defining standardized data models (Authentication, Network Traffic, Endpoint) with accelerated search capabilities. |
| **YARA-L 2.0** | Google Chronicle/SecOps detection language supporting multi-event correlation with `events` and `match` blocks, temporal conditions, and UDM schema references. |
| **EQL (Event Query Language)** | Elastic's sequence detection language enabling ordered multi-event pattern matching with `sequence by` and `maxspan` clauses. |
| **SOAR (Security Orchestration, Automation, and Response)** | Platform automating SOC workflows: enrichment, triage, containment, and ticketing through integration with SIEM, EDR, identity, and network infrastructure. |
| **STIX (Structured Threat Information eXpression)** | Standardized JSON format for representing threat intelligence objects (indicators, threat actors, attack patterns, campaigns) and their relationships. |
| **TAXII (Trusted Automated eXchange of Indicator Information)** | Transport protocol (REST API) for sharing STIX intelligence between TI platforms, SIEMs, and security tools. |
| **Diamond Model** | Intrusion analysis framework connecting four vertices (Adversary, Infrastructure, Capability, Victim) for structured threat intelligence pivoting and correlation. |
| **ATT&CK Navigator** | MITRE tool for visualizing technique coverage as color-coded heat maps across the ATT&CK matrix, enabling gap analysis and coverage scoring. |
| **Mordor / Security Datasets** | Pre-recorded attack telemetry datasets (Windows Event Logs, Sysmon, network captures) from simulated adversary operations, used for detection rule unit testing. |
| **BAS (Breach and Attack Simulation)** | Platforms (AttackIQ, SafeBreach, Picus) that continuously execute adversary simulations against production systems to validate detection coverage and measure regression. |
| **IOC lifecycle** | Management practice assigning expiration periods to indicators (30-90 days for IPs, 6-12 months for domains, indefinite for file hashes) to prevent false positives from stale data. |
