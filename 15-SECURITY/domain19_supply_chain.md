...
modulo: "19.1"
titolo: "Supply Chain Security"
versione: "SLSA v1.0 / Sigstore / CycloneDX 1.7 / SPDX 3.0"
livello: "Advanced"
prerequisiti:
  - "Domain 11 — Malware analysis (Cobalt Strike, DLL sideloading, C2 channels)"
  - "Domain 10 — Container and cloud security (OCI images, Kubernetes admission)"
  - "Domain 14 — Active Directory attacks (DCSync, Golden SAML, Kerberos)"
  - "CI/CD pipeline familiarity (GitHub Actions, GitLab CI, Jenkins)"
  - "Package manager internals (npm, pip, NuGet, Go modules, Cargo)"
obiettivi:
  - "Analyze dependency confusion, typosquatting, and CI/CD pipeline poisoning attack chains and implement ecosystem-specific hardening controls"
  - "Perform forensic timeline reconstruction of major supply chain incidents (SolarWinds SUNBURST, xz CVE-2024-3094, 3CX, Codecov, Kaseya) and extract transferable detection patterns"
  - "Generate and validate SBOMs (CycloneDX/SPDX) using Syft, correlate with Grype/OSV for vulnerability management, and apply VEX triage"
  - "Implement SLSA provenance generation, Sigstore keyless signing (cosign/Fulcio/Rekor), and admission-controller verification in a Kubernetes deployment pipeline"
  - "Design detection rules (Sigma, KQL, SPL, YARA) targeting supply chain attack indicators across package ecosystems, CI/CD platforms, and artifact registries"
tag: [supply-chain, slsa, sigstore, sbom, dependency-confusion, ci-cd-security, solarwinds, xz-backdoor, code-signing, container-signing]
---
...
> 1. Analyze dependency confusion, typosquatting, and CI/CD pipeline poisoning attack chains and implement ecosystem-specific hardening controls.
> 2. Perform forensic timeline reconstruction of major supply chain incidents (SolarWinds SUNBURST, xz CVE-2024-3094, 3CX, Codecov, Kaseya) and extract transferable detection patterns.
> 3. Generate and validate SBOMs (CycloneDX/SPDX) using Syft, correlate with Grype/OSV for vulnerability management, and apply VEX triage.
> 4. Implement SLSA provenance generation, Sigstore keyless signing (cosign/Fulcio/Rekor), and admission-controller verification in a Kubernetes deployment pipeline.
> 5. Design detection rules (Sigma, KQL, SPL, YARA) targeting supply chain attack indicators across package ecosystems, CI/CD platforms, and artifact registries.
...
### Exercise 19.1 — SBOM Generation and Vulnerability Correlation

1. Select a containerized application with 30+ transitive dependencies (e.g., a Node.js or Python web app).
2. Generate a CycloneDX SBOM using Syft: `syft packages <image> -o cyclonedx-json > sbom.cdx.json`.
3. Generate an SPDX SBOM from the same image: `syft packages <image> -o spdx-json > sbom.spdx.json`.
4. Scan both SBOMs with Grype (`grype sbom:sbom.cdx.json --fail-on critical`). Compare findings.
5. Cross-correlate with OSV-Scanner (`osv-scanner --sbom sbom.cdx.json`). Document any discrepancies between Grype and OSV results.
6. Write an OpenVEX document for one identified CVE, marking it "not_affected" with a technical justification.

**Deliverable:** Both SBOMs, Grype and OSV scan reports, VEX document, and a comparison matrix of vulnerability findings.

### Exercise 19.2 — Dependency Confusion Attack and Defense Lab

1. Set up a private npm registry (Verdaccio) and publish an internal scoped package (`@lab/core-utils`).
2. Create a project that depends on `@lab/core-utils` via `package.json`.
3. Simulate a confusion attack: publish `core-utils` (unscoped, version 99.0.0) to the Verdaccio instance acting as a public proxy. Observe which version `npm install` resolves.
4. Implement defenses: configure `.npmrc` with Package Source Mapping, scoped registry pinning, and `npm ci` enforcement.
5. Write a Sigma rule that detects package manager processes connecting to an unexpected registry during build.
6. Run `confused` against the project's lockfile to verify no remaining confusion risks.

**Deliverable:** `.npmrc` configuration, Sigma rule YAML, `confused` scan output, and a written explanation of the resolution logic exploited by the attack.

### Exercise 19.3 — SLSA Provenance and Sigstore Signing Pipeline

1. Create a GitHub Actions workflow that builds a Go binary from source.
2. Integrate `slsa-github-generator` to produce SLSA Level 3 provenance for the binary artifact.
3. Sign the binary using `cosign sign-blob` with keyless (Fulcio/Rekor) signing.
4. Write a verification script that: (a) verifies SLSA provenance with `slsa-verifier`, (b) verifies the cosign signature with expected OIDC issuer and identity constraints, (c) queries Rekor for the signing event entry.
5. Configure a Kyverno ClusterPolicy (or OPA Gatekeeper constraint) that rejects container images without valid cosign signatures in a test Kubernetes cluster.

**Deliverable:** GitHub Actions workflow YAML, verification script, Kyverno/Gatekeeper policy, and Rekor transparency log entry UUID.

### Exercise 19.4 — CI/CD Pipeline Poisoning Detection

1. Set up a Jenkins instance with a Multibranch Pipeline scanning a test repository.
2. Simulate a poisoned pipeline execution (I-PPE): modify a Makefile referenced by the Jenkinsfile to exfiltrate `env` output to a local HTTP listener.
3. Capture the exfiltration using `strace` on the Jenkins agent or network capture on the listener.
4. Write a Sigma rule detecting shell commands spawned by build processes that perform unexpected HTTP POST requests.
5. Implement hardening: pin the Makefile reference to a specific commit SHA, restrict Jenkins agent network egress with firewall rules, and scope credentials to pipeline folders.
6. Verify that the hardened pipeline blocks the simulated attack.

**Deliverable:** Jenkinsfile (before/after hardening), network capture of the exfiltration, Sigma rule, and firewall rules.

### Exercise 19.5 — Supply Chain Incident Forensic Analysis

1. Download the MITRE ATT&CK Navigator layer for SolarWinds SUNBURST (S0559).
2. Map each SUNBURST technique to the corresponding supply chain defense control: which controls from SLSA, Sigstore, SBOM, or hermetic builds would have prevented or detected each step?
3. Repeat for the xz utils backdoor (CVE-2024-3094): map the attack timeline (social engineering, build injection, IFUNC hooking) to detection opportunities (reproducible builds, tarball-vs-git diffing, binary file CI alerts).
4. Create a comparative forensic timeline (SolarWinds vs. xz vs. 3CX) documenting: initial access, persistence mechanism, detection method, time-to-detection, and blast radius.
5. For each incident, identify the single highest-impact defensive control that would have shortened time-to-detection most.

**Deliverable:** ATT&CK Navigator layers, defense-mapping tables, comparative timeline, and a 1,000-word analysis identifying systemic patterns across all three incidents.
...
- SLSA — Supply-chain Levels for Software Artifacts, v1.0 specification. [https://slsa.dev/spec/v1.0/](https://slsa.dev/spec/v1.0/)
- NIST SP 800-161 Rev. 1, Update 1 (November 2024) — Cybersecurity Supply Chain Risk Management Practices for Systems and Organizations. [https://csrc.nist.gov/pubs/sp/800/161/r1/upd1/final](https://csrc.nist.gov/pubs/sp/800/161/r1/upd1/final)
- CycloneDX v1.7 — OWASP SBOM Standard (October 2025). [https://cyclonedx.org/](https://cyclonedx.org/)
- SPDX v3.0 — ISO/IEC 5962 SBOM Standard. [https://spdx.dev/](https://spdx.dev/)
- Executive Order 14028 — Improving the Nation's Cybersecurity (May 2021). [https://www.whitehouse.gov/briefing-room/presidential-actions/2021/05/12/executive-order-on-improving-the-nations-cybersecurity/](https://www.whitehouse.gov/briefing-room/presidential-actions/2021/05/12/executive-order-on-improving-the-nations-cybersecurity/)
- EU Cyber Resilience Act (CRA), adopted 2024. SBOM and vulnerability disclosure requirements.

### Sigstore Ecosystem

- Sigstore — cosign, Fulcio, Rekor documentation. [https://docs.sigstore.dev/](https://docs.sigstore.dev/)
- Rekor v2 GA — improved scalability and witnessing. [https://blog.sigstore.dev/rekor-v2-ga/](https://blog.sigstore.dev/rekor-v2-ga/)
- in-toto — Supply chain layout verification. [https://in-toto.io/](https://in-toto.io/)

### Tooling

- Syft — SBOM generation (Anchore). [https://github.com/anchore/syft](https://github.com/anchore/syft)
- Grype — vulnerability scanner. [https://github.com/anchore/grype](https://github.com/anchore/grype)
- OSV-Scanner — Google's open-source vulnerability scanner. [https://github.com/google/osv-scanner](https://github.com/google/osv-scanner)
- Socket.dev — behavioral supply chain analysis. [https://socket.dev/](https://socket.dev/)
- OpenSSF Scorecard — automated supply chain security scoring. [https://scorecard.dev/](https://scorecard.dev/)

### Incident Reports and Case Studies

- CISA Emergency Directive 21-01 — SolarWinds SUNBURST. [https://www.cisa.gov/emergency-directive-21-01](https://www.cisa.gov/emergency-directive-21-01)
- CVE-2024-3094 — xz utils backdoor. Andres Freund disclosure on oss-security. [https://www.openwall.com/lists/oss-security/2024/03/29/4](https://www.openwall.com/lists/oss-security/2024/03/29/4)
- CVE-2023-29059 — 3CX Desktop App supply chain compromise. Mandiant analysis. [https://www.mandiant.com/resources/blog/3cx-software-supply-chain-compromise](https://www.mandiant.com/resources/blog/3cx-software-supply-chain-compromise)
- Codecov Bash Uploader breach (2021). [https://about.codecov.io/security-update/](https://about.codecov.io/security-update/)
- Kaseya VSA / REvil — CVE-2021-30116, CVE-2021-30119, CVE-2021-30120.
...
|--------|-------------|--------------|
| Domain 11 — Malware | SUNBURST Cobalt Strike Beacon; TEARDROP/RAINDROP in-memory loaders; 3CX DLL sideloading; REvil via Kaseya | S1.6.1, S1.6.3, S1.6.5 |
| Domain 10 — Cloud/Container | Container image signing (cosign); SBOM for container layers; Kubernetes admission controllers; base image poisoning | S2.2, S2.4, S1.4 |
| Domain 14 — AD/Windows | SUNBURST → DCSync → Golden SAML; WDAC code signing enforcement; Authenticode bypass | S1.6.1, S1.5 |
| Domain 17 — Physical/Hardware | Hardware Trojans; PCB implants; PUF authentication; NSA ANT catalog; HSM-backed code signing | S4.2, S4.3, S4.1 |
| Domain 18 — Adversarial ML | ML model supply chain (pickle RCE, ModelHub poisoning); safetensors; ML-BOM proposals | S4 (via Domain 18 S8) |
| Domain 2 — Network | SUNBURST DNS C2 (DGA); network monitoring for supply chain IOCs; CI/CD secret exfiltration via DNS tunneling | S1.6.1, S3.3 |
...
| Term | Definition |
|------|-----------|
| **Dependency Confusion** | An attack exploiting package manager resolution logic where an attacker publishes a higher-versioned package on a public registry with the same name as a private/internal package, causing the package manager to install the malicious public version. |
| **Typosquatting** | Registration of package names that are visually or typographically similar to popular packages, exploiting user typos during installation to deliver malicious code. |
| **SLSA (Supply-chain Levels for Software Artifacts)** | A graduated security framework defining four levels of build integrity guarantees, from documented provenance (L1) to hermetic, reproducible builds with two-person review (L4). |
| **SBOM (Software Bill of Materials)** | A machine-readable inventory of all components (direct and transitive dependencies, embedded libraries) in a software product, enabling vulnerability correlation and license compliance. |
| **Sigstore** | An open-source project providing keyless code signing via Fulcio (ephemeral certificate authority), Rekor (transparency log), and cosign (signing tool), eliminating long-lived key management. |
| **Provenance Attestation** | A signed document (in-toto format) describing how a software artifact was built: builder identity, source repository, build command, input dependencies, and output digest. |
| **VEX (Vulnerability Exploitability eXchange)** | A machine-readable statement declaring whether a known vulnerability actually affects a specific product, reducing false positives in SBOM-driven vulnerability management. |
| **Hermetic Build** | A build process where all inputs are explicitly declared and no network access or undeclared file access occurs during compilation, preventing build-time injection attacks. |
| **RepoJacking** | Claiming a renamed or abandoned GitHub namespace to serve malicious code through dependencies that still reference the old namespace path. |
| **Package Source Mapping** | A registry configuration mechanism (NuGet 6.0+, npm scopes) that binds specific package name patterns to specific registries, preventing namespace confusion. |
| **Reproducible Build** | A build that produces bit-for-bit identical output when given the same source and build configuration, enabling independent verification that distributed binaries match their source. |
| **Code Signing** | Cryptographic signing of software artifacts to prove publisher identity and artifact integrity; compromised signing keys (SolarWinds, 3CX) render this control insufficient without additional provenance verification. |
| **Poisoned Pipeline Execution (PPE)** | An attack where the adversary modifies CI/CD pipeline configuration (directly or indirectly) to execute malicious code during the build process, potentially exfiltrating secrets or tampering with artifacts. |
| **OpenSSF Scorecard** | An automated tool that evaluates open-source projects across security heuristics (branch protection, code review, signed releases, dangerous workflows) and produces a 0-10 score. |
