...
modulo: "28.A"
titolo: "IoT Protocol Security"
versione: "OWASP IoT Top 10 2018 / ETSI EN 303 645 v3.1.3 / NIST IR 8259"
livello: "Advanced"
prerequisiti:
  - "TLS 1.3 and DTLS fundamentals (Domain 9 Chapter 9A)"
  - "Cryptographic primitives — AES-CCM, ECDH, PAKE protocols (Domain 13 Chapter 13A)"
  - "Network architecture and TCP/UDP/IP (Domain 9)"
  - "RF fundamentals and SDR concepts (Domain 20)"
  - "Firmware extraction and reverse engineering basics (Domain 12 Chapter 12B)"
obiettivi:
  - "Analyze MQTT broker security configurations (Mosquitto TLS, ACLs, client certificates), identify misconfigurations via Shodan reconnaissance, and exploit unauthenticated brokers using mosquitto_sub/pub and paho-mqtt"
  - "Evaluate Zigbee key-distribution vulnerabilities — default TC link key, install-code-based joining, Touchlink commissioning — and capture/decrypt Zigbee traffic using KillerBee and Wireshark"
  - "Assess Z-Wave S0 vs S2 security mechanisms, demonstrate the S0 key-exchange weakness, and design defenses against downgrade attacks"
  - "Implement CoAP resource discovery, identify amplification vectors via multicast, and configure DTLS hardening for constrained IoT endpoints"
  - "Compare the security models of BLE pairing modes (Just Works, Passkey, Numeric Comparison, OOB), reproduce KNOB and BLESA attack scenarios in a lab, and apply SweynTooth mitigations"
tag: [iot, mqtt, coap, zigbee, zwave, ble, thread, matter, lorawan, dtls, protocol-security, killerbee, mosquitto]
---

# Domain 28, Chapter 28A — IoT Protocol Security

> **After completing this chapter, you will be able to:** (1) perform full MQTT broker reconnaissance — topic enumeration, $SYS fingerprinting, message injection, and credential brute-force — and implement a hardened Mosquitto configuration with mutual TLS and per-client ACLs; (2) capture Zigbee traffic with KillerBee, extract the network key from a default-TC-link-key join, and demonstrate how install codes prevent key sniffing; (3) analyze Z-Wave S0 and S2 key-exchange flows, exploit the S0 zero-key weakness, and configure S2 Access Control with DSK verification; (4) assess BLE device security through GATT enumeration with gatttool/bettercap, evaluate pairing mode strength, and apply mitigations for KNOB, BLURtooth, BLESA, and SweynTooth; (5) evaluate Matter's PASE/CASE authentication, device attestation via DAC chains, and multi-fabric ACL enforcement.
...
1. **MQTT broker reconnaissance and hardening.** Deploy a Mosquitto broker in a lab environment with default configuration (no auth, no TLS). Using `mosquitto_sub`, subscribe to `#` and `$SYS/#` to enumerate all topics and fingerprint the broker version. Using the Python `paho-mqtt` script from Section 17, perform full topic discovery. Then inject a false sensor reading into a telemetry topic and a spoofed actuator command into a control topic. After documenting the attack surface, harden the broker: enable TLS with mutual certificate authentication, configure per-client ACLs with least-privilege topic access, disable anonymous access, restrict `$SYS/#` to admin users, and set `message_size_limit`. Verify that the previous attacks fail against the hardened configuration.

2. **Zigbee key sniffing with KillerBee.** Using a CC2531 USB dongle with KillerBee firmware (or an ApiMote), capture Zigbee traffic on channel 15 during a device join operation. Use `zbdump` to capture the join to a pcap file. Use `zbgoodfind` with the default TC link key (`ZigBeeAlliance09`) to extract the network key from the Transport Key frame. Open the capture in Wireshark, enter the extracted network key in Protocols > ZigBee > Security Keys, and verify that NWK-layer frames are now decrypted. Document the full attack chain and explain how install-code-based joining (Zigbee 3.0) prevents this attack by deriving a unique TCLK via AES-MMO hash.

3. **CoAP amplification analysis.** Using `aiocoap` or `coap-client`, send a GET request to `coap://[target]:5683/.well-known/core` and measure the response size. Calculate the amplification factor (response bytes / request bytes). Then send a multicast GET to `coap://224.0.1.187/.well-known/core` on a lab network with multiple CoAP devices and document the aggregate response volume. Design and implement a mitigation plan: disable multicast CoAP responses, enforce DTLS on port 5684, restrict `/.well-known/core` to authenticated clients, and implement rate limiting.

4. **BLE device security assessment.** Select a consumer BLE device (smart lock, fitness tracker, or smart bulb). Using `bluetoothctl` and `gatttool`, enumerate all services and characteristics. Identify which characteristics are readable/writable without authentication. Attempt to write a command to an actuator characteristic (e.g., unlock, change color). Using `bettercap`, perform BLE reconnaissance and enumeration. Determine the pairing mode (Just Works, Passkey, Numeric Comparison) and assess its MitM resistance. Document all findings with CVSS scores, CWE classifications, and remediation recommendations per the OWASP IoT Top 10.

5. **LoRaWAN security assessment.** Deploy a ChirpStack LoRaWAN network server locally using Docker. Register a device using both OTAA and ABP activation methods. Capture the Join-Request and Join-Accept exchange for the OTAA device and verify session key derivation. For the ABP device, demonstrate the replay vulnerability: capture an uplink frame, note the frame counter, power-cycle the device (resetting the counter), and attempt to replay the captured frame. Verify that strict frame-counter enforcement rejects the replay. Document the security differences between OTAA and ABP and explain why ABP should never be used in production.
...
- ETSI EN 303 645 — Cybersecurity for Consumer IoT: <https://www.etsi.org/technologies/consumer-iot-security> (retrieved: 2026-05-29)
...
- MQTT v5.0 Specification — OASIS Standard: <https://docs.oasis-open.org/mqtt/mqtt/v5.0/mqtt-v5.0.html> (retrieved: 2026-05-29)
- Eclipse Mosquitto Documentation: <https://mosquitto.org/documentation/> (retrieved: 2026-05-29)
- CoAP — RFC 7252: <https://datatracker.ietf.org/doc/rfc7252/> (retrieved: 2026-05-29)
- DTLS 1.3 — RFC 9147: <https://datatracker.ietf.org/doc/rfc9147/> (retrieved: 2026-05-29)
- KillerBee Framework: <https://github.com/riverloopsec/killerbee> (retrieved: 2026-05-29)
- Matter Specification (CSA): <https://csa-iot.org/all-solutions/matter/> (retrieved: 2026-05-29)
- Zigbee Alliance — Zigbee 3.0 Specification: <https://csa-iot.org/all-solutions/zigbee/> (retrieved: 2026-05-29)
- Z-Wave Alliance — Z-Wave Specification: <https://z-wavealliance.org/> (retrieved: 2026-05-29)
- LoRaWAN Specification (LoRa Alliance): <https://lora-alliance.org/resource_hub/lorawan-specification-v1-0-4/> (retrieved: 2026-05-29)
- Trend Micro — MQTT and CoAP Security Issues: <https://www.trendmicro.com/vinfo/es/security/news/internet-of-things/mqtt-and-coap-security-and-privacy-issues-in-iot-and-iiot-communication-protocols> (retrieved: 2026-05-29)
...
|---|---|---|---|
| §1-3 MQTT/CoAP/AMQP | Domain 9 Ch.9A | TLS/DTLS fundamentals underlying IoT transport security | Domain 9A §5 |
| §4-5 Zigbee/Z-Wave | Domain 13 Ch.13A | AES-CCM, ECDH key exchange primitives | Domain 13A §1.1, §2.2 |
| §6 BLE | Domain 9 Ch.9B §2.4 | BLE security foundations, pairing modes | Domain 9B §2.4 |
| §7 Thread/Matter | Domain 13 Ch.13A | SPAKE2+, J-PAKE, PAKE protocols | Domain 13A |
| §4.6 Key extraction | Domain 12 Ch.12B §2 | Firmware extraction techniques (SPI dump, JTAG, SWD) | Domain 12B §2 |
| §15-16 Gateway/segmentation | Domain 28B | Gateway firmware security, secure boot, update signing | Domain 28B |
...
| Term | Definition |
|---|---|
| **MQTT** | Message Queuing Telemetry Transport — TCP-based publish/subscribe protocol for IoT telemetry (ports 1883/8883) |
| **CoAP** | Constrained Application Protocol — UDP-based REST protocol for resource-constrained IoT devices (RFC 7252) |
| **DTLS** | Datagram Transport Layer Security — TLS-equivalent security over UDP, used by CoAP, Thread, and LwM2M |
| **QoS** | Quality of Service — MQTT delivery guarantee levels: 0 (at-most-once), 1 (at-least-once), 2 (exactly-once) |
| **KillerBee** | Open-source framework for IEEE 802.15.4/Zigbee security assessment using CC2531/ApiMote hardware |
| **Trust Center** | Zigbee network entity managing key distribution; distributes the network key to joining devices via APS Transport Key |
| **Install code** | Unique per-device code (Zigbee 3.0) from which a device-specific TC link key is derived via AES-MMO hash, preventing key sniffing |
| **S2** | Z-Wave Security 2 — ECDH-based key exchange with DSK verification, replacing the vulnerable S0 protocol |
| **DSK** | Device-Specific Key — 16-byte value printed on Z-Wave device labels, authenticating the S2 ECDH exchange against MitM |
| **GATT** | Generic Attribute Profile — BLE data model defining services, characteristics, and descriptors |
| **PASE** | Passcode Authenticated Session Establishment — Matter commissioning protocol using SPAKE2+ |
| **CASE** | Certificate Authenticated Session Establishment — Matter operational protocol using NOC-based mutual authentication |
| **DAC** | Device Attestation Certificate — X.509 certificate proving a Matter device is genuine, signed by a PAI CA |
| **LoRaWAN** | Long Range Wide Area Network — LPWAN protocol using CSS modulation with AES-128 encryption (NwkSKey/AppSKey) |
| **OTAA** | Over-The-Air Activation — LoRaWAN join method deriving session keys from a root AppKey via Join-Request/Accept exchange |
