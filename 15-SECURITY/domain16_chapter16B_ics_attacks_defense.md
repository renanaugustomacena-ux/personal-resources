...
modulo: "16.2"
titolo: "ICS/OT Attack Case Studies, Network Architecture, and Defense"
versione: "MITRE ATT&CK for ICS v16, IEC 62443 Ed.4, NERC CIP v7, NIST SP 800-82 Rev.3, NIS2 Directive 2022/2555, TSA SD-2C"
livello: "Advanced"
prerequisiti:
  - "Domain 16.1 (ICS/OT protocol security, PLC exploitation fundamentals)"
  - "Familiarity with Suricata, Sigma, and YARA rule syntax"
  - "Understanding of Windows internals for OT workstation analysis"
  - "Network packet analysis with Wireshark (ICS dissectors)"
  - "Python scripting for protocol interaction (pymodbus, snap7, pycomm3)"
obiettivi:
  - "Perform deep technical analysis of TRITON, INDUSTROYER, PIPEDREAM, and FrostyGoop attack frameworks including protocol-level exploitation sequences"
  - "Exploit PLC platforms using python-snap7, pycomm3, and Scapy to demonstrate register manipulation, program upload, and memory-area attacks"
  - "Design and deploy ICS detection rules (Suricata, Sigma, YARA) targeting specific ICS malware families and protocol anomalies"
  - "Implement ICS incident response procedures including PLC forensic acquisition, historian SQL forensics, and safe-shutdown coordination"
  - "Map ICS threat actor TTPs to MITRE ATT&CK for ICS and build OT-specific threat hunting playbooks for CHERNOVITE, ELECTRUM, and XENOTIME"
tag: [ics, ot-security, triton, industroyer, pipedream, frostygoop, stuxnet, plc-exploitation, ics-detection, ics-incident-response, mitre-attck-ics, sigma, suricata, yara]
---

# Domain 16, Chapter 16B — ICS/OT Attack Case Studies, Network Architecture, and Defense

> **Learning Objectives.** After completing this module the student will be able to: (1) perform deep technical analysis of TRITON, INDUSTROYER/INDUSTROYER2, PIPEDREAM, and FrostyGoop/BUSTLEBERM frameworks including protocol-level exploitation sequences and payload mechanics; (2) exploit PLCs using python-snap7, pycomm3, pymodbus, and Scapy to demonstrate register manipulation, program upload/download, and direct memory-area attacks; (3) develop and deploy ICS-specific detection rules (Suricata for Modbus/S7comm/IEC 104, Sigma for OT workstation indicators, YARA for ICS malware families) with tuned false-positive management; (4) execute ICS incident response procedures including PLC forensic acquisition via engineering protocols, historian SQL forensics for timeline reconstruction, and safe-shutdown coordination with process engineers; (5) build OT-specific threat hunting playbooks mapped to MITRE ATT&CK for ICS for activity groups CHERNOVITE, ELECTRUM, XENOTIME, and KAMACITE.
...
**Exercise 16.2-1 — PLC program integrity monitoring with python-snap7.**
Using a Siemens S7-1200 or S7-1500 (physical or emulated via Snap7 server): (a) Connect with python-snap7 and download all Organization Blocks (OB1, OB35), Function Blocks, and Data Blocks. Compute SHA-256 hashes for each block and store them as the golden image baseline. (b) Manually modify DB1 on the PLC (change a setpoint value via TIA Portal or snap7 `db_write`). Re-download and re-hash. Confirm that the DB1 hash changed while OB/FB hashes remained constant. (c) Write a Python script that polls the PLC every 60 seconds, downloads all program blocks, hashes them, and compares against the baseline — generating an alert (syslog message) on any mismatch. (d) Document the detection latency (polling interval) and discuss the trade-off between polling frequency and PLC load. Deliverable: the monitoring script, baseline hash file, alert log showing detection of the modified DB, and a trade-off analysis.

**Exercise 16.2-2 — Modbus TCP exploitation and detection with pymodbus and Scapy.**
Deploy a Modbus TCP simulator (e.g., diagslave, ModRSsim2, or pymodbus server) in an isolated lab. (a) Using pymodbus, read holding registers 0–99 (FC03) and coils 0–15 (FC01). Document the baseline values. (b) Using pymodbus, write register 40 to value 9999 (FC06) and coil 5 to ON (FC05). Verify the writes by re-reading. (c) Using Scapy, craft a raw Modbus TCP packet for FC16 (Write Multiple Registers) targeting registers 0–9 with all-zero values. Send and verify. (d) Capture all traffic in Wireshark with the `modbus` dissector. Write a Suricata rule that alerts on any FC06/FC16 write from a source IP not in a whitelist variable `$SCADA_MASTER`. Test the rule against the captured pcap. Deliverable: pymodbus scripts, Scapy crafting script, pcap file, Suricata rule, and rule validation output.

**Exercise 16.2-3 — INDUSTROYER IEC 104 attack sequence reconstruction.**
Using the ESET public analysis of INDUSTROYER's `104.dll` module: (a) Reconstruct the exact IEC 104 command sequence: STARTDT activation → Station Interrogation (C_IC_NA_1) → Select-Before-Operate sequence for Single Command (C_SC_NA_1, Type 45) and Double Command (C_DC_NA_1, Type 46) targeting circuit breaker IOAs. (b) Using Wireshark's IEC 104 dissector on the publicly available INDUSTROYER pcap (or a simulated capture from an IEC 104 test tool), identify each command phase and annotate the APDU type, cause of transmission, and IOA. (c) Write Sigma rules for INDUSTROYER indicators on OT workstations: detection of the `defragsvc` Windows service, TOR traffic from SCADA network segments, and rapid IEC 104 command sequences (>10 commands in <5 seconds). (d) Map the full INDUSTROYER attack to MITRE ATT&CK for ICS techniques across all 12 tactics. Deliverable: annotated pcap analysis, Sigma rules, and a complete ATT&CK mapping table with technique IDs and justifications.

**Exercise 16.2-4 — OT workstation hardening with WDAC and Sysmon.**
On a Windows 10/11 VM configured as an OT engineering workstation: (a) Create a Windows Defender Application Control (WDAC) policy that allows only TIA Portal, WinCC, and standard Windows system processes. Block PowerShell, cmd.exe scripting hosts, Python, and all unsigned executables. Test the policy by attempting to run a Rubber Ducky payload (DuckyScript PowerShell reverse shell). (b) Deploy Sysmon with an OT-specific configuration (based on the SwiftOnSecurity template, augmented with: process creation events for engineering tools, network connection events to PLC subnets, file creation events in project directories). (c) Generate Sigma rules that detect: unexpected process execution on the EWS (anything not in the WDAC whitelist that somehow bypasses WDAC), network connections from the EWS to internet destinations, and modification of PLC project files (.s7p, .zap, .acd) outside of approved maintenance windows. Deliverable: WDAC policy XML, Sysmon configuration XML, three Sigma rules, and test evidence (blocked execution screenshot, Sysmon event log entries).

**Exercise 16.2-5 — ICS incident response tabletop: FrostyGoop/Modbus attack on heating infrastructure.**
Based on the Dragos FrostyGoop/BUSTLEBERM report: (a) Design a tabletop exercise scenario: an attacker has gained network access to a district heating SCADA network and is sending Modbus TCP FC06 (Write Single Register) commands to ENCO controllers, changing temperature setpoints from 70°C to 0°C during winter conditions. (b) Develop the IR playbook: detection (what Modbus traffic anomaly triggers the alert?), containment (how do you isolate the attacker without disrupting heating to 600 buildings?), eradication (how do you restore correct setpoints and verify controller integrity?), and recovery (what operational checks confirm safe resumption?). (c) Document the safe-shutdown decision tree: under what conditions does the IR team recommend manual shutdown of the heating system vs. containment-while-running? (d) Identify the root-cause indicators: how were the ENCO controllers exposed to the attacker (internet-facing Modbus, missing segmentation, default credentials)? Deliverable: tabletop scenario document, IR playbook with decision trees, and a root-cause analysis with remediation recommendations.
...
- MITRE, "ATT&CK for ICS Matrix v19," https://attack.mitre.org/matrices/ics/ (retrieved: 2026-05-29)
- MITRE, "ATT&CK for ICS Techniques," https://attack.mitre.org/techniques/ics/ (retrieved: 2026-05-29)
- Dragos, "FrostyGoop / BUSTLEBERM: ICS Malware Targeting Heating Infrastructure," Dragos Threat Intelligence, 2024
- ESET, "Industroyer2: Industroyer reloaded," https://www.welivesecurity.com/2022/04/12/industroyer2-industroyer-reloaded/ (retrieved: 2026-05-29)
- FireEye/Mandiant, "TRITON Framework Analysis," https://cloud.google.com/blog/topics/threat-intelligence/triton-actor-ttp-profile/ (retrieved: 2026-05-29)
- CISA/NSA/FBI/DOE, "APT Cyber Tools Targeting ICS/SCADA Devices (PIPEDREAM)," Alert AA22-103A, April 2022
- Claroty Team82, "Evil PLC Attack: Weaponizing PLCs," https://claroty.com/team82/research/evil-plc-attack (retrieved: 2026-05-29)
- SOCRadar, "CISA ICS Advisories Recap 2025," https://socradar.io/blog/cisa-industrial-control-systems-ics-advisories-2025/ (retrieved: 2026-05-29)
...
- CVE-2021-29240 through CVE-2021-29247 — Codesys V3 runtime vulnerabilities (CVSS 9.8)
- CVE-2015-5374 — Siemens SIPROTEC relay DoS
- CVE-2018-7522 — Schneider Triconex firmware improper input validation (TRITON)
- Lee, Robert M. and Assante, Michael J., "Analysis of the Cyber Attack on the Ukrainian Power Grid," SANS ICS, 2016

---

## 17. Cross-References (tabular)

| Domain/Chapter | Topic | Relationship to This Chapter |
|---|---|---|
| Domain 9 (Network Security) | TCP/IP attacks, TLS, IDS/IPS | ICS protocols use TCP/IP; ARP spoofing on flat OT networks; TLS wrapping for Modbus/OPC UA/CIP uses Domain 9 TLS stack |
| Domain 12 (Reverse Engineering) | Firmware RE, protocol RE | PLC firmware analysis (§4.3) uses Domain 12 disassembly/decompilation; TriStation protocol RE required Domain 12 network protocol analysis |
| Domain 16A (ICS/OT Protocols) | Protocol internals, PLC exploitation basics | Companion chapter providing protocol frame formats, function-code tables, and hardening guidance that this chapter builds upon with deep attack analysis |
| Domain 17 (Physical/Hardware) | Voltage glitching, PCB RE, debug bypass | S7-1500 firmware decryption required 17B fault injection and 17C PCB analysis; PLC hardware tamper resistance assessed with 17D debug lockout |
| Domain 19 (Supply Chain) | Software supply chain, vendor compromise | Havex distributed via compromised ICS vendor websites; PIPEDREAM exploits shared Codesys V3 runtime creating cross-vendor supply chain risk |
| Domain 27 (Defensive Architecture) | Defense-in-depth, SIEM, zero-trust | ICS network architecture adapts Domain 27 zero-trust to OT constraints; ICS detection complements IT SIEM with protocol-aware behavioral baselines |
...
| Term | Definition |
|---|---|
| **TMR (Triple Modular Redundancy)** | Safety architecture running three independent processors that vote on outputs (2-of-3); used in SIS controllers (e.g., Triconex) for fail-safe operation |
| **TriStation** | Proprietary Schneider protocol (UDP 1502) for engineering workstation communication with Triconex safety controllers; exploited by TRITON |
| **FrostyGoop/BUSTLEBERM** | ICS malware using standard Modbus TCP (FC06/FC16) to manipulate heating-system controllers; notable for weaponizing the simplest ICS protocol |
| **CHERNOVITE** | Dragos-tracked activity group attributed to PIPEDREAM/INCONTROLLER ICS attack framework development |
| **ELECTRUM** | Dragos-tracked activity group associated with INDUSTROYER and Sandworm (GRU Unit 74455) operations against Ukrainian power grid |
| **XENOTIME** | Dragos-tracked activity group attributed to TRITON/TRISIS SIS attack framework targeting petrochemical safety systems |
| **FINS** | Factory Interface Network Service — Omron proprietary ICS protocol (UDP 9600) providing PLC memory read/write and program control |
| **IOA (Information Object Address)** | IEC 60870-5-104 addressing scheme identifying specific data points (breaker status, measurement values) within a substation RTU |
| **SBO (Select-Before-Operate)** | IEC 104 two-step command sequence: SELECT verifies operability, EXECUTE performs the action — mimics legitimate SCADA operator behavior |
| **Golden Image** | Verified, approved version of a PLC program used as the reference for integrity monitoring — any hash mismatch indicates unauthorized modification |
| **Codesys V3 Runtime** | Software PLC runtime licensed by 500+ vendors; a vulnerability in Codesys affects all PLCs sharing the runtime regardless of hardware manufacturer |
| **DUSTTUNNEL** | OT-specific C2 implant within PIPEDREAM designed for air-gapped/firewalled OT networks with intermittent connectivity |
| **RPMB** | Replay Protected Memory Block — tamper-resistant storage partition in eMMC/UFS used for anti-rollback counter storage in Android Verified Boot |

---

## Exercises

**Exercise 1 — TRITON/INDUSTROYER Detection Rule Development.**
Using this module's technical analyses, develop a detection rule set. (a) Write three Suricata rules: TriStation traffic on UDP 1502 from non-SIS-engineering IPs, IEC 104 station-interrogation followed by rapid-fire Single Command (Type 45) sequences, and GOOSE frames from unauthorized MAC addresses. (b) Write two Sigma rules for OT workstation monitoring: py2exe-compiled Python execution on SIS workstations (TRITON indicator), and TOR process execution from OT-zone hosts (INDUSTROYER C2 indicator). (c) Write one YARA rule targeting the TRITON `inject.bin` payload structure. (d) Test all rules against simulated ICS PCAPs and document false-positive tuning.

**Exercise 2 — PLC Exploitation with python-snap7 and pycomm3.**
In an authorized lab with S7 and Rockwell emulators: (a) Using python-snap7, connect to the S7 PLC, read Data Block DB1, read the CPU diagnostic buffer, and perform a PLC Stop/Start cycle. Document S7comm exchanges in Wireshark. (b) Using pycomm3, enumerate controller tags, read a process-value tag, and write a modified setpoint. Document EtherNet/IP CIP exchanges. (c) Create a golden-image integrity monitor that periodically downloads PLC programs, hashes them with SHA-256, and alerts on mismatch. (d) Map each step to MITRE ATT&CK for ICS techniques.

**Exercise 3 — ICS Incident Response: PLC Forensics and Historian Timeline.**
Simulate a compromise in GRFICSv2. (a) Perform PLC forensic acquisition: download program blocks, read diagnostic buffer, capture I/Q/M/DB memory snapshots with SHA-256 hashes and UTC timestamps. (b) Capture and parse a full network PCAP with Wireshark's Modbus dissector to extract the attacker's command sequence. (c) Extract historian data and plot process variables to identify anomalous excursions. (d) Build a unified timeline correlating network events, PLC diagnostic events, and process-variable data. (e) Produce a forensic report following ICS-CERT format with ATT&CK for ICS mapping.

**Exercise 4 — OT Threat Hunting Playbook: CHERNOVITE/PIPEDREAM.**
Develop and execute a hunt playbook targeting PIPEDREAM indicators. (a) Define the hypothesis: CHERNOVITE reconnaissance of Codesys V3 and OPC UA targets. (b) Identify data sources: ICS IDS logs, OT firewall logs, engineering workstation Sysmon logs, OPC UA audit logs. (c) Develop queries for each PIPEDREAM component: Codesys V3 (TCP 11740) from non-engineering IPs, FINS (UDP 9600) from unexpected sources, OPC UA GetEndpoints with SecurityPolicy#None, HTTP to PLC management interfaces. (d) Execute against lab-generated traffic. (e) Document in a threat-hunting report with ATT&CK for ICS mapping.

**Exercise 5 — FrostyGoop-Style Modbus Attack Simulation.**
In a controlled lab: (a) Set up a Modbus TCP server using `pymodbus` simulating a heating controller with temperature setpoint registers. (b) Write a Python attack script that sends FC16 to change the setpoint from 70°C to 0°C. (c) Configure Suricata with ICS rules to detect: unauthorized writes, writes to safety-critical registers, and out-of-range values. (d) Deploy Zeek with Modbus analyzer and verify `modbus.log` capture. (e) Analyze detection gaps if the attacker spoofs the SCADA master's IP. Document limitations and propose additional detection layers.

---

## Readings and References

1. Dragos. "How to Protect Against FrostyGoop." <https://www.dragos.com/blog/protect-against-frostygoop-ics-malware-targeting-operational-technology> (retrieved: 2026-05-29).
2. Remy Jaspers. "Functional Analysis of FrostyGoop ICS Malware." <https://www.remyjaspers.com/blog/frostygoop/> (retrieved: 2026-05-29).
3. MITRE. "ATT&CK for ICS Matrix." <https://attack.mitre.org/matrices/ics/> (retrieved: 2026-05-29).
4. CISA. "ICS Advisories." <https://www.cisa.gov/news-events/ics-advisories> (retrieved: 2026-05-29).
5. NIST. "SP 800-82 Rev. 3 — Guide to OT Security." <https://csrc.nist.gov/pubs/sp/800/82/r3/final> (retrieved: 2026-05-29).
6. Fortiphyd Logic. "GRFICSv2." <https://github.com/Fortiphyd/GRFICSv2> (retrieved: 2026-05-29).
7. donadelden. "awesome-ics-malware." <https://github.com/donadelden/awesome-ics-malware> (retrieved: 2026-05-29).
8. Dragos. "MITRE ATT&CK for ICS Framework." <https://www.dragos.com/mitre-attack-for-ics> (retrieved: 2026-05-29).
9. SOCRadar. "CISA ICS Advisories Recap for 2025." <https://socradar.io/blog/cisa-industrial-control-systems-ics-advisories-2025/> (retrieved: 2026-05-29).
10. Red Teaming ICS. "GRFICS v2 + OpenPLC ICS Cyber Range Setup." <https://www.redteamingics.com/firstrange/> (retrieved: 2026-05-29).
11. SecurityWeek. "FrostyGoop ICS Malware Left Ukrainian City's Residents Without Heating." <https://www.securityweek.com/frostygoop-ics-malware-left-ukrainian-citys-residents-without-heating/> (retrieved: 2026-05-29).
12. MDPI. "Security Aspects of Zones and Conduits in IEC 62443." <https://www.mdpi.com/2624-800X/6/2/52> (retrieved: 2026-05-29).
...
| Module | Relationship |
|--------|-------------|
| Domain 16, Chapter 16A — ICS/OT Protocol Security | Foundation module covering Modbus, DNP3, IEC 61850, OPC UA, EtherNet/IP protocol internals prerequisite to this chapter |
| Domain 12 — Reverse Engineering | Firmware RE (Ghidra headless analysis, disassembly) applied to ICS malware samples (TRITON, INDUSTROYER, PIPEDREAM) |
| Domain 13 — Cryptography | Cryptographic weaknesses exploited by ICS attacks: OPC UA security policy misconfiguration, DNP3 SA key compromise |
| Domain 17B — Fault Injection | Voltage glitching applicable to PLC firmware extraction (S7-1500 firmware decryption via ATECC chip glitching) |
| Domain 17C — PCB RE and Chip Analysis | PCB reverse engineering and firmware extraction from ICS field devices via flashrom/binwalk |
| Domain 7B — Hardware-Level Vulnerabilities | Rowhammer, Plundervolt, and DMA attacks applicable to OT workstation and engineering workstation compromise |
