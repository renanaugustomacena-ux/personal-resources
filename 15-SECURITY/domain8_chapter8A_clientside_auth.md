...
modulo: "8.1"
titolo: "Client-Side Attacks, Authentication, and Session Security"
versione: "OWASP Top 10:2025, CSP Level 3, OAuth 2.1 (draft-ietf-oauth-v2-1), WebAuthn Level 3"
livello: "Advanced"
prerequisiti:
  - "Domain 13 Chapter 13A (cryptographic primitives, hash functions, digital signatures)"
  - "Domain 13 Chapter 13B §§1–3 (TLS, certificate pinning, PKI)"
  - "HTTP fundamentals (request/response, headers, cookies, status codes)"
  - "Browser Developer Tools proficiency"
obiettivi:
  - "Discover, exploit, and remediate all XSS variant classes (reflected, stored, DOM-based, mutation, blind, universal) using context-aware payloads and framework-specific bypass techniques"
  - "Bypass CSP policies via JSONP endpoints, AngularJS sandbox escapes, script gadgets, and strict-dynamic abuse, then deploy Trusted Types as a definitive DOM XSS defense"
  - "Exploit OAuth 2.0/OIDC flows (redirect_uri bypass, state omission, PKCE downgrade) and JWT vulnerabilities (alg:none, RS256→HS256 confusion, kid/jku injection) using jwt_tool and Burp Suite"
  - "Perform CSRF attacks against SameSite=Lax and double-submit cookie defenses, and assess CORS misconfigurations for cross-origin authenticated data theft"
  - "Implement defense-in-depth authentication stacks (session token entropy, refresh token rotation, Argon2id password hashing, WebAuthn/FIDO2) with detection engineering via Sigma rules and SIEM correlation"
tag: [security, xss, csrf, cors, csp, oauth, jwt, saml, clickjacking, session-management, webauthn, trusted-types, prototype-pollution, authentication]
---
...
> **Lab environment.** All exercises assume access to a deliberately vulnerable web application (DVWA, OWASP Juice Shop, or a custom target) and an intercepting proxy (Burp Suite Community/Professional). Detailed setup, step-by-step walkthroughs, and flag validation are in `tutorials/tutorial_domain8_ch8A_xss_csrf_auth_lab.md`.

**Exercise 1 — XSS exploitation chain (reflected → stored → DOM-based).**
Deploy OWASP Juice Shop behind a reverse proxy with a Report-Only CSP (`script-src 'self' 'unsafe-inline'`). (a) Identify a reflected XSS sink in the search functionality using Burp Intruder with the SecLists `Fuzzing/XSS/` payload set. Document the injection context (attribute, tag body, JavaScript string). (b) Escalate to stored XSS via the product review feature: craft a payload that exfiltrates `document.cookie` to a Burp Collaborator callback. Confirm the cookie is `HttpOnly`-protected and explain why `localStorage` theft succeeds instead. (c) Locate a DOM-based XSS in the client-side router by tracing `location.hash` through the source code to an `innerHTML` sink. Exploit it without any server-side reflection. Write a Sigma rule that detects all three variants in WAF access logs.

**Exercise 2 — CSP bypass via JSONP endpoint and script gadget.**
Configure a strict CSP on a test application: `script-src 'nonce-<random>' 'strict-dynamic'; object-src 'none'; base-uri 'self'`. (a) Add a JSONP endpoint (`/api/search?callback=`) to the allowlisted origin. Demonstrate that an attacker can load a `<script>` tag pointing to the JSONP endpoint with `callback=alert(document.domain)//`, bypassing the nonce requirement. Explain why `strict-dynamic` does not prevent this. (b) Remove the JSONP endpoint and instead load AngularJS 1.6 from a CDN that is in the CSP allowlist. Inject `<div ng-app ng-csp>{{$eval.constructor('alert(1)')()}}</div>` and confirm execution. (c) Remediate both bypasses: remove JSONP, pin AngularJS to 2+, and deploy Trusted Types (`require-trusted-types-for 'script'`). Verify that `element.innerHTML = userInput` now throws a `TypeError`. Write a Nuclei template that flags CSPs containing `unsafe-inline` or allowlisted JSONP-capable domains.

**Exercise 3 — JWT algorithm confusion and `kid` injection.**
Stand up a Node.js API that issues RS256-signed JWTs via a `/login` endpoint and exposes the public key at `/.well-known/jwks.json`. (a) Using `jwt_tool.py -X k -pk public_key.pem`, forge a token with `alg: HS256` and the RS256 public key as the HMAC secret. Authenticate as `admin`. Explain the root cause (the server reads `alg` from the token header instead of enforcing a server-side allowlist). (b) Modify the server to read the signing key from a file path derived from the `kid` header. Inject `kid: ../../../../dev/null` and sign with an empty key. Confirm authentication bypass. (c) Inject `kid: ' UNION SELECT 'attacker-secret'--` (SQLite backend) and sign with `attacker-secret`. Confirm bypass. (d) Harden the server: pin `algorithms: ['RS256']` in `jwt.verify()`, validate `kid` against an allowlist, and add `exp`, `iss`, `aud` claim enforcement. Verify all three attacks are blocked.

**Exercise 4 — CSRF with SameSite=Lax bypass via top-level GET navigation.**
Deploy a target application (Django or Express) with a state-changing endpoint at `GET /account/change-email?new=attacker@evil.com` (a deliberately insecure design). Set the session cookie with `SameSite=Lax` (the browser default). (a) From an attacker-controlled page, use `<a href="https://target.com/account/change-email?new=attacker@evil.com">Click me</a>` to trigger a top-level navigation. Confirm the session cookie is sent (Lax allows top-level GETs). (b) Attempt the same attack with a `POST` form — confirm the cookie is blocked. (c) Exploit the 2-minute Lax+POST window: set a cookie via `Set-Cookie` without an explicit `SameSite` attribute, then within 2 minutes submit a cross-site POST form. Observe the cookie being sent (Chromium compatibility behavior). (d) Remediate: move state changes to POST-only endpoints, set `SameSite=Strict` on the session cookie, and add a CSRF token with `Origin` header validation. Write a Splunk query that detects cross-origin POST requests missing the `X-CSRF-Token` header.

**Exercise 5 — OAuth `redirect_uri` exploitation and authorization code theft.**
Configure a local OAuth 2.0 authorization server (Keycloak or a minimal Express implementation) with a registered client whose `redirect_uri` is `https://legitimate-app.com/callback`. (a) Test prefix-match bypass: submit `redirect_uri=https://legitimate-app.com/callback/../../../attacker.com` and observe whether the authorization server redirects the code to the attacker domain. (b) Test open redirect chaining: add an open redirect vulnerability at `https://legitimate-app.com/redirect?url=https://attacker.com`. Submit `redirect_uri=https://legitimate-app.com/redirect?url=https://attacker.com` and capture the authorization code at the attacker endpoint via the `Referer` header. (c) Test PKCE downgrade: send an authorization request with `code_challenge_method=plain` and observe whether the server accepts it, effectively negating PKCE protection. Then omit PKCE parameters entirely and confirm the server still issues a code. (d) Harden: enforce exact-match `redirect_uri` validation, require `code_challenge_method=S256`, reject requests without PKCE, and bind the authorization code to the `redirect_uri` used in the initial request. Write a Sigma rule that detects `redirect_uri` values containing `redirect=`, `url=`, `@`, or `%2F%2F`.

---

## Readings and References

- OWASP, "Cross Site Scripting Prevention Cheat Sheet," OWASP Cheat Sheet Series. https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html (retrieved: 2026-05-29).
- OWASP, "DOM based XSS Prevention Cheat Sheet," OWASP Cheat Sheet Series. https://cheatsheetseries.owasp.org/cheatsheets/DOM_based_XSS_Prevention_Cheat_Sheet.html (retrieved: 2026-05-29).
- OWASP, "Content Security Policy Cheat Sheet," OWASP Cheat Sheet Series. https://cheatsheetseries.owasp.org/cheatsheets/Content_Security_Policy_Cheat_Sheet.html (retrieved: 2026-05-29).
- OWASP, "Cross-Site Request Forgery Prevention Cheat Sheet," OWASP Cheat Sheet Series. https://cheatsheetseries.owasp.org/cheatsheets/Cross-Site_Request_Forgery_Prevention_Cheat_Sheet.html (retrieved: 2026-05-29).
- OWASP, "Session Management Cheat Sheet," OWASP Cheat Sheet Series. https://cheatsheetseries.owasp.org/cheatsheets/Session_Management_Cheat_Sheet.html (retrieved: 2026-05-29).
- OWASP, "Authentication Cheat Sheet," OWASP Cheat Sheet Series. https://cheatsheetseries.owasp.org/cheatsheets/Authentication_Cheat_Sheet.html (retrieved: 2026-05-29).
- OWASP, "OAuth2 Cheat Sheet," OWASP Cheat Sheet Series. https://cheatsheetseries.owasp.org/cheatsheets/OAuth2_Cheat_Sheet.html (retrieved: 2026-05-29).
- OWASP, "XSS Filter Evasion Cheat Sheet," OWASP Cheat Sheet Series. https://cheatsheetseries.owasp.org/cheatsheets/XSS_Filter_Evasion_Cheat_Sheet.html (retrieved: 2026-05-29).
- PortSwigger, "Cross-Site Scripting (XSS) Cheat Sheet — 2026 Edition," Web Security Academy. https://portswigger.net/web-security/cross-site-scripting/cheat-sheet (retrieved: 2026-05-29).
- PortSwigger, "Using form hijacking to bypass CSP," PortSwigger Research. https://portswigger.net/research/using-form-hijacking-to-bypass-csp (retrieved: 2026-05-29).
...
- PortSwigger, "Web Security Research — All Articles," PortSwigger Research. https://portswigger.net/research/articles (retrieved: 2026-05-29).
- OWASP, "Cross Site Scripting (XSS)," OWASP Foundation. https://owasp.org/www-community/attacks/xss/ (retrieved: 2026-05-29).
- OWASP, "Cross Site Request Forgery (CSRF)," OWASP Foundation. https://owasp.org/www-community/attacks/csrf (retrieved: 2026-05-29).
- MDN Web Docs, "Cross-site request forgery (CSRF) — Security," Mozilla Developer Network. https://developer.mozilla.org/en-US/docs/Web/Security/Attacks/CSRF (retrieved: 2026-05-29).
- Red Sentry, "JWT Vulnerabilities List: 2026 Security Risks & Mitigation Guide." https://redsentry.com/resources/blog/jwt-vulnerabilities-list-2026-security-risks-mitigation-guide (retrieved: 2026-05-29).
- AppSec Master, "JWT Security Guide 2026 — JSON Web Token Best Practices and Risks." https://www.appsecmaster.net/blog/jwt-security-guide-json-web-token-best-practices/ (retrieved: 2026-05-29).
- OWASP, "Content Security Policy," OWASP Foundation Controls. https://owasp.org/www-community/controls/Content_Security_Policy (retrieved: 2026-05-29).
- OWASP, "Testing for Content Security Policy," OWASP Web Security Testing Guide. https://owasp.org/www-project-web-security-testing-guide/latest/4-Web_Application_Security_Testing/02-Configuration_and_Deployment_Management_Testing/12-Test_for_Content_Security_Policy (retrieved: 2026-05-29).
- NVD, "CVE-2020-11022 — jQuery before 3.5.0," National Vulnerability Database. https://nvd.nist.gov/vuln/detail/CVE-2020-11022 (retrieved: 2026-05-29).
- NVD, "CVE-2015-9235 — JWT alg:none bypass in multiple libraries," National Vulnerability Database. https://nvd.nist.gov/vuln/detail/CVE-2015-9235 (retrieved: 2026-05-29).
- NVD, "CVE-2022-23529 — jsonwebtoken prototype pollution," National Vulnerability Database. https://nvd.nist.gov/vuln/detail/CVE-2022-23529 (retrieved: 2026-05-29).
- Greadme, "What Is a Content Security Policy (CSP)? Complete Guide (2026)." https://www.greadme.com/blog/security/what-is-content-security-policy-complete-guide (retrieved: 2026-05-29).
...
| Module | Relationship |
|--------|--------------|
| Domain 8 Chapter 8B — Server-Side Attacks and API Security | SSRF (8B §4) and deserialization (8B §6) originate from client-facing inputs processed in §2–§4 but execute server-side; stored XSS payloads may enter via SQLi or SSTI in 8B |
| Domain 9 — Network Security Monitoring | WAF rules (§2.8, §10.3) and CSP violation reporting (§10.2) feed the same SIEM pipeline as IDS/IPS alerts in Domain 9; ModSecurity CRS tuning in §13.3 bridges web and network layers |
| Domain 13 Chapter 13A — Cryptographic Primitives | JWT cryptographic attacks (§4.3) require understanding of HMAC vs RSA signing (13A §§3–4); Argon2id password hashing (§11.4) builds on hash function properties from 13A §2 |
| Domain 13 Chapter 13B — TLS, PKI, and Applied Cryptography | SAML signature validation (§4.4) depends on XML canonicalization and digital signature standards (13B §5); TLS certificate pinning (13B §3) underpins HSTS enforcement in §8.3 |
| Domain 14 Chapter 14A — Kerberos and Active Directory | OAuth token theft (§4.2) parallels TGT theft in Kerberos; JWT algorithm confusion (§4.3 RS256→HS256) parallels Kerberos encryption downgrade attacks (14A §6); session fixation (§4.5) mirrors Kerberos ticket reuse |
| Domain 6 — Exploit Mitigation and Bypass | CSP bypass techniques (§2.6) are the web equivalent of mitigation bypass in Domain 6; defense-in-depth erosion (Domain 6 §8) applies: CSP alone fails if the allowlist includes JSONP endpoints |
...
| Term | Definition |
|------|------------|
| **Same-Origin Policy (SOP)** | Browser security boundary that restricts scripts from one origin (scheme + host + port) from reading data belonging to a different origin; the foundational web security model. |
| **Cross-Site Scripting (XSS)** | Injection attack in which an adversary causes a victim's browser to execute attacker-controlled JavaScript within a trusted origin, enabling session theft, DOM manipulation, or credential harvesting. |
| **Content Security Policy (CSP)** | HTTP response header that instructs the browser to enforce an allowlist of permitted resource sources (scripts, styles, images, frames), mitigating XSS and data injection attacks. |
| **Trusted Types** | W3C browser API (enforced via CSP `require-trusted-types-for 'script'`) that prevents DOM XSS by requiring dangerous sinks (`innerHTML`, `eval`) to accept only typed wrapper objects rather than raw strings. |
| **Cross-Site Request Forgery (CSRF)** | Attack in which an attacker's website causes a victim's browser to submit an authenticated request to a target application, performing unintended state-changing operations using the victim's cookies. |
| **SameSite Cookie Attribute** | Cookie attribute (`Strict`, `Lax`, `None`) that controls whether the browser sends the cookie on cross-site requests; `Lax` (default since Chrome 80) blocks cross-site POST cookies but allows top-level GET navigations. |
| **JSON Web Token (JWT)** | Compact, URL-safe token format (RFC 7519) consisting of a Base64url-encoded header, payload, and signature; used for stateless authentication and authorization in web APIs. |
| **OAuth 2.0** | Authorization framework (RFC 6749) that enables third-party applications to obtain limited access to a user's resources via authorization grants, access tokens, and scopes without exposing the user's credentials. |
| **PKCE (Proof Key for Code Exchange)** | OAuth 2.0 extension (RFC 7636) that binds an authorization code to the client that requested it via a `code_verifier`/`code_challenge` pair, preventing authorization code interception attacks on public clients. |
| **Clickjacking** | UI redressing attack in which the target application is loaded in a transparent iframe on the attacker's page, causing the victim to unknowingly interact with the target's UI elements (buttons, links, forms). |
| **DOM Clobbering** | Technique in which an attacker injects HTML elements with `id` or `name` attributes to shadow JavaScript global variables, manipulating application logic without executing any script directly. |
| **WebAuthn / FIDO2** | W3C and FIDO Alliance standards for phishing-resistant, public-key-based authentication using hardware or platform authenticators; the credential is origin-bound, and the private key never leaves the authenticator device. |
| **Mutation XSS (mXSS)** | XSS variant in which a payload appears benign to a sanitizer but transforms into executable HTML when the browser re-parses and re-serializes the DOM, bypassing string-based sanitization. |
| **Cookie Tossing** | Attack in which an adversary controlling a subdomain sets a cookie on the parent domain (e.g., `.example.com`), overwriting or shadowing legitimate cookies used by sibling subdomains for CSRF defense or session management. |
| **Cross-Origin Resource Sharing (CORS)** | HTTP mechanism that allows a server to indicate which origins are permitted to read its responses via `Access-Control-Allow-Origin` and related headers, selectively relaxing the Same-Origin Policy. |
