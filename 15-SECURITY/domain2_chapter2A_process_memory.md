...
modulo: "2.1"
titolo: "Process Address Space and Memory Management"
versione: "Linux 6.x, glibc 2.39+, x86_64 / AArch64"
livello: "Advanced"
prerequisiti:
  - "Domain 1 (ELF loading sequence, PT_LOAD segments, mmap during load_elf_binary)"
  - "Virtual memory concepts (pages, page tables, TLB, MMU)"
  - "Linux kernel basics (kernel vs user mode, system calls, /proc filesystem)"
  - "C programming with pointers, mmap, and memory allocation"
  - "Basic exploitation concepts (buffer overflow, ASLR)"
obiettivi:
  - "Describe the canonical x86_64 and AArch64 user-space address layout including stack, heap, mmap region, and vDSO"
  - "Trace mmap and brk system call internals through VMA creation, page fault handling, and PTE installation"
  - "Demonstrate ASLR bypass techniques including format-string leaks, brute-force, partial overwrite, and proc/maps reading"
  - "Explain the Stack Clash (CVE-2017-1000364) and Dirty COW (CVE-2016-5195) exploitation mechanisms and their mitigations"
  - "Construct auditd, Sigma, and eBPF detection rules for suspicious memory operations (mprotect W+X, MAP_FIXED abuse)"
tag: [process-memory, aslr, mmap, page-tables, stack-clash, dirty-cow, kernel-exploitation, virtual-memory]
---
...
1. **Address Space Mapping Lab.** Write a C program that prints its own `/proc/self/maps`, then uses `mmap` to create anonymous RW, anonymous RX, file-backed RO, and `MAP_HUGETLB` mappings. After each mapping, re-read `/proc/self/maps` and document the new VMA entries. Verify the permissions match the requested `prot` flags. Reference: `tutorials/tutorial_domain2_ch2A_process_memory_lab.md`.

2. **ASLR Entropy Measurement.** Write a script that spawns 1000 instances of a PIE binary and collects the base addresses of the executable, libc, the stack, and the vDSO from each `/proc/PID/maps`. Compute the observed entropy (in bits) for each region and compare against the theoretical values in §5. Test with `mmap_rnd_bits` at 28 and 32.

3. **Stack Clash Verification.** Compile the `stack_clash_demo.c` from §14.2 with and without `-fstack-clash-protection`. Run both versions under `strace` and observe the `mmap`/`mprotect` calls. On a patched kernel, verify that `stack_guard_gap` prevents the clash. On an unpatched VM (kernel < 4.11.5), demonstrate the actual corruption.

4. **Dirty COW Detection Pipeline.** On a VM running a deliberately vulnerable kernel (< 4.8.3), trigger the Dirty COW PoC from §15.2 against a test file. Then implement detection using: (a) auditd rules from §15.4 monitoring `madvise(MADV_DONTNEED)` frequency, (b) file integrity monitoring comparing `/proc/PID/mem` against disk, and (c) a Sigma rule for anomalous `/proc/self/mem` write patterns.

5. **Ret2dir Attack Surface Audit.** On a test system, verify whether `/proc/self/pagemap` exposes PFNs (check kernel version and `CAP_SYS_ADMIN` requirement). Measure the kernel's direct-map `PAGE_OFFSET` randomization range with KASLR enabled. Document the sysctl and kernel config options (`CONFIG_STRICT_DEVMEM`, `CONFIG_IO_STRICT_DEVMEM`, `init_on_alloc`, `init_on_free`) that mitigate ret2dir.

---

## Readings and References

- Linux kernel `mm/mmap.c` — mmap implementation: https://elixir.bootlin.com/linux/latest/source/mm/mmap.c (retrieved: 2026-05-29)
- Linux kernel `mm/memory.c` — page fault handler: https://elixir.bootlin.com/linux/latest/source/mm/memory.c (retrieved: 2026-05-29)
- CVE-2016-5195 — Dirty COW race condition: https://nvd.nist.gov/vuln/detail/CVE-2016-5195 (retrieved: 2026-05-29)
- CVE-2017-1000364 — Stack Clash (Linux): https://nvd.nist.gov/vuln/detail/CVE-2017-1000364 (retrieved: 2026-05-29)
- Qualys Security Advisory — Stack Clash: https://www.qualys.com/2017/06/19/stack-clash/stack-clash.txt (retrieved: 2026-05-29)
- MITRE ATT&CK T1055 — Process Injection: https://attack.mitre.org/techniques/T1055/ (retrieved: 2026-05-29)
- MITRE ATT&CK T1014 — Rootkit: https://attack.mitre.org/techniques/T1014/ (retrieved: 2026-05-29)
- Kemerlis et al., "ret2dir: Rethinking Kernel Isolation" (USENIX Security 2014): https://www.usenix.org/conference/usenixsecurity14/technical-sessions/presentation/kemerlis (retrieved: 2026-05-29)
- Linux kernel exploitation resources collection: https://github.com/xairy/linux-kernel-exploitation (retrieved: 2026-05-29)
- "How to Bypass Basic Exploit Mitigation — ASLR": https://andy.codes/blog/security-articles/2025-11-02-exploit-mitigation-aslr.html (retrieved: 2026-05-29)
- Linux kernel `include/linux/mm_types.h` — mm_struct and vm_area_struct: https://elixir.bootlin.com/linux/latest/source/include/linux/mm_types.h (retrieved: 2026-05-29)
- `proc(5)` man page — /proc/PID/maps, /proc/PID/pagemap: https://man7.org/linux/man-pages/man5/proc.5.html (retrieved: 2026-05-29)
...
|---------|-------|-------------|
| `domain1_chapter1A_elf_foundations.md` | ELF loading (load_elf_binary), PT_LOAD mmap, GOT overwrite from brk | Kernel mmap calls during ELF segment loading; brk heap adjacency to GOT |
| `domain1_chapter1B_elf_advanced.md` | RELRO mprotect, vDSO, AT_SYSINFO_EHDR, TLS heap allocation | mprotect for RELRO is the same syscall as §8.1; vDSO mapped near stack |
| `domain2_chapter2B_syscall_seccomp_ptrace.md` | syscall dispatch, seccomp filtering of mprotect, userfaultfd, ptrace | mmap/mprotect/brk dispatched via entry_SYSCALL_64; userfaultfd hooks page faults |
| `domain2_chapter2C_caps_ns_lsm.md` | CAP_IPC_LOCK, CAP_SYS_ADMIN, SELinux execmem, cgroup memory | Capabilities gate mlock, pagemap PFN reads; SELinux blocks W+X mprotect |
| `domain3_chapter3A_stack_format_integer.md` | Format string ASLR leak, stack canary, stack layout | Format string %p leaks stack/libc addresses; stack placement from §4 |
| `domain5_chapter5A_kernel_exploitation.md` | Kernel heap spray, msg_msg, SLUB, physmap spray | ret2dir and PTE overwrite payloads; kernel spray primitives from userspace |

---

## Glossary

- **VMA (Virtual Memory Area):** Kernel data structure (`vm_area_struct`) representing a contiguous range of virtual addresses with uniform permissions and backing.
- **ASLR (Address Space Layout Randomization):** Kernel mechanism randomizing base addresses of executable, libraries, stack, heap, and vDSO per execve.
- **KPTI (Kernel Page Table Isolation):** Meltdown mitigation maintaining separate user and kernel page tables, switching CR3 on privilege transitions.
- **PTE (Page Table Entry):** Hardware-level 64-bit entry mapping a virtual page to a physical frame with permission bits (RW, NX, USER).
- **Stack Clash:** Vulnerability class where large stack allocations skip guard pages, landing in adjacent memory regions.
- **Dirty COW:** CVE-2016-5195 — race condition in the COW page fault handler allowing writes to read-only file-backed mappings.
- **Direct Map (Physmap):** Kernel linear mapping of all physical RAM at `PAGE_OFFSET`; used for `kmalloc`, slab allocator, and ret2dir attacks.
- **Maple Tree:** Kernel data structure (replacing red-black tree in 6.1+) for ordered VMA range lookup in `mm_struct`.
- **vDSO (Virtual Dynamic Shared Object):** Kernel-synthesized ELF mapped into every process for fast userspace implementation of select syscalls.
- **MAP_FIXED:** mmap flag placing a mapping at an exact address, silently replacing existing mappings — a destructive operation.
- **COW (Copy-on-Write):** Memory optimization where forked processes share physical pages until one writes, triggering a private copy.
- **THP (Transparent Huge Pages):** Kernel mechanism automatically promoting 512 contiguous 4KB pages into 2MB huge pages for TLB efficiency.
- **Guard Page:** Unmapped page (PROT_NONE) placed below thread stacks to detect overflow via SIGSEGV.
- **SMEP/SMAP:** Intel hardware features preventing the kernel from executing (SMEP) or accessing (SMAP) user-space pages.
