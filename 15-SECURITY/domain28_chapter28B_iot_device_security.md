...
fase: "Domain 28 — IoT Security"
modulo: "28.B"
titolo: "IoT Device Security"
versione: "OWASP IoT Top 10 2018 / NIST IR 8259 / ETSI EN 303 645 v3.1.3 / US Cyber Trust Mark / PSA Certified"
livello: "Advanced"
prerequisiti:
  - "Domain 28 Chapter 28A — IoT protocol security (MQTT, CoAP, Zigbee, Z-Wave, BLE, LoRaWAN)"
  - "Domain 12 Chapter 12B — firmware extraction, reverse engineering tools (binwalk, Ghidra)"
  - "Domain 17 — physical security, fault injection fundamentals"
  - "Domain 11 — malware architecture, C2 design patterns, botnet mechanics"
  - "Embedded systems fundamentals — UART, SPI, I2C, ARM/MIPS architecture"
obiettivi:
  - "Execute a complete IoT hardware assessment — identify UART/JTAG/SWD interfaces, extract firmware via SPI flash dump, and bypass debug protection using voltage glitching with ChipWhisperer"
  - "Perform end-to-end firmware analysis using binwalk, EMBA, and Ghidra: extract filesystem, discover hardcoded credentials, identify vulnerable libraries via cve-bin-tool, and emulate firmware with firmadyne"
  - "Assess firmware update security — demonstrate unsigned OTA exploitation via mitmproxy, implement secure signing with ECDSA, and configure anti-rollback protection using OTP fuses"
  - "Analyze IoT botnet architectures (Mirai scanner/loader/C2, Mozi DHT P2P) and deploy Cowrie/Dionaea honeypots to capture botnet propagation telemetry"
  - "Conduct a structured IoT security assessment following the OWASP IoT Top 10, producing a risk-scored report with CVSS v3.1, CWE classifications, and actionable remediation recommendations"
tag: [iot, firmware, uart, jtag, swd, spi, binwalk, ghidra, emba, mirai, botnet, upnp, tr-069, owasp-iot, chipwhisperer, firmware-signing]
---

# Domain 28, Chapter 28B — IoT Device Security

> **After completing this chapter, you will be able to:** (1) identify and exploit hardware debug interfaces (UART, JTAG, SWD, SPI, I2C) on IoT device PCBs, including baud-rate detection, JTAGulator pin identification, and OpenOCD-based flash extraction; (2) perform complete firmware analysis from acquisition through unpacking, filesystem extraction, credential discovery, binary vulnerability analysis, and emulation with firmadyne and QEMU; (3) exploit unsigned firmware update mechanisms via OTA MitM and implement secure countermeasures (ECDSA signing, anti-rollback via OTP fuses, A/B slot atomic updates); (4) analyze Mirai botnet architecture at the source-code level (scanner, loader, C2 protocol) and deploy honeypots for botnet intelligence collection; (5) conduct a full IoT security assessment covering hardware, firmware, network, and cloud layers, scoring findings against OWASP IoT Top 10 with CVSS and CWE.
...
1. **UART and SPI flash extraction lab.** Obtain a consumer IoT device (a used router or IP camera from a secondhand market). Open the enclosure and photograph the PCB. Identify candidate UART pins using a multimeter (GND continuity) and logic analyzer (TX activity during boot). Determine the baud rate using pulse-width measurement. Connect via `picocom` and document what the serial console provides (U-Boot shell, root shell, or login prompt). If a U-Boot shell is accessible, modify boot arguments to `init=/bin/sh` and obtain a root shell. Then identify the SPI flash chip (read the part number), attach an SOIC-8 test clip, and extract the firmware using `flashrom` on a Raspberry Pi. Verify the dump integrity by reading twice and comparing MD5 hashes.

2. **Complete firmware analysis with binwalk and EMBA.** Take the firmware dump from Exercise 1 (or download a router firmware from the vendor's website). Run `binwalk -E` for entropy analysis and identify compressed vs. encrypted vs. plaintext sections. Run `binwalk -eM` for recursive extraction. Extract the squashfs filesystem with `unsquashfs` (or `sasquatch` for vendor-patched variants). Run `firmwalker` to discover credentials, certificates, and sensitive URLs. Run `cve-bin-tool` against the extracted filesystem to identify known-vulnerable libraries (OpenSSL, BusyBox, Dropbear). Run EMBA for comprehensive automated analysis. Load the web server binary in Ghidra, identify the architecture, and search for `strcpy`, `sprintf`, `system()`, and `popen()` calls with user-controlled input. Document all findings with CWE classifications.

3. **Firmware modification and OTA MitM.** Using the extracted firmware from Exercise 2: (a) modify `/etc/rc.local` to add a reverse shell, (b) add an SSH public key to `/root/.ssh/authorized_keys`, (c) repack the squashfs filesystem with `mksquashfs`, (d) rebuild the firmware image. Then set up an OTA MitM using `mitmproxy` in transparent mode with a DNS redirect to serve the modified firmware when the device checks for updates. If the device accepts the modified firmware (no signature verification), document the attack chain as a finding with CVSS score. Then implement the fix: generate an ECDSA P-256 signing key pair, sign the firmware with `openssl dgst -sha256 -sign`, embed the public key in the bootloader, and verify that the device now rejects unsigned modifications.

4. **IoT botnet analysis and honeypot deployment.** Analyze the Mirai source code (available on GitHub): trace the scanner (`bot/scanner.c`), the credential table (identify all 62 default username/password pairs), the killer module (`bot/killer.c`), and the C2 protocol (`bot/attack.c`). Document the architecture as a diagram showing the scanner, report server, loader, bot, and C2 relationships. Then deploy a Cowrie SSH/Telnet honeypot on a lab network, redirect port 23 traffic to Cowrie via iptables, and capture credential attempts and post-login commands over 48 hours. Analyze the captured data: what credentials were attempted, what payloads were downloaded, what architecture-detection commands were executed. Write YARA rules matching the downloaded payloads and Suricata rules matching the C2 registration traffic.

5. **Structured IoT security assessment.** Select a consumer IoT device and perform a full assessment covering all four layers: (a) Hardware — identify all debug interfaces, attempt UART console access, assess debug protection (RDP/APPROTECT), extract flash if possible; (b) Firmware — analyze filesystem, discover credentials, identify vulnerable components, check for firmware signing and anti-rollback; (c) Network — capture traffic during normal operation, verify TLS certificate validation (attempt MitM), test MQTT/CoAP endpoints for authentication, analyze cloud API for IDOR and broken auth; (d) Cloud — enumerate the device-to-cloud API, test for shadow/twin manipulation, assess credential rotation. Score each finding using CVSS v3.1 with IoT-specific adjustments (AV:P for hardware attacks, S:C for cross-device pivot potential). Map findings to OWASP IoT Top 10 and ETSI EN 303 645 provisions. Produce a professional report following the template in §6.6.

---

## Readings and References

- OWASP IoT Top 10 (2018): <https://owasp.org/www-project-internet-of-things/> (retrieved: 2026-05-29)
- OWASP IoT Security Testing Guide: <https://owasp.org/www-project-iot-security-testing-guide/> (retrieved: 2026-05-29)
- NIST IR 8259 — Foundational Cybersecurity Activities for IoT Device Manufacturers: <https://csrc.nist.gov/pubs/ir/8259/final> (retrieved: 2026-05-29)
- ETSI EN 303 645 v3.1.3 — Cybersecurity for Consumer IoT (2024-09): <https://www.etsi.org/technologies/consumer-iot-security> (retrieved: 2026-05-29)
- US Cyber Trust Mark Program: <https://www.fcc.gov/cybersecurity-certification-mark> (retrieved: 2026-05-29)
- PSA Certified: <https://www.psacertified.org/> (retrieved: 2026-05-29)
- binwalk v3 (Rust rewrite): <https://github.com/ReFirmLabs/binwalk> (retrieved: 2026-05-29)
- EMBA — Embedded Analyzer: <https://github.com/e-m-b-a/emba> (retrieved: 2026-05-29)
- firmadyne — Firmware Emulation: <https://github.com/firmadyne/firmadyne> (retrieved: 2026-05-29)
- Cowrie SSH/Telnet Honeypot: <https://github.com/cowrie/cowrie> (retrieved: 2026-05-29)
- ChipWhisperer — Fault Injection Platform: <https://github.com/newaetech/chipwhisperer> (retrieved: 2026-05-29)
- RouterSploit — Exploitation Framework for Embedded Devices: <https://github.com/threat9/routersploit> (retrieved: 2026-05-29)
- cve-bin-tool: <https://github.com/intel/cve-bin-tool> (retrieved: 2026-05-29)
- Mirai Source Code Analysis — Usenix Security 2017: <https://www.usenix.org/conference/usenixsecurity17/technical-sessions/presentation/antonakakis> (retrieved: 2026-05-29)
...
|---|---|---|---|
| §1 Hardware interfaces | Domain 17 — Physical Security | JTAG/SWD exploitation, fault injection, voltage glitching | Domain 17 §3-4 |
| §2 Firmware analysis | Domain 12 Ch.12B | binwalk, Ghidra, flash extraction, binary RE | Domain 12B §2 |
| §3 Firmware updates | Domain 19 — Supply Chain | Unsigned firmware as supply-chain vulnerability | Domain 19 |
| §4 IoT botnets | Domain 11 — Malware | C2 design patterns, propagation, persistence | Domain 11A §5 |
| §5 UPnP/TR-069 | Domain 28A | Protocol-layer attacks complementing device-layer assessment | Domain 28A §1-3 |
| §7 Standards | Domain 27A §2 | Saltzer-Schroeder design principles applied to IoT | Domain 27A §2 |
...
| Term | Definition |
|---|---|
| **UART** | Universal Asynchronous Receiver/Transmitter — serial debug interface commonly exposing bootloader or root shells on IoT devices |
| **JTAG** | Joint Test Action Group — hardware debug interface providing CPU halt, single-step, memory read/write, and flash access |
| **SWD** | Serial Wire Debug — ARM-specific two-pin debug interface (SWDIO, SWCLK), functionally equivalent to JTAG with fewer pins |
| **JTAGulator** | Hardware tool that systematically tests all pin combinations to identify JTAG/SWD interfaces on unknown headers |
| **binwalk** | Open-source firmware analysis tool scanning for file signatures, extracting embedded components, and performing entropy analysis |
| **EMBA** | Embedded Analyzer — automated firmware analysis platform combining static analysis, dynamic emulation, and CVE matching |
| **firmadyne** | Full-system firmware emulation platform using QEMU with network services for IoT firmware security testing |
| **RDP** | Read-Out Protection — STM32 debug protection mechanism with three levels (0: open, 1: flash read disabled, 2: permanent debug disable) |
| **APPROTECT** | nRF52 access-port protection register disabling the SWD debug port; bypassable via voltage glitching on early silicon revisions |
| **Mirai** | IoT botnet (2016) propagating via Telnet default credentials, source-released, spawning dozens of variants |
| **Mozi** | IoT botnet using BitTorrent DHT for P2P C2 communication, taken down in 2021 via a kill-switch command distributed through its own DHT |
| **UPnP IGD** | Universal Plug and Play Internet Gateway Device — protocol allowing LAN devices to request NAT port mappings without authentication |
| **TR-069/CWMP** | CPE WAN Management Protocol — ISP-to-router management protocol enabling remote configuration, firmware push, and credential extraction |
| **OTP fuse** | One-Time Programmable fuse — irreversible hardware element used for anti-rollback counters and debug-disable flags |
| **MUD** | Manufacturer Usage Description (RFC 8520) — device-emitted profile describing expected network behavior for automated NAC enforcement |
